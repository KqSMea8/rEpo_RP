<?  	ob_start();
	...
	$Config['CronJob'] = '1';
		...
		require_once($Prefix."admin/language/english.php");
	...
	if(sizeof($arryCompany)>0){
		...
		if(!empty($ZoomMeeting)){
			...
			if(empty($resentMeetings->meetings)) exit; 
		}
		if(!empty($ZoomWebinar)){
			$recentWebinars = $objMeeting->listDashboardWebinars();
			if(empty($recentWebinars->meetings)) exit;
		}
		/*---------------added by sanjiv ------------------------*/
		...
		foreach($arryCompany as $key=>$values){
			...
			unset($CmpDatabase);
			//$values['DisplayName'] = 'DemoVstacks'; //for testing
