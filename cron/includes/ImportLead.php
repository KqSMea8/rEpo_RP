	...
	foreach ($Sheets as $Index => $Name){
		...
		foreach ($Spreadsheet as $Key => $Row){
			...
			foreach($DbColumnArray as $Key => $Heading){
				$i = $parameters[$Key];
				$arrayLead[$Count][$Key]=trim($Row[$i]);
			}
			$arrayLead[$Count]["AssignTo"]=$AssignTo;
			$arrayLead[$Count]["AssignType"]='Users';

			/************************/
			if(empty($arrayLead[$Count]["Country"])){
				$arrayLead[$Count]["Country"] = $parameters['LocationCountry'];
			}




			if(!empty($arrayLead[$Count]["country_id"])){
				unset($arryCountry); unset($arryState); unset($arryCity);
				$arryCountry = $objRegion->GetImportCountryID($arrayLead[$Count]["country_id"]);
				$arrayLead[$Count]["country_id"]=$arryCountry[0]['country_id']; //updated by chetan on 20june2017				
