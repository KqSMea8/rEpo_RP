...
function DashboardIconNew($depID) {
        ...
        if ($_SESSION['AdminType'] == "admin") {
            ...
            if (!empty($DefaultIcon)) {
                ...
                $joinsql = "inner join permission p on (i.ModuleID =m.ModuleID and p.UserID='" . $_SESSION['UserID'] . "')";
            }
        }
...
function DashboardIconRoleGroupNew($depID,$GroupID=0) {
        ...
        if ($_SESSION['AdminType'] == "admin") {
            ...
            if (!empty($DefaultIcon)) {
                ...
                $joinsql = "inner join permission_group p on (i.ModuleID =m.ModuleID and p.GroupID='" . $GroupID . "')";
            }
        }
   ...
   function getSettingCaption($settingKey) {
    ...
    function GetShipMethodName($ShippingMethod, $ShippingMethodVal) {
        ...
        if ($ShippingMethod == 'UPS') {
            ...
            $strSQLQuery = "SELECT service_type FROM ups_service_type where service_value='" . $ShippingMethodVal . "'";
        } elseif ($ShippingMethod == 'USPS') {
            $strSQLQuery = "SELECT service_type FROM usps_service_type where service_value='" . $ShippingMethodVal . "'";
        } elseif ($ShippingMethod == 'Fedex') {

            $strSQLQuery = "SELECT service_type FROM fedex_service_type where service_value='" . $ShippingMethodVal . "'";
        }
...
/*********************************/
			 ...
			 function getRightMenuByDepId($depID,$Parent,$EditView)
                {   
                  ...
                  $strSQLQuery = "select * from ".$_SESSION['CmpDatabase'].".admin_right_modules where depID='".$depID."' and Parent='".$Parent."' and EditView in (0,".$EditView.")  ORDER BY ModuleID asc, OrderBy asc";
