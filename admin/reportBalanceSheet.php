...
ValidateAdminSession('reportBalanceSheet.php');
?>
      ...
      $TotalAccountBalance=0;
       foreach($rootBankAccountName as $key=>$values){
		...
		if($RetainedEarning>0 && $values['BankAccountID']==$RetainedEarning && $CurrentPeriod!=''){
			...
			$NettBalance = $BeginningBalance + $PnLAmount_RE555;			
			...
			$NettBalanceRE=($accountdataRE[0]['CrAmnt']-$accountdataRE[0]['DbAmnt']) + $PnLAmount_CurrentRE555 ;// -  $NettBalance;
