<?php	require_once("../includes/pdf_comman.php");
	...
	$pdf->ezSetDy(-5);
        $ds = 0;
    if($NumLine>0){
        $Head1 = '<b>SKU</b>'; $Head2 = '<b>Description</b>'; $Head3 = '<b>Qty Invoiced</b>'; $Head4 = '<b>Unit Price</b>'; $Head5 = '<b>Dropship</b>';$Head6 = '<b>Cost</b>';$Head7 = '<b>Discount</b>'; $Head8 = '<b>Taxable</b>'; $Head9 = '<b>Amount</b>'; 
		...
		for($Line=1;$Line<=$NumLine;$Line++) { 
			...
			if(!empty($arrySaleItem[$Count]["SerialNumbers"])){
			 ...
			 if(empty($arrySaleItem[$Count]['Taxable'])) $arrySaleItem[$Count]['Taxable']='No';
			    ...
			    $arryDef[$i][$Head3] = $arrySaleItem[$Count]["qty_invoiced"];
			    $arryDef[$i][$Head4] = number_format($arrySaleItem[$Count]["price"],2);
                            $arryDef[$i][$Head5] = $DropshipCheck;
                            $arryDef[$i][$Head6] = number_format($arrySaleItem[$Count]["DropshipCost"],2);
        ...
        $pdf->ezTable($data,'','',array('cols'=>array($Head1=>array('justification'=>'left','width'=>'50'),$Head2=>array('justification'=>'left' ),$Head3=>array('justification'=>'left','width'=>'60'),$Head4=>array('justification'=>'left','width'=>'60'),$Head5=>array('justification'=>'left','width'=>'50'),$Head6=>array('justification'=>'left','width'=>'45'),$Head7=>array('justification'=>'left','width'=>'50'),$Head8=>array('justification'=>'left','width'=>'45'),$Head9=>array('justification'=>'right','width'=>'100')), 'shaded'=>0,'shadeCol'=>array(0.9,0.9,0.9), 'showLines'=>2 , 'xPos' =>300 ,'width'=>580,'fontSize'=>8,'showHeadings'=>0, 'colGap'=>2, 'rowGap'=>2) );
		...
		$TotalTxt .=  "Sub Total : ".number_format($subtotal,2);
		$TotalTxt .= "\nTax : ".number_format($taxAmnt,2);
		if($arrySale[0]['MDType']!='' && $CustDisAmt>0){ $TotalTxt .="\n".$arrySale[0]['MDType'].":".$CustDisAmt;}
		$TotalTxt .= "\nFreight : ".number_format($Freight,2);
		$TotalTxt .= "\nTotal Discount : ".number_format($TDiscount,2);
