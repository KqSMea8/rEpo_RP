<?
	...
	$PostedDate = ($arrySale[0]['PostedDate']>0)?(date($_SESSION['DateFormat'], strtotime($arrySale[0]['PostedDate']))):(NOT_MENTIONED);
	$Approved = ($arrySale[0]['Approved'] == 1)?('Yes'):('No');

	$ClosedDate = ($arrySale[0]['ClosedDate']>0)?(date($_SESSION['DateFormat'], strtotime($arrySale[0]['ClosedDate']))):(NOT_MENTIONED);

	$Comment = (!empty($arrySale[0]['Comment']))? (stripslashes($arrySale[0]['Comment'])): (NOT_MENTIONED);
	$Head1 = 'Left'; $Head2 = 'Right';
	$data = array(
		array($Head1 => "Credit Memo ID# :", $Head2 => $arrySale[0]["CreditID"]),
		...
		array($Head1 => "Status :", $Head2 => $arrySale[0]['Status']),
