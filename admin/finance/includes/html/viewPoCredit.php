...
<TABLE WIDTH="100%"   BORDER=0 align="center" CELLPADDING=0 CELLSPACING=0 >
	<tr>
		...
		<? /*if(empty($_GET['po'])){ ?>
			<a class="fancybox add fancybox.iframe" href="RmaListCredit.php?link=editRmaCredit.php" >Add RMA</a>
		<? } */?>
		   <? if($num>0){?>
<input type="button" class="export_button"  name="exp" value="Export To Excel" onclick="Javascript:window.location='export_rma_credit.php?<?=$QueryString?>';" />
    ...
    <tr align="left"  >
     <!-- <td width="0%" class="head1" ><input type="checkbox" name="SelectAll" id="SelectAll" onclick="Javascript:SelectCheckBoxes('SelectAll','OrderID','<?=sizeof($arryReturn)?>');" /></td>-->
       <td width="10%" class="head1" >RMA Date</td>
        <td width="10%" class="head1" >RMA Type</td>
...
<a href="vRmaCredit.php?view=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&po=<?=$_GET['po']?>"><?=$view?></a>
<a href="editRmaCredit.php?edit=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&po=<?=$_GET['po']?>"><?=$edit?></a>
<a href="editRmaCredit.php?del_id=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&po=<?=$_GET['po']?>" onclick="return confirmDialog(this, '<?=$ModuleName?>')"><?=$delete?></a> 
<a href="pdfRmaCredit.php?o=<?=$values['OrderID']?>" ><?=$download?></a>
