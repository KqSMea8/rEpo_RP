<script src="js/cashReceipt.js?<?=time()?>"></script>
...
$( document ).ready(function() {
	...
	$("#CustomerName").change(function() {
		var PaymentTerm = $("#CustomerName :selected").attr("PaymentTerm");
...
<a href="viewCashReceipt.php" class="back">Back</a>
 ...
 <table width="100%" border="0" cellpadding="5" cellspacing="0" >
	<tr>
	 ...
	 <td  align="left">
		  ...
		  <input name="ReceivedAmount" type="text" class="inputbox" id="ReceivedAmount" style="width: 90px;" maxlength="10" onkeypress="return isDecimalKeyNeg(event);"  onchange="Javascript:setBalanceAmount();"   value="<?=$arryTransaction[0]['OriginalAmount']?>"/>
	...
	<tr id="InnerTr" >
	 ...
	 <tr>
        ...
        <td  align="left">
                        ...
                        <select name="CustomerName" id="CustomerName" class="<?=$ClassName?>" <?=$Disabled?>  onChange="return checkContra();" >
                            <option value="">---Select---</option>
                            <?php foreach($arryCustomerList as $values){?>
                            <option PaymentTerm="<?=$values['PaymentTerm']?>"  value="<?=$values['custID']?>" <?=($CustIDForContra==$values['custID'] || $arryTransaction[0]['CustID']==$values['custID'])?('selected'):('')?>><?=$values['CustomerName']?></option>
                            <?php }?>
...
setBankCurrency('<?=$ModuleCurrency?>');
 ...
 <tr style="display: none;" id="CardChargeTr">
            <td  align="right" class="blackbold"> Charge Card Credit  : </td>
            <td  align="left"> 
<input type="checkbox" name="CardCharge" id="CardCharge" value="1" onclick="Javascript:ShowCardDetail();">
</td>
         </tr>
...
<select name="CheckFormat" class="inputbox" id="CheckFormat" onchange="Javascript:PrintCheck();">
	...
	<tr>
	 <td  align="right" style=" padding-top: 10px; vertical-align: top;" class="blackbold">Payment Date  : <span class="red">*</span></td>
		...
		?>
		<input id="Date" name="Date" readonly="" class="<?=$ClassDate?>" value="<?=$Date?>"  type="text" <?=$Disabled?> onchange="Javascript:checkContra();"> 
...
<textarea id="ReferenceNo" class="<?=$ClassName?>" <?=$Disabled?> type="text" name="ReferenceNo" maxlenghth="200"><?=stripslashes($arryTransaction[0]['ReferenceNo'])?></textarea>
 ...
 <tr>
            <td colspan="2" id="CardDetail" align="center" style="display:none">
<?    include("../includes/html/box/card_charge_form.php"); ?></td>
        </tr>
...
<br><br>
<? 
	include("includes/html/box/cash_receipt_unposted.php"); 
?>
