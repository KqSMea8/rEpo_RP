...
function getCurrentStatus(){  
	var  sendParam='';
  sendParam='BackgroundExec=Vendor&pid='+<?=$_SESSION['pid']?>+'&totalCount='+<?=$_SESSION['EXCEL_TOTAL']?>+'&randomval='+Math.random();
//alert(sendParam);
//return false;
	$.ajax({
    type: "POST",
...
//alert(data.count);
		...
		setTimeout(function(){getCurrentStatus();}, 6000); return;
		}else if(parseInt(data.per)>0 && data.status=='0'){ 
     			var tCount = <?=$_SESSION['EXCEL_TOTAL']?>;
     			mess_vendor = "Total Vendor to import from excel sheet : "+tCount;
    			mess_vendor += "<br>Total Vendor imported into database : "+data.count;
    			mess_vendor += "<br>Vendor already exist in database : "+ (tCount-data.count);
         		$("#vendorComplete").show();
         		$("#ErrorMsg").html(mess_vendor);
         	}else if(parseInt(data.count)=='0' && parseInt(data.per)=='0' && data.status=='0'){ 
          		var tCount = <?=$_SESSION['EXCEL_TOTAL']?>;
    			mess_vendor += "<br>Vendor already exist in database : "+ (tCount-data.count);
			mess_vendor += "<br>Please try another excel!";
         		$("#ErrorMsg").html(mess_vendor);
				$("#vendorImportProcess").remove();
         	}else { 
     			mess_vendor = "Failed! Please try again.";
         		$("#ErrorMsg").text(mess_vendor);
			$("#vendorImportProcess").remove();
         	} 
    }
	});
...
#cper1{height: auto;}
              ...
              <form name="form1" action id="vendorImport" method="post" onSubmit="" enctype="multipart/form-data">
<table width="100%"  border="0" cellpadding="0" cellspacing="0" id="vendorImportProcess">
         <tr>
                  <td align="left"  >
		</td>
		</tr>    
	 ...
	 <input name="Complete" type="submit" style="display:none" id="vendorComplete" onClick="unhook()" class="button" value="Complete" />&nbsp;&nbsp;&nbsp;
	 <input name="Cancel" type="submit" id="vendorCancel" onClick="unhook()" class="button" value="Cancel" />&nbsp;&nbsp;&nbsp;
				  </td></tr> 
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0" >
		   ...
		   <table width="100%" border="0" cellpadding="5" cellspacing="1"  class="borderall">
			...
			<br><?=IMPORT_SHEET_FORMAT_MSG?></td>
				...
				$sel='';
				if(isset($_SESSION['DuplicayColumn'])){
