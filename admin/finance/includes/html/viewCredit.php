...
<TABLE WIDTH="100%"   BORDER=0 align="center" CELLPADDING=0 CELLSPACING=0 >
	<tr>
		...
		<? if($num>0){?>
		<input type="button" class="export_button"  name="exp" value="Export To Excel" onclick="Javascript:window.location='export_cust_credit.php?<?=$QueryString?>';" />
		<? } ?>

		<? if(empty($_GET['po'])){ ?>
		<!--  <a href="ShowCustCreditList.php" class="fancybox add fancybox.iframe">Add RMA</a> -->
		<? } ?>
...
<table <?=$table_bg?>>
    ...
    <tr align="left"  >
      <td width="12%" class="head1" >RMA Date</td>
      <td width="9%"  class="head1" align="center" >RMA Type</td>
      <td width="12%"  class="head1" align="center">RMA Number</td>
    ...
    <td align="center"><? 
	 ...
	 ?></td>
      <td  align="center" class="head1_inner">
		...
		<a href="vCustCredit.php?view=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&rtn=<?=$values['ReturnID'];?>"><?=$view?></a>
		<a href="editCustCredit.php?edit=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&rtn=<?=$values['ReturnID'];?>"><?=$edit?></a>
		<a href="editCustCredit.php?del_id=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&rtn=<?=$values['ReturnID'];?>" onclick="return confirmDialog(this, '<?=$ModuleName?>')"><?=$delete?></a> 
		<a href="pdfCustCredit.php?RTN=<?=$values['OrderID']?>"><?=$download?></a>
	</td>
    </tr>
    <?php }  // foreach end //?>
