...
function SelectCheck(MainID,ToSelect,Opt)
{	
	var flag,i;
...
function SetCheckedTotal(line){
	...
	/***************
	var TotalDepositPrior = document.getElementById("TotalDepositPrior").value;
	var TotalCheckPrior = document.getElementById("TotalCheckPrior").value;

	TotalAmount = (TotalAmount - parseFloat(TotalCheckPrior)) + parseFloat(TotalDepositPrior);
	/***************/

	var TotalNetChangePrior = document.getElementById("TotalNetChangePrior").value;
	TotalAmount = TotalAmount - parseFloat(TotalNetChangePrior);



	document.getElementById("TotalDebitCreditByCheck").value = TotalAmount.toFixed(2);
...
function validateReconcile(EditID){
		...
		var NumRecords = parseInt(document.getElementById("NumRecords").value);
		...
		var EndingBankBalance = document.getElementById("EndingBankBalance").value;
		var EndingBalance = document.getElementById("EndingBalance").value;

		EndingBankBalance = parseFloat(EndingBankBalance).toFixed(2);
		EndingBalance = parseFloat(EndingBalance).toFixed(2);

		 
 		/*if(document.getElementById("CompleteReconcile").checked){
			if(EndingBalance!=EndingBankBalance){
				alert("Ending Bank Balance must be equal to Gl Ending Balance which is "+EndingBalance);
				return false;
			}
		}*/
    ...
    $(document).ready(function(){	
		...
		EndingBankBalance = parseFloat(EndingBankBalance).toFixed(2);
		EndingBalance = parseFloat(EndingBalance).toFixed(2);
		TotalDebitCreditByCheck = parseFloat(TotalDebitCreditByCheck).toFixed(2);

 

           if(TotalDebitCreditByCheck == EndingBankBalance && (TotalDebitCreditByCheck > 0 || TotalDebitCreditByCheck < 0))
               {  
                   document.getElementById("Reconcile").style.display = 'inline';
                   document.getElementById("Status").value = 'Reconciled';
                   document.getElementById("SaveButton").style.display = 'none';
...
function getTransactionList(Type){
	...
	if(Type=='D'){
		...
		$("#totalBottomMonth").hide();
		$("#totalBottomDMonth").show();
		$("#totalBottomCMonth").hide();

		$("#totalBottomPrior").hide();
		$("#totalBottomDPrior").show();
		$("#totalBottomCPrior").hide();
		...
		$("#totalBottomMonth").hide();
		$("#totalBottomDMonth").hide();
		$("#totalBottomCMonth").show();

		$("#totalBottomPrior").hide();
		$("#totalBottomDPrior").hide();
		$("#totalBottomCPrior").show();
		...
		$("#totalBottomMonth").show();
		$("#totalBottomDMonth").hide();
		$("#totalBottomCMonth").hide();

		$("#totalBottomPrior").show();
		$("#totalBottomDPrior").hide();
		$("#totalBottomCPrior").hide();
...
<div  align="center" class="message">
    <?php if (!empty($_SESSION['mess_reconcile'])) { ?><?=
...
<table id="search_table"  cellspacing="1" cellpadding="0" border="0"  align="left" >
 ...
 <td  align="left" <?=$HideUnRecMonth?> >&nbsp;&nbsp;&nbsp;&nbsp;Unreconciled Listing From :&nbsp;
                                                        <select name="UnRecMonth" id="UnRecMonth" class="datebox">                              
                                                            <?php for($i=1;$i<=36;$i++) {?>
                                                            <option value="<?=$i?>" <?php if($_GET['UnRecMonth'] == $i){echo "selected";}?>>
                                                            &nbsp;<?=$i?>
                                                            </option>
                                                            <? } ?>
                                                        </select>  Months
...
<table <?=$table_bg?>>
	...
	<? }else{ ?>
      <tr align="left" style="background-color:#fff;" >
		   ...
		   Ending GL Balance : <? echo number_format($EndingGLBalance,2); ?>
 &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                    Ending Bank Balance : 
                    <input type="text" value="<?=number_format($arryMonthReconcil[0]['EndingBankBalance'],2,'.','');?>" onkeypress="return isDecimalKeyNeg(event);" maxlength="10" style="width: 90px;" id="EndingBankBalance" class="inputbox" name="EndingBankBalance">
                    <?=$Config['Currency']?>
 &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                     <?php if($_GET['edit'] > 0){?>
		...
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<label  id="CompleteLabel" <?php  if(empty($num2) && !empty($AccountID)){
		echo 'style="display:inline;"';
	}else if(($_GET['edit'] > 0) && ($arryMonthReconcil[0]['TotalDebitCreditByCheck'] == $arryMonthReconcil[0]['EndingBankBalance'])){ echo 'style="display:inline;"'; } else {echo 'style="display:none;"'; }?>><input type="checkbox" name="CompleteReconcile" id="CompleteReconcile" value="1">Complete</label>
                    ...
                    <input name="Submit" type="button" <?php  if(empty($num2) && !empty($AccountID) ){
		echo 'style="display:inline;"';
	}else if(($_GET['edit'] > 0) && ($arryMonthReconcil[0]['EndingBankBalance'] == $arryMonthReconcil[0]['TotalDebitCreditByCheck'])){?>style="display:inline;"<?php } else {?>style="display:none;"<?php }?> id="Reconcile" onClick="Javascript: validateReconcile(<?=$_GET['edit']?>);" class="button" value="Reconcile" />
...
?>
    ...
    <tr align="left">
		...
		<td width="13%"  align="right" class="head1">Credit (<?=$Config['Currency']?>)</td>
                <td width="5%" align="center" class="head1">Action<input type="checkbox" name="SelectAll" id="SelectAll" onclick="Javascript:SelectCheck('SelectAll', 'Reconcil_','0');" style="display:none1"></td>
    </tr>
  ...
  if($num2>0){
	...
	$ArryPaymentCheck = array('Purchase','Other Expense','Spiff Expense','Adjustment');
 	$LastMonthSeparator=0;
  	foreach($arryTransaction as $key=>$values){
		$PaymentID=0;
		if($_GET['edit'] > 0){
                    $PaymentID = $objReport->checkPaymentIDExist($_GET['edit'], $values['PaymentID']);
                }
	...
	/***********************/
	if(!empty($Config['LastMonthReconcileID']) && $values['PaymentDate'] < $FromDate && $LastMonthSeparator!=1){
		$TotalNetChangeMonth = round(($TotalDebit - $TotalCredit),2);
		$TotalDepositMonth = round($TotalDeposit,2);
		$TotalCheckMonth = round($TotalCheck,2);
		echo '<tr>
			<td colspan="7" class="heading" align="right">
			<span id="totalBottomMonth">Total  : '.number_format($TotalNetChangeMonth,2).'</span> 
			<span id="totalBottomDMonth" style="display:none"> Deposit: '.number_format($TotalDepositMonth,2).'</span>   
			<span id="totalBottomCMonth" style="display:none"> Checks: '.number_format($TotalCheckMonth,2).'</span>  
			</td>
		</tr>';


		echo '<tr><td  colspan="7" class="selectedbg" height="30"><strong>Prior Month Items</strong></td></tr>';
		$LastMonthSeparator=1;
	}	
	/***********************/
	$DebitAmnt = round($values['DebitAmnt'],2);
	$CreditAmnt = round($values['CreditAmnt'],2);

	$TotalDebit += $DebitAmnt;
	$TotalCredit += $CreditAmnt;


	if(in_array($values['PaymentType'],$ArryPaymentDeposit)){
		$TotalDeposit += $DebitAmnt - $CreditAmnt;
	}else if(in_array($values['PaymentType'],$ArryPaymentCheck)){ 
		$TotalCheck += $DebitAmnt - $CreditAmnt;
	}
  ?>
...
?>
      ...
      <td align="right"><strong><?=number_format($DebitAmnt,2)?></strong><input type="hidden" value="<?=$DebitAmnt?>" id="DebitAmnt_<?=$Line;?>" name="DebitAmnt_<?=$Line;?>"></td>
      <td align="right"><strong><?=number_format($CreditAmnt,2)?></strong><input type="hidden" value="<?=$CreditAmnt?>" id="CreditAmnt_<?=$Line;?>" name="CreditAmnt_<?=$Line;?>"></td>
      <td align="center"><?//=$values['PaymentID']?><input type="checkbox" value="<?=$values['PaymentID']?>"  <?php if(in_array($PaymentID,$arryPaymentID)){echo "checked";}?> onclick="SetReconcilByCheck(<?=$Line;?>)" id="Reconcil_<?=$Line;?>" name="Reconcil_<?=$Line;?>"></td>
...
$TotalDepositPrior=0;
$TotalCheckPrior=0;
$TotalNetChangePrior=0;
/*********************************/
if(!empty($Config['LastMonthReconcileID']) && $LastMonthSeparator==1){
	$TotalNetChangePrior = round(($TotalNetChange - $TotalNetChangeMonth),2);
	$TotalDepositPrior = round(($TotalDeposit - $TotalDepositMonth),2);
	$TotalCheckPrior = round(($TotalCheck - $TotalCheckMonth),2);
	echo '<tr>
		<td colspan="7" class="heading" align="right">
		<span id="totalBottomPrior">Total  : '.number_format($TotalNetChangePrior,2).'</span> 
		<span id="totalBottomDPrior" style="display:none"> Deposit: '.number_format($TotalDepositPrior,2).'</span>   
		<span id="totalBottomCPrior" style="display:none"> Checks: '.number_format($TotalCheckPrior,2).'</span>  
		</td>
	</tr>';
}
/*********************************/
...
?>
 ...
 <tr  >
     	<td  colspan="5" class="head1"  align="right"> Ending GL Balance : <?=number_format($EndingGLBalance,2)?>   </td>
	 <td colspan="2"  class="head1" align="right">
	<span id="totalBottom">Total Net Change : <?=number_format($TotalNetChange,2)?></span> 
	<span id="totalBottomD" style="display:none">Total Deposit: <?=number_format($TotalDeposit,2)?></span>   
	<span id="totalBottomC" style="display:none">Total Checks: <?=number_format($TotalCheck,2)?></span>  
<input type="hidden" name="EndingBalance" id="EndingBalance" value="<?=$Balance?>" readonly>

<input type="hidden" name="TotalCheckPrior" id="TotalNetChangePrior" value="<?=$TotalNetChangePrior?>" readonly>
<input type="hidden" name="TotalDepositPrior" id="TotalDepositPrior" value="<?=$TotalDepositPrior?>" readonly>
<input type="hidden" name="TotalCheckPrior" id="TotalCheckPrior" value="<?=$TotalCheckPrior?>" readonly>
     ...
     <?php if($num2>0){ ?>
        ...
        <div  style="text-align:center;width:100%;padding-top: 15px;">
            ...
            <div>
                            ...
                            <input type="hidden" name="TotalDebitByCheck" id="TotalDebitByCheck" value="<?=$arryMonthReconcil[0]['TotalDebitByCheck55'];?>">
                            <input type="hidden" name="TotalCreditByCheck" id="TotalCreditByCheck" value="<?=$arryMonthReconcil[0]['TotalCreditByCheck55'];?>">
                            <input type="hidden" name="TotalDebitCreditByCheck" id="TotalDebitCreditByCheck" value="<?=$arryMonthReconcil[0]['TotalDebitCreditByCheck55'];?>">
...
<?php  if(empty($num2) && !empty($AccountID) ){ ?>
<input type="hidden" name="Status" id="Status" value="Reconciled">
<? }else if(($_GET['edit'] > 0) && ($arryMonthReconcil[0]['EndingBankBalance'] == $arryMonthReconcil[0]['TotalDebitCreditByCheck'])){?>
<input type="hidden" name="Status" id="Status" value="Reconciled">
<?php } else {?>
...
<input type="hidden" name="NumRecords" id="NumRecords" value="<?=$num2;?>">
 </form>
  </div> 
  </td>
 </tr>
...
<script language="JavaScript1.2" type="text/javascript">
    ...
    $(document).ready(function() {
	  <?php if($_GET['edit'] > 0){ ?>
		SelectCheck('SelectAll', 'Reconcil_',1);
	  <?php } ?>
	...
	$("#EndingBankBalance").trigger("keyup");
