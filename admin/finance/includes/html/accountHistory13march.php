...
<table <?=$table_bg?>>
  ...
  if($num>0 || $BeginningBalance>0){
	...
	if($values["PaymentType"] == "Sales" || $values["PaymentType"] == "Other Income"){

		 $categoryType =  stripslashes(ucwords($values["CustomerName"]));
	}
	else if($values["PaymentType"] == "Purchase" || $values["PaymentType"] == "Other Expense")
	{
		$categoryType =  stripslashes(ucwords($values["SupplierName"]));

	}else if($values["PaymentType"] == "Journal Entry" || $values["PaymentType"] == "Vendor Invoice Entry" || $values["PaymentType"] == "Vendor Invoice")
	{
               if(!empty($values["CustomerName"])){
		 $categoryType =  stripslashes(ucwords($values["CustomerName"]));
		}else if(!empty($values["VendorName"])){
		 $categoryType =  stripslashes(ucwords($values["VendorName"]));
		}else{
			$categoryType = "";
                 }
	}else{
  ...
  ?>
    ...
    <td><?=$categoryType555?>
	 <?php 
 if(!empty($values["CustomerName"])){
	echo stripslashes(ucfirst($values["CustomerName"]));
 }else if(!empty($values["VendorName"])){
	echo stripslashes(ucfirst($values["VendorName"]));
 }?>
	</td> 
    <td>
...
<td align="right"><strong><? if($Line==1){ echo number_format($BeginningBalance,2); }?></strong></td>
