...
var SubTotal = price*qty;
	...
	function calculateGrandTotal(TaxAllow=0) {
...
if(document.getElementById("freightTxSet") != null){
	...
	if(fr!='' && fr>0 && tax>0 && freightTaxSet =='Yes'){		
...
console.log(taxAmnt);
		...
		//ali 10 may 2018
		var freightdiscounted= $("#freightdiscount").val();
		if(typeof(freightdiscounted) !="undefined" && freightdiscounted !='') {
			subtotal=subtotal-freightdiscounted;
		}
		// ali 10 may 2018
...
(empty($costofgood))?($costofgood=""):("");
(empty($HideCol))?($HideCol=""):("");
...
<? if($_SESSION['TrackInventory'] ==1 && $Config['SelectOneItem']!='1'){?>
...
<?}?>
			...
			<td width="9%" class="heading" <?=$HideCol?>>Invoiced</td>
...
<? if($_SESSION['TrackInventory'] ==1 && $Config['SelectOneItem']!='1'){?>
 <td><div <?=$style?>><?=stripslashes($arrySaleItem[$Count]["warehouse_code"])?>
		<input type="hidden" name="WID<?=$Line?>" id="WID<?=$Line?>" class="textbox" style="width:150px;"  maxlength="50" onkeypress="return isAlphaKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["WID"])?>"/></div>
		</td>
<? }?>
<? if($_SESSION['TrackInventory'] ==1 && $Config['SelectOneItem']!='1'){?>
...
if($arrySaleItem[$Count]['qty'] == $total_received ){

$QtyDisable = 'class = "disabled" readonly="readonly"';
}else{

$QtyDisable = 'class = "textbox"';
}
?>        
<td>
...
<input <?=$disable?> type="text" value="<?=$total_received?>" data-qty="y" name="qty<?=$Line?>" id="qty<?=$Line?>" onkeypress="return isNumberKey(event);" maxlength="6" size="5" <?=$rd;?>  <?=$QtyDisable?>>
...
//($arrySaleItem[$Count]["DropshipCheck"] != 1 && (
 if($_SESSION['TrackInventory'] ==1 && $_SESSION['SelectOneItem']!='1'){
  if((($arrySaleItem[$Count]["evaluationType"] == 'Serialized' || $arrySaleItem[$Count]["evaluationType"] == 'Serialized Average')
           || (!empty($ItemEvType) && ($ItemEvType[0]["evaluationType"] == 'Serialized' || $ItemEvType[0]["evaluationType"] == 'Serialized Average') )) && $arrySaleItem[$Count]["DropshipCheck"] != 1){
        //End//
?>
        <a  class="fancybox slnoclass fancybox.iframe" href="addSerial.php?id=<?= $Line ?>&MainSku=<?=$mainSku?>&cond=<?=stripslashes($arrySaleItem[$Count]["Condition"])?>&WID=<?=stripslashes($arrySaleItem[$Count]["WID"])?>&OrderID=<?=$_GET['edit']?>" id="addItem" ><img src="../images/tab-new.png"  title="Serial number">&nbsp;Add S.N.</a>
        <?php }?>
...
<? }?>
       <td align="right">
	   <input type="hidden" name="discount<?=$Line?>" style="color:red;" id="discount<?=$Line?>" readonly="" class="disabled"  size="6" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["discount"])?>"/>
...
if($TotalAmount!=''){
			$TotalAmount = $TotalAmount -$TDiscount - $arrySale[0]['FreightDiscount'];
...
<?=$TaxCaption?> : <input type="text" align="right" name="taxAmnt" id="taxAmnt" class="textbox" value="<?=$taxAmnt?>" onkeypress="return isDecimalKey(event);" onblur="calculateGrandTotal(1);"  size="13" style="text-align:right;"/><br><br>


		

		<?  if(!empty($ActualFreight)){ ?>
		Actual Freight : <input type="text" align="right" name="ActualFreight" id="ActualFreight" class="disabled" readonly value="<?=$ActualFreight?>" size="13" maxlength="10" onkeypress="return isDecimalKey(event);"  style="text-align:right;"/>
		<br><br>
...
<!-- added by ali 10/may/2018 -->
<?php if(!empty($arrySale[0]['FreightDiscounted']) ) {
		 ?>Freight Discount: <input type="text" align="right" name=freightdiscount id="freightdiscount" class="textbox" value="<?php echo $arrySale[0]['FreightDiscount']; ?>" size="13" maxlength="10" onkeypress="return isDecimalKey(event);" onblur="calculateGrandTotal();" style="text-align:right;color:red;"/><br><br>   <?php } ?>
<input type="hidden" name="freightdiscounted" value="<?php echo $arrySale[0]['FreightDiscounted']; ?>">
<!-- added by ali 10/may/2018 -->
