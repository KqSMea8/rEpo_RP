...
function GlobalRestockingVal(){
    ...
    $("table.order-list").find("tr.itembg").each(function () {
		...
		document.getElementById("Restocking_fee").value = "";
     }
...
<tbody>
	...
	<input type="text" class="normal" name="item_taxable<?=$Line?>" id="item_taxable<?=$Line?>" value="<?=stripslashes($arryPurchaseItem[$Count]['Taxable'])?>" readonly size="2" maxlength="20"  />
       ...
       <td align="right"><input type="text" align="right" name="amount<?=$Line?>" id="amount<?=$Line?>" class="disabled ReAmnt" readonly size="15" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value="<?=stripslashes($arryPurchaseItem[$Count]["amount"])?>"/></td>
