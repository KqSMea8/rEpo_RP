 ...
 <table width="100%" id="myTable" class="order-list"  bgcolor="<?=$bgcolor?>" cellpadding="0" cellspacing="1">
    <tr align="left"  >
		...
		<td width="16%" class="heading" >Description</td>
	...
	?>
	 ...
	 <td><?=str_replace(",",", ",$arryPurchaseItem[$Count]["SerialNumbers"])?></td>
	...
	<? 
     ...
     <tr class='itembg'>
		...
		if($arryPurchase[0]['Restocking']==1 && $arryPurchase[0]['Restocking_fee']>0){
			$ReStocking = '<span style="color:red;">('.number_format($arryPurchase[0]['Restocking_fee'],2).')</span>';
...
if($arryPurchase[0]['Currency']!=$Config['Currency']){			
			...
			if($arryPurchase[0]['Restocking']==1 && $arryPurchase[0]['Restocking_fee']>0){
				...
				if($arryPurchase[0]['Currency']!=$Config['Currency']){			 
					echo '<br><span class="red">('.round(GetConvertedAmount($arryPurchase[0]['ConversionRate'], $ReStockingAmnt) ,2).' '.$Config['Currency'].')</span>';
				}
			}
