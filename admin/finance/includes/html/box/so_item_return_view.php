...
<table width="100%" id="myTable" class="order-list" cellpadding="0"
	...
	<tr align="left">
		...
		<td width="8%"  class="heading">Description</td>
	...
	for($Line=1;$Line<=$NumLine;$Line++) {
		...
		?>
	<tr class='itembg'>
		<td><?=stripslashes($arrySaleItem[$Count]["sku"])?>&nbsp;&nbsp;<a
			class="fancybox reqbox  fancybox.iframe"
			href="../sales/reqItem.php?id=<?=$Line?>&oid=<?=$arrySaleItem[$Count]['id']?>"
			id="req_link<?=$Line?>" <?=$ReqDisplay?>><img
			src="../images/tab-new.png" style="display: none;" border="0"
			title="Additional Items"></a></td>

		<td>
		<div <?=$style?>><?=stripslashes($arrySaleItem[$Count]["Condition"])?></div>
		</td>
		...
		<td><span style="display:<?=$TaxShowHide?>"> <? /*if(!empty($arrySaleItem[$Count]["RateDescription"]))
		echo $arrySaleItem[$Count]["RateDescription"].' : ';
		echo number_format($arrySaleItem[$Count]["tax"],2);
		*/

		?> </span> <?=$arrySaleItem[$Count]['Taxable']?></td>
	...
	<tr class='itembg'>
		<td colspan="18" align="right"><input type="hidden" name="NumLine"
			id="NumLine" value="<?=$NumLine?>" readonly maxlength="20" /> <input
