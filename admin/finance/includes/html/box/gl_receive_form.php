...
function GetCurrencyRate(CurrencyGL,Line){
	...
	if(ConfigCurrency!=CurrencyGL){
		var SendUrl ='action=getCurrencyRate&CustomerCurrency='+CurrencyGL+'&r='+Math.random(); 
		$.ajax({
			type: "GET",
			async:false,
...
<? $HideCurrency = ($CurrencySelected == $Config['Currency'])?('style="display:none"'):(''); ?>        
       ...
       <td align="right">
<a href="Javascript:AddGlToTransaction(<?=$Line?>);" id="addrog_<?=$Line?>" class="add" style="float:left;display:none;">Add</a>
