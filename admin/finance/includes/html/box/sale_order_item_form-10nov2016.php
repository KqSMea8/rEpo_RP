...
// var serial_value_sel = row.find('input[name^="serial_value"]').val();
            ...
            if (qty > 0) {
                var linkhref = $(this).attr("href") + '&total=' + qty + '&sku=' + serial_sku +'&SerialValue='+serial_value_sel+'&cond='+Condition;
...
<?php if(empty($disable)){?><a href="#" onclick="javascript:selectItem('<?=$Line?>','1');return false;" ><img src="../images/view.gif" border="0" title="Search"></a>&nbsp;&nbsp;<a class="fancybox reqbox  fancybox.iframe" href="reqItem.php?id=<?=$Line?>&oid=<?=$arrySaleItem[$Count]['id']?>" id="req_link<?=$Line?>" <?=$ReqDisplay?>><img src="../images/tab-new.png" style="display:none;" border="0" title="Additional Items"></a><?php }?>
		...
		<input data-OrgQty="y" type="hidden" name="Org_Qty<?=$Line?>" id="Org_Qty<?=$Line?>" value="<?=stripslashes($arrySaleItem[$Count]["qty"])?>" readonly=""/>
                <!--End--> 
               <?php	
	//2feb by chetan//
...
<select name="Condition<?=$Line?>" id="Condition<?=$Line?>" class="textbox" <?php if($_GET['edit']>0){ ?>onchange="getItemCondionQty('<?=stripslashes($arrySaleItem[$Count]['sku'])?>','<?=$Line?>',this.value)" <?php }?> style="width:110px;"><option value="">Select </option><?=$ConditionSelectedDrop?></select>
...
<?php if($_GET['invoice']!='' && $arrySaleItem[$Count]["RecurringCheck"] == 'on' ){   $RecDis =''; }else{ $RecDis ='display:none;'; } ?>
...
<textarea name="DesComment<?=$Line?>" id="DesComment<?=$Line?>" style="height:100px;" maxlength="100"><?=stripslashes($arrySaleItem[$Count]["DesComment"])?></textarea>
        ...
        <td><input <?=$disable?> data-qty="y" type="text" name="qty<?=$Line?>" id="qty<?=$Line?>" class="textbox" size="3" maxlength="6" onkeypress="return isNumberKey(event);" value="<?=stripslashes($arrySaleItem[$Count]['qty_invoiced'])?>"/>
...
?>
        <a  class="fancybox slnoclass fancybox.iframe addSerialItem" href="addSerial.php?id=<?= $Line ?>" id="addItem<?= $Line ?>" ><img src="../images/tab-new.png"  title="Serial number">&nbsp;Add S.N.</a>
        <?php }else{?>
...
<!--by chetan 9feb--->
<?php if($disable){?>
            <input  <?=$disable?>   type="text" name="price<?=$Line?>" id="priced<?=$Line?>" class="textbox" size="10" maxlength="15" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["price"])?>"/>
     <?php }else{?>        

 <input   type="text" name="price<?=$Line?>" id="price<?=$Line?>" class="textbox" size="10" maxlength="15" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["price"])?>"/>
<? }//End//?>



<input type="hidden" align="right" name="CustDiscount<?=$Line?>" id="CustDiscount<?=$Line?>" readonly class="disabled"  value="<?=$arrySaleItem[$Count]['CustDiscount']?>" size="13" style="text-align:right;"/>
<!---------Amit Singh-------->
       ...
       <td>
          <input type="text" <? if($arrySaleItem[$Count]["DropshipCheck"] == 1){?>style="display:none;"<?php }?> name="avgCost<?=$Line?>" id="avgCost<?=$Line?>" value="<?=$arrySaleItem[$Count]["avgCost"]?>" class="disabled textbox avgCost" maxlength="6" size="6" readonly>
           <input  <?php if($arrySaleItem[$Count]["DropshipCheck"] != 1 || $disable){?>style="display:none;"<?php }?> type="text" name="DropshipCost<?=$Line?>" id="DropshipCost<?=$Line?>"  class="textbox" size="6" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["DropshipCost"])?>"></td>
<!--By chetan 18Jan--->
       <td><input <?php if(empty($disable)){?> class="textbox" <?php }else{ echo $disable; }?> type="text" name="discount<?=$Line?>" id="discount<?=$Line?>" size="5" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["discount"])?>"/></td>
<!--End--->
