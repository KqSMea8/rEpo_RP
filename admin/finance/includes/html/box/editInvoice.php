<?php 
	...
	/*******Credit Card Process**********/
	if(!empty($_GET['OrderID']) && !empty($_GET['Amnt'])){
		$OrderID = base64_decode($_GET['OrderID']);			
		$Amnt = base64_decode($_GET['Amnt']);

		$arrySalesBrief = $objSale->GetSalesBrief($OrderID, '', ''); 
		$TotalInvoiceAmount = $arrySalesBrief[0]['TotalInvoiceAmount'];
		$CardOrderID = $OrderID;
		$PaymentTerm = $arrySalesBrief[0]['PaymentTerm'];
		include_once("includes/html/box/edit_invoice_credit.php"); 
 
 
		if($_GET['Action']=='PCard' && $AmountToCharge>0 && !empty($CreditCardFlag)){			
		...
		}else if($_GET['Action']=='VCard' && $AmountToRefund>0  && !empty($CreditCardFlag)){
			//$Config['SalesInvoiceExtraCharge'] = $_GET['ExtraCharge'];			 
			...
			$_SESSION['mess_Invoice'] = str_replace("[ErrorMSG]", "" , CARD_PROCESSED_FAILED);
		}
