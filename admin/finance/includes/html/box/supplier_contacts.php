...
<?	
  ...
  ?>
	...
	if(!empty($values["City"]))$ContactInfo .= ', <br>'.htmlentities($values["City"], ENT_IGNORE);
	if(!empty($values["State"]))$ContactInfo .=	', '.stripslashes($values["State"]);

	$ContactInfo .= ', <br>'.stripslashes($values["Country"]).' - '.stripslashes($values["ZipCode"]).'';
