...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<form name="form1" action="<?=$ActionUrl?>" method="post"
		...
		enctype="multipart/form-data"><? if (!empty($_SESSION['mess_supplier'])) {?>
	<tr>
	...
	<tr>
		...
		<table width="100%" border="0" cellpadding="5" cellspacing="0"
			...
			<tr>
				<td align="right" class="blackbold" width="40%">Company Name :<span
					class="red">*</span></td>
			...
			<tr>
				<td align="right" class="blackbold">Last Name :<span class="red">*</span>
				</td>
			...
			<tr>
				...
				<? for($i=0;$i<sizeof($arryCurrency);$i++) {?>
					<option value="<?=$arryCurrency[$i]['code']?>"
					<?  if($arryCurrency[$i]['code']==$CurrencySelected){echo "selected";}?>>
						<?=$arryCurrency[$i]['name']?></option>
						<? } ?>
				</select></td>
			</tr>
...
?> <input id="SupplierSince" name="SupplierSince" readonly=""
			...
			<tr>
				...
				<td align="left"><select name="ShippingMethod" class="inputbox"
					id="ShippingMethod">
					<option value="">--- None ---</option>
					<? for($i=0;$i<sizeof($arryShippingMethod);$i++) {?>
					<option value="<?=$arryShippingMethod[$i]['attribute_value']?>"
					<?  if($arryShippingMethod[$i]['attribute_value']==$arrySupplier[0]['ShippingMethod']){echo "selected";}?>>
						<?=$arryShippingMethod[$i]['attribute_value']?></option>
						<? } ?>
				</select></td>
			</tr>
	  ...
	  <tr>
		...
		-->
			...
			<? }else{
				...
				echo _("<div style='font-size:15px'>Vendor Have No Login Detail</div> <div><input name='Email' type='hidden' value='".$arrySupplier[0]['Email']."' /><input type='hidden' name='ganeratelogin' value='ganerate'><input type='hidden' name='AddType' value='LoginPermission'>  <input type='hidden' name='SuppID' id='SuppID' value='".$_GET['edit']."' /><input type='submit' name='ganerate' value='Ganerate' class='button'></div>");
				$HideSubmit=1;
			}
              } ?>
	...
	<tr>
		...
		<input name="Submit" type="submit" class="button" id="SubmitButton"
			value=" <?=$ButtonTitle?> " /> <input type="hidden" name="SuppID"
			id="SuppID" value="<?=$_GET['edit']?>" /> <input type="hidden"
			name="UserID" id="UserID" value="<?=$arrySupplier[0]['UserID']?>" />
