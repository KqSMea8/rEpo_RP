...
if(!empty($arrySaleTax)){
 ...
 /*for($i=0;$i<sizeof($arryBankAccountList);$i++) {
	$GlOptions .= '<option value="'.$arryBankAccountList[$i]['BankAccountID'].'">'.ucwords($arryBankAccountList[$i]['AccountName']).' ['.$arryBankAccountList[$i]['AccountNumber'].']</option>';
 }*/

$GlOptions='';
for($i=0;$i<sizeof($arryBankAccountList);$i++) {
	$GlOptions .= '<option value="'.$arryBankAccountList[$i]['BankAccountID'].'">'.ucwords(stripslashes($arryBankAccountList[$i]['AccountName'])).' ['.$arryBankAccountList[$i]['AccountNumber'].']</option>';
}
?>	
...
?>
		<option BankFlag="<?=$arryBankAccountList[$i]['BankFlag']?>" BankCurrency="<?=$BankCurrency?>" value="<?=$arryBankAccountList[$i]['BankAccountID']?>">
		<?=ucwords($arryBankAccountList[$i]['AccountName']);?>
...
<input type="hidden" name="BankCurrencySel<?=$Line?>" id="BankCurrencySel<?=$Line?>" value="">
</td>
...
<input type="text" onkeypress="return isDecimalKey(event);" maxlength="20" size="8"  class="textbox"  value="" id="BankConversionRate<?=$Line?>" name="BankConversionRate<?=$Line?>" onChange="Javascript:GetBankCurrencyRate(1);" style="display:none" >
...
<?    $JournalComment = !empty($arryJournalEntry[$Count]['Comment'])?($arryJournalEntry[$Count]['Comment']):(""); ?>


<input type="text" name="Comment<?=$Line?>" id="Comment<?=$Line?>" class="inputbox"  maxlength="100" value="<?=stripslashes($JournalComment)?>"/></td>
...
<tfoot>
	...
	<td>&nbsp;<input type="text" align="right" name="TotalDebit" id="TotalDebit" size="15" value="0.00" style="width:90px;" onkeypress="return isDecimalKey(event);" readonly/></td>
	<td>&nbsp;<input type="text" align="right" name="TotalCredit" id="TotalCredit" size="15" value="0.00" style="width:90px;" onkeypress="return isDecimalKey(event);" readonly /></td>
	<td>&nbsp;</td>
    </tr>
</tfoot>
</table>
