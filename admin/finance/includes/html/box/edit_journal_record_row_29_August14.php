...
<select name="AccountID<?=$Line?>" class="inputbox" id="AccountID<?=$Line?>" onChange="Javascript: SetAccountName(this.value,<?=$Line?>);">
	...
	<?php if(!empty($arryJournalEntry[$Count]['EntityName'])){?>
		...
		<span id="hideEntityName<?=$Line?>"><?=ucwords($arryJournalEntry[$Count]['EntityName'])?>&nbsp;(<?=ucfirst($arryJournalEntry[$Count]['EntityType'])?>)</span>
		<a href="javascript:void();" id="showName_<?=$Line?>" class="showEntityType">Change</a>
		<select name="EntityType<?=$Line?>" class="inputbox" id="EntityType<?=$Line?>" style="width:150px; display:none;" onChange="Javascript: CustomerList(<?=$Line?>);">
					<option value="">--- Select ---</option>
			...
			<select name="EntityID<?=$Line?>" class="inputbox" id="EntityID<?=$Line?>" style="width:150px;display:none;" onChange="Javascript: SetEntityName(this.value,<?=$Line?>);"><option value="">Loading...</option></select>
		<input type="hidden" name="EntityName<?=$Line?>" id="EntityName<?=$Line?>" value="">

		<a href="javascript:void();" id="hideEntityType_<?=$Line?>" class="showEntityData" style="display:none;" >Hide</a>
		<?php } else {?>
		<select name="EntityType<?=$Line?>" class="inputbox" id="EntityType<?=$Line?>" style="width:150px;" onChange="Javascript: CustomerList(<?=$Line?>);">
					<option value="">--- Select ---</option>
					<option value="customer">Customer</option>
					<option value="employee">Employee</option>
					<option value="supplier">Supplier</option>
...
<td>&nbsp;<input type="text" align="right" name="TotalDebit" id="TotalDebit" size="15" value="<?=$arryJournal[0]['TotalDebit']?>" style="width:90px;" /></td>
<td>&nbsp;<input type="text" align="right" name="TotalCredit" id="TotalCredit" size="15" value="<?=$arryJournal[0]['TotalCredit']?>" style="width:90px;" /></td>
	<td>&nbsp;</td>
    </tr>
</tfoot>
</table>
...
function CustomerList(numRow){
		...
		httpObj.onreadystatechange = function StateListRecieve(){
			if (httpObj.readyState == 4) {
                               ...
                               if(httpObj.responseText != '<option value="">--- Select ---</option>'){
				  document.getElementById("EntityID"+numRow).innerHTML  = httpObj.responseText;
                                }else{
                                    
                                     document.getElementById("EntityID"+numRow).innerHTML  = '';
                                     document.getElementById("EntityName"+numRow).value  = '';
                                }
				ShowHideLoader('');
