...
<table <?=$table_bg?>>
 ...
 $ClubMethodArry = array('prepayment', 'check', 'electronic transfer');
  ...
  if($num>0 || $BeginningBalance>0){
	...
	/*************************/
	/*************************/
	if($PrevName == $Name && $PrevPaymentDate == $values["PaymentDate"] && $PrevPaymentType == $values["PaymentType"] && $PrevMethod == $values['Method'] && in_array($Method,$ClubMethodArry) ){
	...
	if($SameCheck==0 && ($TotalClubPaid>0 || $TotalClubReceived>0)){		
		...
		echo '<td align="right">';
		if($TotalClubReceived>0 || $TotalClubReceived<0){echo number_format($TotalClubReceived,2);}
		echo '</td>';

		echo '<td align="right">';
		if($TotalClubPaid>0 || $TotalClubPaid<0){echo number_format($TotalClubPaid,2);}
		echo '</td>';
  ...
  ?>
    ...
    <td><?=(!empty($ReceiptID))?($ReceiptID):(stripslashes($values["ReferenceNo"]))?></td> 
    <td><?=$Name?></td> 
    <td>
...
if($TotalClubPaid>0 || $TotalClubReceived>0){		
	echo '<tr>
	<td>'.date($Config['DateFormat'], strtotime($PrevPaymentDate)).'</td>
	<td>'.$CheckRef.'</td>
	<td>'.$PrevName.'</td>
