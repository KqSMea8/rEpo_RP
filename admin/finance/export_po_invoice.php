...
$filename = "VendorInvoices_".date('d-m-Y').".xls";
if($num>0){
	...
	$header = "Invoice Date\tInvoice #\tPO Number\tVendor\tAmount\tCurrency\tInvoice Paid";
	...
	foreach($arryInvoice as $key=>$values){
		...
		$line = $PostedDate."\t".$values['InvoiceID']."\t".$values['PurchaseID']."\t".stripslashes($values["SuppCompany"])."\t".$values['TotalAmount']."\t".$values['Currency']."\t".$Paid."\n";
