<?php
	...
	if(!empty($_GET["f"]) && !empty($_GET["t"])){
		...
		for($i=$FromYear;$i<=$ToYear;$i++) {
			$arryNumOrder = $objPurchase->GetNumInvByYear($i,$_GET['f'],$_GET['t'],$_GET['s'],$_GET['p']);
			#$rand = rand(10,100);
			$valuesBar[$i] = $arryNumOrder[0]['TotalInvoice']+$rand;
...
$values_color=imagecolorallocate($img,73,86,58);//values color
		...
		imagestring	($img,2,$img_width-170,2, $xname, $txt_color);
