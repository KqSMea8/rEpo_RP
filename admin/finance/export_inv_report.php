...
$filename = "Purchase_Invoice_Report_".date('d-m-Y').".xls";
if($num>0){
	...
	foreach($arryInvoice as $key=>$values){
		...
		$line = $values['InvoiceID']."\t".$PostedDate."\t".$values['PurchaseID']."\t".$OrderDate."\t".stripslashes($values["SuppCompany"])."\t".$values['TotalAmount']."\t".$values['Currency']."\t".$Paid."\n";
