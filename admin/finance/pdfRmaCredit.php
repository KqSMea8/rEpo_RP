<?	require_once("../includes/pdf_comman.php");
	...
	$ModuleName = "Credit Number";

	if(!empty($_GET['o'])){
		$arryPurchase = $objPurchase->GetPurchaserma($_GET['o'],'','Credit');
		$OrderID   = $arryPurchase[0]['OrderID'];	
		if($OrderID>0){
			$arryPurchaseItem = $objPurchase->GetPurchaseItemCreditRMA($OrderID);
	...
	require_once("includes/pdf_po_order_credit.php");
	...
	$pdf->ezSetDy(-5);
    if($NumLine>0){
        $Head1 = '<b>SKU</b>'; $Head2 = '<b>Description</b>'; $Head3 = '<b>Qty Ordered</b>'; $Head4 = '<b>Qty Returned</b>';$Head5 = '<b>Unit Price</b>'; $Head6 = '<b>Taxable</b>'; $Head7 = '<b>Amount</b>'; 
		...
		for($Line=1;$Line<=$NumLine;$Line++) { 
			...
			$total_received = $objPurchase->GetQtyReceived($arryPurchaseItem[$Count]["id"]);
                        $subtotal_val = ($arryPurchaseItem[$Count]["qty_returned"])*($arryPurchaseItem[$Count]["price"]);
...
if(empty($arryPurchaseItem[$Count]['Taxable'])) $arryPurchaseItem[$Count]['Taxable']='No';
            ...
            $arryDef[$i][$Head4] = $arryPurchaseItem[$Count]["qty_returned"];
            $arryDef[$i][$Head5] = number_format($arryPurchaseItem[$Count]["price"],2);
            $arryDef[$i][$Head6] = $arryPurchaseItem[$Count]['Taxable'];
            $arryDef[$i][$Head7] = number_format($subtotal_val,2);
            $data[] = $arryDef[$i];
            $i++;

			$subtotal += $arryPurchaseItem[$Count]["amount"];
                        $subtotal_val_price += $subtotal_val;
        ...
        $pdf->ezTable($data,'','',array('cols'=>array($Head1=>array('justification'=>'left','width'=>'50'),$Head2=>array('justification'=>'left'),$Head3=>array('justification'=>'left','width'=>'60'),$Head4=>array('justification'=>'left','width'=>'90'),$Head5=>array('justification'=>'left','width'=>'40'),$Head6=>array('justification'=>'right','width'=>'80')), 'shaded'=>0,'shadeCol'=>array(0.9,0.9,0.9), 'showLines'=>2 , 'xPos' =>300 ,'width'=>500,'fontSize'=>8,'showHeadings'=>0, 'colGap'=>2, 'rowGap'=>2) );
        $pdf->setStrokeColor(0,0,0,1);

                $subtotal_val_price = number_format($subtotal_val_price,2);
		...
		$TotalTxt =  "Sub Total : ".$subtotal_val_price."\nTax : ".$taxAmnt."\nFreight : ".$Freight."\nGrand Total : ".$TotalAmount;
