...
$filename = "Vendor_Credit_Memo_".date('d-m-Y').".xls";
if($num>0){
	...
	$header = "Posted Date\tCredit Memo ID#\tVendor\tAmount\tCurrency\tStatus\tApproved";
	...
	foreach($arryPurchase as $key=>$values){
		...
		$line = $PostedDate."\t".$values["CreditID"]."\t".stripslashes($values["SuppCompany"])."\t".$values['TotalAmount']."\t".$values['Currency']."\t".stripslashes($values["Status"])."\t".$Approved."\n";
