...
//end
	/*if(!ValidateForSelect(frm.PaidFrom, "Payment Account")){//pk
		return false;
	}
	...
	if(frm.Method.value == "Check") {
       ...
       /* if(PDate < CurrentPeriodDate && BackFlag == 0) //pk
            {
...
function checkContra(){ 
	...
	if(BankCurrency==''){
		...
		SetVendorInvoice(SuppCode,0,1); //pk
...
function SetVendorInvoice(SuppCode,con,curPage){
		...
		var SendUrl = 'ajax_pay.php?action=getVendorInvoice&SuppCode='+SuppCode+'&TransactionID='+TransactionID+'&BankCurrency='+escape(BankCurrency)+'&PaymentDate='+escape(PaymentDate)+'&confirm='+con+'&r='+Math.random()+'&select=1&curP='+escape(curP)+'&key='+escape(SearchKey)+'&transferVendorFund=1'; 
...
function AddToTransaction(Line){
	...
	if(errorMsg!=''){
		...
		//$("#savedList").html('<strong>Saving.......</strong>');
		//$("#addrow_"+Line).hide(); 
		var sendParam='&action=AddToTransaction&SuppCode='+escape(SuppCode)+'&Amount='+escape(Amount)+'&InvoiceID='+escape(InvoiceID)+'&OrderID='+escape(OrderID)+'&Method='+escape(Method)+'&CheckNumber='+escape(CheckNumber)+'&ConversionRate='+escape(ConversionRate)+'&PaymentType=Invoice&FromVendorPayment=1&TransferFund=1&r='+Math.random();  
...
function ListTransaction(){
	...
	var sendParam='&TransactionID='+TransactionID+'&ContraTransactionID='+ContraTransactionID+'&action=ListTransaction&transferVendorFund=1&r='+Math.random();  
	if(TransactionID>0){
		$("#save_payment").attr("disabled",false);
		$("#save_payment").show();
	}else{
...
function AddCreditToTransaction(Line){
	...
	if(errorMsg!=''){
		...
		//$("#addrow_"+Line).hide(); 
		var sendParam='&action=AddToTransaction&SuppCode='+escape(SuppCode)+'&Amount='+escape(Amount)+'&CreditID='+escape(CreditID)+'&OrderID='+escape(OrderID)+'&Method='+escape(Method)+'&CheckNumber='+escape(CheckNumber)+'&ConversionRate='+escape(ConversionRate)+'&PaymentType=Credit&FromVendorPayment=1&TransferFund=1&r='+Math.random();  
		$.ajax({
			type: "GET",
			async:false,
			url: 'ajax_pay.php',
...
/*************************************/   
 function AddGlToTransaction(Line){
	...
	var Description = document.getElementById("DescGL"+Line).value;
	var TransactionID = $("#TransactionID").val();	
	var SuppCode = $("#SuppCode").val();
	var PaidAmount = parseFloat($("#PaidAmount").val());
	...
	if(errorMsg!=''){
		...
		$("#savedList").html('<strong>Saving.....</strong>');
		//$("#addrow_"+Line).hide(); 
		var sendParam='&action=AddToTransaction&SuppCode='+escape(SuppCode)+'&Amount='+escape(Amount)+'&AccountID='+escape(AccountID)+'&ConversionRate='+escape(ConversionRate)+'&Method='+escape(Method)+'&Description='+escape(Description)+'&CheckNumber='+escape(CheckNumber)+'&BankCurrency='+escape(BankCurrency)+'&PaymentType=GL&FromVendorPayment=1&TransferFund=1&r='+Math.random();  
		$.ajax({
			type: "GET",
			async:false,
			url: 'ajax_pay.php',
...
function setBankCurrency(ModuleCurrency){
	...
	if(SuppCode!=''){
		...
		var BankCurrency = $("#SuppCode :selected").attr("currency");
