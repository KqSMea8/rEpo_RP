...
function validateForm(frm){
	...
	if($("#Method").val() == "Credit Card"){		 
		if($('#CardCharge').is(':checked')) { 			 
			var InValidCard = CheckCardForm();
			if(InValidCard==1)return false;
		} 
	} 
	...
	if(document.getElementById("TotalOriginalAmount") != null){
		TotalOriginalAmount = parseFloat($("#TotalOriginalAmount").val());
	}
	if(parseFloat(ReceivedAmount) != parseFloat(TotalOriginalAmount)){
...
function checkContra(){  
	...
	if(BankCurrency==''){
		var PaidToAccount =$("#PaidTo option:selected").text();
		alert("Please define Bank Currency for "+PaidToAccount+".");
		document.getElementById("PaidTo").focus();		
	}else if(custID>0 && PaidTo>0){  
		if(ContraFlag==''){
...
function SetCustomerInvoice(custID,con){
	...
	var SendUrl = 'ajax_receipt.php?action=getCustomerInvoice&custID='+custID+'&TransactionID='+TransactionID+'&BankCurrency='+escape(BankCurrency)+'&PaymentDate='+escape(PaymentDate)+'&confirm='+con+'&r='+Math.random()+'&select=1&key='+escape(SearchKey); 
 ...
 function SetApPayAmntByCheck(line){
       ...
       remainInvAmnt = PaidAmount-parseFloat(invAmnt);
            ...
            if(document.getElementById("Vendor_invoice_check_"+line).checked){
                 ...
                 if(PaidAmount=='0' || PaidAmount!='' || remainInvAmnt > parseFloat(document.getElementById("vendorinvoice_amnt_"+line).value))
                     {
...
/***************************************************/
	...
	remainInvAmnt = ReceivedAmount-parseFloat(invAmnt);
		...
		if(ReceivedAmount=='0' || ReceivedAmount!='' || remainInvAmnt > parseFloat(document.getElementById("invoice_amnt_"+line).value)){
			document.getElementById("payment_amnt_"+line).value =  parseFloat(document.getElementById("invoice_amnt_"+line).value); 
		}else{ 
...
/*************************************/  
 ...
 function AddToTransaction(Line){
	...
	if(errorMsg!=''){
		...
		//$("#savedList").html('<strong>Saving.......</strong>');
		//$("#addrow_"+Line).hide(); 
		var sendParam='&action=AddToTransaction&CustID='+escape(CustID)+'&Amount='+escape(Amount)+'&InvoiceID='+escape(InvoiceID)+'&OrderID='+escape(OrderID)+'&Method='+escape(Method)+'&CheckNumber='+escape(CheckNumber)+'&ConversionRate='+escape(ConversionRate)+'&PaymentType=Invoice&FromCashReceipt=1&r='+Math.random();  
...
/*************************************/  
 function AddContraToTransaction(Line){
	...
	if(CustID>0 && SuppCode!=''){
		...
		$("#savedList").html('<strong>Saving.....</strong>');
		//$("#addrov_"+Line).hide(); 
		var sendParam='&action=AddToTransaction&CustID='+escape(CustID)+'&SuppCode='+escape(SuppCode)+'&Amount='+escape(Amount)+'&InvoiceID='+escape(InvoiceID)+'&ConversionRate='+escape(ConversionRate)+'&PaymentType=Contra Invoice&FromCashReceipt=1&OrderID='+escape(OrderID)+'&Method='+escape(Method)+'&CheckNumber='+escape(CheckNumber)+'&r='+Math.random();  
		$.ajax({
			type: "GET",
			async:false,
			url: 'ajax_receipt.php',
...
/*************************************/  
 function AddGlToTransaction(Line){
	...
	if(errorMsg!=''){
		...
		$("#savedList").html('<strong>Saving.....</strong>');
		//$("#addrow_"+Line).hide(); 
		var sendParam='&action=AddToTransaction&CustID='+escape(CustID)+'&Amount='+escape(Amount)+'&AccountID='+escape(AccountID)+'&ConversionRate='+escape(ConversionRate)+'&Method='+escape(Method)+'&CheckNumber='+escape(CheckNumber)+'&BankCurrency='+escape(BankCurrency)+'&PaymentType=GL&FromCashReceipt=1&r='+Math.random();  
		$.ajax({
			type: "GET",
			async:false,
			url: 'ajax_receipt.php',
...
function setBalanceAmount(){
	var BalanceAmount = 0;
	if(document.getElementById("TotalOriginalAmount") != null){
		BalanceAmount = parseFloat(document.getElementById("ReceivedAmount").value) - parseFloat(document.getElementById("TotalOriginalAmount").value);	
	}
 ...
 function ListTransaction(){
	...
	$.ajax({
		...
		success: function (responseText) { 			
			...
			var BalanceAmount = 0;
			if(document.getElementById("TotalOriginalAmount") != null){				
				//document.getElementById("ReceivedAmount").value = document.getElementById("TotalOriginalAmount").value;	
				BalanceAmount = parseFloat(document.getElementById("ReceivedAmount").value) - parseFloat(document.getElementById("TotalOriginalAmount").value);			
      ...
      function SetPayAmntByCheckCr(line){
       ...
       remainCrAmnt = ReceivedAmount-parseFloat(CrAmnt);
            ...
            if(document.getElementById("credit_check_"+line).checked){
                 ...
                 if(ReceivedAmount=='0' || ReceivedAmount!='' || remainCrAmnt > parseFloat(document.getElementById("credit_amnt_"+line).value))
                     {
...
function SetPayCreditAmount(opt){
	var CustomerCreditAmount = parseFloat(document.getElementById("CustomerCreditAmount").value);
	...
	if(opt==1){ //by checkbox
		if(document.getElementById("credit_amount_check").checked){
			document.getElementById("payment_credit_amount").value = CustomerCreditAmount; 
			...
			document.getElementById("openBalanceCredit").value = CustomerCreditAmount;
			document.getElementById("addrowcredit").style.display = 'none';  
		}
	}else{
		if(payment_credit_amount>0){
			if(payment_credit_amount>CustomerCreditAmount){
				alert("You can apply a maximum amount of "+CustomerCreditAmount+".");
				payment_credit_amount = CustomerCreditAmount;
				document.getElementById("payment_credit_amount").value = CustomerCreditAmount; 
			}
			openBalance = CustomerCreditAmount - payment_credit_amount;
...
/*************************************/  
 function AddCreditAmountToTransaction(){
	...
	if(CustID==''){
		...
		var sendParam='&action=CheckTransaction&CustID='+escape(CustID)+'&TransactionID='+escape(TransactionID)+'&PaymentType=CreditAmount&r='+Math.random(); 
		...
		$.ajax({
			...
			success: function (responseText) {  
				if(responseText==1){
					errorMsg = "Customer credit amount has been already saved by other user.";
				}else{
					errorMsg = responseText;
				}
			}
	...
	if(errorMsg!=''){
		...
		var sendParam='&action=AddToTransaction&CustID='+escape(CustID)+'&Amount='+escape(Amount)+'&PaymentType=CreditAmount&r='+Math.random();  
		$.ajax({
			type: "GET",
			async:false,
			url: 'ajax_receipt.php',
...
function setBankCurrency(ModuleCurrency){ 
	var BankCurrency = $("#PaidTo :selected").attr("currency");
