...
function ListTransaction(){
	...
	$.ajax({
		...
		success: function (responseText) { 			
			...
			if(document.getElementById("total_saved_payment") != null){				
				document.getElementById("PaidAmount").value = document.getElementById("total_saved_payment").value;
			}
...
/*************************************/   
 function AddGlToTransaction(Line){
	...
	if(errorMsg!=''){
		...
		$("#savedList").html('<strong>Saving.....</strong>');
		//$("#addrow_"+Line).hide(); 
		var sendParam='&action=AddToTransaction&SuppCode='+escape(SuppCode)+'&Amount='+escape(Amount)+'&AccountID='+escape(AccountID)+'&ConversionRate='+escape(ConversionRate)+'&PaymentType=GL&r='+Math.random();  
		$.ajax({
			type: "GET",
			async:false,
			url: 'ajax_pay.php',
