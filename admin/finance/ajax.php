	...
	switch($_GET['action']){
		...
		case 'SupplierAddress':
  		 ...
  		 case 'getCurrencyRate': 
			...
			$AjaxHtml = $ConversionRate;
		 	break;
		  case 'getBankCurrencyRate': 
			global $Config;			
			if($_GET['FromCurrency']!=$_GET['ToCurrency']){			
				$ConversionRate=CurrencyConvertor(1,$_GET['FromCurrency'],$_GET['ToCurrency'],$_GET['Module']);		 
			}else{
				$ConversionRate=1;
			}
        ...
        switch($_GET['actionn']){
			...
			if(!empty($_GET['Range'])){
			 ...
			 $arryAccount = $objBankAccount->getAccountTypeByRange($_GET['Range']);
		 	  if(!empty($arryAccount[0]["AccountTypeID"])){
