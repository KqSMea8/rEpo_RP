...
.controller('chatController',['$scope','$http','$location','$timeout',function($scope,$http,$location,$timeout){
	...
	thisObj.notificationnumber = 0;
	thisObj.notificationnumberWithRef={};

	thisObj.supportext=['xls','pdf','xlsx','doc','docx'];
	...
	identity.designation=(myinfo.designation)?myinfo.designation:'';
	...
	console.log(typeof(getCookie('eznetcrmchat'))+'--'+getCookie('eznetcrmchat'));
		...
		if(getCookie('eznetcrmchat')!='' && getCookie('eznetcrmchat')!=null && getCookie('eznetcrmchat')!='null'){
					...
					let tmpobj={
					 ...
					 $scope.$apply(function() {
					 	...
					 	console.log(tmpobj);
					   	...
					   	if(iiii==0){
					  	 	thisObj.currentchatwith=thisObj.chatwith[0];	
					   	}
			...
			console.log('eznetcrmchatlogin');
			console.log(getCookie('eznetcrmchatlogin'));
	...
	thisObj.changeChattab=function(index){
		...
		thisObj.currentchatwith=thisObj.chatwith[index];

		var notificationcount=thisObj.notificationnumberWithRef[thisObj.currentchatwith.refid];
		thisObj.notificationnumber -=parseInt(notificationcount);
		thisObj.notificationnumberWithRef[thisObj.currentchatwith.refid]=0;
	...
	thisObj.sendChat=function(keyupevent,index){
		...
		messagedata.url="";
		if(!thisObj.ChatDataByRoom[parseInt(thisObj.chatwith[index].refid)]){
			thisObj.ChatDataByRoom[parseInt(thisObj.chatwith[index].refid)]=[];
		}

		thisObj.ChatDataByRoom[parseInt(thisObj.chatwith[index].refid)].push(messagedata);
		thisObj.chatmessagetextarea[index]="";

		var objDiv = jQuery(".chat-wrap.active .chat-box");
	...
	thisObj.clickUploadlink=function(obj){		
		jQuery(angular.element('#uploader-input-'+obj)).trigger('click');		
	}
	...
	socket.on('config',function(data){	
		...
		addCommpanySettingCookies(data);
		 ...
		 $scope.$apply(function() {	 	
		 	...
		 	thisObj.identity.config=data;
		 	thisObj.identity.config.fileshareP=true;
	...
	socket.on('Agent-chat-history',function(data){
		...
		if(data.items.length > 0){
			for (var a in data.items){
				...
				messagedata.url="";
				console.log('ref id'+data.items[a].refid);
				if(!messageDatalist[data.items[a].refid]){
							messageDatalist[parseInt(data.items[a].refid)]=[];
					}
					console.log(messagedata);
				messageDatalist[parseInt(data.items[a].refid)].push(messagedata);	
		...
		console.log('Message List re');
		console.log(messageDatalist);
				$scope.$apply(function() {				
						
		   				thisObj.ChatDataByRoom=messageDatalist;	
		   				console.log(thisObj.ChatDataByRoom);			
				});
	});
	...
	socket.on('receive',function(data){
		...
		if(!thisObj.ChatDataByRoom[data.refid]){
		 ...
		 $scope.$apply(function() {
		   ...
		   if(thisObj.chatwith[thisObj.activeTab].refid!=data.refid)
		   {
		   	console.log('pre notification');
		   	console.log(thisObj.notificationnumber);
		  	 	if(Number.isNaN(thisObj.notificationnumber) ==true){
		  	 		thisObj.notificationnumber=0;
		  	 	}
		  	 	console.log('pre notification');
		   		console.log(thisObj.notificationnumber);
				thisObj.notificationnumber  = parseInt(thisObj.notificationnumber) + 1;

				if(!thisObj.notificationnumberWithRef[parseInt(data.refid)] ){
					thisObj.notificationnumberWithRef[parseInt(data.refid)]=0;
				}
				thisObj.notificationnumberWithRef[parseInt(data.refid)] +=1;
...
//	    $('#imageReceivedMessage').text("> "+m);
	...
	socket.on('visitor-close-chat', function(data){
		...
		var msg=chatwithData.name+' Left the Chat';
		let messagedata={};
		...
		if(!thisObj.ChatDataByRoom[parseInt(data.refid)]){
			thisObj.ChatDataByRoom[parseInt(data.refid)]=[];
		}
		$scope.$apply(function() {
					 thisObj.ChatDataByRoom[parseInt(data.refid)].push(messagedata);
					   });
				...
				if(userids[a]!=identity.myuserid){	
					 $scope.$apply(function() {
					 	thisObj.ChatDataByRoom[parseInt(data.refid)]=[];
					   	...
					   	if(thisObj.activeTab==0 && thisObj.chatwith.length > 0){
							thisObj.currentchatwith=thisObj.chatwith[0];
						}
	...
	$('body').on('change', '.uploder',function(e){
		...
		//return false;	
		console.log(thisObj.chatwith[thisObj.activeTab]);	
		 var refid=thisObj.chatwith[thisObj.activeTab].refid;	
		 var room=thisObj.chatwith[thisObj.activeTab].room;
		 var agentid=thisObj.chatwith[thisObj.activeTab].userid;
        ...
        reader = new FileReader();
         ...
         reader.onload = function(evt){
        	 ...
        	 console.log(ext);
        	 console.log(thisObj.supportext.indexOf(ext));
        	 if(thisObj.supportext.indexOf(ext)!==-1){
             var jsonObject = {
            ...
            //	console.log(jsonObject);
             socket.emit('user image', jsonObject);
        		 ...
        		 custompopup('invalid file format');
        	 }
         };
...
function addLoginCookies(data){
	...
	function clearAllCookies(){		
		...
		document.cookie="eznetcrmcompanysetting=;Path=/;";		
		document.cookie="addCommpanyChatAcceptCookies=;Path=/;";
		document.cookie="eznetcrmchatlogin=;Path=/;";
				
	}
	function clearAllCookiesNotLogin(){
		document.cookie="eznetcrmchat=;Path=/;";		
		document.cookie="eznetcrmcompanysetting=;Path=/;";		
		document.cookie="addCommpanyChatAcceptCookies=;Path=/;";
