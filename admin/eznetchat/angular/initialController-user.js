...
.controller('initialController',['$scope','$http','$location','$timeout','Upload','Cropper',function($scope,$http,$location,$timeout,Upload,Cropper){
	var thisObj = this;	
	var file, data;
	thisObj.tabs = [{
	...
	if(SignatureObj){
	thisObj.Allmysignature=InitialObj;
	}
...
thisObj.isshowbtnupload=function(index){
 ...
 thisObj.isnotmyimage=function(index){
    ...
    thisObj.CropDrawSignature=function(){ 
    	var c = document.getElementById("canvasinitial");
		var ctx = c.getContext("2d");    
		
		var myCanvas = document.getElementById('resizedrawcanvasinitial');
...
// 	     // (thisObj.preview || (thisObj.preview = {})).dataUrl = dataUrl;
 ...
 thisObj.clearDraw=function(){
	   ...
	   var canvas_buffer = document.getElementById('canvas_buffer_initial');
	      var contextbuffer = canvas_buffer.getContext('2d');
	      var canvas = document.getElementById('canvasinitial');
    ...
    thisObj.SaveSignature=function(url){    	
    			...
    			if(thisObj.currentTab=="typesignature" && jQuery(".signname-initial.active").length==0){
    			...
    			var h=parseFloat(jQuery(".signname-initial.active").height())+30;
    			var w=parseFloat(jQuery(".signname-initial.active").width())+30;
    			var c=jQuery(".signname-initial.active").attr("data-class");
    			jQuery(".content-inputhide-initial").addClass(c);
    			var fs=jQuery(".signname-initial.active").css("font-size");
    			var ff=jQuery(".signname-initial.active").css("font-family");
    			console.log(ff);
    			ff=extractText(ff);
    			...
    			fs=parseFloat(fs);
    			jQuery(".content-inputhide-initial").css("font-size",fs);
    			var ww=parseFloat(jQuery(".content-inputhide-initial").width());
    			var hh=parseFloat(jQuery(".content-inputhide-initial").height());
    		
	    			var rw=(w-ww)/2;
	    			var rh=(h-hh)/2;
    			
    				rh=rh+parseFloat(fs);
    				var c = document.getElementById("drawimage-initial");
    				...
    				ctx.fillText(jQuery(".inputbox-initial").val(), 8, fs-8);
    				SaveSignature(ctx.canvas.toDataURL());
    ...
    thisObj.AddSignature=function(){
    	...
    	var pagenumber=jQuery("#initials-modal").attr("data-page");
 
    	var left=jQuery("#initials-modal").attr("data-left");
    	var top=jQuery("#initials-modal").attr("data-top");
    	var code=jQuery("#initials-modal").attr("data-code");
    	...
    	console.log("width  "+jQuery(".class-signaturefield-"+code).find('img').width());
		console.log("height  "+jQuery(".class-signaturefield-"+code).find('img').height());
    	...
    	jQuery('#initials-modal').modal('hide');    	
    	
    	isinitial=1;
    	defaultinitial=thisObj.mysignature;
    	initialid=thisObj.ActiveMySignature;
    	setDefaultInitial(initialid);
    	if(isset=="undefined" ||  isset!= "false"){
      ...
      function setDefaultInitial(signatureId){
   		var request=jQuery.ajax({
		    	...
		    	data:{
		    		...
		    		initialid=signatureId;
		    		isinitial=1;
    				defaultinitial=thisObj.mysignature;	
    				jQuery("#changeInitial").show();		    		
		    	}
    		});    
    }
