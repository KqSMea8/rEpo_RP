...
<?php
	...
	********************************************/
	...
	//$Config['Env'] = "pilot"; //test

	//'------------------------------------
	//' Payflow API Credentials 
	//'------------------------------------
	/*$Config['API_User']="hostbillvirtual";	
	$Config['API_Password']="Mkb!C11";
	$Config['API_Vendor']="virtualstacks";
	$Config['API_Partner']="PayPal";*/

	$Config['API_User']=$arryProvider[0]['ppfUserId'];
	$Config['API_Password']=$arryProvider[0]['ppfPassword'];
	$Config['API_Vendor']=$arryProvider[0]['ppfVendor'];
	$Config['API_Partner']=$arryProvider[0]['ppfPartnerId'];

 
	
	// BN Code
	$Config['sBNCode'] = "PF-CCWizard";
	...
	*/
	...
	if ($Config['Env'] == "pilot") 
	{
		$Config['API_Endpoint'] = "https://pilot-payflowpro.paypal.com";
		$Config['PAYPAL_URL'] = "https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=";
	}
	else
	{
		$Config['API_Endpoint'] = "https://payflowpro.paypal.com";
		$Config['PAYPAL_URL'] = "https://www.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=";
	}
	...
	*/
	...
	'--------------------------------------------------------------------------------------------------------------------------------------------	
	*/
	function DirectPayment( $paymentType, $paymentAmount, $creditCardType, $creditCardNumber, $expDate, $cvv2, $firstName, $lastName, $street, $city, $state, $zip, $countryCode, $currencyCode, $orderdescription, $ORIGID)
	{
		...
		if ("Sale" == $paymentType)
		{
			...
			/*$nvpstr .= "&BA_DESC=purchase Time magazine&BILLINGTYPE=MerchantInitiatedBilling&PAYMENTTYPE=any
&BA_CUSTOM=magazine subscription";*/
		...
		elseif ("Authorization" == $paymentType)
		{
			$nvpstr = "&TENDER=P";
			//$nvpstr .= "&TRXTYPE=A";			 
			$nvpstr .= "&TRXTYPE=A&ACTION=D&AMT=1&ORIGID=".$ORIGID;
			 
		}
		elseif ("Detail" == $paymentType)
		{
			//https://developer.paypal.com/docs/classic/payflow/reporting/
			 
			$nvpstr .= "&TRXTYPE=I&ORIGID=".$ORIGID;
			 
		}
		elseif ("Void" == $paymentType)
		{
			
			 
			$nvpstr .= "&TRXTYPE=V&ORIGID=".$ORIGID;
			 
		}
		elseif ("Refund" == $paymentType)
		{
			
			 
			$nvpstr .= "&TRXTYPE=C&ORIGID=".$ORIGID;
			 
		}
		elseif ("Fee" == $paymentType)
		{
			
			 
			//$nvpstr .= "&TRXTYPE=A&ACTION=D&TENDER=P&AMT=100000&BAID=".$ORIGID;

			$nvpstr .= "&TRXTYPE=A&ACTION=D&TENDER=P&AMT=1&ORIGID=".$ORIGID;
		...
		if ("Detail" != $paymentType){
			...
			// the Payflow SDK.
			// By setting Verbosity to MEDIUM, you can view the processor's raw response values. This setting is more \93verbose\94
...
//echo $nvpstr;exit;
	...
	*/
	function hash_call($nvpStr,$unique_id)
	{
		...
		$API_Vendor = $Config['API_Vendor'];
		$API_Partner = $Config['API_Partner'];
		$PROXY_HOST = $Config['PROXY_HOST'];
		$PROXY_PORT = $Config['PROXY_PORT'];
		$USE_PROXY = $Config['USE_PROXY'];
		$sBNCode = $Config['sBNCode'];
		/**************************/
		...
		if (curl_errno($ch)) 
		{
			...
			//echo $_SESSION['curl_error_no'].' # '.$_SESSION['curl_error_msg'];exit;
