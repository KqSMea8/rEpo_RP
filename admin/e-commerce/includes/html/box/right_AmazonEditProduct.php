<div class="right-search">
  <h4>
    <? if(!empty($arryProduct[0]['Name'])) echo stripslashes($arryProduct[0]['Name']); ?>
  </h4>
<div class="right_box">
    <ul class="rightlink">
      <!--  <li <?= ($_GET['tab'] == "Basic") ? ("class='active'") : (""); ?>><a href="<?= $EditUrl?>Basic">Manage Listing</a></li>-->
      <li <?= ($_GET['tab'] == "ActiveItems") ? ("class='active'") : (""); ?>><a href="<?= $EditUrl ?>ActiveItems"> Active Items</a></li>
      <li <?= ($_GET['tab'] == "QueuedProducts") ? ("class='active'") : (""); ?>><a href="<?= $EditUrl ?>QueuedProducts"> Queued Products </a></li>
      <li <?= ($_GET['tab'] == "UnlistedProduct") ? ("class='active'") : (""); ?>><a href="<?= $EditUrl ?>UnlistedProduct"> Unlisted Product </a></li>
	  <li <?= ($_GET['tab'] == "SubmissionHistory") ? ("class='active'") : (""); ?>><a href="<?= $EditUrl ?>SubmissionHistory">Submission History</a></li>
    </ul>
<?php

 if(isset($fiterSet[0]['Value'])){
 $FiterData = (array) json_decode($fiterSet[0]['Value']);
}
 ?>
</div>	
                        ...
                        <label>Conditions: </label>
						 <div class="sel-wrap">
						 	<select class="inputbox" id="ItemCondition" name="ItemCondition" title="Condition" >
								...
								<option value="Collectible" <?=($FiterData['ItemCondition']=='Collectible') ? 'selected' : ''?>>Collectible</option>
								<option value="Refurbished" <?=($FiterData['ItemCondition']=='Refurbished') ? 'selected' : ''?>>Refurbished</option>
							</select>
						 </div>
					 </fieldset>
					 <fieldset>
