...
function validateVariant(frm){
	...
	if( ValidateForSimpleBlank(frm.variantname, "Variant Name")
		...
		&& ValidateForSelect(frm.required, "Values Required")
                     ...
                     //  if(document.getElementById('variantname').value.match(letters)){
                            /*code for option check*/

                        var optionvalid=true;
                        var group = $('input[name="mytext[]"]');
                           if (group.length > 0 && $('#mdlettersnum').is(":visible")){
                               group.each(function () {
                                 if(($(this).val()=='')){
                                    
                                     optionvalid= false;
                                     if($(this).parents('.optiondiv').find('.error').length==0){ 
                                       $(this).parents('.optiondiv').append('<div class="error">Please fill this field</div>');
                                     }else{ 
                                       $(this).parents('.optiondiv').find('.error').html('Please fill this field');
                                   }  
                                 }
                                 /*else if(!$(this).val().match(letterNumber)){
                                 window.alert("Special Characters Not Allowed");
                                 optionvalid= false;  
                                   if($(this).parents('.optiondiv').find('.error').length==0)
                                       $(this).parents('.optiondiv').append('<div class="error">Special Characters Not Allowed</div>');
                                   else
                                       $(this).parents('.optiondiv').find('.error').html('Special Characters Not Allowed');
                                 }*/else{
                                     
                                     $(this).parents('.optiondiv').find('.error').remove();
                                 }                           
                                      });
                           //}
                              if(optionvalid==true){
