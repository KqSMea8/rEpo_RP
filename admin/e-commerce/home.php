<?php
	...
	$arryCount=$objOrder->ListOrdersCurrentYear($_GET);
	$TotalOrders=$arryCount[0]['NumCount'];	
	/*******/ 
       ...
       // $GetCustomer = $objCustomer->getCustomers('','','','','');
        $GetCustomer = $objCustomer->getCustomersCurrentYear('','','','','');
        $TotalCustomer = $objCustomer->numRows();
        
        $GetCategory = $objCategory->GettotalActiveCategory();
        //$GetCategory = $objCategory->GettotalActiveCategoryCurrentYear();
        $TotalCategory = $objCategory->numRows();
        
        $GetProduct = $objProduct->GettotalActiveProduct();
        // $GetProduct = $objProduct->GettotalActiveProductCurrentYear();
		...
		for($i=1;$i<=$maxDays;$i++) {
			...
			$ttAmnt = $totalOrderAmnt;
			$value[$i-1] = (float)$ttAmnt;
			$month[] = $i;
		}
