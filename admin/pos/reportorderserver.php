...
/**************************************************/
	...
	if(!empty($_GET) && ($_GET['search'] =="Search")){
		...
		$OrderList =  $objorder->getOrderListbyOrderServer(array('vendor_id'=>trim($_GET['vendor']),'OrderStatus'=>trim($_GET['OrderStatus']),'f'=>trim($_GET['f']),'t'=>trim($_GET['t']),'OrderID'=>trim($_GET['OrderID']),'limit'=>$limit,'offset'=>$offset,'location'=>trim($_GET['location'])));
		
		$OrderListTotal =  $objorder->getOrderListbyOrderServer(array('vendor_id'=>trim($_GET['vendor']),'OrderStatus'=>trim($_GET['OrderStatus']),'f'=>trim($_GET['f']),'t'=>trim($_GET['t']),'OrderID'=>trim($_GET['OrderID']),'location'=>trim($_GET['location'])));
