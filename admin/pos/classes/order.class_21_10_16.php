...
class order extends dbClass
{
	...
	function getOrderListbyOrderType($where=array()){
		...
		$strSQLQuery =  "select  b.currency_code, sum(b.gross)as gross, sum(b.quantity) as quantity, sum(b.discount) as discount, sum(b.net) as net, sum(b.tax) as tax, sum(b.include_tax) as include_tax,b.order_type from (select a.*,(sum(a.item_price)+(if(a.modifiers_price IS NULL,0,a.modifiers_price))) as gross,
		sum(a.item_quantity)as quantity, sum(a.item_discount_price)+a.order_discount_price as discount, 
		sum(a.item_price)+(if(a.modifiers_price IS NULL,0,a.modifiers_price))- sum(a.item_discount_price)-a.order_discount_price +sum(a.item_total_tax_price)+a.order_total_tax_price as net,sum(a.item_total_tax_price)+a.order_total_tax_price as tax,sum(item_total_include_tax_price)+order_total_include_tax_price as include_tax
	...
	function getOrderListbyOrderServer($where=array()){
		...
		$strSQLQuery =  "select  b.currency_code,sum(b.gross)as gross, sum(b.quantity) as quantity, sum(b.discount) as discount, sum(b.net) as net, sum(b.tax) as tax, sum(b.include_tax) as include_tax,b.server_id,b.server_name from (select a.*,(sum(a.item_price)+(if(a.modifiers_price IS NULL,0,a.modifiers_price))) as gross,
		sum(a.item_quantity)as quantity, sum(a.item_discount_price)+a.order_discount_price as discount, 
		sum(a.item_price)+(if(a.modifiers_price IS NULL,0,a.modifiers_price))- sum(a.item_discount_price)-a.order_discount_price +sum(a.item_total_tax_price)+a.order_total_tax_price as net,sum(a.item_total_tax_price)+a.order_total_tax_price as tax,sum(item_total_include_tax_price)+order_total_include_tax_price as include_tax
