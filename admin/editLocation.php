<?php
	...
	if(!empty($_POST)) {
		...
		if(!empty($ValidateErrorMsg)){
			$_SESSION['mess_loc'] = $ValidateErrorMsg;
			$ActionUrl = 'editLocation.php';
			if (!empty($_POST['locationID'])) {
				$ActionUrl .= '?edit='.$_POST['locationID'];
			}
	...
	if($_GET['edit']>0){ 
		...
		if(empty($arryLocation[0]["locationID"])){
			header("location: ".$RedirectUrl);
			exit;
		}
	}else{
		...
		$arryLocation = $objConfigure->GetDefaultArrayValue('location'); 
	}
