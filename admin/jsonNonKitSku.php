...
$fetch = "select p1.Sku from inv_bill_of_material p1 inner join inv_items i on p1.item_id = i.ItemID where  LCASE(p1.Sku) like '".strtolower(trim($term))."%' and i.ItemType='Non Kit' order by p1.Sku Asc";
$query=mysql_query($fetch) or mysql_error($fetch);
 
while ($row = mysql_fetch_array($query)) {
    
    
    $inv_items[]=array( "label" => $row['Sku'], "value" => $row['Sku'] );
