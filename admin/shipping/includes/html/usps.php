...
if(!empty($_POST)){ 
	...
	$packageType=(!empty($_POST['packageType']))?($_POST['packageType']):('');
	...
	$AccountNumber=$DeliverySignature=$COD=$CustomValue=$InsureAmount=$CODAmount=$CollectionType=$PackageSize='';
	$DSOptionsType = $FirstClassMailType=$uspsLabel=$CustAccountNumber=$SaveCustAccount='';
...
{	
	...
	if(ValidateForSelect(form1.ShippingMethod, "Shipping Method")
			&& ValidateForSelect(form1.FirstClassMailType, "First Class Mail Type")
			&& ValidateForSimpleBlank(form1.PackageSize, "Package Size")
			&& ValidateForSelect(form1.AccountType, "Account Type")
			&& ValidateForSimpleBlank(form1.Weight, "Weight In Gram")
		...
		var pkt = document.getElementById("PackageSize").value;
...
<div id="accordion">
<?php include 'includes/html/box/from_usps_shipmet.php';?>
<?php include 'includes/html/box/to_usps_shipment.php';?>
</div>

<?php include 'includes/html/box/package_usps_shipment.php';?>
