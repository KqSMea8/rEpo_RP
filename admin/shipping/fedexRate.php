...
//$arryAddressTo = $objShipment->defaultAddressBook("ShippingTo");
	...
	$trackId=$_POST['track'];
	$multipleTrack=explode(":",$trackId);  
	$TotalAmount=0;
	$PostalCodeD=$_POST['PostalCodeD'];
	if(!empty($_POST['SuppCode'])){
		require_once($Prefix."classes/supplier.class.php");
		$objSupplier=new supplier();
		$PostalCodeS = $objSupplier->GetVendorZipCode('',$_POST['SuppCode'],'shipping');
	}else{
		$PostalCodeS=$_POST['PostalCodeS'];
	}
	//echo $PostalCodeS;exit;
if(!empty($_POST['track']) && !empty($PostalCodeS) && !empty($PostalCodeD)){

	for($i=0;$i<sizeof($multipleTrack);$i++){

    	$trackIdDeatil= trim($multipleTrack[$i]);
	 
	if(!empty($trackIdDeatil)){
    ...
    $request = $objTrack->trackRequest($trackIdDeatil);
    ...
    if(!empty($response->TrackDetails->CarrierCode)){	
		...
		$shipDate = $response->TrackDetails->Events->Timestamp;
...
$request['RequestedShipment']['ShipTimestamp'] = $shipDate; //date('c');
...
$TotalAmount += $rateReply->RatedShipmentDetails[0]->ShipmentRateDetail->TotalNetCharge->Amount;
