...
function addComment(){  
	...
	var module_type = $("#module_type").val();
	MultiComment = $("#Comment").val();
	if(newcomment.length < 2 && module_type==null){
		alert("Please enter comment!");
		return false;
	}
  	sendParam='Action=AddComment&module_type='+ module_type +'&view_type='+ view_type + '&comment='+newcomment+'&module_type='+module_type+'&MultiComment='+ MultiComment + '&order_id=<?=$_GET['edit'] ?>';
	$.ajax({
    type: "POST",
    async:false,
    url: 'ajax.php',
...
function deleteComment(commentID, masterCommentID){  
	...
	if(confirm("Are you sure you want to delete this comment!")==false){
  	...
  	sendParam='Action=DeleteComment&commentID='+ commentID +'&masterCommentID='+ masterCommentID + '&MultiComment='+ MultiComment+'&order_id=<?=$_GET['edit']?>';
	$.ajax({
    type: "POST",
    async:false,
    url: 'ajax.php',
                  ...
                  <?php 
                   ...
                   <tr>
					...
					<tr class="hideC">
                      <td width="30%" align="right" valign="top" =""  class="blackbold"> Comment :<span class="red">*</span> </td>
                      <td width="56%"  align="left" valign="top">
                       <textarea name="newcomment" id="newcomment" class="inputbox" maxlength="200" rows="6"><?=stripslashes($arryComments[0]['comment'])?></textarea>
					  </td>
                    </tr>
