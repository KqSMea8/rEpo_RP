...
//Added by chetan 21Sep. 2016//

//if(!empty($_POST['Submit']) || isset($_SESSION['process'])){//

if( $_POST['Submit']=='Save' || isset($_SESSION['process'])){
?>
...
function getCurrentStatus(){  
	var  sendParam='';
  sendParam='BackgroundExec=Customer&pid='+<?=$_SESSION['pid']?>+'&totalCount='+<?=$_SESSION['EXCEL_TOTAL']?>+'&randomval='+Math.random();
    ...
    success: function (responseText) { 
		var data = jQuery.parseJSON(responseText);
			if(data.per>=0){ 
     		...
     		if(parseInt(data.per)<100 && data.status=='1'){ setTimeout(function(){getCurrentStatus();}, 6000); return;}
			var mess_customer = '';
     		if(parseInt(data.per)>0 && data.status=='0'){ 
     			var tCount = <?=$_SESSION['EXCEL_TOTAL']?>;
     			mess_customer = "Total Customer to import from excel sheet : "+tCount;
    			mess_customer += "<br>Total Customer imported into database : "+data.count;
    			mess_customer += "<br>Customer already exist in database : "+ (tCount-data.count);
         		$("#customerComplete").show();
         		$("#ErrorMsg").html(mess_customer);
         	}else if(parseInt(data.count)=='0' && parseInt(data.per)=='0' && data.status=='0'){ 
          		var tCount = <?=$_SESSION['EXCEL_TOTAL']?>;
    			/*mess_customer += "<br>Customer already exist in database : "+ (tCount-data.count);
			mess_customer += "<br>Please try another excel!";*/

			mess_customer += "<br>Customer data has been imported successfully!";

         		$("#ErrorMsg").html(mess_customer);
			$("#customerImportProcess").remove();
         	}else { 
     			mess_customer = "Failed! Please try again.";
         		$("#ErrorMsg").text(mess_customer);
			$("#customerImportProcess").remove();
         	} 
    }
	});
...
#cper1{height: auto;}
              ...
              <form name="form1" id="customerImport" method="post" onSubmit="" enctype="multipart/form-data">
<table width="100%"  border="0" cellpadding="0" cellspacing="0" id="customerImportProcess">
         <tr>
                  <td align="left"  >
		</td>
		</tr>    
                ...
                <div class="unit size1of2 maintain-width"> 
				 ...
				 <td align="1center">
	 <input name="AssignTo" type="hidden" value="<?=(isset($_POST['AssignTo'])) ? $_POST['AssignTo'] : '';?>" />
	 <input name="FolderID" type="hidden" value="<?=(isset($_POST['FolderID'])) ? $_POST['FolderID'] : '';?>" />
	 <input name="Complete" type="submit" style="display:none" id="customerComplete" onClick="unhook()" class="button" value="Complete" />&nbsp;&nbsp;&nbsp;
	 <input name="Cancel" type="submit" id="customerCancel" onClick="unhook()" class="button" value="Cancel" />&nbsp;&nbsp;&nbsp;
				  </td></tr> 
...
<?=EXCEL_MAP_MSG?> 
	 ...
	 <table width="100%" border="0" cellpadding="10" cellspacing="1"  class="borderall">
		...
		?>
                    ...
                    <td  class="blackbold" valign="top" height="40"  align="right"> <?=$Heading?> : <?php //echo$mand?></td>
