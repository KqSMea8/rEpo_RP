...
function GetTerrLocation(){
	var TerritoryIDVal = $("#TerritoryID option:selected").map(function(){ return this.value }).get().join(",");
	if(TerritoryIDVal!=''){	
		$("#TerrLocation").html('<img src="../images/loading.gif">');
		var SendUrl = "&action=GetTerritoryLocation&TerritoryID="+escape(TerritoryIDVal)+"&r="+Math.random();
		...
		success: function (responseText) {			
			$("#TerrLocation").html(responseText);
			ShowManager();
...
<tr>
	...
	<!--select name="TerritoryID" id="TerritoryID" class="inputbox" onchange="Javascript:GetTerrLocation();">
	<option value="">--- Select ---</option>
	  <?php $objTerritory->getTerritoryOption(0,0,$arryTerritoryAssign[0]['TerritoryID']);?>
	<option value="None">--- None ---</option>
	</select-->

	<select name="TerritoryID[]" class="inputbox" id="TerritoryID" multiple style="height:150px;" onchange="Javascript:GetTerrLocation();">
