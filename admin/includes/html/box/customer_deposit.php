...
if($arryCustomer[0]['CustCode']!=''){
	...
	$LinkPrefix='';
	if($Config['CurrentDepID']!=8) $LinkPrefix = '../finance/';
...
<? if(sizeof($arryCash)>0){ ?>
<tr align="left"  >
	<td width="13%"  class="head1" >Cash Receipt No#</td>
	<td width="13%" class="head1" >Cash Receipt Date</td>
	<td width="13%"   class="head1" >GL Posting Date</td>
	<td width="20%"   class="head1" >Deposited To</td>
...
<?
	...
	$arryDtAmount=$objBankAccount->GetTransactionDataAmount($values["TransactionID"],$CustID,'');	
...
<td align="center">
  <a class="fancybox vSalesPayWidth fancybox.iframe"  href="<?=$LinkPrefix?>vCashReceipt.php?view=<?=$values['TransactionID']?>&CustID=<?=$CustID?>" onMouseover="ddrivetip('<center>View Receipt Summary</center>', 140,'')"; onMouseout="hideddrivetip()" ><strong><?=number_format($AmountTotal,2)?></strong></a>
