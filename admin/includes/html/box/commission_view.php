...
<tr>
	<td  align="right"  width="48%" valign="top"  >&nbsp;&nbsp;Sales Structure :</td>
...
<?=($arrySalesCommission[0]['SalesStructureType']==1)?('Subscription'):('Standard');?> 	
...
<? if($Dashboard==1 && $_SESSION['AdminType'] == "employee"){
	$TerritoryName = 'No territory assigned.';
	$arryTerritoryID = $objTerritory->GetTerritoryByAssign($_SESSION['AdminID']); 
	foreach($arryTerritoryID as $key=>$values){
		$arryTerritoryName = $objTerritory->GetTerritoryNameByID($values['TerritoryID']); 
		$TerritoryName = '';
		foreach($arryTerritoryName as $key2=>$values2){
			$TerritoryName .= stripslashes($values2['Name']).', ';
		}
		$TerritoryName = rtrim($TerritoryName,", ");
	}

?>
<tr>
	<td align="right" class="blackbold" valign="top" >
	Commission Tier and Teritory :
	</td>
	<td align="left" valign="top" >
	...
	if($arrySalesCommission[0]['TargetFrom']!='' && $arrySalesCommission[0]['TargetTo']!='')
	{
		...
		$Range = 'Range: '.$TargetFrom.' - '.$TargetTo;
		echo '<span class="button_small" onMouseover="ddrivetip(\'<center>'.$Range.'</center>\', 150,\'\')"; onMouseout="hideddrivetip()">Range</span>';
	}
	
	 echo ' <span class="button_small" onMouseover="ddrivetip(\'<center>'.$TerritoryName.'</center>\', 200,\'\')"; onMouseout="hideddrivetip()">Territory</span>';
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <!--<tr>
		<td  align="right"   valign="top"  width="48%">Spiff Tier :</td>
		<td align="left" valign="top">
...
<tr>  
	...
	<td align="left"><b><?=number_format($TotalSales,2).' '.$Config["Currency"]?></b></td>
  </tr>
 <!--tr>  
	<td align="right">&nbsp;&nbsp;Sales Amount for Commission : </td>
	<td align="left"><b><?=number_format($TotalSalesComm,2).' '.$Config["Currency"]?></b></td>
  </tr>	
 <tr>  
	<td align="right">&nbsp;&nbsp;Sales Amount for Spiff : </td>
	<td align="left"><b><?=number_format($TotalSalesSpiff,2).' '.$Config["Currency"]?></b></td>
  </tr-->
 <tr>  
	<td align="right">&nbsp;&nbsp;Sales Commission : </td>
	<td align="left"><b><?=number_format($TotalCommission,2).' '.$Config["Currency"]?></b></td>
  </tr>
