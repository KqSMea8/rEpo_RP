...
<tr>
  <td  valign="top" align="left">
	<? if($arryLog[0]['loginID']>0){ 
	 ...
	 if($arryLog[0]['UserType']=='employee'){
		$UserLink = '<a class="fancybox fancybox.iframe" href="userInfo.php?view='.$arryLog[0]['EmpID'].'&cmp='.$CmpID.'" >'.stripslashes($arryLog[0]["UserName"]).'</a>';
		$Email = $arryLog[0]['Email'];
	 }else if($arryLog[0]['UserType']=='customer'){
		$UserLink = '<a class="fancybox fancybox.iframe" href="custInfo.php?CustID='.$arryLog[0]['UserID'].'&cmp='.$CmpID.'">'.stripslashes($arryLog[0]['CustomerName']).'</a>';
		$Email = $arryLog[0]['CustomerEmail'];
	}else if($arryLog[0]['UserType']=='customerContact'){
		$UserLink = stripslashes($arryLog[0]['CustContactName']);
		$Email = $arryLog[0]['CustContactEmail'];
	}else if($arryLog[0]['UserType']=='vendor'){
		$UserLink = '<a class="fancybox fancybox.iframe" href="vendorInfo.php?SuppID='.$arryLog[0]['UserID'].'&cmp='.$CmpID.'" >'.stripslashes($arryLog[0]["VendorName"]).'<a>';
		$Email = $arryLog[0]['VendorEmail'];
	}else{
		$UserLink = ADMINISTRATOR;
...
?>
  ...
  <tr>
	  ...
	  <tr>
        <td  align="right"   class="blackbold"   > User Type : </td>
        <td><?=ucwords($arryLog[0]['UserType'])?></td>
      </tr>
	<tr>
...
<table <?=$table_bg?>>
	...
	<td width="20%"  class="head1" >Page Name</td>     
	<td width="20%"  class="head1" >Heading</td>
	<td  class="head1" >Url</td>
	<td width="20%" class="head1" >View Time</td>
...
<td><?=stripslashes($values["PageHeading"])?></td> 
<td><?=stripslashes($values["PageUrl"])?></td>  
<td>
<?  if($values['ViewTime']>0){
	echo date($Config['DateFormat']." ".$Config['TimeFormat'],strtotime($values['ViewTime']));
}
?>
</td>   
