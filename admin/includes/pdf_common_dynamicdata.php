...
//echo 'ddd';die;
  ...
  //echo $_GET['o'].'dd';
   ...
   //$_GET['listview']='1';
   //$_GET['tt']=1;
   $recordpdftemp1 = $objConfig->GetSalesPdfTemplate($_GET);
   //PR($recordpdftemp1);
   //PR($_SESSION);
   if(!empty($recordpdftemp1[0]['id'])){
     //echo $recordpdftemp1[0]['id'];
        //if($_SESSION['AdminType']=='admin' || $_SESSION['AdminID']==$recordpdftemp1[0]['AdminID']){
         $recordpdftemp=$recordpdftemp1;
        //}
   }
}
//echo '<pre>'; print_r($recordpdftemp);die;
/* * start company tab** */
$logoSize = (!empty($recordpdftemp[0]['LogoSize'])) ? ($recordpdftemp[0]['LogoSize']) : ('180');
//                echo $logoSize;die;
...
$infoNewMargin=($informationAlign == 'left') ? ('0') : ('180');
...
$SpecialSigned = (!empty($recordpdftemp[0]['SpecialSigned']))?($recordpdftemp[0]['SpecialSigned']):('');
}else{
  $SpecialSigned = (!empty($recordpdftemp[0]['SpecialSigned']))?($recordpdftemp[0]['SpecialSigned']):('Once signed, please Fax mail or e-mail it to the provided address.');
}*/
$SpecialSigned = (!empty($recordpdftemp[0]['SpecialSigned']))?($recordpdftemp[0]['SpecialSigned']):('');
/* * *end Special Tab*** */
$FooterContent = (!empty($recordpdftemp[0]['FooterContent'])) ? ($recordpdftemp[0]['FooterContent'] . '<br/><br/>') : ('');
...
$SalesPersonD = (!empty($recordpdftemp[0]['SalesPersonD'])) ? ($recordpdftemp[0]['SalesPersonD']) : ('show');
$CreatedByD = (!empty($recordpdftemp[0]['CreatedByD'])) ? ($recordpdftemp[0]['CreatedByD']) : ('show');

if ($_GET['ModuleDepName'] == 'Sales' || $_GET['ModuleDepName'] == 'SalesInvoice')
        { 

           if($SalesPersonD=='hide'){unset($Infodata['Sales Person']);}
           if($CreatedByD=='hide'){unset($Infodata['CreatedBy']);}
        }
      //PR($Infodata);
