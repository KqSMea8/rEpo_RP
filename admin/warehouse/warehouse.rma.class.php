...
class warehouserma extends dbClass {
 ...
 $strQuery = "insert into w_warehouse_location (WID,Address,city_id, state_id, ZipCode, country_id,OtherState,OtherCity)values('" . $WID . "','" . addslashes($Address) . "', '" . $main_city_id . "', '" . $main_state_id . "','" . addslashes($ZipCode) . "', '" . $country_id . "','" . $OtherState . "','" . $OtherCity . "' )";
    ...
    function GetReceiptRmaListing($OrderID, $ReceiptID, $Module) {
        ...
        $strSQLQuery = "select o.*,o.Freight as FreightAmt,o.taxAmnt as wtaxAmnt,o.wCode as wCode, s.CustID, s.ConversionRate from w_receipt o  left outer join s_order s on o.OrderID = s.OrderID   where 1" . $strAddQuery . " order by o.OrderID desc";
            ...
            if (!empty($id) && $arryDetails['qty' . $i] > 0) {
                ...
                if (!empty($Recipt_ID)) {
                    ...
                    if ($id > 0) {
                        ...
                        $sql = "insert into w_receipt_item(ReceiptID,OrderID, item_id, ref_id, sku, description, on_hand_qty, qty, qty_received,qty_invoiced,qty_returned,qty_receipt , price, tax_id, tax, amount, Taxable, req_item,SerialNumbers,Type,Action,Reason,discount,`Condition`) values('" . $receipt_id . "','" . $arryItem[$Count]["OrderID"] . "', '" . $arryItem[$Count]["item_id"] . "' , '" . $arryDetails['id' . $i] . "', '" . addslashes($arryItem[$Count]["sku"]) . "', '" . addslashes($arryItem[$Count]["description"]) . "', '" . $arryItem[$Count]["on_hand_qty"] . "', '" . $arryItem[$Count]["qty"] . "', '0', '0','0','" . $qty_receipt . "', '" . $arryItem[$Count]["price"] . "', '" . $arryItem[$Count]["tax_id"] . "', '" . $arryItem[$Count]["tax"] . "', '" . $arryDetails['amount' . $i] . "', '" . $arryItem[$Count]["Taxable"] . "', '" . addslashes($arryItem[$Count]["req_item"]) . "','" . $SerialNumbers . "', '" . $arryDetails['Type' . $i] . "', '" . $arryDetails['Action' . $i] . "', '" . $arryDetails['Reason' . $i] . "', '" . $arryDetails['discount' . $i] . "' , '" . $arryDetails['Condition' . $i] . "')";
                    ...
                    $sql1 = "insert into w_receipt_item(ReceiptID,OrderID, item_id, sku, description, on_hand_qty, qty, qty_received,qty_invoiced,qty_returned,qty_receipt , price, tax_id, tax, amount, Taxable, req_item,SerialNumbers,Type,Action,Reason,discount,`Condition`) values('" . $receipt_id . "','" . $arryItem[$Count]["OrderID"] . "', '" . $arryItem[$Count]["item_id"] . "' ,  '" . addslashes($arryItem[$Count]["sku"]). "', '" . addslashes($arryItem[$Count]["description"]) . "', '" . $arryItem[$Count]["on_hand_qty"] . "', '" . $arryItem[$Count]["qty"] . "', '0', '0','0','" . $qty_receipt . "', '" . $arryItem[$Count]["price"] . "', '" . $arryItem[$Count]["tax_id"] . "', '" . $arryItem[$Count]["tax"] . "', '" . $arryDetails['amount' . $i] . "', '" . $arryItem[$Count]["Taxable"] . "', '" . addslashes($arryItem[$Count]["req_item"]) . "','" . $SerialNumbers . "', '" . $arryDetails['Type' . $i] . "', '" . $arryDetails['Action' . $i] . "', '" . $arryDetails['Reason' . $i] . "', '" . $arryDetails['discount' . $i] . "' , '" . $arryDetails['Condition' . $i] . "' )";
	...
	function AddCreditSalesRmaByWarehouse($arryDetails,$totalAmountForCredit){
		...
		$strSQLQuery = "INSERT INTO s_order SET Module = 'Credit', OrderDate='".$OrderDate."', SaleID ='".$SaleID."', QuoteID = '".$QuoteID."', SalesPersonID = '".$SalesPersonID."', SalesPerson = '".addslashes($SalesPerson)."', InvoiceID = '".$InvoiceID."',
			...
			TotalAmount = '".$totalAmountForCredit."', TotalInvoiceAmount = '".addslashes($TotalInvoiceAmount)."', Freight ='0.00', taxAmnt ='".addslashes($taxAmnt)."', CreatedBy = '".addslashes($_SESSION['UserName'])."', AdminID='".$_SESSION['AdminID']."',AdminType='".$_SESSION['AdminType']."',PostedDate='".$Config['TodayDate']."',UpdatedDate='".$Config['TodayDate']."',
	...
	function AddReplacementSalesRmabyWarehouse($arryDetails,$totalAmountForReplacement){
     ...
     function GetSaleReceiptItem($OrderID, $SalesID){
        ...
        $strSQLQuery = "select i.*,c.valuationType as evaluationType,itm.ItemID,itm.CategoryID from w_receipt_item i  left outer join inv_items itm on i.item_id=itm.ItemID left outer join inv_categories c on c.CategoryID =itm.CategoryID where 1" . $strAddQuery . " order by i.id asc";
        return $this->query($strSQLQuery, 1);
    }
   ...
   function getWareHouseByAction($Action) {  
    ...
    function ReceiptNo($ReceiptNo, $editID) {
        $strSQLQuery = (!empty($editID)) ? (" and OrderID = '" . $editID . "'") : ("");
...
foreach($allserials as $serial){
	...
	function  warehouseRmaList($arryDetails)
	{
		...
		$strAddQuery = "where o.Module='RMA'  and o.Status!='Closed' and o.Status!='Parked'";
		$SearchKey   = strtolower(trim($key));
		...
		if($Config['GetNumRecords']==1){
			...
			$Columns = "   o.OrderDate, o.ReturnDate, o.OrderID, o.SaleID,o.ReturnID,o.InvoiceID, o.CustCode, o.CustomerName, o.TotalAmount, o.CustomerCurrency,o.ReturnPaid,o.SalesPerson   ";
			if($Config['RecordsPerPage']>0){
				$strAddQuery .= " limit ".$Config['StartPage'].",".$Config['RecordsPerPage'];
			}
	...
	$NumLine = sizeof($arryItem);
 	...
 	/******** Credit  ******/
  	if(sizeof($arrayCrData)>0){
		...
		foreach($arrayCrData as $arrayCrValue){
			$sqlCrd = "insert into s_order_item(OrderID, item_id, sku, description, on_hand_qty, qty,  price, amount, discount, req_item,SerialNumbers,`Condition`) values('".$CreditID."', '".$arrayCrValue['item_id']."' , '".addslashes($arrayCrValue['sku'])."', '".addslashes($arrayCrValue['description'])."', '".$arrayCrValue["on_hand_qty"]."', '".$arrayCrValue["qty"]."', '".$arrayCrValue["price"]."', '".$arrayCrValue['amount']."' , '".$arrayCrValue['discount']."',  '".addslashes($arrayCrValue["req_item"])."','".addslashes($arrayCrValue['SerialNumbers'])."','".addslashes($arrayCrValue['Condition'])."')";
	...
	function CustomerRmaPostToGL($ReceiptID){
		global $Config;		 
	 	if($ReceiptID>0){
			...
			$ConversionRate=1;
			$PaymentType = 'Customer RMA (Re-Stocking)';
			if($Restocking==1){
