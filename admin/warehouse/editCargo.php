<?php 
	/**************************************************/
	$ThisPageName = 'viewCargo.php'; $EditPage = 1;
	/**************************************************/
	...
	$ModuleName = "Cargo";
	$RedirectURL = "viewCargo.php?curP=".$_GET['curP'];
	if(empty($_GET['tab'])) $_GET['tab']="general";

	$EditUrl = "editCargo.php?edit=".$_GET["edit"]."&curP=".$_GET["curP"]."&tab="; 
	...
	/*
	 ...
	 if($_GET['del_id'] && !empty($_GET['del_id'])){
		$_SESSION['mess_supplier'] = SUPP_REMOVED;
		$objWarehouse->RemoveCargo($_GET['del_id']);
		header("Location:".$RedirectURL);
		exit;
	}
	 ...
	 if($_GET['active_id'] && !empty($_GET['active_id'])){
		$_SESSION['mess_supplier'] = SUPP_STATUS_CHANGED;
		$objWarehouse->changeCargoStatus($_GET['active_id']);
		header("Location:".$RedirectURL);
		exit;
	}
	 ...
	 if ($_POST) {
			...
			{
			//echo "hii";
			$UpdateId=$objWarehouse->UpdateCargo($_POST);
			$_SESSION['mess_supplier'] = GENERAL_UPDATED;
			$RedirectURL = "viewCargo.php";
			
			}
			 if (empty($_POST['LicenseNo']) && empty($_POST['SuppID'])) {
				$errMsg = ENTER_EMAIL;
			 } else {
				...
				if (!empty($_POST['SuppID'])) {
					...
					*/
					...
					if($objSupplier->isEmailExists($_POST['LicenseNo'],'')){
						$_SESSION['mess_supplier'] = EMAIL_ALREADY_REGISTERED;
					}else{	
						$LastInsertId = $objWarehouse->AddCargo($_POST); 
...
*/
	...
	if(!empty($_GET['edit'])) {
		$arryCargo = $objWarehouse->GetCargo($_GET['edit'],'','');
		$cargo_id  = $_GET['edit'];	
...
*/
	...
	//print_r($arryShippingMethod);
	//exit;
	$arryCustomField = $objConfigure->CustomFieldList($CurrentDepID,'Cargo','');
