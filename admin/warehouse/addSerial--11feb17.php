...
if(!empty($_GET['sku'])){
    ...
    $arrySerialNumber = $objPurchase->selectSerialNumberForItembyID($inoiceOrderID,$_GET['item_id'],$_GET['sku']);
    
    $allserials = explode(",",$arrySerialNumber[0]['SerialNumbers']);
	$serialValue=$objPurchase->getSerialDetails($sku,$condition,$allserials);
	
	
$selectedData=$objPurchase->GetOrderItembyOrderIDAndSKU($orderid,$sku,$condition);
