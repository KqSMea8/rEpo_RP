...
(empty($RtnInvoiceMess))?($RtnInvoiceMess=""):("");
?>	
...
<script language="JavaScript1.2" type="text/javascript">
	...
	$("table.order-list").on("click", "#ibtnDel", function (event) {
		...
		calculateGrandTotal();
            ...
            var bin = row.find('input[name^="bin"]').val();

             //alert(bin);
            if (qty > 0) {
                var linkhref = $(this).attr("href") + '&total=' + qty +'&binid='+bin;
	...
	$(elm).autocomplete({
		...
		minLength: 1,select: function( event, ui ) {
				...
				if(ui.item.id>0){
					 ...
					 jQuery('#bin'+lin).val(ui.item.id);
					 jQuery('#bnName'+lin).val(ui.item.label);
...
<?php if($_SESSION['InventoryLevel']==1){?>
<td width="5%" class="heading">Bin</td>
<? }?>
...
if(!empty($arrySaleItem[$Count]["qty_picked"]) && $arrySaleItem[$Count]["qty_picked"]>0 && $arrySaleItem[$Count]["qty_picked"] == $arrySaleItem[$Count]["qty"]){
$arrySaleItem[$Count]["qty_shipped"] = $arrySaleItem[$Count]["qty_picked"];
}


if(empty($arryShip[0]['ShipmentStatus'])) $arryShip[0]['ShipmentStatus']='';


		if($arrySaleItem[$Count]["qty_shipped"] == $arrySaleItem[$Count]["qty"]  || $arryShip[0]['ShipmentStatus'] == 'Shipped')
		{  
			...
			$textFld .= '<input type="hidden"  name="qty'.$Line.'" id="qty'.$Line.'" onkeypress="return isNumberKey(event);" maxlength="6" value ="'.$arrySaleItem[$Count]["qty_shipped"].'" size="5" data-qty="y"  class="textbox">';
			  ...
			  $textFld = '<input type="text" value="'.$arrySaleItem[$Count]["qty_shipped"].'" value="'.$ChildQty.'" name="qty'.$Line.'" data-qty="y" id="qty'.$Line.'" '.$childReadOnly.'  onkeypress="return isNumberKey(event);" maxlength="6" size="5"   class="'.$childClass.'">';
		  }
			if(!empty($_GET['ship'])){
...
#$arryBin = $objWarehouse->getBinByWarehouse($arrySaleItem[$Count]["WID"]);

if($_SESSION['InventoryLevel']==1 && !empty($arrySaleItem[$Count]['bin'])){ //binlocation based
 $arrayBin = $objItem->BinLocationByWarehouse($arrySaleItem[$Count]["WID"],$arrySaleItem[$Count]['bin']);

$binName = $arrayBin[0]['binlocation_name'];
//$binName = $arrayBin[0]['binid'];
...
<input type="text" name="bnName<?=$Line?>" id="bnName<?=$Line?>" class="textbox"    onclick="Javascript:SetAutoCompleteBin(this,<?=$Line?>);" size="6"   value="<?=stripslashes($binName)?>"/>
<input type="hidden" name="bin<?=$Line?>" id="bin<?=$Line?>" value="<?=stripslashes($arrySaleItem[$Count]['bin'])?>" readonly />




<!--<select name="bin<?=$Line?>" class="textbox" style="width:80px;" id="bin<?=$Line?>"  >
...
if($values['binid'] == $arrySaleItem[$Count]['bin']){
$selected2 = "selected";
}else{
...
$selected2 = "";
            ...
            echo '<option value="'.$values['binid'].'"  '.$selected2.'>'.$values['binlocation_name'].'</option>';
        }
    }else{
...
<?}?>
<td><div <?=$style?>>
<select name="Condition_val<?=$Line?>" disabled id="Condition_val<?=$Line?>" class="disabled"  <?php if($_GET['edit']>0){ ?>onchange="getItemCondionQty('<?=stripslashes($arrySaleItem[$Count]['sku'])?>','<?=$Line?>',this.value)" <?php }?> style="width:80px;"><option value="">Select </option><?=$ConditionSelectedDrop?></select>
...
<a  class="fancybox slnoclass fancybox.iframe" href="../finance/addSerial.php?id=<?= $Line ?>&cond=<?=$arrySaleItem[$Count]["Condition"]?>&total=<?=$arrySaleItem[$Count]["qty_shipped"]?>&sku=<?=stripslashes($mainSku)?>&WID=<?=$arrySaleItem[$Count]["WID"]?>&LineID=<?=$arrySaleItem[$Count]["id"]?>" id="addItem" ><img src="../images/tab-new.png"  title="Serial number">&nbsp;Select S.N.</a>
<? }else{?>
<a  class="fancybox slnoclass fancybox.iframe" href="../finance/addSerial.php?id=<?= $Line ?>&cond=<?=$arrySaleItem[$Count]["Condition"]?>&sku=<?=$mainSku?>&WID=<?=$arrySaleItem[$Count]["WID"]?>&LineID=<?=$arrySaleItem[$Count]["id"]?>" id="addItem" ><img src="../images/tab-new.png"  title="Serial number">&nbsp;Select S.N.</a>
<? }?>
        <?php }?>
             <textarea style="display:none;" name="serial_value<?= $Line ?>" id="serial_value<?= $Line ?>"><?=$arrySaleItem[$Count]["SerialNumbers"]?></textarea>
<textarea name="SerialValue<?= $Line ?>" style="display:none;" id="SerialValue<?= $Line ?>"><?=$arrySaleItem[$Count]["SerialNumbers"]?></textarea>
...
<!--td>
           ...
           <input  <?php if($arrySaleItem[$Count]["DropshipCheck"] != 1){?>style="display:none;"<?php }?> type="text" name="DropshipCost<?=$Line?>" id="DropshipCost<?=$Line?>"  class="textbox formactive" size="3" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["DropshipCost"])?>"></td-->
...
<input type="hidden" align="right" name="CustDisType" id="CustDisType" class="disabled"  value="<?=$arrySale[0]['CustDisType']?>" />
		...
		<div id="ActualFreightDiv" <?=($arrySale[0]['ActualFreight']>0)?(''):('style="display:none"')?>>Actual Freight : <input type="text" align="right" name="ActualFreight" id="ActualFreight" class="disabled" readonly value="<?=$arrySale[0]['ActualFreight']?>" size="13" style="text-align:right;"/>
		<br><br></div>
