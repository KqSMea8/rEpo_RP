...
<?php if ($_SESSION['SelectOneItem']!='1'){?>
		...
		<td width="8%" class="heading">Qty Pick</td>
		<?php }?>
...
$subtotal=0;$TotalQtyReceived=0;$TotalQtyOrdered=0;$costofgood=0;
	for($Line=1;$Line<=$NumLine;$Line++) { 
		$Count=$Line-1;	
		$total_receivedAry = $objSale->GetQtyInvoiced($arrySaleItem[$Count]["id"]);
		$total_received = (!empty($total_receivedAry[0]['qty_picked']))?($total_receivedAry[0]['qty_picked']):(0);
...
<td><div <?=$style?>><?=stripslashes($arrySaleItem[$Count]["warehouse_code"])?></div></td>
   ...
   <td><div <?=$style?>><?=stripslashes($arrySaleItem[$Count]["bin"])?></div></td>
...
<span class="heading">To Date:</span>&nbsp;<?php echo stripslashes($arrySaleItem[$Count]["ToDate"]); }?>
	...
	<?if($module!='Quote' && $module!='CreditNote'){?>
       <td><?=$arrySaleItem[$Count]["qty_picked"]?></td>
