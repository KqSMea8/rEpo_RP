...
<input type="hidden" name="req_item<?=$Line?>" id="req_item<?=$Line?>" value="<?=stripslashes($arryRMAItem[$Count]['req_item'])?>" readonly />
		...
		<td><input type="text" value="<?=$ValReciept;?>" size="4" readonly="" class="disabled">
			...
			?>
		      ...
		      <a  class="fancybox slnoclass fancybox.iframe" href="addSerial.php?id=<?  echo $Line ?>&OrderID=<?php echo $arryRMAItem[$Count]['OrderID']?>&item_id=<?php echo $arryRMAItem[$Count]['item_id']?>" id="addItem" ><img src="../images/tab-new.png"  title="Serial number">&nbsp;Select S.N.</a>
		        <?php }?>
