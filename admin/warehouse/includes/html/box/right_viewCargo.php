				...
				<form class="admin_r_search_form" action="<?=$SelfPage?>" method="get" enctype="multipart/form-data" name="form2" onSubmit="return LoaderSearch();">
                    <fieldset>
						...
						<select name="sortby" id="sortby">
								...
								<option value="s.Country" <? if($_GET['sortby']=='s.Country') echo 'selected';?>>Country</option>
								<option value="s.State" <? if($_GET['sortby']=='s.State') echo 'selected';?>>State</option>
								<option value="s.City" <? if($_GET['sortby']=='s.City') echo 'selected';?>>City</option>
