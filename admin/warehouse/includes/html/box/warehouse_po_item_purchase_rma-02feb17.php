...
<input type="hidden" name="req_item<?=$Line?>" id="req_item<?=$Line?>" value="<?=stripslashes($arryRMAItem[$Count]['req_item'])?>" readonly />
		...
		<td><?=$WhRMAType?>
		<input type="hidden" name="Type<?= $Line ?>" id="Type<?= $Line ?>" value="<?=$WhRMAType?>"  /></td>
		...
		<td><input type="text" value="<?=$ValReciept;?>" size="4" readonly="" class="disabled">
		     ...
		     <?php if($arryRMAItem[$Count]["qty"] == $ValReciept) { ?>
		      ...
		      <?php }else { 
				$QtyFlag=1;
			?>
		       <?=$textFld;?><?php if($arryRMAItem[$Count]["DropshipCheck"] != 1 && $arryRMAItem[$Count]["evaluationType"] == 'Serialized'){ ?>
		                    <br> <a  class="fancybox slnoclass fancybox.iframe" href="addSerial.php?id=<?= $Line ?>&OrderID=<?=$arryRMAItem[$Count]['OrderID']?>&item_id=<?=$arryRMAItem[$Count]['item_id']?>" id="addItem" ><img src="../images/tab-new.png"  title="Serial number">&nbsp;Select S.N.</a>
		        <?php }?>
...
<!--span style="display:<?=$TaxShowHide?>">
	   ...
	   <input type="text" align="right" name="amount<?=$Line?>" id="amount<?=$Line?>" class="disabled" readonly size="13" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value="<?=number_format($amount,2)?>"/></td>
...
<tfoot>
    ...
    <tr class='itembg'>
		...
		$Freight = $arryRMAItem[0]['Freight']; // number_format($arrySale[0]['Freight'],2);
		...
		Re-Stocking Fee: <input type="text" align="right" name="Restocking_fee"
                ...
                onblur="calculateGrandTotal();" style="width:130px;text-align:right;" /> <br><br>
		<?php } else { echo "";}?>


<?=$TaxCaption?> : <input type="text" align="right" name="taxAmnt" id="taxAmnt" class="disabled" readonly value="" size="13" style="text-align:right;"/><br><br>
