...
<td class="heading rest_td" width="5%" <?=($arryRMA[0]['Restocking']=='1')?(''):('style="display: none;"')?> >Re-Stocking Fee</td>
...
<tbody>
	...
	for($Line=1;$Line<=$NumLine;$Line++) { 
		...
		$qty_receipt = (!empty($arryRMAItem[$Count]["qty_receipt"]))?($arryRMAItem[$Count]["qty_receipt"]):('');
		$qty_returned = (!empty($arryRMAItem[$Count]["qty_returned"]))?($arryRMAItem[$Count]["qty_returned"]):('');
		
		if($qty_returned>0 && $qty_returned == $qty_receipt)
		{  
 		...
 		$tax_auths = (!empty($arryPurchase[0]['tax_auths']))?($arryPurchase[0]['tax_auths']):('');

		if($tax_auths=='Yes' && $arryRMAItem[$Count]['Taxable']=='Yes'){	
                    $TaxShowHide = 'inline';
		}else{
			$TaxShowHide = 'none';
		}
		...
		$ReqDisplay = !empty($arryRMAItem[$Count]['req_item'])?(''):('style="display:none"');
				...
				$warehouseSelectedDrop  =$objCondition-> GetWarehouseDropValue($arryRMAItem[$Count]["WID"]);
...
$mainSku = $arryRMAItem[$Count]["sku"];
	...
	$req_item = (!empty($arryRMAItem[$Count]['req_item']))?($arryRMAItem[$Count]['req_item']):('');
...
<input type="hidden" name="WID<?=$Line?>" id="WID<?=$Line?>" class="textbox"  value="<?=stripslashes($arryRMAItem[$Count]["WID"])?>"/>
 </div></td>
		    ...
		    <td>  
		     <?php if($arryRMAItem[$Count]["qty"] == $ValReciept) { ?>
		      ...
		      <?php }else { 
				$QtyFlag=1;
			?>
		       <?=$textFld;?><?php if( ($arryRMAItem[$Count]["evaluationType"] == 'Serialized' || $arryRMAItem[$Count]["evaluationType"] == 'Serialized Average')  ){ ?>
		                    <br> 
...
<!--span style="display:<?=$TaxShowHide?>">
	...
	<td class="rest_td" <?=($arryRMA[0]['Restocking']=='1')?(''):('style="display: none;"')?> ><input type="text" name="restocking_fee<?=$Line?>"
				id="restocking_fee<?=$Line?>"    class="textbox"
				size="3" maxlength="10" onkeypress="return isDecimalKey(event);"
				value="<?=stripslashes($arryRMAItem[$Count]["fee"])?>" <?=($arryRMAItem[$Count]['Type']=="C" || $arryRMAItem[$Count]['Type']=="AC")?(""):('style="display: none;"')?> /></td>


       <td align="right">
	   
	   <input type="text" align="right" name="amount<?=$Line?>" id="amount<?=$Line?>" class="disabled ReAmnt" readonly size="13" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value=""/></td>
...
<tfoot>
 ...
 <input type="hidden" name="RestockingGlobal" id="RestockingGlobal" value="<?=$RestockingGlobal?>" readonly maxlength="20"  />
		...
		<?	
		$taxAmnt = (!empty($arryRMAItem[0]['taxAmnt']))?($arryRMAItem[0]['taxAmnt']):('');
		
 
		$Freight = $arryRMA[0]['Freight'];  
		$Restocking_fee = $arryRMA[0]['Restocking_fee'];
		 
		$TotalAmount = (!empty($arryRMAItem[0]['TotalAmount']))?($arryRMAItem[0]['TotalAmount']):('');
		?>
		<br>
