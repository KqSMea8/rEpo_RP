...
<thead>
    <tr align="left" >
		...
		<td width="15%" class="heading">Transfer Qty</td>
...
<tbody>
	...
	for($Line=1;$Line<=$NumLine;$Line++) { 
		$Count=$Line-1;	
		if($arryTransferItem[$Count]["qty_received"] == $arryTransferItem[$Count]["qty"])
		{
		  $dd = "disabled";
		  $rd = "readonly";
		  }else{
		  ...
		  $remainQty = $arryTransferItem[$Count]["qty"]-$arryTransferItem[$Count]["qty_received"];
		  
	?>
     <tr class='itembg'>
		<td>
		 <input type="text" name="sku<?=$Line?>" id="sku<?=$Line?>" class="disabled" readonly size="10" maxlength="10"  value="<?=stripslashes($arryTransferItem[$Count]["sku"])?>"/>
        ...
        <td><?=stripslashes($arryTransferItem[$Count]["description"])?>
		<input type="hidden" name="description<?=$Line?>" id="description<?=$Line?>" class="textbox" style="width:150px;"  maxlength="50" onkeypress="return isAlphaKey(event);" value="<?=stripslashes($arryTransferItem[$Count]["description"])?>"/>
		</td>
        
        <!--td><input type="text" value="<?=$arryTransferItem[$Count]["qty_received"]?>" size="5" readonly="" class="disabled" id="received_qty<?=$Line?>" name="received_qty<?=$Line?>"></td-->
		<td><input type="text" value="<?=$arryTransferItem[$Count]["qty"]?>" size="5" readonly="" class="disabled" id="ordered_qty<?=$Line?>" name="ordered_qty<?=$Line?>"></td>
        ...
        <td><input type="text" name="price<?=$Line?>" id="price<?=$Line?>" readonly="" class="disabled" class="textbox" size="10" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arryTransferItem[$Count]["price"])?>"/></td>
		<!--td><input type="text" name="discount<?=$Line?>" id="discount<?=$Line?>" readonly="" class="disabled" class="textbox" size="10" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arryTransferItem[$Count]["discount"])?>"/></td-->
       <!--td>
		<input type="text" name="tax<?=$Line?>" id="tax<?=$Line?>" readonly="" class="disabled" value="<?=$arryTransferItem[$Count]['tax'];?>">
		<input type="hidden" name="tax_id<?=$Line?>" id="tax_id<?=$Line?>" readonly="" class="disabled"  value="<?=$arryTransferItem[$Count]['tax_id'];?>">
	   </td-->
       <td align="right">
	   
	   <input type="text" align="right" name="amount<?=$Line?>" id="amount<?=$Line?>" class="disabled" readonly size="15" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value="<?=stripslashes($arryTransferItem[$Count]["amount"])?>"/></td>
