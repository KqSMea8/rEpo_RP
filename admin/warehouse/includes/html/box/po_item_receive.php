...
<?php if ($_SESSION['SelectOneItem']!='1'){?>
   		<td width="7%" class="heading" >Warehouse</td>
...
<? }?>
		...
		<td width="6%" class="heading" >Qty Ordered</td>
		<td width="9%" class="heading" >Total Qty Received</td>
		<td width="8%" class="heading" >Qty Received</td>
		<!--td width="8%"  class="heading" >Unit Price</td>-->
<?php if ($_SESSION['SelectOneItem']!='1'){?>
		<td width="8%" class="heading">Dropship Cost</td>
		<td width="6%" class="heading" >Taxable</td>
<? }?>
		<!--<td width="10%" class="heading" align="center" >Amount Received</td>-->
    </tr>
</thead>
<tbody>
	<? $subtotal=$TotalQtyReceived=$amount=$TotalAmount=0;
	$QtyFlag=0;
...
//echo $total_received;
//if($total_received>2) $total_received  = $total_received/2; 
...
if($arryPurchase[0]['EDIRefNo']!='' && !empty($InvID)){
			$arrySerialItem = $ediObj->GetSaleSerialItem($InvID,$arryPurchaseItem[$Count]["sku"],$DisplayName);

#print_r($arrySerialItem);
			$arryPurchaseItem[$Count]['SerialNumbers'] = $arrySerialItem[0]['SerialNumbers'];
}
...
<? }?>
	...
	<td><?=$arryPurchaseItem[$Count]["qty"]?><input type="hidden" name="ordered_qty<?=$Line?>" id="ordered_qty<?=$Line?>" class="disabled" readonly size="5"  value="<?=$arryPurchaseItem[$Count]["qty"]?>"/></td>
	<td><?=number_format($total_received)?><input type="hidden" name="total_received<?=$Line?>" id="total_received<?=$Line?>" class="disabled" readonly size="5" maxlength="6" onkeypress="return isNumberKey(event);" value="<?=$total_received?>"/></td>
	<td><input type="<?=$QtyType?>" name="qty<?=$Line?>" id="qty<?=$Line?>" class="textbox" size="5" maxlength="6" onkeypress="return isNumberKey(event);" value=""/>
	<br>
	<?php  if(($arryPurchaseItem[$Count]["evaluationType"] == 'Serialized' || $arryPurchaseItem[$Count]["evaluationType"] == 'Serialized Average') && $arryPurchase[0]['OrderType'] != 'Dropship' && $total_received < $ordered_qty && $Config['SelectOneItem']!=1){ ?>
...
<textarea style="display:none;" name="serial_value<?= $Line ?>" id="serial_value<?= $Line ?>"><?=$arryPurchaseItem[$Count]["SerialNumbers"]?></textarea>
<textarea style="display:none;" name="serialdesc<?= $Line ?>" id="serialdesc<?= $Line ?>"></textarea>
	<input type="hidden" name="evaluationType<?= $Line ?>" id="evaluationType<?= $Line ?>" value="<?=$arryPurchaseItem[$Count]["evaluationType"]?>"  />
	<input type="hidden" name="DropshipCheck<?=$Line?>" id="DropshipCheck<?=$Line?>" class="textbox" value="<?=$arryPurchaseItem[$Count]["DropshipCheck"]?>"/>

<input type="hidden" name="price<?=$Line?>" id="price<?=$Line?>" class="disabled" readonly size="15" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=$arryPurchaseItem[$Count]["price"]?>"/>

<!--  Amount define -->

<input type="hidden" align="right" name="amount<?=$Line?>" id="amount<?=$Line?>" class="disabled" readonly size="13" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value="<?=number_format($amount,2)?>"/><input type="hidden" align="right" name="freight_cost<?=$Line?>" id="freight_cost<?=$Line?>" class="textbox" readonly size="5" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value=""/>
...
<? }?>
	<!--<td align="right"><input type="text" align="right" name="amount<?=$Line?>" id="amount<?=$Line?>" class="disabled" readonly size="13" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value="<?=number_format($amount,2)?>"/><input type="hidden" align="right" name="freight_cost<?=$Line?>" id="freight_cost<?=$Line?>" class="textbox" readonly size="5" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value=""/>
<input type="hidden" align="right" name="weight<?=$Line?>" id="weight<?=$Line?>" class="textbox" readonly size="5" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value="<?=$weight?>"/>
<input type="hidden" align="right" name="volume<?=$Line?>" id="volume<?=$Line?>" class="textbox" readonly size="5" maxlength="10" onkeypress="return isDecimalKey(event);" style="text-align:right;" value="<?=$volume?>"/>
</td>-->
...
?>
		...
		<input type="hidden" align="right" name="subtotal" id="subtotal" class="disabled" readonly value="<?=$subtotal?>" size="13" style="text-align:right;"/>
...
<?=$TaxCaption?> : <input type="text" align="right" name="taxAmnt" id="taxAmnt" class="disabled" readonly size="13" style="text-align:right;"/><br><br>
