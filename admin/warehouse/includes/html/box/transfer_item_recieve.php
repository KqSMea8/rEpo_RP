...
<thead>
    <tr align="left"  >
		...
		<td width="10%" style="display:none;" >Qty Ordered</td>
		<td width="13%" class="heading" >Transfer Qty</td>
...
<tbody>
	...
	for($Line=1;$Line<=$NumLine;$Line++) { 
		...
		if($ref==1){
			$qty_ordered = $arryTransferItem[$Count]["qty"];
          //echo $qty_ordered = $objWrecieve->GetQtyTransfer($arryTransferItem[$Count]["id"]);exit;	
		   $total_Received = $objWrecieve->GetTransferQtyRecieved($arryTransferItem[$Count]["id"]);
	...
	?>
     <tr bgcolor="<?=$bgcolor?>">
        <td><?=stripslashes($arryTransferItem[$Count]["sku"])?>
		...
		<input type="hidden" name="id<?=$Line?>" id="id<?=$Line?>" value="<?=$arryTransferItem[$Count]["id"]?>" readonly maxlength="20"  />
		</td>
        <td><?=stripslashes($arryTransferItem[$Count]["description"])?></td>

        <td><?=number_format($arryTransferItem[$Count]["qty"])?>
		<input type="hidden" name="total_received<?=$Line?>" id="total_received<?=$Line?>" class="disabled" readonly size="5"  value="<?=number_format($arryTransferItem[$Count]["qty"])?>"/></td>
       ...
       <td><?=number_format($arryTransferItem[$Count]["price"],2)?><input type="hidden" name="price<?=$Line?>" id="price<?=$Line?>" class="disabled" readonly size="15" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=$arryTransferItem[$Count]["price"]?>"/></td>
       <td> <? if(!empty($arryTransferItem[$Count]["RateDescription"]))
				echo $arryTransferItem[$Count]["RateDescription"].' : ';
				echo number_format($arryTransferItem[$Count]["tax"],2);
				
			?>
	   <select name="tax<?=$Line?>" id="tax<?=$Line?>" class="disabled" style="width:120px;display:none">
			<option value="0">None</option>
			<? for($i=0;$i<sizeof($arryPurchaseTax);$i++) {?>
			<option value="<?=$arryPurchaseTax[$i]['RateId'].':'.$arryPurchaseTax[$i]['TaxRate']?>" <? if($arryPurchaseTax[$i]['RateId']==$arryTransferItem[$Count]['tax_id']){echo "selected";}?>>
