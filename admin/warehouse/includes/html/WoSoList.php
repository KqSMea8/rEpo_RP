...
function SelectSO(OrderID){	
	ResetSearch();
	window.parent.location.href = "editWorkOrder.php?so="+OrderID;
}


function SlectSo(SaleID,OrderID,CustomerName,CustID,CustCode){
ResetSearch();
				window.parent.document.getElementById("SaleID").value=SaleID;
				window.parent.document.getElementById("OrderID").value=OrderID;
				window.parent.document.getElementById("CustomerName").value=CustomerName;
...
<!--<a href="Javascript:void(0);" onMouseover="ddrivetip('<?=CLICK_TO_SELECT?>', '','')"; onMouseout="hideddrivetip()" onclick="Javascript:SlectSo('<?=$values["SaleID"]?>','<?=stripslashes($values["OrderID"])?>','<?=stripslashes($values["CustomerName"])?>','<?=$values["CustID"]?>','<?=$values["CustCode"]?>');"><?=$values['SaleID']?></a>-->

<a href="Javascript:void(0);" onMouseover="ddrivetip('<?=CLICK_TO_SELECT?>', '','')"; onMouseout="hideddrivetip()" onclick="Javascript:SelectSO('<?=stripslashes($values["OrderID"])?>');"><?=$values['SaleID']?></a>

<!--a href="SoInvoiceList.php?key=<?=$values['SaleID']?>&link=editShipment.php" onMouseover="ddrivetip('<?=CLICK_TO_SELECT?>', '','')"; onMouseout="hideddrivetip()"><?=$values['SaleID']?></a--></td>
			<td>
