...
<TABLE WIDTH="100%"   BORDER=0 align="center" CELLPADDING=0 CELLSPACING=0 >
	<tr>
		...
		<? if($num>0){?>
		<input type="button" class="export_button"  name="exp" value="Export To Excel" onclick="Javascript:window.location='export_Picking.php?<?=$QueryString?>';" />
		...
		<?php  
         ...
         ?>
            <a href="SorderList.php?link=editPicking.php" class="fancybox add fancybox.iframe">Add <? if($_GET['module']=='Picking'){ echo 'Picking';}else{ echo 'Picking';}?></a>
        <?php }else{?>
            
            <a href="selectbatch.php" class="fancybox add fancybox.iframe">Add <? if($_GET['module']=='Picking'){ echo 'Picking';}else{ echo 'Picking';}?></a><!--added by chetan 6May-->
        <?php } } ?>
...
<table <?=$table_bg?>>
    ...
    <tr align="left"  >
      <td width="10%" class="head1" >Picking Date</td>
      <td width="10%"  class="head1"  >Picking #</td>
...
<span class="<?=$cls?>"><?=$values['ShipmentStatus']?></span>
      ...
      <td  align="center" class="head1_inner">
		...
		<a href="vPicking.php?view=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&ship=<?=$values['ShippingID'];?>&module=<?=$_GET['module']?>"><?=$view?></a>
<? if($arryInvoice[0]['PostToGL']!=1){?>
		<a href="editPicking.php?edit=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&ship=<?=$values['ShippingID'];?>&module=<?=$_GET['module']?>"><?=$edit?></a>
<? }?>
<? if($values['ShipmentStatus'] != 'Shipped'){?>
		<a href="editPicking.php?del_id=<?=$values['OrderID']?>&curP=<?=$_GET['curP']?>&ship=<?=$values['ShippingID'];?>&module=<?=$_GET['module']?>" onclick="return confirmDialog(this, 'Picking')"><?=$delete?></a> 
<? }?>
