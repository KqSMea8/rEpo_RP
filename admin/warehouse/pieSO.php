<?php
	...
	$OrderTotal = 0;
	$module = $_GET['module'];
	$ModuleName = "OutBound ".$_GET['module'];
	$_GET['y'] = date("Y");
	//$_GET['st'] = 'Completed';
	$_GET['st'] = '';
	...
	if(!empty($_GET['y'])){
		for($i=1;$i<=12;$i++) {
				...
				$arryNumOrder = $objShipment->GetNumShippingByYear($_GET['y'],$from,$to,'','',$_GET['st']);
				$valuesBar[$MonthName] = $arryNumOrder[0]['TotalShipping'];
				$OrderTotal +=  $arryNumOrder[0]['TotalShipping']; 
				$arryLegend[] = $MonthName;
				$arryValue[] = $arryNumOrder[0]['TotalShipping'];
