<?php 
	...
	if(!empty($_GET['po'])){
		...
		$RedirectURL = "viewPoReceipt.php?po=".$_GET['po'];
	}
...
if (!empty($_GET["customview"]) ) {
	...
	$colValue=$colRule='';
        ...
        foreach ($arryQuery as $colRul) {
                ...
                if ($colRul['columnname'] == 'ReceiptPaid') {
                   
      
                       if(strtolower($colRul['value'])=='paid' ){
                          $colRul['value'] = 1; 
				//$colRule .= " and o.ReceiptPaid='1'"; 
			}else if(strtolower($colRul['value'])=='unpaid' ){
                            $colRul['value'] = '';
				//$colRule .= " and o.ReceiptPaid!='1' and o.ReceiptPaid!='2'";
			}else {
                            $colRul['value'] = 2;
				//$colRule .= " and o.ReceiptPaid='2' ";
			}
...
/****************************End Custom Filter****************************************/
	...
	$Config['GetNumRecords'] = 1;
        $arryCount=$objPurchase->ListReceipt($_GET);
