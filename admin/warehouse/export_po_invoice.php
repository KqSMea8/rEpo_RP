...
$filename = "Purchase_Invoice_".date('d-m-Y').".xls";
if($num>0){
	...
	$header = "Invoice Date\tInvoice Number\tPO Number\tOrder Date\tVendor\tAmount\tCurrency\tInvoice Paid";
	...
	foreach($arryInvoice as $key=>$values){
		...
		$line = $PostedDate."\t".$values['InvoiceID']."\t".$values['PurchaseID']."\t".$OrderDate."\t".stripslashes($values["SuppCompany"])."\t".$values['TotalAmount']."\t".$values['Currency']."\t".$InvoicePaid."\n";
