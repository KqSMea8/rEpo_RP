...
if($num>0){
	...
	$header = "Return Date\tReturn Number\tPO Number\tOrder Date\tVendor\tAmount\tCurrency\tAmount Paid";
	...
	foreach($arryReturn as $key=>$values){
		...
		$line = $PostedDate."\t".$values['ReturnID']."\t".$values['PurchaseID']."\t".$OrderDate."\t".stripslashes($values["SuppCompany"])."\t".$values['TotalAmount']."\t".$values['Currency']."\t".$InvoicePaid."\n";
