<?php
	...
	$objBom=new bom();        
	$EditUrl = "editInternalBinOrder.php?edit=".$_GET["edit"]."&curP=".$_GET["curP"].""; 
	(!$_GET['curP'])?($_GET['curP']=1):(""); // current page number	
	/**************************************************/
		if((!empty($_GET['popup']))||(!empty($_GET['edit'])))  {
		
			if(!empty($_GET['popup']))	 {
				$arryAssemble = $objWarehouse->ListBinProduction($_GET['popup'],'','','','');
				...
				$arryAssemble = $objWarehouse->ListBinAssembly($_GET['edit'],'','','','');
		...
		if(!empty($_POST))	{
				...
				if(!empty($_GET['edit'])&&empty($_GET['popup']))	{
					...
					$updateItemID=$objWarehouse->updateAssemblyIntoBin($_POST,$NumLine,$arryAssemble[0],$_GET['edit']);
