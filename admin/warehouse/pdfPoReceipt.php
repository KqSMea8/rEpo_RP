...
$GenerateInvoice = ($arryPurchase[0]['GenrateInvoice']=='1')?("Yes"):("No");
	...
	$data = array(
		array($Head1 => "Receipt # :", $Head2 => $arryPurchase[0]["ReceiptID"]),
		array($Head1 => "Receipt Date :", $Head2 => $InvoiceDate),
		array($Head1 => "Item Received Date :", $Head2 => $ReceivedDate),
		array($Head1 => "Generate Invoice :", $Head2 => $GenerateInvoice),
			array($Head1 => "Receipt Status :", $Head2 => $arryPurchase[0]['ReceiptStatus']),
  ...
  /********* CODE FOR SERIAL NUMBER ***************/
	...
	// or write to a file
	$file_path = 'upload/pdf/'.$arryPurchase[0]["ReceiptID"].".pdf";
	$pdfcode = $pdf->output();
