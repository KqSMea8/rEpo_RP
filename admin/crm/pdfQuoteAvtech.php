<?	if($_SERVER['HTTP_REFERER'] == '') { echo exit(); }
...
require_once("../includes/pdf_comman.php");
	...
	require_once("includes/pdf_orderAvtech.php");
...
$arryDef[$i][$Head5] = $Head5;$arryDef[$i][$Head6] = $Head6;$arryDef[$i][$Head7] = $Head7;$arryDef[$i][$Head8] = $Head8;
	    ...
	    $description = stripslashes($arryQuoteItem[$Count]["description"]);
	    if(!empty($arryQuoteItem[$Count]["DesComment"]))  $description .= "\n<b>Comments: </b>".stripslashes($arryQuoteItem[$Count]["DesComment"]);


            $arryDef[$i][$Head1] = stripslashes($arryQuoteItem[$Count]["sku"]);
            $arryDef[$i][$Head2] = stripslashes($arryQuoteItem[$Count]["description"]);

 	     $arryDef[$i][$Head2] = $description;
            ...
            $arryDef[$i][$Head8] =$arryQuote[0]['CustomerCurrency']." ". number_format($arryQuoteItem[$Count]["amount"],2);
            $data[] = $arryDef[$i];
            $i++;
        ...
        $pdf->ezTable($data,'','',array('cols'=>array($Head1=>array('justification'=>'left','width'=>'60','showLines'=>2 ),$Head2=>array('justification'=>'left'),$Head3=>array('justification'=>'left','width'=>'60'),$Head4=>array('justification'=>'left','width'=>'90'),$Head5=>array('justification'=>'left','width'=>'60'),$Head6=>array('justification'=>'left','width'=>'80'),$Head7=>array('justification'=>'left','width'=>'42'),$Head8=>array('justification'=>'right','width'=>'80')), 'shaded'=>0,'shadeCol'=>array(0.9,0.9,0.9), 'showLines'=>-1 , 'xPos' =>300 ,'width'=>500,'fontSize'=>8,'showHeadings'=>0, 'colGap'=>6, 'rowGap'=>10) );
...
/***********/
		...
		$Freight = $arryQuote[0]['CustomerCurrency']." ".number_format($arryQuote[0]['Freight'],2);
...
$pdf->setLineStyle(0.5);
$pdf->ezText('<b>Term & Condition</b>',8,array('justification'=>'left', 'spacing'=>'1.5','shaded'=>0, 'showLines'=>0 , 'xPos' =>176 ,'width'=>250,'fontSize'=>12,'showHeadings'=>1, 'colGap'=>2, 'rowGap'=>2));
if($arryQuote[0]['9acf9a']!=''){
$pdf->ezSetDy(-20);
$pdf->ezText($arryQuote[0]['9acf9a'],8,array('justification'=>'left', 'spacing'=>'1.5'));
}
