...
if ($_GET['del_id'] && !empty($_GET['del_id'])) {
    $_SESSION['mess_opp'] = OPP_REMOVE;
    $objLead->RemoveOpportunity($_GET['del_id']);
    header("Location:" . $RedirectURL);
}
...
if ($_GET['active_id'] && !empty($_GET['active_id'])) {
    ...
    $objLead->changeOpportunityStatus($_GET['active_id']);
    header("Location:" . $RedirectURL);
}
...
//For array to string conversion by niraj 10feb16
    ...
    $objConfig->connect();
//echo "hello";die;
    $objLead->UpdateCountyStateCityforOpp($arryRgn, $_POST['leadID']);
...
if (!empty($_GET['edit'])) {
    ...
    $PageHeading = 'Edit Opportunity Detail for: '.stripslashes($arryOpportunity[0]['OpportunityName']);
    
    $OpportunityID = $_GET['edit'];
 ...
 if ($arryOpportunity[0]['AssignType'] == "Users" || $arryOpportunity[0]['AssignType'] == '') {
		$arryEmp = array();
        ...
        for ($i = 0; $i < sizeof($arryEmp); $i++) {
			...
			if(!isset($arryEmp[$i]['Image'])){ $arryEmp[$i]['Image'] ='';}
            if($arryEmp[$i]['Image'] == '') {
...
if (!empty($arryOpportunity) && $arryOpportunity[0]['Status'] != '') {
...
$arryHeadLead=$objField->getHead('',102,1);//By Chetan29/6/15//
//echo "<pre/>";print_r($arryHead);
