...
/**************************************************************/
	...
	$EditUrl = "trashEmail.php?edit=".$_GET["view"]."&module=".$_GET["module"]."&curP=".$_GET["curP"]."&tab="; 
	...
	$objConfig->connect();  
        ...
        if(isset($_GET['flag_id']) && !empty($_GET['flag_id']) && ($_GET['flag_id']> 0))
        {
            ...
            $RedirectURL = "trashEmail.php?curP=".$_GET['curP'];
        ...
        {
            /*********Get Email Data***********/ 
            $arryTrashItems = $objImportEmail->GetTrashEmail($OwnerEmailId,'',$activeDataEmailID[EmailId]);
          
          /*********Count Records************/
            $Config['GetNumRecords']=1;
            $arryCount = $objImportEmail->GetTrashEmail($OwnerEmailId,'',$activeDataEmailID[EmailId]);
            $num=$arryCount[0]['NumCount'];
        
        }else {
            /*********Get Email Data***********/ 
            $arryTrashItems = $objImportEmail->TrashEmailWithSearch($OwnerEmailId,'',$activeDataEmailID[EmailId],$_GET['sortby'],$_GET['key']);
            /*********Count Records************/
            $Config['GetNumRecords']=1;
            $arryCount = $objImportEmail->TrashEmailWithSearch($OwnerEmailId,'',$activeDataEmailID[EmailId],$_GET['sortby'],$_GET['key']);
            $num=$arryCount[0]['NumCount'];
        }
