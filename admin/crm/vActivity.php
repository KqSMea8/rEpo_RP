<?php
 ...
 /**************************************************/
    $ThisPageName = 'viewActivity.php';   if(!empty($_GET['pop']))$HideNavigation = 1;
    /**************************************************/
	...
	$EditUrl = "editActivity.php?edit=".$_GET["view"]."&module=".$_GET["module"]."&curP=".$_GET["curP"]."&mode=".$mode."&tab=".$_GET['tab']; 
	$ActionUrl = $EditUrl.$_GET["tab"];
   }
	...
	$objCustomer=new Customer(); 
		...
		/*****************************/
		if(!empty($_GET['act_id']) && !empty($_GET['select_del_id'])){
		...
		if (!empty($_GET['view'])) {
	     ...
	     $arryActivity = $objActivity->GetActivity($_GET['view'],'');
             $PageHeading = 'View Activity for: '.stripslashes($arryActivity[0]['subject']);
		...
		if($arryActivity[0]['AssignType'] == 'Group'){ 
		 ...
		 $AssignName = (!empty($arryAssignee)) ? $arryAssignee[0]['UserName'] : '';
		}
	...
	if($arryActivity[0]['RelatedType']!=''){
		...
		if($RelatedType=='Opportunity'){
			...
			$RelatedTitle = (!empty($arryCampaign)) ? stripslashes($arryCampaign[0]['campaignname']) : '';
