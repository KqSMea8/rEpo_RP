...
//echo "<pre>";print_r($arryCustomer);exit;
	...
	$salesPerson="";
			if(!empty($arryCustomer[0]['AssignType']) && $arryCustomer[0]['AssignType']=="User")
			{
			...
			else if(!empty($arryCustomer[0]['AssignType']) && $arryCustomer[0]['AssignType']=='Group')
                                           {
			...
			{
			$salesPerson= stripslashes($arryCustomer[0]["FullName"]);
			}
	
		if(($arryCustomer[0]['Status'])=="1")
		{
		$Status="Active";	
		}
		else 
	 ...
	 $Title ="Customer # ".$arryCustomer[0]['FullName'];
