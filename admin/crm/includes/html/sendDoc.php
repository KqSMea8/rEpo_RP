...
if(IsFileExist($Config['C_DocumentDir'], $arryDoc[0]['FileName']) ){
	$DocExist=1;
	?>	
	<a href="../download.php?file=<?=$arryDoc[0]['FileName']?>&folder=<?=$Config['C_DocumentDir']?>" class="download">Download</a>	
<? } else {
	$DocExist=0;
	echo NOT_UPLOADED;
}
...
<form name="formMail" action=""  method="post" onSubmit="return validateMail(this);" enctype="multipart/form-data">
   ...
   <tr>
        ...
        <td   align="left"  >
	<? if(!empty($arryCustomer) && sizeof($arryCustomer)>0){ ?>
	...
	<? }else{ ?>
         	<input type="text" name="ToEmail" id="ToEmail" value="<?=(isset($arryDoc[0]['CustomerEmail'])) ? stripslashes($arryDoc[0]['CustomerEmail']) : '';?>" class="inputbox" >
	<? } ?>	  
		 </td>
      </tr>
   <tr>
