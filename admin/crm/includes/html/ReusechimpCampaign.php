...
<table width="100%"  border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td   align="left" >
			<input name="title" type="text" class="inputbox" id="title" value="<?php echo !empty($reuselist['title'])?$reuselist['title']:'';?>"/></td>
			</tr>
			<tr>
			<td  align="right" width="40%"   class="blackbold">Subject: </td>
			<td   align="left" >
			<input name="subject" type="text" class="inputbox" id="title" value="<?php echo !empty($reuselist['subject'])?$reuselist['subject']:'';?>"/></td>
			</tr>
			<tr>
			<td  align="right" width="40%"  class="blackbold">From Email:<span class="red">*</span> </td>
			<td   align="left" >
			<input name="femail" type="email" class="inputbox" id="title" value="<?php echo !empty($reuselist['from_email'])?$reuselist['from_email']:'';?>"/></td>
			</tr>
			<tr>
			<td  align="right" width="40%"  class="blackbold">From Name:<span class="red">*</span> </td>
			<td   align="left" >
			<input name="frname" type="text" class="inputbox" id="title" value="<?php echo !empty($reuselist['from_name'])?$reuselist['from_name']:'';?>"/></td>
			</tr>
			<tr>
			...
			if (is_array($listtamplate['user']) && $num > 0) {
                  ...
                  <option name="Tempid[]" value="<?= (!empty($values['id'])) ? (stripslashes($values['id'])) : (NOT_SPECIFIED) ?>" <?php if($values['id']==$reuselist['template_id']){ echo 'selected'; } ?> ><?= (!empty($values['name'])) ? (stripslashes($values['name'])) : (NOT_SPECIFIED) ?></option>
			      ...
			      <?php 
                        if (is_array($ChimpSegmentList) && $num > 0) {
                            ...
                            foreach($ChimpSegmentList as $key => $values) {
                                ...
                                $LeadNo++;
                            ?>
                  <option name="Segid[]" value="<?= (!empty($values['segment_id'])) ? (stripslashes($values['segment_id'])) : (NOT_SPECIFIED) ?>" <?php if($values['segment_id']==$reuselist['segment_opts']['conditions'][0]['value']){ echo 'selected';}?>><?= (!empty($values['name'])) ? (stripslashes($values['name'])) : (NOT_SPECIFIED) ?></option>
                  <?php 
						} //end foreach
                  } //end if						
						 ?> 
		...
		<tr>
			...
			<div id="SubmitDiv" style="display:none1">
                        <input type="hidden" name="RcmpId" value="<?php echo !empty($reuselist['id'])?$reuselist['id']:'';?>"/>
