            ...
            <td  align="center" valign="top" >
                ...
                <table width="100%" border="0" cellpadding="5" cellspacing="0" class="borderall">
                    ...
                    <tr>
                        ...
                        <td  align="left" >
                            <input name="Name" type="text" class="inputbox" id="Name" value="<?php (!empty($EmailFolderDetails)) ? stripslashes($EmailFolderDetails[0]['FolderName']) : ''; ?>" onblur="<?php if (empty($_GET['edit'])) { ?> return CheckFolderName(this.value,'Add','');<?php } else { ?> return CheckFolderName(this.value,'Edit',<?=$_GET['edit']?>); <?php } ?>"  maxlength="80" onkeypress="return isCharKey(event);"/>
<input type="checkbox" name="IsPublic" <?php if(!empty($EmailFolderDetails) && $EmailFolderDetails[0]['IsPublic']) echo "checked";?> value="1"/> Public
...
<SCRIPT LANGUAGE=JAVASCRIPT>
    function CheckFolderName(FolderName,EditType,FolderID){
       ...
       {
        var  sendParam='';
		ModPID = <?=(isset($_SESSION['ModuleParentID'])) ? $_SESSION['ModuleParentID'] : '';?>
        sendParam='create_folder=checkFolderName&foldername='+FolderName+'&folderid='+FolderID+'&AdminId='+<?=$_SESSION['AdminID']?>+'&CompId='+<?=$_SESSION['CmpID']?>+'&ModuleID='+ModPID+'&randomval='+Math.random();
