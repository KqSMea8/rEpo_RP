...
function SelectOpp(OppName,OppID,LeadID){
	...
	*/
	...
	//if(LeadID>0){
		...
		var SendUrl = "&action=OppAddInfo&OppID="+escape(OppID)+"&r="+Math.random();		
		...
		$.ajax({
			...
			if(responseText["Currency"]!='')
			{
				window.parent.document.getElementById("CustomerCurrency").querySelector("option[value="+responseText["Currency"] +"]").selected = true;	//added on 23Jan2017 by chetan//
			}
	...
	//}else{
		...
		$.ajax({
			...
			success: function (responseText) {
					...
					if(responseText["Currency"]!='')
					{
						window.parent.document.getElementById("CustomerCurrency").querySelector("option[value="+responseText["Currency"] +"]").selected = true;
					}
					parent.jQuery.fancybox.close();
			}
...
<td width="22%"  class="head1" >Opportunity Name</td>
 ...
 <td><a href="Javascript:void(0);" onclick="Javascript:SelectOpp('<?=$values['OpportunityName']?>','<?=$values['OpportunityID']?>','<?=$values['LeadID']?>')" onMouseover="ddrivetip('<?=CLICK_TO_SELECT?>', '','')"; onMouseout="hideddrivetip()"><?=stripslashes($values['OpportunityName'])?></a></td>
