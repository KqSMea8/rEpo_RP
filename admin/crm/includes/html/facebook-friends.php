...
//	console.log('Access Token' + accessToken);
  ...
  FB.init({
     appId      : '1608313902793535',//'211257059004768',
  ...
  function testAPI() {
   ...
   // console.log('Welcome!  Fetching your information.... ');
    FB.api('/me?fields=gender,name,birthday,age_range,email,first_name,last_name,picture,friendlists,link', function(response) {
...
function logout() {
FB.logout(function(response) { window.location="http://dev.vstacks.in/erp_old/erp/admin/crm/facebook.php";// Amit Singh
...
<div id="status"></div>
<div id="login" style="text-align: center; margin-top: 10px;">

<button onclick="checkLoginState();" class="fblogin" style="float:none; display: none;" ></button>
...
<? } ?>
		...
		$i=0;
		 ...
		 unset($result);
			 $result = $facebook->api('/'.$userid['id'].'?fields=birthday,name,link,age_range,cover,first_name,last_name,gender'); //print_r($result); Amit Singh
				if($result['id']){
					echo '<li>';
					...
					echo '<div class="pdata"><div class="plable"><div class="plable"><label>Gender</label><span class="pcoln">:</span></div>&nbsp;<div class="presult">'.$result['gender'].'</div></div></div></div>';	
