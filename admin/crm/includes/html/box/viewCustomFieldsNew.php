...
if(is_array($arryField)){
    ...
    if($values['fieldname'] == 'CustType'){
        ...
        if($arrayVal['CustType']=='c'){$flabel = 'Customer'; $cusVal = $CustomerName;}
            ...
            echo ''.$intr.'
                    ...
                    {
                        $res = (isset($arrayVal[$values['fieldname']])) ? $arrayVal[$values['fieldname']] : '';
                    ...
                    }elseif($values['fieldname'] == 'Mobile')
                    {
                        //$res = $arrayVal[$values['fieldname']];
												...
												$res = (isset($arrayVal[$values['fieldname']])) ? $arrayVal[$values['fieldname']] : '';
												if(!empty($res)) {
												   $res = PhoneNumberFormat($res); 
												} 
												//  changes ended  by nisha for phone no patter
                        echo ($res)? $res.'<a href="javascript:void(0);" onclick=\"call_connect("call_form","to","'.$arrayVal[$values['fieldname']].'","'.$VId.'","'.$country_code.'","'.$country_prefix.'","'.$callfor.'")\" class="call_icon">
                        <span class="phone_img"></span></a>'
                        : '<span class="red">Not specified.</span>'; 
			    ...
			    $dateTime = '';
					...
					$sD = 'startDate'; $sT = 'startTime';
					
					if(!empty($arrayVal[$sD])){
						$dateTime = $arrayVal[$sD]." ".$arrayVal[$sT]; 
					}
                            }elseif($values['fieldname'] == 'closeDate'){ 
					$sD = 'closeDate'; $sT = 'closeTime';     
				...
				if(!empty($arrayVal[$sD])){
				$dateTime = $arrayVal[$sD];
				}
			    }
                           
                               
                            $res =  ((isset($arrayVal[$values['fieldname']]) && $arrayVal[$values['fieldname']] > 0)? date($Config['DateFormat']." ".$Config['TimeFormat'], strtotime($dateTime)) : '' );
                        }else{
                            $res =  ((isset($arrayVal[$values['fieldname']]) && $arrayVal[$values['fieldname']] > 0)? date($Config['DateFormat'], strtotime($arrayVal[$values['fieldname']])) : '' );    
                        } 
			 ...
			 $resStatus = (isset($arrayVal['Status'])) ? $arrayVal['Status'] : '';

                        if($resStatus == 1 || $resStatus==''  || $resStatus == 'Yes'){ echo "Active";}else{echo "InActive";}
                    ...
                    {
                        if($values['fieldname'] == 'Notification' && !empty($arrayVal['Notification']) ){ echo "Yes";}
                        elseif($values['fieldname'] == 'reminder' && !empty($arrayVal['reminder']) ){ echo "Yes";}
                        elseif($values['fieldname'] == 'sendnotification' && !empty($arrayVal['sendnotification']) ){ echo "Yes";}
                        elseif($values['fieldname'] == 'Taxable' && !empty($arrayVal['Taxable']) && $arrayVal['Taxable'] == 'Yes' ){echo "Yes";}else{echo "No";}
                        
                    }
  		
                    elseif($values['fieldname'] == 'DefaultAccount'){
 
			$DefaultAccountId = (isset($arrayVal[$values['fieldname']])) ? $arrayVal[$values['fieldname']] : '';
                    ...
                    {?>
                        ...
                        <script type="text/javascript">
                                        $(function () {
                                                var that = this;
                                                $("#jRate").jRate({
                                                        rating: "<?=(!empty($arryLead)) ? stripslashes($arryLead[0]['Rating']) : '';?>",
                                                        strokeColor: 'black',
                                                        readOnly: true,
                                                        width: 20,
                                                        height: 20,
 ...
 //echo (!empty($arrayVal[$values['fieldname']]))?($arrayVal[$values['fieldname']]).' ['.$arrayVal[$values['fieldlabel']].': '.stripslashes($arrayVal[$values['fieldname']]]).']'):(NOT_SPECIFIED);
                       ...
                       echo (isset($arrayVal[$values['fieldname']])) ? stripslashes($arrayVal[$values['fieldname']]) : NOT_SPECIFIED; 
                        }    
                    } 
...
/*****************/ 
             ...
             if($values['fieldname'] == 'RelatedType' && !empty($arrayVal['RelatedType']) )
                    {  
            
                         if($arrayVal['RelatedType']=='Lead'){
                            $relatedToValue = (isset($arrayVal['RelatedTo']))  ? $arrayVal['RelatedTo'] : '';
                            if($relatedToValue>0)$arryLead = $objLead->GetLeadBrief($relatedToValue);
                            $final = (!empty($arryLead)) ? $arryLead[0]['FirstName'].' '.$arryLead[0]['LastName'] : '';
                            ...
                            $final = (!empty($arryCampaign)) ? $arryCampaign[0]['campaignname'] : '';
