...
function GetSalesCommChart(){
	...
	var salesCommPersonType=document.getElementById("sp").value;
	var dataString="empId="+escape(salesCommEmp)+"&y="+escape(salesCommYr)+"&salesPerson="+escape(salesCommPersonType);
	 $('#divSalesReport').hide();
...
function GetSalesPerson(str,locationID){
	...
	var dataString="sp="+escape(str)+"&action=SalesPersonData";
...
if($sPersonType==0) {
	...
	<select name="sp" class="inputbox" onChange="Javascript: GetSalesPerson(this.value,'<?php echo $_GET['locationID']?>');" style="margin:10px;" id="adminSalesPerson">
		 ...
		 <option value="0" <?if(isset($_GET["sp"])){ echo ($_GET["sp"]=='0')?("selected"):("");}?>>Employees</option>
         <option value="1" <?if(isset($_GET["sp"])){ echo ($_GET["sp"]=='1')?("selected"):("");}?>>Vendor</option>
	</select>
<?php } ?>
