...
<? } ?>
		...
		$i=0;
			foreach($search_user['userdata'] as $result){
				if($result['id']){
					echo '<li>';
					echo '<div class="top"><div class="pfname"><a href="'.$result['link'].'" target="_blank">'.$result['name'].'</a></div><div class="image-set" align="center"><span class="pimg"><a href="'.$result['link'].'" target="_blank"><img src="https://graph.facebook.com/'.$result['id'].'/picture?type=small&width=80&height=80"></a></span>';
			...
			<!--<a href="javascript:void(0);" onclick="jQuery(this).parents(\'li\').find(\'.add-dashboard\').toggleClass(\'active\')">Add To CRM</a> -->
			       ...
			       <span class="exiting-profile"><a name="Add Existing" title="Add Existing" href="javascript:void(0);" onclick="addtoexistingcrm(\''.$i.'\')"></a></span></div>';
					...
					echo '<div class="pdata"><div class="plable"><label>Screen Name</label><span class="pcoln">:</span></div>&nbsp;<div class="presult">'.$result['username'].'</div></div>';
