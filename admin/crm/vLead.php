...
if(!empty($_GET['popLead'])){  $HideNavigation = 1; $hide = 1; } 
/**************************************************/
...
//By chetan 4Dec//
		  ...
		  if($_POST['AddRemd']!='')
		  {
		  	$Res =  $objActivity->AddQuickActivityReminder($_POST);    
		  	header("Location:".$ViewUrl."Event");
		  }
...
//End//
		...
		if(isset($_GET['select_del_id']) && $_GET['select_del_id'] && !empty($_GET['select_del_id'])){
		...
		if (!empty($_GET['view'])) {
			 ...
			 $objLead->updateAsViewMarked($_GET['view'],'c_lead','leadID');
		...
		if($_GET['tab']=="Event"){
			...
			$arryActivityEvent=$objActivity->ListActivity('','lead',$_GET['view'],$_GET['key'],$_GET['sortby'],$_GET['asc']);
			$numEvent=$objActivity->numRows();
			$pagerLink=$objPager->getPager($arryActivityEvent,$RecordsPerPage,$_GET['curP']);
			(count($arryActivityEvent)>0)?($arryActivityEvent=$objPager->getPageRecords()):("");
