<?php 
   ...
   if (!empty($_GET['PostEdi'])) {
   if($_GET['receipt']==1){
    $recID = $objPurchase->allowEDIdata($_GET['PostEdi'],'Receipt',$_GET['refd'],$_GET['invid']);
    $_SESSION['mess_purchase'] = 'PO Edi Receipt has been generated successfully ';
    }else{
    $recID = $objPurchase->allowEDIdata($_GET['PostEdi'],'Receipt',$_GET['refd'],$_GET['invid']);
    if(!empty($recID)){
    $invId = $objPurchase->allowEDIdata($recID,'Invoice',$_GET['refd'],$_GET['invid']);
    }
    $_SESSION['mess_purchase'] = 'PO Edi Receipt & invoice has been generated successfully ';
   }
    #$objPurchase->CreateCloneOrder($_GET['PostEdi'],'Invoice');
    header("location:viewPO.php?module=Order");
    exit;
}
