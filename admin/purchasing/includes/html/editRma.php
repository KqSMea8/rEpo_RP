...
function validateForm(frm){
	...
	var OrderTy = '<?=$arryPurchase[0]['OrderType']?>';
...
if(document.getElementById("serial_value"+i).value == '' && document.getElementById("qty"+i).value!=''){
		if((document.getElementById("evaluationType"+i).value == 'Serialized' || document.getElementById("evaluationType"+i).value == 'Serialized Average') && document.getElementById("DropshipCheck"+i).value != 1 && OrderTy!='Dropship'){
...
<input name="ReturnID" type="text" class="datebox" id="ReturnID" value="<?=$arryPurchase[0]['ReturnID']?>"  maxlength="20" onKeyPress="Javascript:ClearAvail('MsgSpan_ModuleID');return isUniqueKey(event);" oncontextmenu="return false"  onBlur="Javascript:ClearSpecialChars(this);CheckAvailField('MsgSpan_ModuleID','ReturnID','<?=$_GET['edit']?>');"  />
...
<input id="ExpiryDate" name="ExpiryDate" readonly="" class="datebox" value="<?=$ExpiryDate?>"  type="text" > 
	...
	<td   align="left" >
      ...
      <? if($arryPurchase[0]['EDIRefNo']!=''){?>
  <tr>
<td  align="right"   class="blackbold" > Edi Ref No  : </td>
			<td   align="left" >
	<input name="EDIRefNo" type="text" class="disabled" id="EDIRefNo" readonly value="<?php echo stripslashes($arryPurchase[0]['EDIRefNo']); ?>"  maxlength="100" />          
		</td>
    </tr>
<? }?>
    <tr>
...
$arryShipStand['ShippingState'] = $arryPurchase[0]['wState']; 
$arryShipStand['ShippingCountry'] = $arryPurchase[0]['wCountry']; 
$arryShipStand['ConversionRate'] = $arryPurchase[0]['ConversionRate']; 
$arryShipStand['ShippingZipCode'] = $arryPurchase[0]['wZipCode']; 
$arryShipStand['ShippingCompany'] = $arryPurchase[0]['SuppCompany']; 
$arryShipStand['ShippingAddress'] = $arryPurchase[0]['wAddress']; 
$arryShipStand['ShippingCity'] = $arryPurchase[0]['wCity']; 
$arryShipStand['ShippingMobile'] = $arryPurchase[0]['wMobile']; 
$arryShipStand['ShippingFax'] = $arryPurchase[0]['Fax']; 
$arryShipStand['ShippingName'] = $arryPurchase[0]['wName']; 
$arryShipStand['ShippingLandline'] = $arryPurchase[0]['wLandline']; 
$arryShipStand["INVNumber"] = $arryPurchase[0]['InvoiceID']; 
$arryShipStand["SALENUMBER"] = $arryPurchase[0]['PurchaseID']; 
$arryShipStand["REFERENCE_NUMBER"] = $arryPurchase[0]['ReturnID']; 
