...
function validateForm(frm){
	...
	if(CreditIDVal!=''){
		if(!ValidateMandRange(document.getElementById("PoCreditID"), "Credit Note ID",3,20)){
			return false;
		}
	}
	...
	if( ValidateForSelect(frm.ClosedDate, "Expiry Date")
		...
		if(CreditIDVal!='' && OrderID==''){
			...
			SendExistRequest(Url,"PoCreditID", "Credit Note ID");
			return false;	
		}else{
			ShowHideLoader('1','S');
			return true;	
