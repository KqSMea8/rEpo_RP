...
function validateForm(frm){
	...
	if(OrderType=="Dropship"){
		if(!ValidateForSelect(frm.SaleID, "Sales Order")){        
		    return false;
		}
	}
...
var comType = document.getElementById("comType").value;
	...
	if(PaymentTerm == 'prepayment'){
		...
		if(!ValidateForSelect(frm.CreditCardVendor, "Credit Card Vendor")){        
		    return false;
		}
	}
...
if(document.getElementById("Currency") != null){
	if(!ValidateForSelect(frm.Currency, "Currency")){        
	    return false;
	}
...
function setPrepaidFreight(){	
	...
	if(document.getElementById("PrepaidFreight").value=="1"){
		...
		if(ShippingMethod!=''){
			var ShipVendor = $("#ShippingMethod :selected").attr("ShipVendor");
 
			if(ShipVendor!=''){
				$("#PrepaidVendor").val(ShipVendor);	
			}
		}
		$("#PrepaidVendor").select2();
		if(ShippingMethod=='Fedex' && TrackingNo!='' && SuppCode!='' && PostalCodeD!=''){
			var SendParam = 'action=GetShippingRate&track='+TrackingNo+'&SuppCode='+SuppCode+'&PostalCodeD='+PostalCodeD;
			//alert(SendParam);
...
jQuery('document').ready(function(){
	$("#PrepaidFreight,#ShippingMethod,#wZipCode").change(function(){
			setPrepaidFreight();
	});	
	 
	$("table").on("blur", 'input[name^="TrackingNo"]', function (event) {
		setPrepaidFreight();
	});
...
function setShipTo(ProcessCal){
	...
	/***PK*******/
	if(ProcessCal==1){		 
		 if(document.getElementById("SuppCode").value=='' || document.getElementById("Currency").value==''){
			document.getElementById("OrderType").value = 'Standard';
			alert('Please select vendor and currency first for dropship order.');
			return false;
		 }
	} 
	/**********/
...
// added by karishma for EDI 10 dec 2015
 window.parent.document.getElementById("EDICompId").value=responseText["EDICompId"];
 window.parent.document.getElementById("EDISalesCompName").value=responseText["EDICompName"];
...
<? if(!empty($_GET['edit']) && $arryPurchase[0]['EDIRefNo']!='') {?> <tr> <td align="right" class="blackbold">EDI Ref No :</td> <td align="left" colspan="3"><input name="EDIRefNo" type="text" class="disabled" readonly style="width: 90px;" id="EDIRefNo" value="<?php echo stripslashes($arryPurchase[0]['EDIRefNo']); ?>" maxlength="20" /></td> </tr> <? } ?>
...
<tr>
        ...
        <td   align="left">
<? if(!empty($_GET['edit']) && $arryPurchase[0]['EDIRefNo']=='') {?>
...
<? }?>	
	...
	<td  align="right" class="blackbold"> <div id="SoTitle">Sales Order # :<span class="red">*</span></div> </td>
        <td   align="left">
<div id="SoVal">
<input name="SaleID" id="SaleID" type="text" class="disabled" value="<?=$SaleID?>" readonly style="width:90px;" maxlength="30" />
<? if(empty($_GET['edit']) || empty($SaleID) || $arryPurchase[0]['EDIRefNo']!='') {?>
...
<!--tr>
 ...
 <td  align="right" class="blackbold">Payment Method  :</td>
	...
	$ShippingCarrier = $arryShippingMethod[$i]['attribute_value'];
if(!empty($ShippingCarrier)){
	$ShipVendor = $objConfigure->GetDefaultShipVendor($ShippingCarrier);
?>
	<option ShipVendor="<?=$ShipVendor?>" value="<?=$ShippingCarrier?>" <?  if( $arryPurchase[0]['ShippingMethod']==$ShippingCarrier){echo "selected";}?>>
	<?=$ShippingCarrier?>
	</option>
				<? }} ?>
		</select> 
