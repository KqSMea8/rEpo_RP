...
<tbody>
	...
	<td><div ><?=stripslashes($arryPurchaseItem[$Count]["Condition"])?></div></td>
	...
	<div>
       ...
       <td><input <?=$DisplayN?> type="<?=$QtyType?>" name="qty<?=$Line?>" id="qty<?=$Line?>" class="textbox" size="5" maxlength="6" onkeypress="return isNumberKey(event);" value="<?=$qty_rma?>"/>
		...
		<?php }?>
		<input type="hidden" name="serial_value<?= $Line ?>" id="serial_value<?= $Line ?>" value="<?=$arryPurchaseItem[$Count]["SerialNumbers"]?>"  /></span>
