	...
	if(restock=='1'){
		...
		$("table.order-list").find("tr.itembg").each(function () {
			...
			if(objtr.find('.Retype').val()=="C" || objtr.find('.Retype').val()=="AC"){
				...
				$('#Restocking_fee').val(total.toFixed(2));
			}
		});
	}else{
...
function frameDisplay(ln){
 ...
 var sku = document.getElementById("mainSku"+ln).value;
 var condition = document.getElementById("condition"+ln).value;

var total = document.getElementById("qty"+ln).value;
var WID = document.getElementById("WID"+ln).value;
var item_id = document.getElementById("item_id"+ln).value;
var OrderID= <?=$InvoiceOrderID?>;
//class="fancybox slnoclass fancybox.iframe" href="addPoSerialRma.php?id=<?= $Line ?>&OrderID=<?=$InvoiceOrderID?>&item_id=<?=$arryPurchaseItem[$Count]['item_id']?>&sku=<?=$mainSku?>" id="addItem"
...
$.fancybox.open({
                                ...
                                href:'addPoSerialRma.php?id='+ln+'&OrderID='+OrderID+'&item_id='+item_id+'&WID='+WID+'&condition='+condition+'&total='+total+'&sku='+sku,
                                type: 'iframe',
  width:'50%', height:'75%',
                                helpers   : { 
...
<thead>
		<tr align="left"  >
				...
				<td width="8%" class="heading" >Type <?=$TypeInfo?></td>
				<td width="8%" class="heading" >Action</td>
				<td width="8%" class="heading" >Reason</td>
				<td width="10%" class="heading" >Total Qty Invoiced</td>
				...
				<td class="heading rest_td" width="5%" <?=($arryPurchase[0]['Restocking']=='1')?(''):('style="display: none;"')?> >Re-Stocking Fee</td>
...
$checkProduct=$objItem->checkItemSku($arryPurchaseItem[$Count]["sku"]);
	...
	if(!empty($arryPurchaseItem[$Count]["SerialNumbers"]) || ($arryPurchaseItem[$Count]["evaluationType"] == 'Serialized' || $arryPurchaseItem[$Count]["evaluationType"] == 'Serialized Average') ){
            $SlNoHide = 'inline';
	}
...
$arryPurchaseItem[$Count]["amount"] =0.00;
	...
	?>
     <tr class='itembg'>
		...
		<input type="hidden" name="mainSku<?=$Line?>" id="mainSku<?=$Line?>" value="<?=$mainSku?>" readonly maxlength="20"  />
...
<input type="hidden" name="WID<?=$Line?>" id="WID<?=$Line?>" class="textbox"  value="<?=stripslashes($arryPurchaseItem[$Count]["WID"])?>"/>-->
	...
	<div>
		...
		<?php }?>
         ...
         <td><?=number_format($total_rma)?><input type="hidden" name="total_returned<?=$Line?>" id="total_returned<?=$Line?>" class="disabled" readonly size="5" maxlength="6" onkeypress="return isNumberKey(event);" value="<?=$total_rma?>"/></td>
       <td><input <?=$DisplayN?> type="<?=$QtyType?>" name="qty<?=$Line?>" id="qty<?=$Line?>" class="textbox" size="5" maxlength="6" onkeypress="return isNumberKey(event);" value="<?=$qty_rma?>"/>
		...
		<?php  if(($arryPurchaseItem[$Count]["evaluationType"] == 'Serialized' || $arryPurchaseItem[$Count]["evaluationType"] == 'Serialized Average')  && $total_rma < $total_received){ ?>
		<span style="display:<?=$SlNoHide?>;"  id="serial<?= $Line ?>">
		<a  <?=$DisplayN?>    onclick=" frameDisplay('<?= $Line ?>');" ><img src="../images/tab-new.png"  title="Serial number">&nbsp;Add S.N.</a>
		<?php }?>
		<?php //$arryPurchaseItem[$Count]["SerialNumbers"]?>
		<input type="hidden" name="serial_value<?= $Line ?>" id="serial_value<?= $Line ?>" value="<?php if(!empty($_GET['edit'])){echo $arryPurchaseItem[$Count]["SerialNumbers"];}?>"  /></span>
		...
		<input type="hidden" name="DropshipCost<?=$Line?>" id="DropshipCost<?=$Line?>" class="textbox" value="<?=$arryPurchaseItem[$Count]["DropshipCost"]?>"/>
		</td>
...
<input type="text" class="normal" name="item_taxable<?=$Line?>" id="item_taxable<?=$Line?>" value="<?=stripslashes($arryPurchaseItem[$Count]['Taxable'])?>" readonly size="2" maxlength="20"  />
	...
	<td class="rest_td" <?=($arryPurchase[0]['Restocking']=='1')?(''):('style="display: none;"')?> ><input type="text" name="restocking_fee<?=$Line?>"
				id="restocking_fee<?=$Line?>"   class="textbox" class="textbox"
				size="3" maxlength="10" onkeypress="return isDecimalKey(event);"
				value="<?=stripslashes($arryPurchaseItem[$Count]["fee"])?>" <?=($arryPurchaseItem[$Count]['Type']=="C" || $arryPurchaseItem[$Count]['Type']=="AC")?(""):('style="display: none;"')?> /></td>
...
<tfoot>
 ...
 <input type="hidden" name="OrderType" id="OrderType" value="<?=$arryPurchase[0]['OrderType'];?>" class="inputbox" readonly />
		<?	
...
?>
 <div id="ActualFreightDiv" <?=$ActualFreightDisplay?>>Actual Freight :
		...
		<div id="ReSID" <?=($arryPurchase[0]['Restocking']=='1')?('style="display: inline;"'):('style="display: none;"')?>>
            <span style="color:red;">Re-Stocking Fee :</span> <input type="text"  style="text-align: right; color:red;" name="Restocking_fee"
                id="Restocking_fee" class="disabled" readonly  value="<?=$Restocking_fee?>" size="15" maxlength="10"
