...
<? if($ShowList==1){ ?>
 ...
 <br>
 <div class="cb"></div>
<input type="submit" name="DeleteButton" class="button" style="float:right;margin-bottom:5px;" value="Delete" onclick="javascript: return ValidateMultiple('overtime record','delete','NumField','OvID');">
<? } ?>
...
<table width="100%" align="center" cellpadding="3" cellspacing="1" id="myTable"  >
    ...
    <tr align="left"  >
      ...
      <td width="12%" class="head1" >Duration</td>
       <td  width="13%" align="center" class="head1" >Overtime Rate</td>
     <td  width="14%" align="center" class="head1" >Overtime Hours</td>
      <td width="4%"  align="center" class="head1 head1_action" ><input type="checkbox" name="SelectAll" id="SelectAll" onclick="Javascript:SelectCheckBoxes('SelectAll','OvID','<?=sizeof($arryOvertime)?>');" />
	  </td>
    </tr>
  ...
  if(is_array($arryOvertime) && $num>0){
	$flag=true;
	$Line=0; $TotalDuration = 0; $TotalOvertime = 0; $TotalOvEmp=0;
	...
	/*if(!empty($values["InTime"]) && !empty($values["OutTime"])){
		...
		$TotalOvertime += $values["Hours"];
	
		$OvHour = GetHourMinute($values["Hours"]);
	}*/

	$Duration = ConvertToSecond($values["TimeDuration"]) - $BreakTime;
	if($Duration>0){$TotalDuration += $Duration; $Duration = time_diff($Duration); 
	$TotalOvertime += $values["Hours"];$OvHour = GetHourMinute($values["Hours"]);}
  ...
  ?>
	...
	<? if($NewEmpID != $values['EmpID']){ ?>
		...
		<? if($TotalOvEmp>0 && $Line>1){?>
		 ...
		 <strong>Total Overtime Hours : <?=GetHourMinute($TotalOvEmp)?></strong>
		  </td>
		</tr>
		<? $TotalOvEmp=0; } ?>
    ...
    <tr align="left" class="<?=$bgclass?>">
      ...
      <td><?=date($Config['DateFormat'].", l",strtotime($values["OvDate"]))?></td>
     <td><? if(!empty($values["InTime"])) echo date($Config['TimeFormat'],strtotime($values["InTime"])); ?></td>
      ...
      <td><?=($Duration!=0)?($Duration):('')?></td>
       <td align="center"><?=$values["OvRate"]?></td>
     <td align="center" ><?=$OvHour?></td>
      <td align="center" class="head1_inner"><!--a href="<?=$RedirectUrl?>&del_id=<?=$values['OvID']?>" onclick="return confirmDialog(this, 'Attendance Record')"><?=$delete?></a--> 
	  <input type="checkbox" name="OvID[]" id="OvID<?=$Line?>" value="<?=$values['OvID']?>" />
