...
function ValidateForm(frm)
{
	...
	if( ValidateForSelect(frm.MainEmpID, "Employee") 
		&& ValidateMandDecimalField(frm.Amount,"Loan Amount")
 	){
		...
		*/
		...
		if(document.getElementById("Approved1").checked){
			...
			if(!ValidateMandDecimalField(frm.Rate,"Interest Rate")){
				return false;	
			}
		}
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0" >
 ...
 <? 
	if($_GET['edit'] >0){	
		include("includes/html/box/loan_edit.php");
	}else{ 
...
<input type="hidden" name="LoanID" id="LoanID" value="<?=$_GET["edit"]?>" />
<input type="hidden" name="MainEmpID" id="MainEmpID" value="<?=$arryLoan[0]['EmpID']?>" />
<input type="hidden" name="AmountDue" id="AmountDue" value="<?=$AmountDue?>" />
<input type="hidden" name="OldApproved" id="OldApproved" value="<?=$arryLoan[0]['Approved']?>" />
