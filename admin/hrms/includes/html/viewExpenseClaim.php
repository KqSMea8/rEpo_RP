...
<a href="editExpenseClaim.php" class="add">Add <?=$ModuleName?></a>

<? if($num>0){?>
	<input type="button" class="export_button"  name="exp" value="Export To Excel" onclick="Javascript:window.location='export_expense_claim.php?<?=$QueryString?>';" />
	<input type="button" class="print_button"  name="exp" value="Print" onclick="Javascript:window.print();"/>
<? } ?>

<? if($_GET['sc']!='') {?>
  <a href="viewExpenseClaim.php" class="grey_bt">View All</a>
<? }?>
...
<table <?=$table_bg?> >
    ...
    <tr align="left"  >
       ...
       <td width="12%"  class="head1" >Claim Amount (<?=$Config['Currency']?>) </td>
...
<a href="editExpenseClaim.php?edit=<?=$values['ClaimID']?>&curP=<?=$_GET['curP']?>" ><?=$edit?></a>
<? } ?>

<a href="editExpenseClaim.php?del_id=<?=$values['ClaimID']?>&curP=<?=$_GET['curP']?>" onclick="return confirmDialog(this, '<?=$ModuleName?>')"  ><?=$delete?></a>  
...
<tr >  <td  colspan="8" id="td_pager">Total Record(s) : &nbsp;<?php echo $num;?>  &nbsp;<?php if(count($arryExpenseClaim)>0){?>
