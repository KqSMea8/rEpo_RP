...
<table width="100%"  border="0" cellpadding="0" cellspacing="0" >
	...
	<tr <?=$HideTableRow?>>
	 ...
	 <tr>
		 <td colspan="4" align="left" class="head">Bank Details

<a class="fancybox add fancybox.iframe" href="viewBank.php?emp=<?=$_GET['emp']?>">Manage Bank Detail</a>
...
<table width="100%" border="0" cellpadding="5" cellspacing="0" class="borderall" >
	...
	<td align="left" >
<input id="PayCycle" name="PayCycle" class="disabled" readonly size="15" value="<?=$GlobalPayCycle?>"  type="text" maxlength="10" size="15" autocomplete="off" > 
...
<input id="HourRate" name="HourRate" class="textbox" size="15" value="<?=$arrySalary[0]['HourRate']?>"  type="text" maxlength="10" onkeypress="return isDecimalKey(event);Javascript:ClearMsg();" size="15" autocomplete="off" > 
	...
	<tr id="yearlysalarytr" style="display:none">
	<td align="right" class="blackbold" valign="top">Yearly Salary :<span class=red>*</span></td>
	<td align="left" >
<input id="YearlySalary" name="YearlySalary" class="textbox" size="15" value="<?=$arrySalary[0]['CTC']?>"  type="text" maxlength="10" onkeypress="return isDecimalKey(event);Javascript:ClearMsg();" size="15" autocomplete="off" onblur="Javascript:SetHourlyRate();" >  <?=$Config['Currency']?> 

	</td>
	</tr>

	<tr id="hourlyratetemptr" style="display:none">
	<td align="right" class="blackbold" valign="top">Hourly Rate :</td>
	<td align="left" >
<input id="HourRateTemp" name="HourRateTemp" class="disabled" readonly size="15" value="<?=$arrySalary[0]['HourRate']?>"  type="text" maxlength="10" onkeypress="return isDecimalKey(event);Javascript:ClearMsg();" size="15" autocomplete="off" >
	</td>
	</tr>

	<tr id="paycycletr" style="display:none">
...
?>
<input id="PayCycleRate" name="PayCycleRate" class="disabled" readonly size="15" value="<?=$arrySalary[0]['PayCycleRate']?>"  type="text" maxlength="10" size="15" autocomplete="off" >
	</td>
	</tr>
...
$FieldValue='';
	foreach($arryHead as $key=>$values){
		...
		if(!empty($arrySalaryDb[$values['headID']])){
...
<input type="hidden" name="EmpWorkingHour" id="EmpWorkingHour" value="<?=$EmpWorkingHour?>">
<input type="hidden" name="GlobalPayCycle" id="GlobalPayCycle" value="<?=$GlobalPayCycle?>">
<input type="hidden" name="NumWorkingDays" id="NumWorkingDays" value="<?=$NumWorkingDays?>">
...
function SetSalaryTR(){
	if(document.getElementById("PayRate1").checked==true){		
		$("#yearlysalarytr").show(); 
		$("#hourlyratetemptr").show();
		$("#paycycletr").show();
		$("#hourlyratetr").hide(); 
	}else{
		$("#yearlysalarytr").hide(); 
		$("#hourlyratetemptr").hide();
		$("#paycycletr").hide();
		$("#hourlyratetr").show(); 
	}
}
...
function SetHourlyRate(){
	var CTC = parseFloat(Trim(document.getElementById("YearlySalary")).value);
	var EmpWorkingHour = parseFloat(Trim(document.getElementById("EmpWorkingHour")).value);
	var NumWorkingDays = parseInt(Trim(document.getElementById("NumWorkingDays")).value);
	var GlobalPayCycle = document.getElementById("GlobalPayCycle").value;
	var HourRateTemp = 0;
	var PayCycleRate = 0;

	if(CTC!='' && !isNaN(CTC)){	
		if(GlobalPayCycle=='Weekly'){
			PayCycleRate = CTC/52;
			HourRateTemp = (PayCycleRate/NumWorkingDays)/EmpWorkingHour;
		}else if(GlobalPayCycle=='Bi-Weekly'){
			PayCycleRate = CTC/26;
			HourRateTemp = (PayCycleRate/(NumWorkingDays*2))/EmpWorkingHour;
		}else if(GlobalPayCycle=='Semi-Monthly'){
			PayCycleRate = CTC/24;
			HourRateTemp = (PayCycleRate/(NumWorkingDays*2))/EmpWorkingHour;
		}else if(GlobalPayCycle=='Monthly'){
			PayCycleRate = CTC/12;
			HourRateTemp = (PayCycleRate/(NumWorkingDays*4))/EmpWorkingHour;
		}else{
			PayCycleRate = (CTC/NumWorkingDays);
			HourRateTemp = CTC/(NumWorkingDays*EmpWorkingHour);
		}


		HourRateTemp = parseFloat(HourRateTemp).toFixed(2);
		PayCycleRate = parseFloat(PayCycleRate).toFixed(2);
		$("#HourRateTemp").val(HourRateTemp); 
		$("#PayCycleRate").val(PayCycleRate); 
	}	
...
function ValidateForm(frm)
{
	...
	/**********************/
	if(document.getElementById("PayMethod").value=='H'){ //Hourly
		if(document.getElementById("PayRate0").checked==true){
			if(!ValidateForSimpleBlank(document.getElementById("HourRate"), "Hourly Rate")){
				return false;
			}			
		}else if(document.getElementById("PayRate1").checked==true){
			if(!ValidateForSimpleBlank(document.getElementById("YearlySalary"), "Yearly Salary")){
				return false;
			}			
		}
		ShowHideLoader(1,'S');
