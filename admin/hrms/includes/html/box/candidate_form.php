...
function validateCandidate(frm){
	...
	if( ValidateForSelect(frm.Vacancy,"Job Vacancy")
		...
		&& ValidateOptionalResume(frm.Resume,"Resume")
...
<input id="date_of_birth" name="date_of_birth" readonly="" class="datebox" value="<?=$arryCandidate[0]['date_of_birth']?>"  type="text" >         </td>
 ...
 <tr>
        ...
        <td  align="left"  >
	 <input name="Mobile" type="text" class="inputbox" id="Mobile" value="<?=stripslashes($arryCandidate[0]['Mobile'])?>"     maxlength="20" onkeypress="return isNumberKey(event);"  />			</td>
      </tr>	
...
if(IsFileExist($Config['CandidateDir'],$arryCandidate[0]['Image'])){ 
	$OldImage =  $arryCandidate[0]['Image'];
	...
	&nbsp;<a href="Javascript:void(0);" onclick="Javascript:RemoveFile(\''.$Config['CandidateDir'].'\',\''.$arryCandidate[0]['Image'].'\',\'ImageDiv\')">'.$delete.'</a><input type="hidden" name="OldImage" value="'.$OldImage.'"></div>';
...
<tr>
	...
	if(IsFileExist($Config['CandResumeDir'],$arryCandidate[0]['Resume'])){ 
		$OldResume = $arryCandidate[0]['Resume'];		
	?>
	<br><br>
...
&nbsp;
	<a href="Javascript:void(0);" onclick="Javascript:RemoveFile('<?=$Config['CandResumeDir']?>', '<?=$arryCandidate[0]['Resume']?>', 'ResumeDiv')"><?=$delete?></a>
	<br><br>
	</div>
