...
function EmployeeShow(){
	if(document.getElementById('ExistingEmployee').value == 0){
		$("#maintablerow tr").hide();
		$(".fixedtr").show();		
		$("#pswd-info-wrap").css('top', "430px");  		 
	}else{
		$("#maintablerow tr").show();
		$("#pswd-info-wrap").css('top', "1680px");  		 
	}
}
	...
	if( ValidateRadioButtons(frm.Gender, "Gender")
		...
		&& ValidateForSimpleBlank(frm.LastName, "Last Name")
			...
			if(document.getElementById('ExistingEmployee').value == 1){
				...
				if(!ValidateForSelect(frm.catID,"Category")){
					return false;
				}
				...
				if(!ValidateForSelect(frm.JobTitle,"Designation")){
					return false;
				}
				if(!ValidateForSelect(frm.JobType,"Job Type")){
					return false;
				}
				if(!ValidateForSelect(frm.ExperienceYear,"Total Years of Experience")){
					return false;
				}
				if(!ValidateForSelect(frm.ExperienceMonth,"Total Months of Experience")){
					return false;
				}
...
<table width="100%" border="0" cellpadding="5" cellspacing="0" class="borderall" id="maintablerow">
<tr class="fixedtr">
	 <td colspan="2" align="left" class="head">Personal Details</td>
</tr>

<tr class="fixedtr" >
		<td  align="right" valign="center"  class="blackbold" width="35%">New Employee : </td>
	 ...
	 <span id="MsgSpan_Email"></span>		</td>
	  ...
	  <? if(empty($_GET['edit'])){ ?>
        ...
        <td   align="left" class="blacknormal">
            <input name="Password" type="Password" class="inputbox" id="Password" onkeypress="ClearMsg();" onmousedown="ClearMsg();"
                   value="<?php echo stripslashes($arryEmployee[0]['Password']); ?>" maxlength="15" autocomplete="off"/>
