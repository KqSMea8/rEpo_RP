...
function validateVendor(frm){
	...
	if(VendorCode!=''){
		...
		DataExist = CheckExistingData("isRecordExists.php","&VendorCode="+escape(VendorCode)+"&editID="+document.getElementById("VendorID").value, "VendorCode","Vendor Code");
	...
	DataExist = CheckExistingData("isRecordExists.php","&VendorName="+escape(document.getElementById("VendorName").value)+"&editID="+document.getElementById("VendorID").value, "VendorName","Vendor Name");
	if(DataExist==1)return false;
	/**********************
	...
	DataExist = CheckExistingData("isRecordExists.php", "&Type=Vendor&Email="+escape(document.getElementById("Email").value)+"&editID="+document.getElementById("VendorID").value, "Email","Email Address");
	if(DataExist==1)return false;
	/**********************/
...
<table width="100%" border="0" cellpadding="5" cellspacing="0" class="borderall">
 ...
 <tr>
	...
	<input name="VendorCode" type="text" class="datebox" id="VendorCode" value="<?=stripslashes($arryVendor[0]['VendorCode'])?>"  maxlength="20" onKeyPress="Javascript:ClearAvail('MsgSpan_VendorCode');return isAlphaKey(event);" onBlur="Javascript:CheckAvailField('MsgSpan_VendorCode','VendorCode','<?=$_GET['edit']?>');" onMouseover="ddrivetip('<?=BLANK_ASSIGN_AUTO?>', 220,'')"; onMouseout="hideddrivetip()"/>
   ...
   <tr>
        ...
        <td   align="left" >
<input name="VendorName" type="text" class="inputbox" id="VendorName" value="<?=stripslashes($arryVendor[0]['VendorName'])?>"  maxlength="40" onKeyPress="Javascript:ClearAvail('MsgSpan_Company');return isAlphaKey(event);" onBlur="Javascript:CheckAvailField('MsgSpan_Company','VendorName','<?=$_GET['edit']?>');"/>
