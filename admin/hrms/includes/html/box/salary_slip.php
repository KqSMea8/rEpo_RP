...
<? 
$AllField = ''; $Total = 0; $Deduction = 0;  $OtherAmount=0; $OtherAddition=0;
$TotalEarnings = 0; $TotalDeduction = 0;
/******** Abbas ********/

// Break in Array
$arryfirst=(array_slice($arryPayCategory,0,2));
$arrysecond=(array_slice($arryPayCategory,2));

// reverse 
$reversefirst=array_reverse($arrysecond);

// again break in aaray
$arrysecond_first=(array_slice($reversefirst,0,1));
$arrysecond_last=(array_slice($reversefirst,1));

// reverse 
$reversesecond=array_reverse($arrysecond_last);

// merge array
$firstmerge=array_merge($arrysecond_first,$reversesecond);
$finalmerge=array_merge($arryfirst,$firstmerge);

/*******End ***********/

foreach($finalmerge as $key=>$values_cat){
	$arryHead = $objPayroll->getHead('',$values_cat['catID'],$catEmp,0);
...
?>			
	...
	<tr <?=$HideUnwanted?>>
		 ...
		 <td align="right" ><strong> <? if($SubTotal>0) echo number_format($SubTotal);  else echo '0'; ?>
		  ...
		  <?  if($values_cat['catGrade']=="A" || $values_cat['catGrade']=="B" || $values_cat['catGrade']=="E"){
...
<? //if($values_cat['catGrade']=="B"){ // Start Overtime and Bonus 
	...
	if($values_cat['catGrade']=="B" && $OtherShown!=1){  $OtherShown=1;
			if($arrySalary[0]['Arrear']>0 || $arrySalary[0]['Overtime']>0 || $arrySalary[0]['Bonus']>0 || $arrySalary[0]['Commission']>0){ ?>
		<tr <?=$HideUnwanted?>>
		...
		<? } ?>
 		<? if($arrySalary[0]['Commission']>0){
			  $OtherAmount += round($arrySalary[0]['Commission']);
			?>
		 <tr>
		  <td align="left" class="blackbold">Sales Commission : </td>
		  <td align="right" >
			<?  echo round($arrySalary[0]['Commission']); ?>
		   </td>
		</tr>
		<? } ?>
 ...
 <? 
	...
	$CurrentPM = ($NetPM + round($OtherAddition,2) )- $Deduction;
