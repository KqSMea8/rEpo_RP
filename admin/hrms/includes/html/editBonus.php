...
function ValidateForm(frm)
{
	...
	if( ValidateForSelect(frm.MainEmpID, "Employee") 
		...
		if(parseInt(frm.Amount.value) > parseInt(frm.NetSalary.value)){
			alert("Bonus amount should not be greater than Net Salary.");
			return false;	
		}
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0" >
 ...
 <? 
	if($_GET['edit'] >0){	
		include("includes/html/box/bonus_edit.php");
	}else{ 
		include("includes/html/box/bonus_form.php");
	} 
?>
...
<input type="hidden" name="BonusID" id="BonusID" value="<?=$_GET["edit"]?>" />
<input type="hidden" name="MainEmpID" id="MainEmpID" value="<?=$arryBonus[0]['EmpID']?>" />
<input type="hidden" name="OldApproved" id="OldApproved" value="<?=$arryBonus[0]['Approved']?>" />
