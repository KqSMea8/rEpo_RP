   ...
   <tr align="left"  >
		...
		for ($j = 0; $j <= sizeof($reportHeader); $j++) {
			...
			if($reportHeader[$j][0] != 'Date' ){
				echo '<td class="head1" width="" >'.$reportHeader[$j][0].'</td>';
                              }else{
...
echo $numHead = $numHead-1;
}

if($arryReport[0]['BreakCheck'] == 'Yes'){

$width = 400;}else{ $width =250;}
//echo $numHead = $numHead;
	for ($j = 0; $j <= sizeof($reportHeader); $j++) {
...
echo '<td class="head1" colspan="'.$numHead.'" > </td>';
			for($dat=0;$dat<count($arrYDate);$dat++){
				echo '<td class="head1">
					 <table width="'.$width.'" align="center" cellpadding="3" cellspacing="1"  >
...
for ($i = 0; $i <= sizeof($reportHeader); $i++) {
	...
	<? }elseif($reportHeader[$i][1] == "Department"){?>
		<td>
		<? if(!empty($values[$reportHeader[$i][1]])){ echo $values[$reportHeader[$i][1]]; }?>
		</td>
	<? }elseif($reportHeader[$i][1] == "JobTitle"){?>
		<td>
		<? if(!empty($values[$reportHeader[$i][1]])){ echo $values[$reportHeader[$i][1]]; }?>
		</td>
...
?> 
		...
		<table width="<?=$width?>"  align="center" cellpadding="3" cellspacing="1" id="myTable" >

		<tr align="left" class="<?=$bgclass?>">
		<td width="30"><?=$values[$arrYDate[$dat2]]['Intime']?></td>
		<td width="30"><?=$values[$arrYDate[$dat2]]['OutTime']?></td>
		<td width="30"><? echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["LO"]; ?></td> 			<td width="30"> <? echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["LI"];?></td>						
<? if($arryReport[0]['BreakCheck'] == 'Yes'){
//echo count($arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["SO"]);?>

<td><?  echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["SO"][0]; ?></td> 			
<td> <? echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["SI"][0];?></td>

<td><?  echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["SO"][1]; ?></td> 		
<td> <? echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["SI"][1];?></td>

<td><?  echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["SO"][2]; ?></td> 			
<td> <? echo $arryBreakList[$values['EmpID']][$arrYDate[$dat2]]["SI"][2];?></td>	
