...
function ShowEmpListing(){
	ResetSearch();
	location.href = "EmpList.php?d="+document.getElementById("Department").value+"&id="+$("#SelID").val();
}


function SetEmployee(EmpID,UserName,JobTitle){	
	ResetSearch();
	var RedirectUrl = document.getElementById("link").value;
	if(RedirectUrl!=''){
		window.parent.location.href = RedirectUrl+"?emp="+EmpID;
	}else{
		var SelID = $("#SelID").val();
...
<TABLE WIDTH="100%"   BORDER=0 align="center" CELLPADDING=0 CELLSPACING=0 height="500">
 ...
 <? //if($_GET["d"]>0){?>
<form name="frmSrch" id="frmSrch" action="EmpList.php" method="get" onSubmit="return ResetSearch();">
