<?php
	...
	$BotTextHeight = 10;
		...
		//image filed
		 $img_width=750; 
		 $img_height=480;
		 $yname='Number of Days';//y-axis name
		//border margins
		 $margins=40;
		 $ymargin=70;
...
$values_color=imagecolorallocate($img,73,86,58);//values color
		...
		imageline($img,$img_width-$ymargin,2,$img_width-$ymargin,$LineHeight,$xyline_color);
		 ...
		 $max_value=(max($valuesBar)>50)?(max($valuesBar)):(50); 
		 ...
		 for($i=0;$i< $total_bars; $i++){
			...
			imagestring($img,0,$x1+3,$y1-10,$tempVal,$values_color);
			imagestringup($img,2,$x1,$img_height-$BotTextHeight,$key,$txt_color);

		} 

		imagestringup($img,5,0,220, $yname, $txt_color);
