...
class iContactApi {
	...
	**/
	protected function __construct() {
		// Check for constants
		$aConstantMap = array(
			'ICONTACT_APIVERSION' => '2.2', 
			// 'ICONTACT_APISANDBOXURL', 
			'ICONTACT_APPID'       => 'appId', 
			//'ICONTACT_APIURL'	   => 'https://app.icontact.com/icp/a/1231763/c/2715/', 
	...
	**/
	...
	public function get_contact_info($email){
		...
		$ch = curl_init("https://app.icontact.com/icp/a/".$this->setAccountId()."/c/".$this->setClientFolderId()."/contacts/email=".urlencode($email));
	...
	**/
	...
	public function deleteContact($ContactId) {
		// Delete the list
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/contacts/{$ContactId}", 'delete');
	}
  ...
  public function subscriptionStatus($subscriptionId,$sStatus = 'normal') {
		...
		$aValidStatuses = array('normal', 'pending', 'unsubscribed');
		// Setup the subscription and make the call
		$aSubscriptions = $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/subscriptions/{$subscriptionId}", 'PUT', 
			
			array('status'    => $sStatus), 'subscriptions');
		// Return the subscription
		return $aSubscriptions;
	}
  public function GetContactSubscriptionStatus($limit = 0) {
		// Valid statuses
		//$aValidStatuses = array('normal', 'pending', 'unsubscribed');
		// Setup the subscription and make the call
		
		$aSubscriptions = $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/subscriptions?limit={$limit}&offset={$limit}", 'GET',null);
	...
	public function GetSubscriptionWithContact($iContactId, $iListId, $sStatus = 'normal') {
		// Valid statuses
		$aValidStatuses = array('normal', 'pending', 'unsubscribed');
		// Setup the subscription and make the call
	...
	**/
	...
	public function getContactsWithListId($listId) {
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/contacts?status=total&listId={$listId}", 'GET');
	}
	...
	**/
	...
	public function getListsWithId($listId) {
		// Make the call and return the lists
	...
	**/
	...
	//Some custom methods for elabrating the power of iContact API
	public function getContactWithEmail($Email) {
		// Make the call and return the data
		$this->aSearchParameters['status'] = 'total';
		$this->aSearchParameters['email'] = $Email;
