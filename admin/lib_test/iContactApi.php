...
class iContactApi {
	...
	**/
	 ...
	 * @return object
	**/
	public function addList($sName, $bEmailOwnerOnChange = true, $bWelcomeOnManualAdd = false, $bWelcomeOnSignupAdd = false, $sDescription = null, $iWelcomeMessageId=null, $sPublicName = null) {
		// Setup the list
                if(empty($iWelcomeMessageId)){
		$aList = array(
			'name'               => $sName, 
	 ...
	 * @return object
	**/
	public function addMessage($sSubject, $iCampaignId, $sTextBody = null, $sMessageName = null, $sHtmlBody = null, $iListId = null, $sMessageType = 'normal') {
		// Valid message types
	...
	**/
	public function sendMessage($sIncludeListIds, $iMessageId, $sExcludeListIds = null, $sExcludeSegmentIds = null, $sIncludeSegmentIds = null, $sScheduledTime = null) {
		...
		// Return the send
                //print_r($aSends); //'$iMessageId'         => $iMessageId,
		return $aSends;
	}
	 ...
	 * @return bool|object
	 **/
	public function updateContact($iContactId, $sEmail = null, $sPrefix = null, $sFirstName = null, $sLastName = null, $sSuffix = null, $sStreet = null, $sStreet2 = null, $sCity = null, $sState = null, $sPostalCode = null, $sPhone = null, $sFax = null, $sBusiness = null, $sStatus = 'normal') {
		// Valid statuses
		$aValidStatuses = array('normal', 'bounced', 'donotcontact', 'pending', 'invitable', 'deleted');
		// Contact placeholder
		$aContact       = array();
	...
	**/
	public function getCampaigns() {
        ...
        public function getCampaign($iCampaignId) {
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/campaigns/{$iCampaignId}", 'GET', null, 'campaign');
	}
	...
	**/
	public function getLists() {
		...
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/lists/{$listId}", 'GET', null, 'list');
	}
	/**
	...
	**/
	...
	//Create Segments 
	public function addSegment($sName, $sDisc= null, $sListId) {
		// Setup the list
		$aSegm = array(
			'name'               => $sName, 
			'description'        => $sDisc, 
		...
		// Make the call
		$aSegments = $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/segments", 'POST', array($aSegm), 'segments');
		// Return the list
		return $aSegments;
	}
	//Grab All Segments
	public function getSegments() {
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/segments", 'GET');
	}
	
	//Grab a Segment
	public function getSegment($segmentId) {
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/segments/{$segmentId}", 'GET', null, 'segment');
	}
	//Update Segment  $sListId,  //'listId'         => $sListId
        public function updateSegment($segmentId,$sName, $sDisc= null,$sListId) {
		// Setup the list
		$aSegment = array(
			'name'               => $sName, 
			'description'        => $sDisc,
		...
		// Return the list
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/segments/{$segmentId}", 'POST',$aSegment, 'segment');
	}
        // Delete Segment
        public function deleteSegment($iSegId) {
		// Delete the list
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/segments/{$iSegId}", 'delete');
	}
	//Create Campaign , $forwardToFriend
	public function addCampaign($name, $description = null, $fromName , $fromEmail , $clickTrackMode='0', $useAccountAddress = '0',$subscriptionManagement='0',$forwardToFriend='0',$archiveByDefault='0',$street = null, $city = null, $state = null, $zip = null, $country = null) {
		// Valid statuses
                    //$aValidStatuses = array('normal', 'bounced', 'donotcontact', 'pending', 'invitable', 'deleted');
		// Contact placeholder
		...
		if (!empty($name)) {
			// Add the new Name
			$aCampaign['name'] = (string) $name;
		}
		// Check for Discription
		if (!empty($description)) {
			// Add the Discription
			$aCampaign['description'] = (string) $description;
		}
		// Check for a from mail
		if (!empty($fromEmail)) {
			// Add the new last name
			$aCampaign['fromEmail'] = $fromEmail;
		}
		// Check for a from Name
		if (!empty($fromName)) {
			// Add the From Name
			$aCampaign['fromName'] = (string) $fromName;
		}
                
                //if (!empty($forwardToFriend)) {
			
			$aCampaign['forwardToFriend'] = (string)$forwardToFriend;
		//}
                //if ($subscriptionManagement!=0) {
			
			$aCampaign['subscriptionManagement'] =(string)$subscriptionManagement;
		//}
                //if ($clickTrackMode) {
			// on or off the Track Mode
			$aCampaign['clickTrackMode'] = (string)$clickTrackMode;
		//}
                
                //if (!empty($useAccountAddress)) {
			
			$aCampaign['useAccountAddress'] =(string) $useAccountAddress;
		//}
		 
                // Check for a street
		if (!empty($street)) {
			// Add the new street
			$aCampaign['street'] = (string)$street;
		}
		// Check for a city
		if (!empty($city)) {
			// Add the new city
			$aCampaign['city'] =  (string)$city;
		}
		// Check for a state
		if (!empty($state)) {
			// Add the new state
			$aCampaign['state'] = (string)$state;
		}
		// Check for a postal code
		if (!empty($zip)) {
			// Add the new postal code
			$aCampaign['zip'] = (string)$zip;
		}
		// Check for a Country
		if (!empty($country)) {
			// Add the Country
			$aCampaign['country'] = (string)$country;
		}
                //if (!empty($archiveByDefault)) {
			
			$aCampaign['archiveByDefault'] = (string)$archiveByDefault;
		//}
               //print_r($aCampaign);
                //die;
		/* Check for a valid status
		...
		// Make the call
		$aCampaigns = $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/campaigns", 'POST', array($aCampaign), 'campaigns');
		// Return the campaign
		return $aCampaigns;
	}
        
        //Update Campaign
        public function updateCampaign($campaignid,$name, $description = null, $fromName , $fromEmail , $clickTrackMode='0', $useAccountAddress = '0',$subscriptionManagement='0',$forwardToFriend='0',$archiveByDefault='0',$street = null, $city = null, $state = null, $zip = null, $country = null) {
		// Valid statuses
                    //$aValidStatuses = array('normal', 'bounced', 'donotcontact', 'pending', 'invitable', 'deleted');
		// Contact placeholder
		...
		// Make the call
		$aCampaigns = $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/campaigns/{$campaignid}", 'POST', array($aCampaign), 'campaigns');
		// Return the campaign
		return $aCampaigns;
	}
        // DElete Contact
		...
		// Delete the contact
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/contacts/{$contId}", 'delete');
	}
        
        // Get Subscription
        public function getSubscribtions() {
		// Setup the subscription and make the call
		$aSubscriptions = $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/subscriptions", 'GET','subscriptions');
		// Return the subscription
		return $aSubscriptions;
	}
        /* Get a Subscription not worked
        public function getSubscribtion($subsId) {
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/subscriptions/{$subsId}", 'GET', null, 'subscriptions');
	}*/
