...
<tr>
	...
	$inputdata = ($searchdata) ? $searchdata : $_SESSION['searchdata'];
	foreach($inputdata as $value){ ?>
		...
		if($value['type'] == 'select'){ 
			...
			$fldvar.= $objCategory->getCategoriesforCustomSearch(0, 0,$_SESSION['PostData']['CategoryID']);

			}else{

			$fldvar.= $csearch->selectOptions($value['fieldname'], $Viewdata['moduleID'], $_SESSION['PostData'][$value['fieldname']]);    
		...
		if($value['type'] == 'text'){ 
			if($value['fieldname'] == 'Sku'  || $value['fieldname'] == 'description'){		
            	...
            	$fvar.='<td valign="bottom"><input '.(($value['fieldname'] == 'description') ? 'style="width:300px"' : 'autocomplete = "on"' ).' id="'.$value['fieldname'].'" name="'.$value['fieldname'].'" size="15"  class="textbox" value="'.(($_POST[$value['fieldname']]=="") ? $_GET['keyword'] :  $_POST[$value['fieldname']]).'"  type="text" placeholder="'.$value['fieldlabel'].' Search"></td>'; 	
			 }else{ 

			$fldvar.= '<td valign="bottom"><input id="'.$value['fieldname'].'" name="'.$value['fieldname'].'" size="15"  class="textbox" value="'.$_GET['keyword'].'"  type="text" placeholder="'.(($_POST[$value['fieldname']]=="") ? $_GET['keyword'] :  $_POST[$value['fieldname']]).' Search"></td>';
...
<tr><td>
<?php if($_POST['go'] || $_SESSION['PostData']){ ?>
    ...
    <td align="center"><?php 
        if(!empty($values[$value['fieldname']])  || $values[$value['fieldname']] == '0'){
            ...
            { 
                ...
                $res = $csearch->selectedVal($values[$value['fieldname']],$value['fieldname']);
...
<!----Add by chetan on 11Jan----->
	...
	if(($values['itemType'] == 'Kit' || $values['itemType'] == 'Non Kit') && !empty($arryKit)){  
		...
		{	
			...
			if($PrimaryArry[0]['Primary']==1 && !empty($PrimaryArry)){
			...
			$NonPairCost = $pairTotCostNon/$NonPairQty;
...
<span class="bom1" ><b>Partials</b><br><? echo '<a class="fancybox fancybox.iframe"  href="csviewcondqty.php?sku='.$values['Sku'].'&condition='.$condi.$serialNo.'&WID='.$_POST['warehouse'].'" title="view"> <span title="Partials"><b>'.(($NonPairQty>0)?$NonPairQty:0).'</b></span></a>';//updated 30Mar2017 by chetan//
      ...
      <? echo $csearch->showOnSO($condi,$ItemDtl[0]['ItemID'],$checked,$postdata); ?>
        <td align="center"><?=($onHand)?></td>
				<!--td align="center"><?=$totCost." ".$Config['Currency']?></td-->
...
$Config['warehouse'] = $_POST['warehouse'];
				...
				$condQtyCost = $objItems->getTotQtByItemIdsOnCond($values['ItemID'],$condi,$_POST['warehouse']); //updated by chetan on 4Apr2017//  //for search item//	
				...
				//$TotavgCost = $totcondAvgCost/$TotCondQtyCost[0]['condition_qty'];
				$avgPrice = $csearch->compItemTableHtml($condi,$ItemDtl[0],$arr,$_POST['warehouse']); //updated by chetan on 3Apr2017//
				$SoQty  = $csearch->getQtyOrderfr('sale',$allBomItemidArr,$condi,'',$_POST['warehouse']);
...
<span class="bom3" title="On hand" style="cursor: pointer;"><b>On hand</b><br><?php if($OnQty>0){?><?=($OnQty)?> 
				<? }else{ echo '0';}?></span>
...
<!--table  id="list_table">
    ...
    <td class="head1">Total Qty </td>
     ...
     <tr>
<td><?=$avgPrice[0]['price']?></td>
    <td><?=$TocondQtyCost[0]['condition_qty']?> </td> 
...
?>	
				...
				<!--td align="center"><?=$avgPrice[0]['price']." ".$Config['Currency']?></td-->
<td align="center"><span class="bom3" style="width: 70px;"><b>On hand</b><br><?if($postdata['currency']!=''){?><?=$csearch->getCurrencyConverted($avgPrice[0]['price'], $Config['Currency'])?> <?}else{ echo $avgPrice[0]['price']; }?>
...
<? if($postdata['currency']!=''){echo $csearch->getCurrencyConverted($totcondAvgCost, $postdata['currency']);}//by chetan 2Feb//?></span></td>
		...
		if (is_array($allcondition) && $numcond > 0) 
		{
			foreach ($allcondition as $key => $condi) {
				...
				$OnQty =$condQtyCost[0]['condition_qty']+$InvQty[0]['InvQty'];
				$onHand = intval($OnQty)-intval($SoQty);
				//$onHand = $condQtyCost[0]['condition_qty'];
				?>
				...
				<td align="center">	<?php  echo $onHand;?></td>
				<td align="center">
<span class="bom3" style="width: 70px;"><b>On hand</b><br><?php if($postdata['currency']!=''){  echo $csearch->getCurrencyConverted($avgPrice[0]['price'], $Config['Currency']); }else{ echo '<b>'.number_format($avgPrice[0]['price'],2).'</b>';} ?>
