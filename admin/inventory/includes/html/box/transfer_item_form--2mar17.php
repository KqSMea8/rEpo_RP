...
<td><select name="Condition<?=$Line?>" id="Condition<?=$Line?>" class="textbox" style="width:120px;display:<?=$TaxShowHide?>"><option value="">Select Condition</option><?=$ConditionSelectedDrop?></select></td>
        ...
        <td><input type="text" name="qty<?= $Line ?>" id="qty<?= $Line ?>" <? if($_GET['edit']>0){  echo "readonly  "; echo 'class="disabled"'; }else{  echo 'class="textbox"'; } ?> size="5" maxlength="6" onkeypress="return isNumberKey(event);" value="<?= stripslashes($arryTransferItem[$Count]["qty"]) ?>"/>
                    <span id="serialqty1"></span><span style="display:none;"  id="serial<?= $Line ?>"><a  class="fancybox fancybox.iframe" href="editSerial.php?id=<?= $Line ?>&SerialType=adjust"id="addItem"><img src="../images/tab-new.png"  title="Serial number">&nbsp;Add S.N.</a>
                    ...
                    <? if($arryTransferItem[$Count]['valuationType']=="Serialized"){
