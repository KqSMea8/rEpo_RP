...
<div class="had">
Required Items For Alias - <?=$arryAlias44[0]['ItemAliasCode']?> 
</div>
<div id="msg_div" align="center" class="redmsg"></div>


<form name="form1" action=""  method="post" onSubmit="return validateRequired(this);"  enctype="multipart/form-data">
 ...
 <tr >		
	...
	?>
     <tr class="itembg">
		...
		<input type="text" name="sku<?=$Line?>" id="sku<?=$Line?>" onclick="Javascript:SetAutoComplete(this);" onblur="return SetItemReqCode(this.value,'<?=$Line?>');" class="textbox"  size="10" maxlength="15"  value="<?=stripslashes($sku)?>"/>&nbsp;<?if($_GET['edit']>0 || $_GET['bc']>0){?><a class="fancybox fancybox.iframe" href="reqItemList.php?id=<?=$Line?>" ><img src="../images/view.gif" border="0"></a><?}?>
...
<input type="hidden" name="item_sku" id="item_sku" value="<? if(!empty($arryProduct[0]['Sku']))echo stripslashes($arryProduct[0]['Sku']); ?>" class="inputbox" readonly />
<input type="hidden" name="Type" id="Type" value="Alias" class="inputbox" readonly />
<input type="hidden" name="ItemID" id="ItemID" value="<?=$itemID?>" class="inputbox" readonly />
<input type="hidden" name="aliasID" id="aliasID" value="<?=$aliasID?>" class="inputbox" readonly />
<input type="hidden" name="tab" id="tab" value="<?=$_GET['tab']?>" class="inputbox" readonly />
<input type="hidden" name="CatID" id="CatID" value="<?=$_GET['CatID']?>" class="inputbox" readonly />
<input type="hidden" name="curP" id="curP" value="<?=$_GET['curP']?>" class="inputbox" readonly />
