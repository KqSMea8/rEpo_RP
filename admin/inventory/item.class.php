...
//update By Chetan 7Jan(add sell price,purchase cost /evaluationType col.)//
	...
	/********* By Karishma for MultiStore 22 Dec 2015******/
       ...
       $strSQLQuery = "select p1.*,cat.valuationType as evaluationType from inv_items  p1 left join inv_categories cat on cat.CategoryID=p1.CategoryID ".$custJoin." where p1.Sku = '" . $Sku . "' ".$whereAddQuery."";
...
//}
    ...
    function GetBinBySku($Sku) {
        ...
        $strSQLQuery = "select inv_stock.*,w.warehouse_name,w.warehouse_code,w.WID,bin.binlocation_name,binid,q.`condition`,q.condition_qty from inv_bin_stock  inv_stock left outer join w_warehouse w  on inv_stock.Wcode = w.WID left outer join w_binlocation bin  on inv_stock.bin = bin.binid left outer join inv_item_quanity_condition q  on inv_stock.QTID = q.ID " . $strAddQuery;
...
$srPrice =$arryDetails['price' . $i];
						...
						//}
						$strSQLQuery = "insert into inv_serial_item (adjustment_no,warehouse,serialNumber,Sku,disassembly,`Condition`,description,type,UnitCost,ReceiptDate)  values ('".$adjustID."','" . $arryDetails['warehouse'] . "','" . $serial_noum[$j] . "','" . addslashes($arryDetails['sku' . $i]) . "','" . $AID . "','".$arryDetails['Condition' . $i]."','" . addslashes($serial_desc[$j]) . "','Adjustment', '".addslashes($srPrice)."','".$Config['TodayDate']."')";
						$this->query($strSQLQuery, 0);
						//echo   $serial_no[$i]."<br/>"; 
						}
 }
   ...
   /*********Added By karishma based on Condition 6 Jan 2016*********************/
					if($arryDetails['Condition' . $i]!=''){
						...
						if($restbl[0]['total']==0){
							...
							'" . addslashes($arryDetails['qty' . $i]) . "','" . addslashes($arryDetails['price' . $i]) . "','" . $arryDetails['warehouse'] . "')";
							$this->query($strSQLQuery, 0);
						}else{
							// update in tbl
...
//if($Module=='Order' && empty($id)){
					if($arryDetails['Condition'.$i]!=''){
						...
						if($restbl[0]['total']>0){*/
							...
							// update in tbl
							 $UpdateQtysql = "update inv_item_quanity_condition set condition_qty = condition_qty-" . $arryDetails['qty' . $i] . " ,AvgCost =AvgCost-" . addslashes($arryDetails['amount' . $i]) . "  where Sku='" . $arryDetails['sku'.$i] . "' and `condition`='".addslashes($arryDetails['Condition'.$i])."' and WID=1 "; 
							$this->query($UpdateQtysql, 0);
						//}
					}
//}
...
if($WID>0){ $warehouse = $WID;}
        ...
        $strAddQuery .= ($warehouse!='')?(" and s.warehouse='".$warehouse."'"):(" ");
...
$Columns = " s.*, w.warehouse_name ,w.warehouse_code,o.InvoiceID,o.CustCode,o.CustomerName as OrderCustomerName, IF(c.CustomerType = 'Company' and c.Company!='', Company, FullName) as CustomerName,p.SuppCode,p.SuppCompany,p.PurchaseID,pinv.InvoiceID as pInvoiceID,pinv.PurchaseID,pinv.OrderID as pInvID";
 //$strAddQuery .= (!empty($sortby)) ? (" order by " . $sortby . " ") : (" group by o.InvoiceID,s.serialNumber ");
	...
	if($Config['RecordsPerPage']>0){
          ...
          $strSQLQuery = "SELECT ".$Columns."  FROM inv_serial_item s left outer join w_warehouse w on s.warehouse = w.WID left outer join s_order o on o.OrderID=s.OrderID left outer join p_order p on p.OrderID=s.ReceiveOrderID  left outer join p_order pinv on pinv.PurchaseID=p.PurchaseID and pinv.Module='Invoice' left outer join s_customers c on o.CustCode=c.CustCode " . $strAddQuery; 
...
$strSQLTransQuery = "insert into inv_item_quanity_condition 
							...
							'" . addslashes($arryDetails['qty' . $i]) . "','".$to_WID."')";
...
function GetAvgSerialPrice($item_id,$key){



/*$strAddQuery .= (!empty($key['Condition']))?(" and t.TranCondition='".$key['Condition']."' "):("");

  $strAddQuery .= (!empty($key['Sku']))?(" and t.TransactionSku='".$key['Sku']."' "):("");
			$strAddQuery .= (!empty($item_id)) ? (" and t.TransactionItemID='" . $item_id . "'") : ("");
$strAddQuery .= (!empty($key['CatID']))?(" and c.CategoryID='".$key['CatID']."' "):("");
*/
...
$strAddQuery .= (!empty($key['Condition']))?(" and `Condition`='".$key['Condition']."' "):("");
  ...
  $strAddQuery .= (!empty($key['Sku']))?(" and Sku='".$key['Sku']."' "):("");
			//$strAddQuery .= (!empty($item_id)) ? (" and t.TransactionItemID='" . $item_id . "'") : ("");
//$strAddQuery .= (!empty($key['CatID']))?(" and c.CategoryID='".$key['CatID']."' "):("");
//$Sql = "select (SUM(t.ConvertAmt*t.TransactionQty )/SUM(t.TransactionQty)) as price,t.TransactionID ,t.TransactionSku,t.TransactionItemID,SUM(t.TransactionQty) as TotQty from inv_item_transaction t  where 1 and (t.TransactionType ='Adjustment' or t.TransactionType='PO Receipt' ) and  (t.valuationType ='Serialized' or t.valuationType='Serialized Average' ) " . $strAddQuery . "  "; 




 //$Sql = "select SUM(t.ConvertAmt*t.TransactionQty ) as price,t.TransactionID ,t.TransactionSku,t.TransactionItemID,SUM(t.TransactionQty) as TotQty from inv_item_transaction t  where 1 and (t.TransactionType ='Adjustment' or t.TransactionType='PO Receipt' ) and  (t.valuationType ='Serialized' or t.valuationType='Serialized Average' ) " . $strAddQuery . "  "; 
//$rs =  $this->query($Sql, 1);




/*$Sql2 = "select (t.ConvertAmt) as conAmt,s.UsedSerial,t.freight_cost,t.TransactionID ,t.TransactionSku,t.TransactionItemID,c.valuationType,t.TranCondition,
IF(s.UsedSerial='0' OR s.UsedSerial='',1, NULL) 'srQt',t.TransactionQty as TotQty,s.serialID as serialQty,c.CategoryID,i.CategoryID as ItemCat from inv_item_transaction t  right  join inv_serial_item s on ((s.ReceiveOrderID=t.TransactionOrderID) or (s.adjustment_no=t.TransactionOrderID) ) left  join inv_items i on i.ItemID = t.TransactionItemID left  join inv_categories c on c.CategoryID =i.CategoryID where 1 and (t.TransactionType ='Adjustment' or t.TransactionType='PO Receipt' ) and  (t.valuationType ='Serialized' or t.valuationType='Serialized Average' ) and s.UsedSerial='0' " . $strAddQuery . " "; */

$Sql2 = "select count(serialID) as srQt ,SUM(UnitCost) as conAmt  from inv_serial_item where 1 and UsedSerial =0 and warehouse=1 ".$strAddQuery."";
/*
$rs2 =  $this->query($Sql2, 1);
foreach($rs2 as $values){


    $frCost += $values['freight_cost']/$values['TotQty'];
		$TotCost += $values['conAmt']*$values['TotQty'];
		$toQty +=$values['TotQty']; 
}

 $avrageCost[0]['price'] = $TotCost/$toQty ; 
...
//$Sql2 = "select (t.ConvertAmt) as conAmt,t.freight_cost,t.TransactionID ,t.TransactionSku,t.TransactionItemID,t.TranCondition,t.TransactionQty as TotQty,serialID as serialQty,c.CategoryID,i.CategoryID as ItemCat,c.valuationType,s.UsedSerial,s.serialNumber from inv_item_transaction t  left  join inv_serial_item s on (s.ReceiveOrderID=t.TransactionOrderID ) left  join inv_items i on i.ItemID = t.TransactionItemID left  join inv_categories c on c.CategoryID =i.CategoryID where 1 and (t.TransactionType ='Adjustment' or t.TransactionType='PO Receipt' ) and  (t.valuationType ='Serialized' or t.valuationType='Serialized Average' ) " . $strAddQuery . " "; 
...
if($values['valuationType']=='Serialized Average'){

			if($values['UsedSerial'] =='' || $values['UsedSerial'] ==0 ){
...
}else if($values['valuationType']=='Serialized'){
			if($values['UsedSerial'] =='' || $values['UsedSerial'] ==0){
			    $frCost = $values['freight_cost']/$values['TotQty'];
					$TotCost += $values['conAmt']+$frCost;
					$toQty +=$values['srQt']; 
...
echo "Tot=".$TotCost."= TotQty =".$toQty;
//echo "Tot=".$avrageCost[0]['price'];exit;
//echo number_format($avrageCost[0]['price'], 2, '.', ''); exit; 
echo $Sql2; exit;
/*$Sql2 = "select t.ConvertAmt,t.TransactionID ,t.TransactionSku,t.TransactionItemID,(SUM(t.TransactionQty)-COUNT(s.serialID)) as TotQty,COUNT(s.serialID) from inv_item_transaction t  left  join inv_serial_item s on (s.ReceiveOrderID=t.TransactionOrderID and s.UsedSerial=1) where 1 and (t.TransactionType ='Adjustment' or t.TransactionType='PO Receipt' ) and  (t.valuationType ='Serialized' or t.valuationType='Serialized Average' ) " . $strAddQuery . " group by  t.TransactionOrderID,s.ReceiveOrderID"; 

$rs2 =  $this->query($Sql2, 1);

foreach($rs2 as $values){

$TotCost += $values['ConvertAmt']*$values['TotQty'];
$toQty +=$values['TotQty']; 
}

echo  $avrageCost[0]['price'] = $TotCost/$toQty ; 
 echo "<pre>";
print_r($rs2); exit;*/ }
...
return $rs;
    ...
    function GetTransactionForSku($TransactionSku) {
      global $Config;
        $strSQLQuery = "select * from inv_item_transaction where 1 and (TransactionSku = '".$TransactionSku."' OR 	TransactionItemID='".$Config['ItemID']."' ) order by TransactionID desc";
        return $this->query($strSQLQuery, 1);
    }
...
if($_GET['this'] ==1){ echo  $strSQLQuery; exit;}
...
function getItemCondion($Sku,$Condi) {//,$isdefault//
	$strAddQuery = (!empty($Condi))?(" and `condition`='".addslashes($Condi)."'"):("");
	$strAddQuery.= (!empty($WID))?(" and WID = '".addslashes($WID)."' "):("and WID = 1 ");
	$strSQLQuery = "select condition_qty,`condition`,AvgCost,SalePrice,ItemID  from inv_item_quanity_condition where Sku = '" . $Sku . "' and WID=1 ".$strAddQuery." group by `condition` desc ";
	$row = $this->query($strSQLQuery, 1);
	return $row;
}
...
$strAddQuery = (!empty($Condi))?(" and `condition`='".addslashes($Condi)."'"):("");
		 $strSQLQuery = "select condition_qty,`condition`,AvgCost,SalePrice,pricetype,prpercent,fprice,qtyfrom,qtyto from inv_item_quanity_condition where Sku = '" . $Sku . "' and WID=1 ".$strAddQuery;
		$row = $this->query($strSQLQuery, 1);
		return $row;
	}

/*********************/
function getCountItemCondion($Sku,$Condi) {//,$isdefault//
	$strAddQuery = (!empty($Condi))?(" and `condition`='".addslashes($Condi)."'"):("");
	$strAddQuery.= (!empty($WID))?(" and WID = '".addslashes($WID)."' "):("and WID = 1 ");
...
//added by chetan on 19jan2017// updated (sku and condition) by chetan on 15Jan2017//updated again for WID on 3Apr2017//
	...
	$strAddQuery .= ($WID) ? (" AND t1.WID = '".$WID."' ") : (" and t1.WID = 1 ");  //added on 3Apr2017//
        $strSQLQuery = "SELECT Sum(t1.qty) as total FROM ".$table." as t1 LEFT OUTER JOIN ".$jointable." as t2 ON t2.OrderID = t1.OrderID WHERE   t2.Module = 'Order' " . $strAddQuery . " $Cond ";
              if($_GET['QRY']==1 && $for == 'SO'){echo "=>".$strSQLQuery;}
        return $this->query($strSQLQuery, 1);
        }
...
//SELECT SUM(t1.qty_received) as total FROM p_order_item as t1 LEFT OUTER JOIN p_order as t2 ON t2.OrderID = t1.OrderID WHERE t2.Module = 'Receipt' AND t2.OrderDate >= DATE_SUB(NOW(), INTERVAL 7 DAY) and t2.ReceiptStatus='Completed' 
               ...
               if($_GET['QRY']==1 ){echo "=>".$strSQLQuery;}
        return $this->query($strSQLQuery, 1);
        }
...
function GetBOMById($id) {
        ...
        $strSQLQuery = "select bomID from inv_bill_of_material " . $strAddQuery;
        ;
$rs = $this->query($strSQLQuery, 1);
if(!empty($rs[0]['bomID'])){
return $rs[0]['bomID'];
