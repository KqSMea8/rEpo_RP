...
if($num>0){
	...
	$header = "Payment Date\tPayment Method\tPayment Ref #\tInvoice #\tInvoice Date\tOrder #\tOrder Date\tCustomer\tAmount\tCurrency";
	...
	foreach($arryPayment as $key=>$values){		 
		...
		$line = $PaymentDate."\t".stripslashes($values["Method"])."\t".stripslashes($values["ReferenceNo"])."\t".$values['InvoiceID']."\t".$InvoiceDate."\t".$values['SaleID']."\t".$OrderDate."\t".stripslashes($values["CustomerName"])."\t".$values['DebitAmnt']."\t".$values['Currency']."\n";
