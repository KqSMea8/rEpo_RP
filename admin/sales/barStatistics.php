...
if((!empty($_GET['f']) && !empty($_GET['t'])) || $_GET['y']){
		...
		if($_GET['fby']=="Year"){
			...
			for($i=1;$i<=12;$i++) {
				...
				$totalOrderAmnt = $objSale->GetOrderAmountByYear($_GET['y'],$from,$to,$_GET['c'],$_GET['st']);
				$valuesBar[$MonthName] = $totalOrderAmnt;
			...
			for($i=1;$i<=$d;$i++) {
				...
				/*$from = 2014-03-01;
				$to = 2014-03-31;*/
				$totalOrderAmnt = $objSale->GetOrderAmountByMonth($_GET['y'],$from,$to,$_GET['c'],$_GET['st']);
			...
			for($i=$FromYear;$i<=$ToYear;$i++) {
				$totalOrderAmnt = $objSale->GetOrderAmountByYear($i,$_GET['f'],$_GET['t'],$_GET['c'],$_GET['st']);
		...
		*/
		...
		//image filed
		 ...
		 $xname = 'Total Sales Amount:'.$OrderTotal;
		//border margins
		 $margins=60;
		 $ymargin=40;
		 ...
		 $bar_color=imagecolorallocate($img,0,80,123);
...
$values_color=imagecolorallocate($img,73,86,58);//values color
		...
		imageline($img,$img_width-$ymargin,2,$img_width-$ymargin,$LineHeight,$xyline_color);
		 ...
		 $max_value=(max($valuesBar)>100000)?(max($valuesBar)):(100000); 
		 ...
		 for($i=1;$i<=$horizontal_lines;$i++){
			...
			imagestring($img,0,15,$y-2,$v,$values_color);
		 }
		 $ymargin = $ymargin;
		 for($i=0;$i< $total_bars; $i++){
			...
			imagestring($img,0,$x1,$y1-10,$value,$values_color);
			if($_GET['fby'] == "Month"){
			...
			imagestringup($img,5,-1,280, $yname, $txt_color);
			imagestring	($img,2,$img_width-220,2, $xname, $txt_color);
