		...
		var SendUrl = "&action=ItemInfo&ItemID=" + escape(ItemID) + "&proc=" + escape(proc) +"&showcompo="+ showComponent + "&SelID"+SelID+"&r=" + Math.random();
            ...
            $.ajax({
                ...
                success: function(responseText) {
                       ...
                       /*alert(responseText["purchasePrice"]);
                        return false;*/
                      //if(document.getElementById("MDiscount").value == 'Cost')  
			...
			//window.parent.document.getElementById("Req_ItemID" + SelID).value = responseText["ReqItemIDs"];     //By Chetan 4Dec//
/*
...
var prev=window.parent.document.getElementById("VariantINvalues" + SelID).innerHTML;
       ...
       //End
		    ...
		    window.parent.document.getElementById("sku" + SelID).focus();			//By Chetan 31Aug//
    ...
    <tr>
        <td id="ProductsListing" height="400" valign="top">
            ...
            <form action="" method="post" name="form1">
				...
				if(!isset($values["bill_option"])) $values["bill_option"]='';
					...
					?>
                                ...
                                <tr align="left" valign="middle" bgcolor="<?= $bgcolor ?>">
                                    <td>
                                        ...
                                        <?php if($values["bill_option"] == "Yes" || $AliasNum > 0 ){ ?>
                                        ...
                                        <?php } else { //By Chetan 14Aug//?>
                                        <?php if(count($compo) > 0 && $values['itemType'] == 'Kit'){?>
                                        <a href="Javascript:void(0)" onclick="Javascript:$('#compo'+<?=$Line?>).show();" onMouseover="ddrivetip('<?= CLICK_TO_SELECT ?>', '', '')" onMouseout="hideddrivetip()"><?= $values["Sku"] ?></a>
                                        <?php }else{?>
			...
			<?php  //By Chetan 14Aug// 
				...
				<tr id="compo<?=$Line?>" style="display:none">
				<td>Display Component Item</td>
				<td colspan="5">
					<input type="radio" name="yes" id="yes" onclick="Javascript:SetItemCode('<?= $values["ItemID"] ?>', '<?= $values["Sku"] ?>');" value="yes"> Yes&nbsp;&nbsp;
					<input type="radio" onclick="Javascript:SetItemCode('<?= $values["ItemID"]?>', '<?= $values["Sku"]?>');" name="no" id="no" value="no"> No</td>    
				</tr>
