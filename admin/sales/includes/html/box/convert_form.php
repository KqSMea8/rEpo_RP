...
function validate_convert_form(frm){
	...
	if(Trim(document.getElementById("SaleID")).value!=''){
		if(!ValidateMandRange(document.getElementById("SaleID"), "SO Number",3,20)){
			return false;
		}
		...
		httpObj.onreadystatechange = function CheckExistRequest(){
			if (httpObj.readyState == 4) {
				if(httpObj.responseText==1) {
					alert("Sale Order Number already exists in database. Please enter another.");
					return false;
	...
	<tr>
	  ...
	  <tr>
		  ...
		  <table width="100%" border="0" cellpadding="5" cellspacing="1" align="center" bgcolor="#FFFFFF" class="borderall">
			 ...
			 <tr>
        <td  align="right"   class="blackbold" width="30%" > <?=$ModuleIDTitle?> # : </td>
        <td   align="left">
			<?php echo stripslashes($arrySale[0][$ModuleID]); ?>
					 ...
					 <tr>
						  ...
						  <td  align="left">
                            <input name="SaleID" type="text" class="datebox" id="SaleID" value="<?=$NextSalesModuleID?>"  maxlength="20" onKeyPress="Javascript:ClearAvail('MsgSpan_SaleID');return isAlphaKey(event);" onBlur="Javascript:CheckAvailField('MsgSpan_SaleID','SaleID','<?=$_GET['edit']?>');" />
							</td>
						</tr>
					<tr>
						<td   align="center"></td>
