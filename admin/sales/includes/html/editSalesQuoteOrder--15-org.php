...
if($objConfigure->getSettingVariable('SO_SOURCE')==1){
	if(empty($arryCompany[0]['Department']) || substr_count($arryCompany[0]['Department'],2)==1){
		...
		/******/
		$OrderSourceVal = strtolower($arrySale[0]['OrderSource']);
		if($arrySale[0]['EntryBy'] =='C' && ($OrderSourceVal=='amazon' || $OrderSourceVal=='ebay') && $arrySale[0]['ecom_order_id']>0){
			$TaxReadonly = 1;
		}
		/******/
	}
...
function DeleteFileStorage(file_path,file_div,id,ModuleName,Module){
    ...
    $("#dialog-modal").dialog(
    {
		...
		buttons: {
			"Ok": function() {
				...
				$(this).dialog("close");
				var SendParam = "action=delete_file_Storage&file_path="+escape(file_path)+"&id="+id+"&ModuleName="+ModuleName+"&Module="+Module+"&Config=1&r="+Math.random(); 
		    var IsExist = 0;
		    $.ajax({
			type: "GET",
			async:false,
...
document.getElementById("SalesPersonType").value=responseText["SalesPersonType"]; //added by nisha
		}else{
      ...
      document.getElementById("SalesPersonType").value=''; //added by nisha
...
<input name="SalesPersonType" id="SalesPersonType" value="<?php echo stripslashes($arrySale[0]['SalesPersonType']); ?>" type="hidden"><!---//added by nisha----->
...
<input type="hidden" name="TaxReadonly" id="TaxReadonly" class="disabled" readonly value="<?=$TaxReadonly?>" maxlenght="1" >
