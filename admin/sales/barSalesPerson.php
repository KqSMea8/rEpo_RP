...
if((!empty($_GET['f']) && !empty($_GET['t'])) || $_GET['y']){
		...
		if($_GET['fby']=="Year"){
			...
			for($i=1;$i<=12;$i++) {
				...
				$arryNumOrder = $objSale->GetNumSOByYear($_GET['y'],$from,$to,'',$_GET['s'],$_GET['st']);
			...
			for($i=1;$i<=$d;$i++) {
				...
				/*$from = 2014-03-01;
				$to = 2014-03-31;*/
				$arryNumOrder = $objSale->GetNumSOByMonth($_GET['y'],$from,$to,'',$_GET['s'],$_GET['st']);
			...
			for($i=$FromYear;$i<=$ToYear;$i++) {
				$arryNumOrder = $objSale->GetNumSOByYear($i,$_GET['f'],$_GET['t'],'',$_GET['s'],$_GET['st']);
				#$rand = rand(10,100);
