...
class facebookSearcher
{
	...
	public function setOffset($value) {
		$this->_offset = $value; return $this;
	}
	
	public function setAfterid($value) {
		$this->_afterid = $value; return $this;
	}
	...
	protected function buildQueryUrl()
	{
		...
		//Build URL
		$graphURL = "https://graph.facebook.com/search?fields=birthday,age_range,cover,about,email,education,first_name,last_name,picture,address&q=".urlencode($this->_query);
		if(isset($this->_type)) $graphURL .= "&type={$this->_type}";
		if(isset($this->_limit)) $graphURL .= "&limit={$this->_limit}";
		if(isset($this->_offset)) $graphURL .= "&offset={$this->_offset}";
		if(isset($this->_afterid)) $graphURL .= "&__after_id={$this->_afterid}";
