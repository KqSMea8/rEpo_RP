...
class employee extends dbClass
{
		...
		function GetEmployeeBrief($EmpID)
		{
			...
			$strSQLQuery = "select e.EmpID,e.EmpCode,e.UserName,e.Email,e.JobTitle, e.catID, e.Division, d.depID, d.Department from h_employee e left outer join  h_department d on e.Department=d.depID where e.Status=1 ".$strAddQuery." order by d.depID asc,e.UserName asc";
			return $this->query($strSQLQuery, 1);
		}
		...
		function UpdateJobDetail($arryDetails){   
			...
			$Division = $arryRow[0]["Division"];
				...
				,SalaryFrequency='".mysql_real_escape_string($SalaryFrequency)."',shiftID='".mysql_real_escape_string($shiftID)."'
				where EmpID='".mysql_real_escape_string($EmpID)."'"; 
				$this->query($strSQLQuery, 0);
...
$this->query($strSQLQuery, 0);	
		...
		function UpdateDeptHead($Department,$EmpID,$DeptHead){   
			if(!empty($EmpID) && !empty($Department)){
