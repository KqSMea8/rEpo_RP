...
{ 
	...
	function getLeaveBalance($EmpID,$LeaveType){
		...
		if(!empty($EmpID)){
			$strSQLQuery = "select e.EmpID,e.JobType, e.JoiningDate,e.LeaveAccrual,e.ProbationPeriod,e.EligibilityPeriod from h_employee e where  e.EmpID=".mysql_real_escape_string($EmpID)." ";
	...
	function getShortLeave($EmpID=0,$year,$month)
		...
		function addCustomRule($arryDetails)
		{
			...
			$sql = "insert into h_leave_rule (locationID, Heading, Detail, Status, JobType, RuleOn, RuleOpp, RuleValue,Date,LeaveAllowed) values('".$_SESSION['locationID']."', '".mysql_real_escape_string($Heading)."', '".mysql_real_escape_string($Detail)."', '".$Status."', '".mysql_real_escape_string($JobType)."', '".mysql_real_escape_string($RuleOn)."', '".mysql_real_escape_string($RuleOpp)."', '".mysql_real_escape_string($RuleValue)."', '".$Config['TodayDate']."', '".mysql_real_escape_string($LeaveAllowed)."')";		
			$this->query($sql, 0);
			return true;
		}
		function updateCustomRule($arryDetails)
		{
			@extract($arryDetails);	
			if(!empty($RuleID)){ 
				$sql = "update h_leave_rule set Heading = '".mysql_real_escape_string($Heading)."', Detail = '".mysql_real_escape_string($Detail)."', JobType = '".mysql_real_escape_string($JobType)."' , RuleOn = '".mysql_real_escape_string($RuleOn)."', RuleOpp = '".mysql_real_escape_string($RuleOpp)."', RuleValue = '".mysql_real_escape_string($RuleValue)."', Status = '".$Status."' , LeaveAllowed = '".mysql_real_escape_string($LeaveAllowed)."' where RuleID = '".mysql_real_escape_string($RuleID)."'"; 
				$rs = $this->query($sql,0);
			}
	...
	if(!empty($JobType)){
		...
		if(sizeof($arryCustomRule)>0){
			foreach($arryCustomRule as $key=>$values){
				#echo $values['RuleOn'].' '.$values['RuleOpp'].' '.$values['RuleValue'].' : '.$values['LeaveAllowed'].'<br>';exit;
				/////////////
