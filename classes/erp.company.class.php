...
class company extends dbClass
{
		...
		function  ListCompany($id=0,$SearchKey,$SortBy,$AscDesc)
		{
			...
			if($Config['GetNumRecords']==1){
				...
				$Columns = " c.CmpID,c.Status, c.DisplayName, c.CompanyName, c.Email, c.Image, c.ExpiryDate, c.PaymentPlan, c.DefaultCompany, c.DefaultInventoryCompany  ";
		...
		function  GetCompanyBrief($CmpID)
		{
			...
			$strSQLQuery = "select c.CmpID, c.CompanyName, c.DisplayName, c.Email, c.Image, c.Timezone, c.DateFormat, c.TimeFormat, c.SessionTimeout, c.Department, c.currency_id, c.MaxUser,c.RecordsPerPage, c.StorageLimit , c.StorageLimitUnit,c.Storage,c.TrackInventory,c.TrackVariant,c.sync_items,c.sync_type,c.ExpiryDate,c.AdditionalCurrency, c.PunchingBlock, c.PunchingIP, c.LoginBlock, c.LoginIP, c.PaymentPlan,c.DefaultInventoryCompany,c.SelectOneItem,c.spiffDis from company c where c.Status=1 ".$strAddQuery." order by c.CmpID asc";
			return $this->query($strSQLQuery, 1);
		}
...
if(!empty($Timezone)){
	...
	$Timezone = $this->UpdateUpdateTimezone($arryRgn['Country'],$arryRgn['State'],$arryRgn['City']);
			...
			$contents = str_replace("[URL]",$Config['Url'].'ezneterp',$contents);
			$contents = str_replace("[SITENAME]",'eznetERP',$contents);
		...
		/*****************************/
				...
				,UpdatedDate = '".date('Y-m-d')."', Fax='".addslashes($Fax)."', Website='".addslashes($Website)."',MultiSite='".addslashes($MultiSite)."', TrackInventory='".mysql_real_escape_string($TrackInventory)."',TrackVariant='".mysql_real_escape_string($TrackVariant)."', EIN='".mysql_real_escape_string($EIN)."',sync_items='".mysql_real_escape_string($sync_items)."', sync_type='".mysql_real_escape_string($sync_type)."'  ".$addSql."	where CmpID='".mysql_real_escape_string($CmpID)."'"; 
				//echo $strSQLQuery;exit;
				$this->query($strSQLQuery, 0);
		...
		function UpdateAccount($arryDetails){   
			...
			if(!empty($CmpID)){
				...
				$strSQLQuery = "update company set Email='".addslashes($Email)."', RsID='".$RsID."',AutomaticSync='".$AutomaticSync."', Status='".$Status."' ".$AddSql." where CmpID='".$CmpID."'"; 
				$this->query($strSQLQuery, 0);
		...
		function UpdateAdminModules($CmpID,$Department)
		{
			...
			if(!empty($CmpID)){
				...
				$sql1 = "update admin_modules set Status='".$I_EmpStatus."' where ModuleID in('2005','2009','2012','2064') ";
				$this->query($sql1,0);
	...
	function UpdateInventoryModules($CmpID,$TrackInventory)
		{
			...
			if(!empty($CmpID)){
				...
				$sql2 = "update admin_modules set Status='".$MenuStatus."' where Parent in('602', '603','604', '605', '643','644', '654','636',' 637','639', '642','648', '649', '650', '651', '652','653') ";
				$this->query($sql2,0);
				/***********************/
...
function UpdateStandardModules($CmpID,$industry)
		{
	...
	function syncbom($CompanyToImportDB,$AutomaticSyncAction,$Id=''){
		...
		foreach($res as $required){
			...
			$AddedBomId=$this->lastInsertId();
				...
				select optionID,'".$AddedBomId."' as bomID,option_cat,option_code,`req_status`,qty,
...
function UpdateSelectItem($arryDetails){   
	...
	$strSQLQuery = "update company set SelectOneItem='".addslashes($SelectOneItem)."' where CmpID='".$CmpID."'"; 
	$this->query($strSQLQuery, 0);
 ...
 function selectOneSalesOrder($CmpID){
	...
	function UpdateInvSettingOther($arryDetails){   
			extract($arryDetails);
			if(!empty($CmpID)){
				$strSQLQuery = "update company set  TrackInventory='".mysql_real_escape_string($TrackInventory)."',TrackVariant='".mysql_real_escape_string($TrackVariant)."',sync_items='".mysql_real_escape_string($sync_items)."', sync_type='".mysql_real_escape_string($sync_type)."' where CmpID='".mysql_real_escape_string($CmpID)."'"; 
				$this->query($strSQLQuery, 0);
			}
			return 1;
		}
