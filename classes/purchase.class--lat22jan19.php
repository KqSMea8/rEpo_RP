...
function allowEDIdata($order_id,$Module,$db='',$invid=0){    
	...
	if(!empty($invid)){
		 $strqury ="select SerialNumbers from ".$db."s_order_item where OrderID='".$invid."' and Sku ='".$values['sku']."' and `Condition`='".$values['Condition']."'";
	...
	$PercentFreight =0;
       ...
       $PercentFreight = ($arryPurchase[0]['Freight']*$PercentLine)/100;
						...
						if($Module=="Invoice"){
							$stqty = " ,ref_id='".$values['id']."',qty_received='".$values['qty']."',SerialNumbers ='".$SerialNumbers."',freight_cost='".$PercentFreight."' ";
						}else if($Module=="Receipt"){
							$stqty = " , qty_received='".$values['qty']."',ref_id='".$values['id']."',SerialNumbers ='".$SerialNumbers."',freight_cost='".$PercentFreight."' ";
						}else{
							$stqty = " , qty_received='' ";
						}
