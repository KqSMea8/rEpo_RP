...
if(!empty($Timezone)){
	...
	$Timezone = $this->UpdateUpdateTimezone($arryRgn['Country'],$arryRgn['State'],$arryRgn['City']);
		...
		/*****************************/
				...
				,UpdatedDate = '".date('Y-m-d')."', Fax='".addslashes($Fax)."', Website='".addslashes($Website)."', TrackInventory='".mysql_real_escape_string($TrackInventory)."',TrackVariant='".mysql_real_escape_string($TrackVariant)."', EIN='".mysql_real_escape_string($EIN)."',sync_items='".mysql_real_escape_string($sync_items)."' ".$addSql."	where CmpID='".mysql_real_escape_string($CmpID)."'"; 
				echo $strSQLQuery;exit;
				$this->query($strSQLQuery, 0);
		...
		function UpdatePermission($arryDetails){   
			...
			if(!empty($CmpID)){
				/*if(sizeof($Department)>0){
				 ...
				 $strSQLQuery = "update company set MaxUser='".mysql_real_escape_string($MaxUser)."', StorageLimit='".mysql_real_escape_string($StorageLimit)."', StorageLimitUnit='".mysql_real_escape_string($StorageLimitUnit)."', TrackInventory='".mysql_real_escape_string($TrackInventory)."',TrackVariant='".mysql_real_escape_string($TrackVariant)."',sync_items='".mysql_real_escape_string($sync_items)."', Department='".mysql_real_escape_string($Department)."',ErpDomain='".mysql_real_escape_string($ErpDomain)."',WebDomain='".mysql_real_escape_string($WebDomain)."' where CmpID='".mysql_real_escape_string($CmpID)."'"; 
				$this->query($strSQLQuery, 0);
			}
			return 1;
		}
		...
		function UpdateGlobalOther($arryDetails){   
			...
			if(!empty($CmpID) && !empty($RecordsPerPage)){
				$strSQLQuery = "update company set RecordsPerPage='".$RecordsPerPage."', TrackInventory='".mysql_real_escape_string($TrackInventory)."',TrackVariant='".mysql_real_escape_string($TrackVariant)."',sync_items='".mysql_real_escape_string($sync_items)."' where CmpID='".mysql_real_escape_string($CmpID)."'"; 
				$this->query($strSQLQuery, 0);
			}
			return 1;
		}
			...
			if(empty($Department)){
				...
				$C_SpiffStatus=0;				
				 ...
				 if(substr_count($Department,6)==0){  // Item
					$ItemStatus=1; $CustStatus=1; $InvSetting = 0;
				 }
			}
