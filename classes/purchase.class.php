...
$strSQLQuery = "select o.* , s.VAT,s.CST, s.TRN, IF(s.SuppType = 'Individual' and s.UserName!='', s.UserName, s.CompanyName) as VendorName, s.SuppID, s.Email as VendorEmail   from p_order o left outer join p_supplier s ON o.SuppCode = s.SuppCode  where 1 ".$strAddQuery." order by o.OrderID desc";
		...
		function  GetPurchaseItem($OrderID)
		{
			...
			#$strSQLQuery = "select i.*,t.RateDescription from p_order_item i left outer join inv_tax_rates t on i.tax_id=t.RateId where 1".$strAddQuery." order by i.id asc";
                         $strSQLQuery = "select i.*,t.RateDescription,itm.qty_on_hand,itm.weight,itm.wt_Unit, itm.width,itm.ln_Unit, itm.height,itm.wd_Unit, itm.depth,itm.ht_Unit,itm.CategoryID ,itm.sale_tax_rate, itm.purchase_tax_rate ,c.valuationType as evaluationType,w.warehouse_code,w.warehouse_name from p_order_item i left outer join inv_tax_rates t on i.tax_id=t.RateId left outer join inv_items itm on i.item_id=itm.ItemID left outer join inv_categories c on c.CategoryID =itm.CategoryID left outer join w_warehouse w on i.WID=w.WID where 1".$strAddQuery." order by i.id asc";
			return $this->query($strSQLQuery, 1);
		}
...
// Add EDI purchase
		if($EDICompId!='' && $EDISalesCompName!='' && $Module=='Order' ){
			...
			if($EDISalesCompName!='') {
				...
				$_SESSION['CmpDatabase'] = 'erp_'.$EDISalesCompName;
...
$objSale = new sale();
      ...
      $arryDetails['CustCode']=$Customerdetail[0]['CustCode'];
			...
			$arryEdiLocation=$objSale->GetEdiLocationData(1,0,$DBName);
			...
			$arryEdiSelCurrency = $objSale->getEdiCurrency($arryEdiLocation[0]['currency_id'],'');
...
$Commdetail =array();
     ...
     $arryDetails['CityID']=$CustomerBilling[0]['city_id'];
          ...
          $arryDetails['ShippingCityID']=$CustomerShipping[0]['city_id'];
					...
					//Shipping end
					$arryDetails['ShippingAccountCustomer']=$ShippingAccountVendor;
	unset($arryDetails['OrderType']);
					$arryDetails['OrderType']='Standard';
					...
					$order_id = $objSale->AddSale($arryDetails,$DBName);
...
#$strSQL = "update ".$DBName."s_customer_shipping 

$strQuery = "INSERT INTO ".$DBName."s_customer_shipping SET api_account_number = '".addslashes($ShippingAccNO)."',api_name = '".addslashes($ShippingMethod)."',live='1',Fixed='1',CustID = '".addslashes($Customerdetail[0]['Cid'])."'";
	...
	$SElectSaleQury = "select PurchaseID from p_order where OrderID='".$OrderID."'";
		$arryPur = $this->query($SElectSaleQury, 1);
			 ...
			 $strSQL2 = "update p_order set EDIRefNo='EDI/".$EDICompId."/".$_SESSION['CmpID']."/".$order_id."/".$arrySal[0]['SaleID']."',EdiSoID ='".$order_id."' where OrderID='".$OrderID."'"; 
			$this->query($strSQL2, 0);

		 $strSQL = "update ".$DBName."s_order set EDIRefNo='EDI/".$EDICompId."/".$_SESSION['CmpID']."/".$OrderID."/".$arryPur[0]['PurchaseID']."',CustomerPO = '".$arryPur[0]['PurchaseID']."',EDICompId='".addslashes($_SESSION['AdminID'])."',EDICompName='".addslashes($_SESSION['DisplayName'])."',EdiPoID ='".$OrderID."' where OrderID='".$order_id."'";
			$this->query($strSQL, 0);
...
function  ListReceipt($arryDetails)
		{
			...
			/**************/
			$FPostedDate = DefaultDateFormat($FPostedDate);
			$TPostedDate = DefaultDateFormat($TPostedDate);		 
			/**************/
...
$strSQLup = "update s_order_item set DropshipCost ='".$arryDetails['price'.$i]."', DropshipUsed ='1' where item_id =".$arryDetails['item_id'.$i]." and OrderID IN(".$Order_IDs.") and DropshipCheck ='1'"; 
 ...
 function AlterSalesForDropship($order_id, $arryDetails){  
		global $Config;
		//pr($arryDetails,1);	
		extract($arryDetails);
 		//$SaleID = 'SO2111';
		/*****Both Po and So invoice should not be posted to gl**********/
		$sqlPInv = "select OrderID from p_order where Module='Invoice' and PurchaseID ='".$PurchaseID."'   "  ;
		$arryPInv = $this->query($sqlPInv, 1);
		$sqlSInv = "select OrderID from s_order where Module='Invoice' and SaleID ='".$SaleID."'  "  ;
		$arrySInv = $this->query($sqlSInv, 1);
		/************************/
		if(!empty($order_id) && empty($arryPInv[0]["OrderID"]) && empty($arrySInv[0]["OrderID"])){

			$arryPoItem = $this->GetPurchaseItem($order_id); 
			 /*****************/
			$sqlSo = "select OrderID, CustCode, Module, TaxRate, tax_auths, CustomerCurrency, Freight,TDiscount,freightTxSet from s_order where SaleID ='".$SaleID."' and Module='Order' order by OrderID Asc "; //so only
			$arrySoOrderID = $this->query($sqlSo, 1); 
 

			if(!empty($arrySoOrderID[0]['CustCode'])){ //customer info
				$sqlC = "select Cid,MDType,MDAmount,CustCode from s_customers where CustCode ='".$arrySoOrderID[0]['CustCode']."' order by Cid Asc ";
				$arryCust = $this->query($sqlC, 1);
			}
			//ConversionRate	
			$PoConversionRate = 1;
			if(!empty($arrySoOrderID[0]['CustomerCurrency']) && !empty($Currency) && $arrySoOrderID[0]['CustomerCurrency']!=$Currency){
				$PoConversionRate = CurrencyConvertor(1,$Currency, $arrySoOrderID[0]['CustomerCurrency'],'AR',$OrderDate);	
			}
 
			//start so, shipment and invoice
			foreach($arrySoOrderID as $key=>$value){   
				//pr($value,0);
				$TotalAmount=0;	
				$taxAmnt=0;
				$TaxRate=0;	
				if($value['tax_auths']=="Yes" && $value['TaxRate']>0){
					$arrRate = explode(":",$value['TaxRate']);
					$TaxRate = $arrRate[2];
				}

				/********delete so item **************/
 				$strSQLd = "Select s.sku FROM s_order_item s left outer join p_order_item p on (s.sku=p.sku and s.sku!='' and p.OrderID='".$order_id."') where s.OrderID='".$value['OrderID']."' and p.sku is NULL   ";
				$arryddItem = $this->query($strSQLd, 1);
 
				foreach($arryddItem as $key=>$values){ 
					$arrayDelSku[] = $values['sku'];
				} 
				if(!empty($arrayDelSku)){ 
					$DelSku = "'".implode ( "', '", $arrayDelSku )."'"; 
					$strSQLd = "delete from s_order_item where sku in (".$DelSku.") and OrderID='".$value['OrderID']."'  "; 
					//echo $strSQLd.'<br><br>';die;
					$this->query($strSQLd, 0);
				}
				/**********************/
	
				$sql = "select * from s_order_item where OrderID='".$value['OrderID']."' order by id asc";
				$arrySoItem = $this->query($sql, 1);
				$arraySoList=array();
				foreach($arrySoItem as $key=>$values){ 
					$sku = $values['sku']; 
					foreach($values as $key2=>$values2){ 					
						$arraySoList[$sku][$key2] =  $values2;
					}
				}
				/************/


				foreach($arryPoItem as $key=>$valueitem){ //start item 
					...
					if(!empty($sku)){
						...
						if($price>0){
							$price = round(GetConvertedAmount($PoConversionRate, $price) ,2);
						}
						$DropshipCost = $price;



						if(!empty($arraySoList[$sku]['sku'])){ //match
							 //update		 									
							
							if($arryCust[0]['MDType']=='Markup'){
								$totDiscountCal = $price*$arryCust[0]['MDAmount']/100;
		 						$price = $price+ $totDiscountCal;
								$price = round($price,2);
							}else{
								$price = $arraySoList[$sku]['price'];
							}

							$discount = round($arraySoList[$sku]['discount']*$qty,2);
							$amount =  round($price*$qty,2) - $discount;
							$TotalAmount += $amount;
							if($TaxRate>0 && $arraySoList[$sku]['Taxable']=='Yes'){
								$tax = ($amount*$TaxRate)/100;		
								$taxAmnt += $tax;
							...
							$strSQLup = "update s_order_item set DropshipCost ='".$DropshipCost."', price ='".$price."', amount ='".$amount."',DropshipUsed ='1' , DropshipCheck ='1' , qty ='".$qty."' ,`Condition` ='".addslashes($valueitem['Condition'])."' where sku ='".$arraySoList[$sku]['sku']."' and OrderID='".$value['OrderID']."'   "; 
							//echo $strSQLup.'<br><br>';
							$this->query($strSQLup, 0);
							...
							if($TaxRate>0 && $valueitem['sale_tax_rate']=='Yes'){
								$tax = ($amount*$TaxRate)/100;		
								$taxAmnt += $tax;
								
							}

							$strSQLup = "insert into s_order_item set DropshipCost ='".$DropshipCost."', price ='".$price."', amount ='".$amount."',DropshipUsed ='1', qty ='".$qty."',sku ='".addslashes($sku)."' ,item_id ='".addslashes($valueitem['item_id'])."'  ,Taxable ='".addslashes($valueitem['sale_tax_rate'])."' ,description ='".addslashes($valueitem['description'])."'  ,`Condition` ='".addslashes($valueitem['Condition'])."' , OrderID='".$value['OrderID']."' , DropshipCheck ='1' "; 
							//echo $strSQLup.'<br><br>';
							$this->query($strSQLup, 0);
				...
				/***********************/
				if($Freight>0){  
					$SoFreight = round(GetConvertedAmount($PoConversionRate, $Freight) ,2);			}else{
					$SoFreight = $value['Freight'];
				}
				$TotalAmount += $SoFreight;			

				if($value['TDiscount']>0){
					if($TaxRate>0){
						$taxDis = ($value['TDiscount']*$TaxRate)/100;
						$taxAmnt -= $taxDis;
					}
					$TotalAmount -= $value['TDiscount'];
				}
				if($TaxRate>0 && $value['freightTxSet']=="Yes" && $SoFreight>0){
					$taxFreight = ($SoFreight*$TaxRate)/100;		
					$taxAmnt += $taxFreight;
				}					

				if($taxAmnt>0){
					$taxAmnt = round($taxAmnt,2);
					$TotalAmount += $taxAmnt;
				}
				
				$strSQL = "update s_order set Freight ='".$SoFreight."', taxAmnt ='".$taxAmnt."',TotalAmount ='".$TotalAmount."' where OrderID='".$value['OrderID']."'  "; 
				//echo $strSQL.'<br><br>';
				$this->query($strSQL, 0);
				
				
   				/***********************/
			} //end so, shipment and invoice
...
$qty_receivedSql  = "update p_order_item set ".$DBName."qty_received = qty_received+".$qty_received ."  where id='".$arryItem[$Count]["ref_id"]."'  ";
...
//$serial_price = explode(",", $arryDetails['serialPrice' . $i]);
 ...
 $query = 'INSERT INTO inv_serial_item (warehouse,serialNumber,Sku,UsedSerial,ReceiveOrderID,`Condition`,type,UnitCost,description,ReceiptDate,binid)  VALUES ';
    $query_parts = array();
    for ($j = 0; $j < sizeof($serial_no); $j++) {
        $query_parts[] = "('".$arryItem[$Count]["WID"]."','" . $serial_no[$j] . "','" . addslashes($mainSku) . "','0','" . $OrderID . "','".$arryItem[$Count]["Condition"]."','Receive Order','".$NetPriceSr."','".addslashes($serial_desc[$j])."','".$Config['TodayDate']."','".$arryItem[$Count]["binid"]."')";
    }
...
if($numSrId == 0){
	...
	function UpdatePoStatusByReceipt($ReceiptOrderID){
		if(!empty($ReceiptOrderID)){			
			$arryPurchase = $this->GetPurchase($ReceiptOrderID,'','');
...
function allowEDIdata($order_id,$Module,$db='',$invid=0){    
		...
		#echo $order_id."-OrderID=>".$Module."-Module=>".$invid."-db=>".$db; 
		/*****************/
		if($order_id>0){
		
		  $arryPurchase = $this->GetPurchase($order_id,'','');
			$arryPurchaseItem = $this->GetPurchaseItem($order_id);
			#pr($arryPurchaseItem); exit;
			/*****************/ 	
			$selectQury = "select SUM(Qty) as totalqty,SUM(amount) as subtotal from p_order_item where OrderID = '".$order_id."'";
			$arryTotQty = $this->query($selectQury,1);
			$TotalQty = $arryTotQty[0]['totalqty'];
			$subtotal = $arryTotQty[0]['subtotal'];
			...
			if($OrderID>0){
				if($Module=="Invoice"){
					$strAdd = " ,Module='Invoice',ReceivedDate='".$Config['TodayDate']."',GenrateInvoice=1 ";
				}else if($Module=="Receipt"){
					$strAdd = " ,Module='Receipt',ReceivedDate='".$Config['TodayDate']."'  ";
				}
				...
				foreach($arryPurchaseItem as $key=>$values){
					  ...
					  $strSQLQuery = "insert into p_order_item (".$FeildAndValues2.") SELECT ".$FeildAndValues2." FROM p_order_item WHERE OrderID = '".$order_id."' and id = '".$values['id']."' and Sku ='".$values['sku']."' and `Condition`='".$values['Condition']."'"; 
					$this->query($strSQLQuery, 0);
					$id = $this->lastInsertId();
					if(!empty($id)){
	...
	if(!empty($invid)){
		 $strqury ="select SerialNumbers,id from ".$db."s_order_item where OrderID='".$invid."' and Sku ='".$values['sku']."' and `Condition`='".$values['Condition']."' and  RecEdiSerial=0";
		$arrySaleData = $this->query($strqury,1);
		if($arrySaleData[0]['SerialNumbers']!=''){
		    $SerialNumbers = $arrySaleData[0]['SerialNumbers'];
		    
		     $strSerialSQL = "update p_order_item set SerialNumbers ='" .$SerialNumbers."'  where id='" .$id. "' and Sku ='".$values['sku']."' and `Condition`='".$values['Condition']."'"; 
						$this->query($strSerialSQL, 0);
						
						
						 $strEdiitemSQL = "update ".$db."s_order_item set RecEdiSerial ='1'  where id='" .$arrySaleData[0]['id']. "'"; 
						$this->query($strEdiitemSQL, 0);
	...
	$TotalWeight=0;
	$PercentFreight =0;
	     //$weight = $weight*$values['qty'];
						//$volume = $volume*$values['qty'];
					if($values['amount']>0 && $AllocationMethod=='Cost'){	
							$PercentLine = ($values['amount']*100)/$subtotal;
						}else if($values['qty']>0 && $AllocationMethod=='Quantity'){		
							$PercentLine = ($values['qty']*100)/$TotalQty;
						}else if($weight>0 && $AllocationMethod=='Weight'){		
							$PercentLine = ($weight*100)/$TotalWeight;
						}else if($volume>0 && $AllocationMethod=='Volume'){		
							$PercentLine = ($volume*100)/$TotalVolume;
						}
				
				
							//$PercentLine =($values['qty']*100)/$TotalQty;
       $PercentFreight = ($arryPurchase[0]['Freight']*$PercentLine)/100;

						if($Module=="Invoice"){
							$stqty = " ,ref_id='".$values['id']."',qty_received='".$values['qty']."',freight_cost='".$PercentFreight."' ";
						}else if($Module=="Receipt"){
							$stqty = " , qty_received='".$values['qty']."',ref_id='".$values['id']."',freight_cost='".$PercentFreight."' ";
						}else{
							$stqty = " , qty_received='' ";
						}
					
					
						 $strSQL = "update p_order_item set OrderID ='" .$OrderID."',qty_returned='', SaleQty='' ".$stqty."  where id='" .$id. "' and Sku ='".$values['sku']."' and `Condition`='".$values['Condition']."'"; 
						$this->query($strSQL, 0);
					}
				}
				/*****************/					
...
function EdiUpdateSo($arryDetails){
  ...
  if($rows[0]['EDICompName']!='') { $DBName='erp_'.$rows[0]['EDICompName'].'.'; $EDICompName=$rows[0]['EDICompName']; } if($AdminID==''){ $AdminID=$_SESSION['AdminID']; }
    ...
    // Add EDI purchase
     ...
     #pr($Customerdetail); exit;
      ...
      $arryDetails['Currency']=$Customerdetail[0]['Currency'];
      $arryDetails['Status']='Open';
      /*if($Customerdetail[0]['MDType']=='Markup'){
               $arryDetails['MDAmount']=$Customerdetail[0]['MDAmount'];
...
$Commdetail =array();
     	$sqlCom= "select GROUP_CONCAT(EmpSpId  SEPARATOR ',') as Userids,GROUP_CONCAT(VenSpId  SEPARATOR ',') as VendorIDs from ".$DBName."customer_sales_commission where CustomerId ='".addslashes($Customerdetail['0']['Cid'])."' ";
          ...
          $sqlbilling= "select * from ".$DBName."s_address_book where CustID='".addslashes($Customerdetail[0]['Cid'])."' and AddType = 'billing' ";
					...
					//$arryDetails['CustomerName']=$CustomerBilling[0]['Company'];
					//$arryDetails['CustomerCompany']=$CustomerBilling[0]['Company'];
					$arryDetails['BillingName']=$Customerdetail[0]['Company'];
					$arryDetails['CustomerName']=$Customerdetail[0]['Company'];
					$arryDetails['CustomerCompany']=$Customerdetail[0]['Company'];
					$arryDetails['Address']=stripslashes($CustomerBilling[0]['Address']);
					$arryDetails['City']=$CustomerBilling[0]['CityName'];
     ...
     $arryDetails['ShippingCityID']=$CustomerShipping[0]['city_id'];
					...
					$arryDetails['ShippingAccountCustomer']=$rows[0]['ShippingAccountVendor'];
					//Shipping end
					...
					$sqlso = "select  OrderID,SaleID from ".$DBName."s_order where CustomerPO = '".$rows[0]['PurchaseID']."' and PickID='' ";
					$arrySo = $this->query($sqlso, 1);
					
				
					if(!empty($arrySo[0]['OrderID'])){
					$strqry ="update p_order set SaleID='".addslashes($arrySo[0]['SaleID'])."' where OrderID='".$OrderID."'";
					$this->query($strqry, 0);
					
					$order_id = $arrySo[0]['OrderID'];
					$arryDetails['OrderID']=$order_id;
					//$arryDetails['order_id']=$order_id;
...
/*************************END EDI**********************************/
 ...
 function AlterPurchaseForDropship($order_id, $arryDetails){  
		global $Config;
		//pr($arryDetails,1);	
		extract($arryDetails);
		$objSale = new sale();
		/*****Both Po and So invoice should not be posted to gl**********/
		 $sqlPInv = "select OrderID from p_order where Module='Invoice' and PurchaseID ='".$PONumber."'   "  ;
		$arryPInv = $this->query($sqlPInv, 1);
		$sqlSInv = "select OrderID from s_order where Module='Invoice' and SaleID ='".$SaleID."'  "  ;
		$arrySInv = $this->query($sqlSInv, 1);
		...
		/************************/
		if(!empty($order_id) && empty($arryPInv[0]["OrderID"]) && empty($arrySInv[0]["OrderID"])){
			$arrySoItem = $objSale->GetSaleItem($order_id); 
			/*************/				
		
			$sqlPo = "select OrderID, SuppCode, Module, TaxRate, tax_auths, Currency, Freight,freightTxSet from p_order where PurchaseID ='".$PONumber."' and Module='Order' order by OrderID Asc "; //po only
			$arryPoOrderID = $this->query($sqlPo, 1);

			

			//ConversionRate	
			$PoConversionRate = 1;
			if(!empty($arryPoOrderID[0]['Currency']) && !empty($CustomerCurrency) && $arryPoOrderID[0]['Currency']!=$CustomerCurrency){
				$PoConversionRate = CurrencyConvertor(1,$CustomerCurrency, $arryPoOrderID[0]['Currency'],'AP',$OrderDate);	
			}
 	
			//start po, shipment and invoice
			foreach($arryPoOrderID as $key=>$value){ 				
				$TotalAmount=0;	
				$taxAmnt=0;
				$TaxRate=0;	
				if($value['tax_auths']=="Yes" && $value['TaxRate']>0){
				...
				/********delete po item **************/
 				$strSQLd = "Select p.sku FROM p_order_item p left outer join s_order_item s on (p.sku=s.sku and p.sku!='' and s.OrderID='".$order_id."') where p.OrderID='".$value['OrderID']."' and s.sku is NULL   ";
				$arryddItem = $this->query($strSQLd, 1); 
				foreach($arryddItem as $key=>$values){ 
					$arrayDelSku[] = $values['sku'];
				} 

				if(!empty($arrayDelSku)){ //delete
					$DelSku = "'".implode ( "', '", $arrayDelSku )."'"; 
					$strSQLd = "delete from p_order_item where sku in (".$DelSku.") and OrderID='".$value['OrderID']."'  "; 
					//echo $strSQLd.'<br><br>';
					$this->query($strSQLd, 0);
				}
				/****************/
	
				$sql = "select * from p_order_item where OrderID='".$value['OrderID']."' order by id asc";
				$arryPoItem = $this->query($sql, 1);
				$arrayPoList=array();
				foreach($arryPoItem as $key=>$values){ 
					$sku = $values['sku']; 
					foreach($values as $key2=>$values2){ 					
						$arrayPoList[$sku][$key2] =  $values2;
					}
				}
				/************/

				foreach($arrySoItem as $key=>$valueitem){ //start item 
 
					$sku = $valueitem['sku']; 					
					if(!empty($sku)){
						$qty = $valueitem["qty"];
						if($valueitem['DropshipCheck'] ==1){
							$DropshipCost = $valueitem["DropshipCost"];
						}else{
							$DropshipCost = $valueitem["avgCost"];
						}
						if($DropshipCost>0){
						...
						if(!empty($arrayPoList[$sku]['sku'])){ //match
							 //update
							 					 
							if($TaxRate>0 && $arrayPoList[$sku]['Taxable']=='Yes'){
								$tax = ($amount*$TaxRate)/100;		
								$taxAmnt += $tax;
							...
							$strSQLup = "update p_order_item set DropshipCheck ='".$valueitem["DropshipCheck"]."' ,`Condition` ='".addslashes($valueitem['Condition'])."' , price ='".$price."', DropshipCost ='".$DropshipCost."', amount ='".$amount."'  , qty ='".$qty."'  where sku ='".$arrayPoList[$sku]['sku']."' and OrderID='".$value['OrderID']."'   "; 
							//echo $strSQLup.'<br><br>';
							$this->query($strSQLup, 0);
							...
							if($TaxRate>0 && $valueitem['purchase_tax_rate']=='Yes'){
								$tax = ($amount*$TaxRate)/100;		
								$taxAmnt += $tax;
								
							}

							$strSQLup = "insert into p_order_item set DropshipCheck ='".$valueitem["DropshipCheck"]."', price ='".$price."', DropshipCost ='".$DropshipCost."', amount ='".$amount."', qty ='".$qty."',sku ='".addslashes($sku)."' ,item_id ='".addslashes($valueitem['item_id'])."' ,description ='".addslashes($valueitem['description'])."' ,Taxable ='".addslashes($valueitem['purchase_tax_rate'])."'  ,`Condition` ='".addslashes($valueitem['Condition'])."' , OrderID='".$value['OrderID']."'   "; 
							//echo $strSQLup.'<br><br>';
							$this->query($strSQLup, 0);
				...
				/***********************/
				if($Freight>0){  
					$PoFreight = round(GetConvertedAmount($PoConversionRate, $Freight) ,2);			}else{
					$PoFreight = $value['Freight'];
				}
				$TotalAmount += $PoFreight;			

				
				if($TaxRate>0 && $value['freightTxSet']=="Yes" && $PoFreight>0){
					$taxFreight = ($PoFreight*$TaxRate)/100;		
					$taxAmnt += $taxFreight;
				}					
				...
				$strSQL = "update p_order set Freight ='".$PoFreight."', taxAmnt ='".$taxAmnt."',TotalAmount ='".$TotalAmount."' where OrderID='".$value['OrderID']."'  "; 
				//echo $strSQL.'<br><br>';
				$this->query($strSQL, 0);
				
				
   				/***********************/
			} //end po, shipment and invoice
	...
	function isSaleExistDropshipPO($SaleID){
		$strSQLQuery = "select OrderID from p_order where SaleID='".$SaleID."' and OrderType='Dropship' and Module='Order' ";
		$arryRow = $this->query($strSQLQuery, 1);

		$strSQLQuery2 = "select OrderID from s_order where SaleID='".$SaleID."' and OrderType='PO' and Module='Order' and PONumber!='' ";
		$arryRow2 = $this->query($strSQLQuery2, 1);

		if (!empty($arryRow[0]['OrderID']) || !empty($arryRow2[0]['OrderID'])) {
			return true;
		} else {
			return false;
		}			
	...
	function isPoExistForSO($PurchaseID){
			$strSQLQuery = "select OrderID from s_order where PONumber='".$PurchaseID."' and OrderType='PO' and Module='Order'";
			$arryRow = $this->query($strSQLQuery, 1);

			$strSQLQuery2 = "select OrderID from p_order where PurchaseID='".$PurchaseID."' and OrderType='Dropship' and Module='Order' and SaleID!='' ";
			$arryRow2 = $this->query($strSQLQuery2, 1);
	...
	function isEdiSaleItemExist($OrderID,$Sku){
		    $strSQLQuery2 = "select Sku from p_order_item where OrderID='".$OrderID."' and sku='".$Sku."'";
		    $arryRow2 = $this->query($strSQLQuery2, 1);
