...
function  PaymentReport($FromDate,$ToDate,$SalesPersonID,$salesPersonType='')
		...
		function  ListARInvoice($arryDetails)
		{
			...
			if($SearchKey=='yes' && ($sortby=='o.Approved' || $sortby=='') ){
				...
				$strAddQuery .=  " and o.InvoicePaid='Unpaid' and o.PaymentTerm='Credit Card' and o.OrderPaid in ('1','3','4') ";
			}else if($sortby=='o.InvoicePaid'){
				$strAddQuery .= (!empty($SearchKey))?(" and (".$sortby." like '".$SearchKey."')"):("");
...
$EdiPOFromSO = $objConfigure->getSettingVariable('EDI_PO_SO');
 // Add EDI purchase
		...
		$arrySal = $this->query($SElectSaleQury, 1);
			...
			$strSQL = "update ".$DBName."p_order set EDIRefNo='EDI/".$_SESSION['CmpID']."/".$EDICompId."/".$OrderID."/".$arrySal[0]['SaleID']."',EDICompId='".addslashes($_SESSION['AdminID'])."',EDICompName='".addslashes($_SESSION['DisplayName'])."',EdiSoID ='".$OrderID."',SuppCompany ='".$arrySuppAdd[0]['Name']."', Address='".$arrySuppAdd[0]['Address']."', City='".$arrySuppAdd[0]['City']."', State='".$arrySuppAdd[0]['State']."', Country='".$arrySuppAdd[0]['Country']."', ZipCode='".$arrySuppAdd[0]['ZipCode']."',   Mobile='".$arrySuppAdd[0]['Mobile']."', Landline='".$arrySuppAdd[0]['Landline']."', Fax='".$arrySuppAdd[0]['Fax']."', Email='".$arrySuppAdd[0]['Email']."' where OrderID='".$order_id."'";
			$this->query($strSQL, 0);
...
return $rs = $this->query($strSQLQuery, 1);
    ...
    global $Config;
			...
			if($Config['ConversionType']==1){
		     ...
		     $strSQLQuery = "select sum(".$ConvertedAmount.") as TotalSales from s_order o   inner join h_commission c on ".$comCondition." where o.Module='Invoice'  and o.SalesPerson!='' and o.SalesPersonID>0 and o.PostToGL='1' ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1  order by o.InvoiceDate asc, o.OrderID asc";
		...
		function  GetInvPaymentMargin($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0)
		{
			...
			if($Config['ConversionType']==1){
			 ...
			 $strSQLQuery = "select  sum(".$ConvertedAmount.") as TotalSales, o.CustomerCurrency,o.ConversionRate,o.OrderID as InID, o.InvoiceID, o.TotalInvoiceAmount,o.Freight,o.Fee ,o.TDiscount,c.CommPaidOn,cs.CommPercentage  from s_order o inner join h_commission c on ".$comCondition." left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice' and o.SalesPerson!='' and o.SalesPersonID>0 and o.PostToGL='1'  ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc";
		...
		function  GetInvPaymentNonResidualMargin($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0)
		{
			...
			if(!empty($SuppID)){			
		 	...
		 	$strSQLQuery = "select  o.InvoiceID from s_order o  inner join h_commission c on ".$comCondition." left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice'  and o.SalesPerson!='' and o.SalesPersonID>0 and o.PostToGL='1' ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc limit 0,1";
			return $this->query($strSQLQuery, 1);
...
function checkCustomerPO($CustomerPO)
	...
	function  GetInvPaymentCal($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0){
		...
		$strSQLQuery = "select sum(".$ConvertedAmount.") as TotalSales,cs.CommPercentage from s_order o   inner join h_commission c on ".$comCondition." left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice'  and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1' ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1   group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc";
	...
	function  GetInvoiceSpiff($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0){
		...
		$strSQLQuery = "select sum(".$ConvertedAmount.") as TotalSales,o.InvoiceID from s_order o inner join h_commission c on ".$comCondition."  where o.Module='Invoice'  and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1' ".$strAddQuery."   group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc";
	...
	function UpdateEdiPO($arryDetails){
		...
		global $Config;
			extract($arryDetails);
		  	 ...
		  	 $strSQLQuery = "select EdiPoID,EDICompId,EDICompName from s_order  where OrderID = '".$OrderID."'"; 
