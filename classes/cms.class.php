...
class cms extends dbClass
{
		 ...
		 /*     * **********************************************************START FUNCTIONS FOR PAGES**************************************************************************** */
                    ...
                    function addPage($arryDetails) {
                        @extract($arryDetails);
                        $sql = "INSERT INTO  e_pages SET Name='" . addslashes($Name) . "',DisplayMenu='" . addslashes($DisplayMenu) . "', Title = '" . addslashes($Title) . "',Priority = '".$Priority."',UrlCustom = '".$UrlCustom."',UrlHash=MD5('".$UrlCustom."'),MetaKeywords='".addslashes($MetaKeywords)."',MetaTitle='".addslashes($MetaTitle)."',MetaDescription='".addslashes($MetaDescription)."',Status='" . $Status . "',Content='".addslashes($page_content)."'";
                        $this->query($sql, 0);
                    }
        ...
        function UpdateImage($imageName,$PageId)
	{
		$strSQLQuery = "update e_pages set Banner_image='".addslashes($imageName)."' where 	PageId='".mysql_real_escape_string($PageId)."'";
