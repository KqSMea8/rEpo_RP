...
{ 
	...
	{
		@extract($arryDetails);	
		$sql = "insert into h_holiday(locationID, heading, holidayDate, holidayDateTo, Status, Recurring) values('".$_SESSION['locationID']."', '".addslashes($heading)."', '".addslashes($holidayDate)."', '".addslashes($holidayDateTo)."', '".$Status."', '".$Recurring."')";
	...
	{
		@extract($arryDetails);	
		$sql = "update h_holiday set heading='".addslashes($heading)."', holidayDate = '".addslashes($holidayDate)."', holidayDateTo = '".addslashes($holidayDateTo)."', Status = '".$Status."', Recurring = '".$Recurring."'  where holidayID = '".$holidayID."'"; 
		$rs = $this->query($sql,0);
	...
	function isHolidayDateExists($holidayDate,$holidayID)
	{
		...
		$strSQLQuery ="select * from h_holiday where locationID='".$_SESSION['locationID']."'  and CASE WHEN holidayDateTo > 0 THEN ('".$holidayDate."' BETWEEN holidayDate and holidayDateTo) ElSE holidayDate='".$holidayDate."' END  ";
	...
	function isHolidayDateRangeExists($holidayDate, $holidayDateTo,$holidayID)
	{
		$objLeave=new leave();		 
		$i = $holidayDate; 
		while($i<=$holidayDateTo){
			if($objLeave->isHolidayDateExists($i, $holidayID)){
				$ExistFlag = 1; break;
			}
			$DateAry = explode("-",$i);			
	...
	/*******************************************/
	function  ListEntitlement($arryDetails){
		extract($arryDetails);
		global $Config;
		...
		$strAddQuery .= (!empty($sortby))?(" order by ".$sortby." ".$asc):(" order by em.UserName asc, e.LeaveType asc ");

		if($Config['GetNumRecords']==1){
				$Columns = " count(e.EntID) as NumCount ";				
		}else{				
			$Columns = "  e.*,em.EmpCode,em.UserName,em.JobType,em.JobTitle,em.Email,d.Department ";
		 ...
		 $strSQLQuery = "select ".$Columns." from h_entitlement e inner join h_employee em on e.EmpID=em.EmpID left outer join  h_department d on em.Department=d.depID ".$strAddQuery;		
	...
	function  ListLeave($arryDetails){
		extract($arryDetails);
		global $Config;
		$strAddQuery = " where em.locationID='".$_SESSION['locationID']."'";
		$SearchKey   = strtolower(trim($key));
		$strAddQuery .= (!empty($d))?(" and em.Department='".$d."'"):("");
		$strAddQuery .= (!empty($EmpID))?(" and em.EmpID='".$EmpID."'"):("");
		...
		$strAddQuery .= (!empty($sortby))?(" order by ".$sortby." ".$asc):(" order by l.FromDate Desc ");

		if($Config['GetNumRecords']==1){
			$Columns = " count(l.LeaveID) as NumCount ";				
		}else{				
			$Columns = "  l.*, em.EmpCode, em.UserName,em.Email,em.JobTitle,d.Department,e2.UserName as SupervisorName ";
			if($Config['RecordsPerPage']>0){
				$strAddQuery .= " limit ".$Config['StartPage'].",".$Config['RecordsPerPage'];
			}
		...
		$strSQLQuery = "select ".$Columns." from `h_leave` l inner join h_employee em on l.EmpID=em.EmpID left outer join  h_department d on em.Department=d.depID left outer join h_employee e2 on em.Supervisor=e2.EmpID ".$strAddQuery;		
	...
	function GetLeaveByDepartment($Department){
			...
			$strAddQuery .= ($Config['LeaveApplyToMe']==1)?(" and l.EmpID!='".$_SESSION['AdminID']."' "):("");
	...
	function getLeaveBalance($EmpID,$LeaveType){
		if(!empty($EmpID)){
			$strSQLQuery = "select e.EmpID, e.EmpCode, e.UserName,e.Email,e.JobTitle,e.catID,e.Division, e.JobType, e.JoiningDate,e.LeaveAccrual,e.ProbationPeriod,e.EligibilityPeriod,e.ProbationUnit,e.EligibilityUnit,e.shiftID from h_employee e where  e.EmpID='".mysql_real_escape_string($EmpID)."' ";
			$arryEmp = $this->query($strSQLQuery, 1);

			$Y = date("Y");
			/*******************/
			if(!empty($_POST['FromDate'])){			
				$arryFrom = explode("-",$_POST['FromDate']);
				if($arryFrom[0]>0) $Y = $arryFrom[0];
			}
			/*******************/
			...
			if($arryEmp[0]['LeaveAccrual']==1){ 
				...
				//print_r($arryFinalLeave);exit;
				$LeaveEntitle=0;
				if(!empty($arryFinalLeave)){
				...
				//echo $LeaveEntitle;exit;


				$arryJoin  = explode("-",$arryEmp[0]['JoiningDate']);
				$FromDate = $Y."-".$arryJoin[1]."-".$arryJoin[2]; 
				$ToDate = ($Y+1)."-".$arryJoin[1]."-".$arryJoin[2];


				$PrevDate = ($Y-1)."-".$arryJoin[1]."-".$arryJoin[2];
				$CurrentDate = 	$Y.'-'.date('m-d');	
			...
			$sql2 = "select sum(l.Days) as LeaveTaken from `h_leave` l where l.Status in ('Taken','Approved') and l.EmpID='".$EmpID."'" ; 
			if(!empty($LeaveType)) $sql2 .= " and l.LeaveType='".$LeaveType."'";

			$sql2 .= (!empty($FromDate))?(" and ( l.FromDate>='".$FromDate."' or  l.ToDate>='".$FromDate."')"):("");
			$sql2 .= (!empty($ToDate))?(" and (l.FromDate<='".$ToDate."' or  l.ToDate<='".$ToDate."')"):("");
			...
			//echo $LeaveEntitle.'#'.$LeaveTaken;exit;
	...
	function getLeaveByStatus($EmpID,$Status,$LeaveType){
		if(!empty($EmpID)){
			...
			if(!empty($LeaveType)) $sql .= " and LeaveType='".$LeaveType."'";
			$sql .= (!empty($FromDate))?(" and ( FromDate>='".$FromDate."' or  ToDate>='".$FromDate."')"):("");
			$sql .= (!empty($ToDate))?(" and (FromDate<='".$ToDate."' or  ToDate<='".$ToDate."')"):("");
//echo $sql;exit;
			$arryRow = $this->query($sql, 1);
		}
	...
	function getShortLeave($EmpID=0,$year,$month)
		...
		/********************************************/
		/********************************************/
		function getMaxKey($arryFirst){
			$MaxFirst='';			
			if(!empty($arryFirst)){
				$max = max($arryFirst);
				for($i=0;$i<sizeof($arryFirst);$i++){
					if($arryFirst[$i]==$max){
						$MaxFirst = $i;
					}
				}
	...
	if($arryEmp[0]['LeaveAccrual']==1){ 
		...
		if(empty($LeaveAllowedTotal)) $LeaveAllowedTotal= array();
      		if(empty($arryFinalLeave)) $arryFinalLeave= array();
	...
	$JoiningDate = $arryEmp[0]['JoiningDate'];
	...
	$EmploymentPeriod = (strtotime(date('Y-m-d')) - strtotime($arryEmp[0]['JoiningDate']))/(24*3600) + 1;
	$EmploymentUnit = 'Days';
	#if($_GET['d']==1){echo $FromDate.' '.$ToDate; exit;}
 
	if(!empty($JobType)){
		$arryCustomRule=$this->getRuleByType($JobType,$arryEmp[0]['EmpID']);

//echo '<pre>';print_r($arryCustomRule);exit;

		if(sizeof($arryCustomRule)>0){
			foreach($arryCustomRule as $key=>$values){
				#echo $values['RuleOn'].' '.$RuleOpp.' '.$values['RuleValue'].' : '.$values['LeaveAllowed'].'<br>';
				...
				*/
				...
				case 'E': //Employment Period

					if($EmploymentUnit!=$RuleUnit){
						if($EmploymentUnit!='Hrs') $EmploymentPeriod=$EmploymentPeriod*24;
						else if($RuleUnit!='Hrs') $RuleValue=$RuleValue*24;
					}
					
					if($RuleOpp=='E'){	
						if($EmploymentPeriod==$RuleValue){
							$NumRuleTrue++; 
				...
				if($NumRuleTrue == $NumRule){
					$LeaveAllowedTotal[] = $values['LeaveAllowed'].'@'.$values['RuleValue'];
			...
			//echo '<pre>'; print_r($LeaveAllowedTotal); die;
			$MainIndex = 0;	
			if(!empty($LeaveAllowedTotal)){
					
				$arry1=$arry2=$arry3=$arry4=$arry5= array();
					
				for($i=0;$i<sizeof($LeaveAllowedTotal);$i++){					


					$LeaArry = explode("@", $LeaveAllowedTotal[$i]); //main value
					$LeaArrytttt = explode("#", $LeaArry[1]);				
					
					
					if(isset($LeaArrytttt[0])) $arry1[] = $LeaArrytttt[0];	
					if(isset($LeaArrytttt[1])) $arry2[] = $LeaArrytttt[1];	
					if(isset($LeaArrytttt[2])) $arry3[] = $LeaArrytttt[2];	
					if(isset($LeaArrytttt[3])) $arry4[] = $LeaArrytttt[3];	
					if(isset($LeaArrytttt[4])) $arry5[] = $LeaArrytttt[4];					
				}

				unset($max);
				$max1 = $this->getMaxKey($arry1); if(isset($max1)) $max[]= $max1;
				$max2 = $this->getMaxKey($arry2); if(isset($max2)) $max[]= $max2;
				$max3 = $this->getMaxKey($arry3); if(isset($max3)) $max[]= $max3;
				$max4 = $this->getMaxKey($arry4); if(isset($max4)) $max[]= $max4;
				$max5 = $this->getMaxKey($arry5); if(isset($max5)) $max[]= $max5;
			...
			if(!empty($LeaveAllowedTotal[$MainIndex])){
				$LeavvsssArry = explode("@", $LeaveAllowedTotal[$MainIndex]);
				$LeaveAllowed = $LeavvsssArry[0];
			}
			/************/
		}
	}
...
/************************************/
		...
		function checkLeaveHoliday($EmpID,$Date)
		{
			global $Config;
			//holiday check
			$strH ="select holidayID from `h_holiday` where Status='1'  and CASE WHEN holidayDateTo > 0 THEN ('".$Date."' BETWEEN holidayDate and holidayDateTo) ElSE holidayDate='".$Date."' END   ";
			$arryH = $this->query($strH, 1);

			if(!empty($arryH[0]['holidayID'])) {
				return 1;
			}else{		

				/*//leave check approved
				$strL ="select LeaveID,DateDiff('".$Date."',FromDate)+1 as NumDay,LastBalance as LaB from `h_leave` where EmpID='".$EmpID."' and Status in ('Approved','Taken') and WEEKDAY('".$Date."') not in (5,6)  and (FromDate<='".$Date."' and ToDate>='".$Date."') having NumDay<=LaB ";*/


				//leave check approved
				$strL ="select LeaveID, if(FromDateHalf=1 OR ToDateHalf=1,DateDiff('".$Date."',FromDate)+0.5, DateDiff('".$Date."',FromDate)+1 ) as NumDay ,LastBalance as LaB from `h_leave` where EmpID='".$EmpID."' and Status in ('Approved','Taken') and WEEKDAY('".$Date."') not in (5,6)  and (FromDate<='".$Date."' and ToDate>='".$Date."') having NumDay<=LaB ";

				
				//if(!empty($_GET['pk']) && $EmpID==69){echo $strL;exit;}

				$arryL = $this->query($strL, 1);
				if(!empty($arryL[0]['LeaveID'])) { return 1;}
