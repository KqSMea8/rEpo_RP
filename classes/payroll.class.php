...
class payroll extends dbClass
{
		...
		/********** Salary Management *********/
		/*******************************************/
		function  ListSalary($arryDetails){
			extract($arryDetails);
                    	global $Config;
			$strAddQuery = '';
			$SearchKey   = strtolower(trim($key));
			...
			$strAddQuery .= (!empty($sortby))?(" order by ".$sortby." ".$asc):(" order by e.UserName asc ");

			if($Config['GetNumRecords']==1){
				$Columns = " count(s.salaryID) as NumCount ";				
			}else{				
				$Columns = " s.*,DECODE(s.CTC,'". $Config['EncryptKey']."') as CTC,DECODE(s.Gross,'". $Config['EncryptKey']."') as Gross,DECODE(s.NetSalary,'". $Config['EncryptKey']."') as NetSalary,e.EmpCode,e.UserName,e.Email,e.JobTitle, d.Department ";
			...
			$strSQLQuery = "select ".$Columns." from h_salary s inner join h_employee e on s.EmpID=e.EmpID left outer join h_department d on e.Department=d.depID ".$strAddQuery;		
		...
		function addSalary($arryDetails){ 
			...
			/********************/
			@extract($arryDetails);	 
			$sql = "insert into h_salary(EmpID, catEmp, CTC, Gross, NetSalary, SalaryData, updatedDate, BankName, AccountName, AccountNumber, IFSCCode,PayRate,HourRate,PayCycle,PayCycleRate) values('".$EmpID."', '".$catEmp."', ENCODE('".$CTC."','".$Config['EncryptKey']."'), ENCODE('".$Gross."','".$Config['EncryptKey']."'), ENCODE('".$NetSalary."','".$Config['EncryptKey']."'), '".$SalaryData."', '".$Config['TodayDate']."', '".addslashes($BankName)."', '".addslashes($AccountName)."', '".addslashes($AccountNumber)."', '".addslashes($IFSCCode)."', '".addslashes($PayRate)."', '".addslashes($HourRate)."', '".addslashes($PayCycle)."', '".addslashes($PayCycleRate)."')";
			$this->query($sql,0);
			return true;
		}
		...
		function  ListDeclaration($arryDetails){
			...
			$strAddQuery .= (!empty($sortby))?(" order by ".$sortby." ".$asc):(" order by d.depID asc, s.decID asc ");

			if($Config['GetNumRecords']==1){
				$Columns = " count(s.decID) as NumCount ";				
			}else{				
			  ...
			  $strSQLQuery = "select ".$Columns." from h_declaration s inner join h_employee e on s.EmpID=e.EmpID left outer join h_department d on e.Department=d.depID ".$strAddQuery;		
		...
		function deleteDeclaration($decID){
			...
			if(!empty($decID)){
				...
				if($arryRow[0]['document'] !=''){			
					$objFunction->DeleteFileStorage($Config['DeclarationDir'],$arryRow[0]['document']);
				}
		...
		function updateDeclarationFile($document,$decID)
		{
			...
			if(!empty($decID)){
				...
				$DecDir = $Config['FileUploadDir'].$Config['DeclarationDir']; 
	...
	/********** Employee Bank *********/
	function addBankDetail($arryDetails){
		global $Config;
		@extract($arryDetails);	
		if(!empty($EmpID)){
			$sql = "insert into h_bank (EmpID, BankName, AccountName, AccountNumber, IFSCCode, DefaultAccount, UpdatedDate) values('".addslashes($EmpID)."', '".addslashes($BankName)."', '".addslashes($AccountName)."', '".addslashes($AccountNumber)."', '".addslashes($IFSCCode)."', '".addslashes($DefaultAccount)."', '".$Config['TodayDate']."' )";
	...
	function UpdateBankDetail($arryDetails){   
		global $Config;
		extract($arryDetails);
		if(!empty($EmpID) && !empty($BankID)){
			$strSQLQuery = "update h_bank set BankName='".addslashes($BankName)."',AccountName='".addslashes($AccountName)."', AccountNumber='".addslashes($AccountNumber)."', IFSCCode='".addslashes($IFSCCode)."', DefaultAccount='".addslashes($DefaultAccount)."', UpdatedDate = '".$Config['TodayDate']."'  where EmpID='".$EmpID."' and BankID='".$BankID."'  "; 
			$this->query($strSQLQuery, 0);
		}

		return 1;
	}
	function UnDefaultBank($BankID, $EmpID){   
		global $Config;
		extract($arryDetails);
		if(!empty($EmpID) && !empty($BankID)){
			$strSQLQuery = "update h_bank set DefaultAccount='0' where BankID!='".$BankID."'  "; 
			$this->query($strSQLQuery, 0);
		}

		return 1;
	}
	function RemoveBank($BankID,$EmpID){
		if(!empty($EmpID) && !empty($BankID)){
			$strSQLQuery = "Delete FROM h_bank WHERE BankID = '".$BankID."' and EmpID = '".$EmpID."'"; 
			$this->query($strSQLQuery, 0);
			return 1;
		}

	}
	function GetBank($BankID,$EmpID,$DefaultAccount){
		$strSQLQuery = "select b.* from h_bank b where EmpID= '".$EmpID."' ";
