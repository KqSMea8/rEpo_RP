...
function  PaymentReport($FromDate,$ToDate,$SalesPersonID,$salesPersonType='')
		...
		function  GetSale($OrderID,$SaleID,$Module)
		{
			...
			$strAddQuery .= (!empty($Module))?(" and o.Module='".$Module."'"):("");
			$strSQLQuery = "select o.*, IF(c.CustomerType = 'Company' and c.Company!='', Company, FullName) as CustomerName, e.Email as CreatedByEmail,c.Email as CustomerEmail,c.VAT,c.CST,c.PAN,c.TRN ".$newaddcolm." from s_order o left outer join h_employee e on (o.AdminID=e.EmpID and o.AdminType!='admin') left outer join s_customers c on o.CustCode=c.CustCode where 1 ".$strAddQuery." order by o.OrderID desc";
		...
		function  GetSalesBrief($OrderID,$SaleID,$Module)
		{
			...
			$strAddQuery .= (!empty($SaleID))?(" and o.SaleID='".$SaleID."'"):("");
			$strAddQuery .= (!empty($Module))?(" and o.Module='".$Module."'"):("");
			$strSQLQuery = "select o.Module, o.OrderID, o.InvoiceID, o.CreditID, o.InvoiceEntry from s_order o  where 1 ".$strAddQuery." order by o.OrderID desc"; 			
			return $this->query($strSQLQuery, 1);
		}
...
function  GetSaleComponentItem($OrderID,$PidID) 
     ...
     $strAddQuery .= (!empty($OrderID))?(" and i.parent_item_id='".$PidID."'"):("");
		...
		{  
		    ...
		    $strSQLQuery = "INSERT INTO s_order SET Module = '".$Module."',
                                    ...
                                    CreatedDate='".$Config['TodayDate']."',
																	  UpdatedDate='".$Config['TodayDate']."',
																		...
																		IPAddress='".$IPAddress."' ,RecurringOption='".$RecurringOption."',RecurringDate='".$RecurringDate."',BillingPeriod='".$BillingPeriod."',BillingFrequency='".$BillingFrequency."' ";
			...
			$objConfigure->UpdateModuleAutoID('s_order',$Module,$OrderID,$arryDetails[$ModuleID]);  
			...
			if(isset($PaymentTerm)){
				if($PaymentTerm=='Credit Card' && !empty($CreditCardNumber) && !empty($CreditCardType)){
					...
					if(!empty($arryCard[0]['ID'])){
						$sql = "UPDATE s_order_card ".$addsql." where ID='".$arryCard[0]['ID']."'";				$this->query($sql, 1);
					}else{
...
//SaleQty = SaleQty-" . $arryDetails['QtySel'.$i] . "
				if($arryDetails['Condition'.$i]!=''){
				 $sqlCondAdd="select count(*) as total from inv_item_quanity_condition where 
				...
				if($restblAdd[0]['total']>0){
					// update in tbl
					 $AddQtysql = "update inv_item_quanity_condition set SaleQty = SaleQty-" . $arryDetails['QtySel'.$i] . "  where Sku='" . $Mainsku . "' ".$strAddConQuery." ".$strAddWareQuery.""; 
...
if($arryDetails['PONumber']!=''){
 $this->UpdateSaleQtyInPO($arryDetails['PONumber'],$arryDetails['qty'.$i],$arryDetails['sku'.$i],$arryDetails['Condition'.$i]);
}
...
/*********end By bhoodev based on Condition 6 june 2016*********************/
		...
		function UpdateSale($arryDetails){ 
			...
			$strSQLQuery = "UPDATE s_order SET Module = '".$Module."',EntryType='".$EntryType."', EntryInterval='".$EntryInterval."',  EntryMonth='".$EntryMonth."', EntryWeekly = '".$EntryWeekly."', EntryFrom='".$EntryFrom."',EntryTo='".$EntryTo."',EntryDate='".$EntryDate."', OrderDate='".$OrderDate."', InvoiceID = '".$InvoiceID."',SalesPersonID = '".$SalesPersonID."', SalesPerson = '".addslashes($SalesPerson)."',SalesPersonType = '".$SalesPersonType."',
		...
		function GenerateInVoice($InvoiceData)
		{
			...
			$strSQLQuery = "INSERT INTO s_order SET Module = 'Invoice',
                                    ...
                                    CreatedDate='".$Config['TodayDate']."',
																		...
																		IPAddress='".$IPAddress."', EntryBy = '".$EntryBy."', AccountID = '".$AccountID."',FileName = '".$FileName."',ShipAccountNumber = '".addslashes($ShipAccountNumber)."', CountryId='".$CountryId."', StateID='".$StateID."' , CityID='".$CityID."' , ShippingCountryID='".$ShippingCountryID."' , ShippingStateID='".$ShippingStateID."', ShippingCityID='".$ShippingCityID."'  ";
...
function UpdateSaleQtyInPO($PurchaseID,$Qty,$Sku,$Condition){
...
$checkProduct=$objItem->checkItemSku($Sku);
  ...
  $strSQLQuery = "select OrderID from p_order where PurchaseID='" . $PurchaseID . "' and Module='Order'";
	$rs = $this->query($strSQLQuery, 1);
		if(!empty($rs[0]['OrderID'])){

				$strSQLQuery = "update p_order_item set SaleQty = '" .$Qty. "'  where OrderID = '" .$rs[0]['OrderID']. "' and sku='".$Sku."' and `Condition` = '".$Condition."'";
				$this->query($strSQLQuery, 0);
...
function  ListSalePick($arryDetails,$CustomerID=0)
		{
			...
			$strAddQuery .= (!empty($Approved))?(" and o.Approved='1'"):("");
			$strAddQuery .= (!empty($PostToGL))?(" and o.PostToGL='1'"):("");
$strAddQuery .= (!empty($Pick))?(" and (o.PickID!='' or i.DropShipCheck=1) "):(" and i.DropShipCheck=0 ");
...
$batchCol = ",b.PostToGL ";
			...
			if($Config['GetNumRecords']==1){
				...
				$Columns = "  o.*,i.DropShipCheck,COUNT(i.qty) as orderqty ,COUNT(i.qty_shipped) as totship ";
				if($Config['RecordsPerPage']>0){
					$strAddQuery .= " limit ".$Config['StartPage'].",".$Config['RecordsPerPage'];
				}
		          ...
		          $strSQLQuery = "select ".$Columns." ".$batchCol."  from s_order o left join s_order_item i on o.OrderID=i.OrderID  ".$join."  " . $strAddQuery;
