...
class Cartsettings extends dbClass {
	...
	function deleteSliderBanner($id) {
		...
		$objFunction=new functions();  
         ...
         $strSQLQuery = "select Slider_image from e_slider_banner where Id = '".$id."'"; 
		$arryRow = $this->query($strSQLQuery, 1);

		if($arryRow[0]['Slider_image'] != '') {  
			    $objFunction->DeleteFileStorage($Config['ProductsBanner'],$arryRow[0]['Slider_image']); 
		}
