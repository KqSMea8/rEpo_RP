...
class activity extends dbClass {
        ...
        $arryAsign = explode(":", $assignedTo);
         ...
         if($Config['CronEntry']==1){ //cron entry
				$EntryType = 'one_time';
                                ...
                                if ($assign == 'Users') {                   //By Chetan 3AUG//
	...
	if($_SESSION['AdminType'] == "employee" && empty($AssignUser)) {
	    ...
	    $AssignType = 'Users';     //By Chetan 20July//
	}

        //By Chetan3AUG//
         $unsetArray = array("assign","AssignToGroup","AssignToUser","Submit","add_date","EntryDate","EntryType","EntryInterval","EntryMonth",
             "EntryWeekly","EntryFrom","EntryTo","activity_type","activityID",'EmpID');
        ...
        $sql = "insert into c_activity 
                        (assignedTo,AssignType,GroupID,add_date,EntryType,EntryFrom,EntryTo,EntryDate,EntryInterval, EntryMonth, EntryWeekly,$fields)
                        values( '" . $AssignUser . "',
                                '" . $AssignType . "',
                                '" . $GroupID ."', 
                                '" . $Config['TodayDate'] . "',
    ...
    function updateActivity($arryDetails) {
        ...
        if($EntryInterval == 'daily'){$EntryDate = 0;$EntryMonth='';$EntryWeekly = '';}
         ...
         //By Chetan20July//
         $unsetArray = array("assign","AssignToGroup","AssignToUser","Submit","add_date","EntryDate","EntryType","EntryInterval","EntryMonth",
             "EntryWeekly","EntryFrom","EntryTo","activity_type","activityID",'created_by','created_id','EmpID');
        foreach($unsetArray as $arr){unset($arryDetails[$arr]);}
        ...
        $sql = "update c_activity set ".trim($str, ',')." , 
			EntryType='" . $EntryType . "',EntryInterval='" . $EntryInterval . "',  
                        EntryMonth='" . $EntryMonth . "', EntryWeekly='" . $EntryWeekly . "',     
                        EntryFrom='" . $EntryFrom . "',    EntryTo='" . $EntryTo . "',
                        EntryDate='" . $EntryDate . "',  assignedTo  = '" . $AssignUser . "',
                        AssignType  = '" . $AssignType . "',GroupID  = '" . $GroupID . "',activityType = '" . addslashes($activity_type) . "'			
...
$strAddQuery .= (!empty($Config['rows'])) ? ("  and e.RowColor = '#" . $Config['rows'] . "' ") : ("");  //add Rajan 20 jan
...
if($Config['GetNumRecords']==1){
				$Columns = " count(e.activityID) as NumCount ";				
			}else{		

				//$strAddQuery .= " group by e.activityID ";
        $strAddQuery .= (!empty($SortBy)) ? (" order by " . $SortBy . " ") : (" order by e.startDate desc, e.startTime desc,e.activityID ");
        $strAddQuery .= (!empty($asc)) ? ($asc) : (" Desc");	
				$Columns = " e.*,u.Role,u.UserName as created ";
        ...
        $strSQLQuery = "select ".$Columns." from c_activity  e  left outer join  h_employee u on u.EmpID=e.created_id" . $strAddQuery;
	...
	function setRowColorActivity($activityID,$RowColor) {
		if (!empty($activityID)) {
		    $sql = "update c_activity set RowColor='".$RowColor."' where activityID in ( " . $activityID . ")";
		    $rs = $this->query($sql, 0);
		}
    ...
    function GetActivityDeshboard($Type,$limit=7) {
        global $Config;
        ...
        $strSQLQuery .= " order by startDate asc limit 0,".$limit;
...
(CASE WHEN MainTime  = '".$startDateTime."' 
        ...
        ELSE MainTime ".$operator." '" . $startDateTime . "' END 
			...
			$arrRow = $this->query($strSQLQuery, 1);
			return (!empty($arrRow)) ? $arrRow[0]['activityID'] : false;
		}
	}            
   ...
   function GetWorkspaceActivity($activityType) {
		$arryDate = explode(" ",$_SESSION['TodayDate']);

		 $strSQLQuery = "select subject,activityID,activityType,startDate,closeDate,closeTime,startTime from c_activity where status not in ('Held','Not Held') ";

		 $strSQLQuery .= ($_SESSION['vAllRecord'] != 1) ? (" and (FIND_IN_SET(" . $_SESSION['AdminID'] . ",assignedTo) OR created_id='" . $_SESSION['AdminID'] . "') ") : ("");

		//$strSQLQuery .= " and startDate =  '" . $arryDate[0] . "'  ";
		//$strSQLQuery .= " and startTime >  '" . $arryDate[1] . "'  ";
		//$strSQLQuery .= " order by startDate asc limit 0,50 ".$limit;



		switch($activityType){
		   case 'Top':
			$strSQLQuery .= " and startDate='".$arryDate[0]."' and startTime >  '" . $arryDate[1] . "'  ";			
			break;
		   case 'Daily':  	
			$strSQLQuery .= " and startDate='".$arryDate[0]."' ";		
			break;
		   case 'Weekly':  	
			$strSQLQuery .= " and WEEKOFYEAR(startDate)=WEEKOFYEAR('".$arryDate[0]."') ";		
			break;
		   case 'Monthly':  	
			$strSQLQuery .= " and month(startDate)=month('".$arryDate[0]."') ";		
			break;
		  case 'Yearly':  	
			$strSQLQuery .= " and year(startDate)=year('".$arryDate[0]."') ";		
			break;
		}

		$strSQLQuery .= " order by startDate asc limit 0, 50";
		//echo $strSQLQuery;exit;
		return $this->query($strSQLQuery, 1);
	...
	//By chetan 4Dec//
    function AddQuickActivityReminder($PostArr)
    {
        global $Config;
        extract($PostArr);
        $objLead=new lead();
        $LeadRes = $objLead->GetLead($LeadID);
        $sql = "insert into c_activity
        (subject, created_by, created_id, description, startDate, startTime, closeDate, closeTime,status, activityType, location, priority, LeadID, add_date,RelatedType, parent_type, parentID) 
        values( '" . addslashes($LeadRes[0]['LeadName']) . "','" . addslashes($created_by) . "', '" . addslashes($created_id) . "',
            '" . addslashes($LeadRes[0]['description']) . "', '" . $startDate . "','" . $startTime . "','" . $closeDate . "',
        '" . $closeTime . "','Hot','Call', '" . $LeadRes[0]['Address'] . "', 'Low','" . $LeadID . "','" . $Config['TodayDate'] . "','Lead','lead','" . $LeadID . "' )";
        return $this->query($sql, 1);
    }
    //End//
