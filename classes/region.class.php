...
class region extends dbClass
{
		...
		function getCountryWithCode($country_id,$Status)
		{
			//$sql  = " where continent_id=2"; 
			$sql  = " where code !='' "; 
			$sql .= (!empty($country_id))?(" and country_id='".$country_id."'"):("");
		...
		{
			global $Config;
			$strSQLQuery = "select code from  ".$Config['DbMain'].".country where country_id ='".$CountryID."'";
			return $this->query($strSQLQuery, 1);
		}
...
function  GetCountryByIDCode($name)
		{
			$strSQLQuery = "select country_id,used_state from country where LCASE(name)='".mysql_real_escape_string(strtolower(trim($name)))."' or LCASE(code) like '%".mysql_real_escape_string(strtolower(trim($name)))."%'";
			return $this->query($strSQLQuery, 1);
		}
//Update by chetan on 12Feb added col(name)//
		function  GetCountryID($name)
		{
			$strSQLQuery = "select country_id,name from country where LCASE(name)='".mysql_real_escape_string(strtolower(trim($name)))."'";
			return $this->query($strSQLQuery, 1);
		}
function  GetImportCountryID($name)
        {
         $strSQLQuery = "select country_id,name from country where LCASE(name)='".mysql_real_escape_string(strtolower(trim($name)))."' or (code = '".$name."'  or code2 = '".$name."') ";
            return $this->query($strSQLQuery, 1);
        }
...
$strAddQuery = (!empty($country_id))?(" and country_id='".$country_id."'"):("");
			 $strSQLQuery = "select state_id,name from state where ( LCASE(name)='".mysql_real_escape_string(strtolower(trim($name)))."'   OR LCASE(code)='".mysql_real_escape_string(strtolower(trim($name)))."'   ) ".$strAddQuery."";
			//echo $strSQLQuery.'<br>';
			return $this->query($strSQLQuery, 1);
		}
		...
		{
$strAddQuery = (!empty($country_id))?(" and country_id='".$country_id."'"):("");
			$strSQLQuery = "select city_id from city where  LCASE(name)='".strtolower(trim($name))."' ".$strAddQuery."";
			return $this->query($strSQLQuery, 1);
		}
//Update by chetan on 12Feb added col(name)//
		function  GetCityIDSt($name,$state_id, $country_id)
		{
			$strSQLQuery = "select city_id, name from city where country_id='".$country_id."' and state_id='".$state_id."' and LCASE(name)='".strtolower(trim(addslashes($name)))."'";
		...
		{
			global $Config;
			$strSQLQuery = "select name from ".$Config['DbMain'].".state where state_id ='".$state_id."'";
			return $this->query($strSQLQuery, 1);
		}
		...
		function isStateInCountry($country_id)
		{
			$strAddQuery = (!empty($country_id))?(" where s.country_id='".$country_id."'"):("");

			$strSQLQuery = "select s.state_id from state s inner join country c on s.country_id=c.country_id ".$strAddQuery." limit 0 , 1";
			$arryRow = $this->query($strSQLQuery, 1);
			if($arryRow[0]["state_id"]>0) return true;
		}
		...
		function getCityList($state_id,$country_id)
		{
			 $strAddQuery='';
			$strAddQuery .= (!empty($state_id))?(" and c.state_id='".$state_id."'"):("");
			$strAddQuery .= (!empty($country_id))?(" and c.country_id='".$country_id."'"):("");

			$strSQLQuery = "select c.* from city c where 1 ".$strAddQuery." order by c.name";
			return $this->query($strSQLQuery, 1);
		...
		function addCityMulti($ArryRequest){			
			extract($ArryRequest);
		
			$CityArray=explode(",",$name);
			if(sizeof($CityArray)>0){			
				foreach($CityArray as $City){
					if(!empty($City)){
						if(!$this->isCityExists($City,$main_state_id,$country_id)){
							$sql="insert into city (name,state_id,country_id) values('".addslashes(trim($City))."','".$main_state_id."','".$country_id."')";
							$this->query($sql,0);	
						}
					}
		...
		function getCity($city_id,$Status)
		{
			...
			$strAddQuery .= (!empty($Status))?(" and ct.Status='".$Status."'"):("");
			 ...
			 $strSQLQuery = "select ct.*,s.name as state_name,c.name as country_name from city ct left outer join state s on ct.state_id=s.state_id inner join country c on ct.country_id=c.country_id ".$strAddQuery." order by ct.name";
		...
		{
			global $Config;
			$strSQLQuery = "select name from ".$Config['DbMain'].".city where city_id ='".mysql_real_escape_string($city_id)."'";
			return $this->query($strSQLQuery, 1);
		}
		...
		function isCityExists($name, $state_id, $country_id, $city_id)
		{
			$sql ="select * from city where LCASE(name) = '".strtolower(trim($name))."'";
		...
		function isMultiCityExists($name, $state_id, $country_id){			
			$CityArray = explode(",",$name);
			$Existing = '';$addsql='';
			$addsql .= (!empty($state_id))?(" and state_id = '".$state_id."'"):("");
			$addsql .= (!empty($country_id))?(" and country_id = '".$country_id."'"):("");
			if(sizeof($CityArray)>0){
				foreach($CityArray as $City){
					if(!empty($City)){
						$sql ="select * from city where LCASE(name) = '".strtolower(trim($City))."'".$addsql;
						$arryRow = $this->query($sql, 1);
						 
						if (!empty($arryRow[0]['name'])) {
							$Existing = $arryRow[0]['name'];
							break;
						}
					}
				}
		...
		function deleteMultiCity($city_id){
			if(!empty($city_id)){
				$sql = "delete from city where city_id in ( ".$city_id.")";
				$rs = $this->query($sql,0);
			}
			return true;
		}
		 ...
		 function getCurrencyByCode($code)
		{
			global $Config;
			if(!empty($code)){
				$sql="select * from ".$Config['DbMain'].".currencies where code = '".$code."'";
				return $this->query($sql);
			}
		}
		...
		function getZipCode($zipcode_id,$Status){
			...
			$strSQLQuery = "select z.*,ct.name as city_name,s.name as state_name,c.name as country_name from zipcode z inner join city ct on z.city_id=ct.city_id left outer join state s on ct.state_id=s.state_id left outer join country c on s.country_id=c.country_id ".$strAddQuery." order by ct.name";
		
		
			return $this->query($strSQLQuery, 1);
		}
		
		function getZipCodeByZip($zip_code){
			if(!empty($zip_code)){
				$strAddQuery = " where z.zip_code='".$zip_code."'";
		...
		function deleteMultiZipCode($zipcode_id){
			if(!empty($zipcode_id)){
				$sql = "delete from zipcode where zipcode_id in ( ".$zipcode_id.")";
				$rs = $this->query($sql,0);
			}
			return true;
		}
...
//Update by chetan on 12Feb added col(name)//
		...
		{
			$strSQLQuery = "select state_id,name from state where country_id='".$country_id."' and LCASE(code)='".mysql_real_escape_string(strtolower(trim($code)))."'";
			return $this->query($strSQLQuery, 1);
		}
...
{
	$strSQLQuery = "select state_id from state where  LCASE(name)='".mysql_real_escape_string(strtolower(trim($name)))."'";
	return $this->query($strSQLQuery, 1);
}

function  GetCitybyID($name)
	{
		$strSQLQuery = "select city_id from city where LCASE(name)='".mysql_real_escape_string(strtolower(trim($name)))."'";
		return $this->query($strSQLQuery, 1);
	}

/////	
	//By chetan 11Feb//
                function getCountryByCode($code,$Status)
		{
                    $sql  = " where 1 "; 
                    $sql .= (!empty($Status))?(" and Status='".$Status."'"):("");

                    $sql="select * from country ".$sql." and (code = '".$code."'  or code2 = '".$code."') ";
                    return $this->query($sql);
		} 


	function GetStateCodeByName($country_id, $name){
		global $Config;
		if(!empty($country_id) && !empty($name)){
			  $sql = "select code from ".$Config['DbMain'].".state where country_id='".$country_id."' and LCASE(name)='".mysql_real_escape_string(strtolower(trim($name)))."'";
			$arryRow = $this->query($sql, 1);
			if(!empty($arryRow[0]['code']))return $arryRow[0]['code'];
		}		 
	}
