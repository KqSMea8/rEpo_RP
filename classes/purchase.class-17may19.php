...
class purchase extends dbClass
{
		...
		function  ListPurchase($arryDetails)
		{
			...
			(empty($Approved))?($Approved=""):("");
			(empty($InvoicePaid))?($InvoicePaid=""):("");
			(empty($Status))?($Status=""):("");
			(empty($ToApprove))?($ToApprove=""):("");
			(empty($Limit))?($Limit=""):("");
...
function  GetTotalRcieveQtyLeft($OrderID)
		...
		function RemovePurchase($OrderID){
			...
			$objConfig=new admin();	
			if(!empty($OrderID)){
				$strSQLQuery = "select Module,PurchaseID,QuoteID,CreditID from p_order where OrderID='".$OrderID."'"; 
				$arryRow = $this->query($strSQLQuery, 1);
