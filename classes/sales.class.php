...
class common extends dbClass
{
		...
		function GetShippInfoByOrder($OrderID){
		         $strSQLQuery = "select *  from w_shipment where OrderID = '".$OrderID."' order by ShipmentID desc limit 0,1";
			 $results=$this->query($strSQLQuery,1);
			 return $results;
			
		}

		function GetShippInfoByTrackingId($trackingId){
		         $strSQLQuery = "select * from w_shipment where trackingId = '".$trackingId."' order by ShipmentID desc limit 0,1";
			 $results=$this->query($strSQLQuery,1);
			 return $results;
			
		}

		function GetShippingInfoInvoice($trackingId,$InvoiceID){
