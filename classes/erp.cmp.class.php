...
function addFreeOrder($Email){
	...
	function getPackagesById($packid){
		$sql="SELECT * FROM erp_packages WHERE id= '$packid'";
		return $this->query($sql, 1);
	}

	function getPackagesByName($name){
		$sql="SELECT * FROM erp_packages WHERE name= '".$name."' AND package_type='4'";
		return $this->query($sql, 1);
	}
	...
	function AddErpOrder($arryDetails)
	{
...
// Added by shravan 13 feb,2015 for status checking of company
	...
	$sql="SELECT * FROM erp_email_template WHERE CatID='".$CatID."'";
	return $this->query($sql, 1);
    }
...
/** ezneterp start from 30 march 2016***/
   function ezneterpPackagesById($packid){
		 $sql="SELECT * FROM erp_packages WHERE id= '$packid'";
		return $this->query($sql, 1);
	}

	function ezneterpPackagesByName($name){
		$sql="SELECT * FROM erp_packages WHERE name= '".$name."' AND package_type='4'";
		return $this->query($sql, 1);
	}

	function UpdateDepartment($CmpID,$Department,$maxProduct){
		if(!empty($CmpID)){
			 $strSQLQuery = "update company set Department='".mysql_real_escape_string($Department)."',maxProduct='".mysql_real_escape_string($maxProduct)."'  where CmpID='".mysql_real_escape_string($CmpID)."'"; 
			$this->query($strSQLQuery, 0);
		}
		return 1;
	}
	...
	function EzneterpUpdateInventryModule($inventryModule)
		{
	
	           $arrayInventryModule = unserialize($inventryModule);
	           
	           $InventryModule = implode(",", $arrayInventryModule);
	          

				$sql1 = "update admin_modules set Status='1'  where depID = '6' and  ModuleID in(".$InventryModule.") "; //Setting Sub Module: 115
				$this->query($sql1,0);
				
				$sql2 = "update admin_modules set Status='0'  where depID = '6' and  ModuleID NOT IN(".$InventryModule.") "; //Setting Sub Module: 115
				$this->query($sql2,0);
