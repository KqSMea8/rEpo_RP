...
$strAddQuery .= (!empty($WID))?(" and `warehouse`='".$WID."' "):("");
...
*/
 ...
 //By Chetan21Aug//
    ...
    function checkItemAliasSku($Sku) {
                $strSQLQuery = "select c.valuationType as evaluationType, ia.ItemAliasCode,ia.AliasID,ia.sku,ia.description,i.ItemID,i.itemType from inv_item_alias ia left join inv_items i on ia.item_id = i.ItemID left outer join inv_categories c on c.CategoryID =i.CategoryID where LOWER(ia.ItemAliasCode) = '" . strtolower($Sku) . "'";
...
/*********Added By karishma based on Condition********************added more col by chetan on 16feb */
	function getItemCondionQty($Sku,$Condi,$WID=0) {
$strAddQuery = (!empty($Condi))?(" and `condition`='".addslashes($Condi)."'"):("");
$strAddQuery.= (!empty($WID))?(" and WID = '".addslashes($WID)."' "):("and WID = 1 ");
