...
class sale extends dbClass
{
		...
		function  ListSale($arryDetails,$CustomerID=0)
		{
			...
			if((!empty($CustomerID) && $CustomerID!='0')){
				...
				if(!empty($_SESSION['vAllRecord']) || !empty($Config['vAllRecord'])){
					$strAddQuery .= '';
				}else{
					$strAddQuery .= " and (o.SalesPersonID='".$AdminID."' or o.AdminID='".$AdminID."') ";
				}
...
$strAddQuery .= " and (o.PaymentTerm not in ('Credit Card','PayPal') or o.OrderPaid in ('1','3','4')) and o.Approved='1' ";
}
			...
			if($SearchKey==strtolower(ST_CLR_CREDIT)){
				...
				$SearchKey = 'Open';
				$strAddQuery .= " and o.PaymentTerm='Credit Card' and o.OrderPaid in ('1','3','4')";
			}
	...
	if(!empty($Config['CrditStatus'])){
	     if($Config['CrditStatus']==strtolower(ST_CREDIT_APP)){
			...
			$oRTpe = 'Drop Ship';
			// $strAddQuery .= " and (o.PaymentTerm='' or o.PaymentTerm like '%-%') and o.Approved='1'";
			$strAddQuery .= " and (o.PaymentTerm not in ('Credit Card','PayPal') or o.OrderPaid in ('1','3','4')) and o.Approved='1' and o.OrderType !='".$oRTpe."'  and o.Approved='1'";
		}
	}
			...
			$strAddQuery .= (!empty($PostToGL))?(" and o.PostToGL='1'"):("");
				$strAddQuery  .= ($module == 'Order')? " and o.SaleID NOT IN (select SaleID from s_order where Module = 'Invoice'  and  PostToGL=1) " : '';
...
$batchCol = ",b.PostToGL ";
	...
	function  SalesReport($FilterBy,$FromDate,$ToDate,$Month,$Year,$CustCode,$SalesPID,$Status)
			 ...
			 if($OrderStatus == 'Completed') {
						...
						if(!empty($PaymentTerm)){
							...
							//paid or partially refunded
								if($OrderPaid==1 || $OrderPaid==3 || $OrderPaid==4){
		...
		function GetCreditStatusMsg($OrderStatus,$OrderPaid){
			 ...
			 if($OrderStatus == 'Completed') {		                
				...
				if($OrderPaid == '2'){
					...
					$Status = 'Partially Paid'; $StatusCls = 'green';
				}
		         }
		...
		function GetInvoiceStatusMsg($InvoicePaid,$PaymentTerm,$OrderPaid){
			if($InvoicePaid == 'Paid') {
				$StatusCls = 'green';
			} else {
				$StatusCls = 'red';
			...
			if($InvoicePaid=='Unpaid' && ($PaymentTerm=='Credit Card' || $PaymentTerm=='PayPal')  && ($OrderPaid==1 || $OrderPaid==3 || $OrderPaid==4)){
				$StatusCls = 'green';
				$InvoicePaid = $PaymentTerm;
			}
	   
			return '<span class="' . $StatusCls . '">' . $InvoicePaid . '</span>';
		}
		...
		function  GetOrderAmountByMonth($Year,$FromDate,$ToDate,$CustCode,$Status)
		{
			...
			$strSQLQuery = "select o.TotalAmount,o.CustomerCurrency,o.ConversionRate from s_order o where o.Module='Order' and YEAR(o.OrderDate)='".$Year."' ".$strAddQuery." order by o.OrderDate desc";
			$rs = $this->query($strSQLQuery, 1);
			$avgCost=0;
			for($i=0;$i<=count($rs);$i++)
			{

				if(!empty($rs[$i]['TotalAmount'])){
					$ConversionRate=1;		
					if($rs[$i]['CustomerCurrency']!=$Config['Currency']){
						$ConversionRate = $rs[$i]['ConversionRate'];			   
					}
					$avgCost += GetConvertedAmount($ConversionRate, $rs[$i]['TotalAmount']);
				}
...
function  PaymentReport($FromDate,$ToDate,$SalesPersonID,$salesPersonType='')
		...
		function  GetSalesPaymentPer($PaymentID,$FromDate,$ToDate,$EmpID,$SuppID=0)
		{
			...
			$comCondition = 'FIND_IN_SET(c.EmpID,o.SalesPersonID)>0';   
			if(!empty($SuppID)){			
				$strAddQuery .= " and c.SuppID='".$SuppID."'  "; 			 
				$comCondition = 'FIND_IN_SET(c.SuppID,o.VendorSalesPerson)>0';  
                		$strJoinQuery  = " and  cs.VenSpId ='".$SuppID."'" ;
			}else if(!empty($EmpID)){	
				$strAddQuery .= " and c.EmpID='".$EmpID."'";
				$strJoinQuery  = " and  cs.EmpSpId ='".$EmpID."'";
			}
			$strSQLQuery = "select DECODE(p.DebitAmnt,'". $Config['EncryptKey']."') as TotalSales,cs.CommPercentage,o.InvoiceID,o.setSpiffamt, o.InvoicePaid from f_payments p left outer join s_order o on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='') left outer join h_commission c on ".$comCondition." left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery."  where o.Module='Invoice' and o.ReturnID='' and o.CreditID='' and o.InvoicePaid in ('Paid','Part Paid') and p.PaymentType='Sales' ".$strAddQuery." ";
		...
		function  GetSalesPaymentNonResidualPer($PaymentID,$FromDate,$ToDate,$EmpID,$SuppID=0)
		{
			...
			$sql_invoice = "select p.InvoiceID from f_payments p left outer join s_order o on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='') left outer join h_commission c on ".$comCondition." where o.Module='Invoice' and o.ReturnID='' and o.CreditID='' and o.InvoicePaid in ('Paid','Part Paid') ".$strAddQuery."  group by p.InvoiceID order by p.PaymentDate asc limit 0,1";

			$arryInvoice = $this->query($sql_invoice, 1);
			
			if(!empty($arryInvoice[0]["InvoiceID"])){
				 $strInnerQuery = (!empty($PaymentID))?(" and p.PaymentID='".$PaymentID."'"):("");
				 $strSQLQuery = "select DECODE(p.DebitAmnt,'". $Config['EncryptKey']."') as TotalSales,o.InvoicePaid,cs.CommPercentage from f_payments p left outer join s_order o on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='')  left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice' and o.ReturnID='' and o.CreditID='' and p.InvoiceID='".$arryInvoice[0]["InvoiceID"]."'  ".$strInnerQuery." "; 
		...
		function  ListARInvoice($arryDetails)
		{
			...
			if($SearchKey=='yes' && ($sortby=='o.Approved' || $sortby=='') ){
				...
				$strAddQuery .= " and o.Approved='0'";
			}else if($sortby=='o.InvoicePaid' && ($SearchKey=='credit card' || $SearchKey=='paypal')){
				$strAddQuery .=  " and o.InvoicePaid='Unpaid' and o.PaymentTerm like '%".$SearchKey."%' and o.OrderPaid in ('1','3','4') ";
			}else if($sortby=='o.InvoicePaid'){
				$strAddQuery .= (!empty($SearchKey))?(" and (".$sortby." like '".$SearchKey."')"):("");
		...
		function  GetSalesBrief($OrderID,$SaleID,$Module)
		{
			...
			$strAddQuery .= (!empty($SaleID))?(" and o.SaleID='".$SaleID."'"):("");
			$strAddQuery .= (!empty($Module))?(" and o.Module='".$Module."'"):("");
			$strSQLQuery = "select o.Module, o.OrderID, o.InvoiceID, o.CreditID, o.SaleID,  o.InvoiceEntry, o.StatusMsg, o.PaymentTerm,o.CardCharge, o.CardChargeDate, o.InvoiceEntry, o.TotalInvoiceAmount, o.TotalAmount,o.CustomerCurrency from s_order o  where 1 ".$strAddQuery." order by o.OrderID desc"; 					
			return $this->query($strSQLQuery, 1);
		}
...
$strAddQuery .= (!empty($Config['ComShow']))?(" and i.parent_item_id='0'"):("");
			//$strSQLQuery = "select i.*,t.RateDescription from s_order_item i left outer join inv_tax_rates t on i.tax_id=t.RateId where 1".$strAddQuery." order by i.id asc";
                         $strSQLQuery = "select i.*,t.RateDescription,c.valuationType as evaluationType ,itm.qty_on_hand,itm.CategoryID,itm.sale_tax_rate, itm.purchase_tax_rate, w.warehouse_code,w.warehouse_name  from ".$Db."s_order_item i left outer join ".$Db."inv_tax_rates t on i.tax_id=t.RateId left outer join ".$Db."inv_items itm on ( i.item_id=itm.ItemID) left outer join ".$Db."inv_categories c on c.CategoryID =itm.CategoryID left outer join ".$Db."w_warehouse w on i.WID =w.WID where 1 ".$strAddQuery." order by i.id asc"; 
			return $this->query($strSQLQuery, 1);
		}
...
function  GetSaleComponentItem($OrderID,$PidID) 
     ...
     $strAddQuery .= (!empty($OrderID))?(" and i.parent_item_id='".$PidID."'"):("");
		...
		function AddSale($arryDetails,$Db='')
		{  
			global $Config;
			extract($arryDetails);
	...
	//EDI Update	
	$DBName=''; if(!empty($EDIPurchaseCompName)) $EDICompName=$EDIPurchaseCompName; 
		if($EDISalesCompName!='') { 
					$DBName='erp_'.$EDISalesCompName.'.'; 
					$EDICompName=$_SESSION['DisplayName']; 
					$EDICompId=$_SESSION['CmpID']; 
		}
	if($AdminID==''){
	$AdminID=$_SESSION['AdminID'];
	...
	//End EDI Update	
		    ...
		    $strSQLQuery = "INSERT INTO ".$DBName."s_order SET Module = '".$Module."',
																		...
																		VendorSalesPerson = '".addslashes($vendorSalesPersonID)."',
																		InvoiceID = '".$InvoiceID."',
																		Approved = '".$Approved."',
																		Status = '".$Status."',
...
$strSQLQuery .= " ,EDICompId='".addslashes($EDICompId)."' ,EDICompName='".addslashes($EDICompName)."'";
			...
			if(isset($ReSt) && isset($ReStocking)){
				$strSQLQuery .= " ,ReSt='".addslashes($ReSt)."' ,ReStocking='".addslashes($ReStocking)."'  ";	
			}
		...
		function AddUpdateItem($order_id, $arryDetails,$DBName='')
		{  
			global $Config;
			extract($arryDetails);
			
			if($_GET['bh']==1){ pr($arryDetails) ; exit; }
...
$arryProduct=$objItem->GetItemByCategoryID(0,$arryDetails['item_id'.$i]);

if($arryProduct[0]['Spiff']==1){

    if($arryProduct[0]['OverrideItem']==1){
       $Spiffamt = $arryProduct[0]['OverspiffAmt'];
    }else{
       $Spiffamt = $arryProduct[0]['spiffAmt'];
    
    }

     if($arryProduct[0]['spiffType']=='P'){

     $FinalSpiffamt = $arryDetails['amount'.$i]*$Spiffamt/100;
     }else{

     if($arryDetails['amount'.$i]>0){
            $FinalSpiffamt =$Spiffamt*$arryDetails['qty'.$i];
        }else{
         $FinalSpiffamt =0.00;
...
$arryDetails['EntryMonth'.$i] = ''; $arryDetails['EntryWeekly'.$i] = '';
                                          ...
                                          if(!empty($arryDetails['DropshipCheck'.$i]) && $arryDetails['DropshipCheck'.$i]!='' ){$DropshipCheck = 1;$serial_value='';}else{$DropshipCheck = 0;$serial_value=trim($arryDetails['serial_value'.$i]);}

//if($EDIExe==1){ $DropshipCheck =0; } if($DBName!=''){ $id =0; }  //Added for EDI Dropship remove
if($DBName!=''){
					$arryDetails['QtySel'.$i] = $arryDetails['qty'.$i];
					
					$totDiscountCal = $arryDetails['price'.$i]*$arryDetails['MDAmount']/100;
				$totDiscountAmt = $arryDetails['price'.$i] + $totDiscountCal;
				
					$arryDetails['price'.$i]=$totDiscountAmt;
...
/*********end By bhoodev based on Condition 6 june 2016*********************/
			 ...
			 $strSQL = "update s_order set discountAmnt ='".$discountAmnt."',setSpiffamt='".$setSpiffamt."' where OrderID='".$order_id."'"; 
			$this->query($strSQL, 0);
			return true;
                ...
                function  CountSkuSerialNoAndQtyInvoiced($Sku)
		{
			...
			function  selectSerialNumberForItem27nov2017($Sku)
			{
				   ...
				   $strAddQuery = '';
							...
							$strAddQuery .= (!empty($Config['binid']))?(" and `binid`='".mysql_real_escape_string($Config['binid'])."'"):("");
...
$strAddQuery ='';
							...
							if(!empty($Config['GetNumRecords'])){
							     $Columns = " count(serialID) as NumCount ";
									...
									$Columns = " Sku,`Condition`,LineID,UnitCost,serialNumber,serialID,UsedSerial,Status,binid ";
									if($Config['RecordsPerPage']>0)$strAddQuery .= " limit ".$Config['StartPage'].",".$Config['RecordsPerPage'];
							}
...
//$strAddQuery .= (!empty($Config['OrderID']))?(" and `OrderID`='".mysql_real_escape_string($Config['OrderID'])."'"):("");
 ...
 /************************************************/
			 	...
			 	if($arrySale[0]['EDICompName']!=''){
				      $EDIRefNo = explode("/",$arrySale[0]['EDIRefNo']);
		     
				            $strSoQuery = "insert into edi_request(Amount, ReqDate, SalesID,AdminType,AdminID,Type,CustCode,CustID,DeleteDate,CustomerName) values('".$arrySale[0]["TotalAmount"]."' , '".$arrySale[0]["OrderDate"]."', '".$arrySale[0]["SaleID"]."', '".$_SESSION['AdminType']."','".$_SESSION['AdminID']."','SO','".$arrySale[0]["CustCode"]."','".$arrySale[0]["CustID"]."','".$Config['TodayDate']."','".$arrySale[0]["CustomerName"]."')";
...
if(!empty($arrySale[0]['ecom_order_id']) && $arrySale[0]['ecom_order_id']>0){

	$Updatesql = "update e_orders set isCustom = '0'  where OrderID='".$arrySale[0]['ecom_order_id']."' ";
			...
			$strSQL = "delete from s_order_spiff where OrderID='".$OrderID."'"; 
			$this->query($strSQL, 0);
...
PONumber='".$PONumber."',
                ...
                function GenerateInVoiceEntry($arryDetails)
		{
			...
			$strSQLQuery = "INSERT INTO s_order SET Module = 'Invoice', ConversionRate='".$ConversionRate."', EntryType='".$EntryType."', EntryInterval='".$EntryInterval."',EntryMonth='".$EntryMonth."', EntryWeekly = '".$EntryWeekly."', EntryFrom='".$EntryFrom."',EntryTo='".$EntryTo."',EntryDate='".$EntryDate."', OrderDate='".$OrderDate."', SaleID ='".$ReferenceNo."', QuoteID = '".$QuoteID."', SalesPersonID = '".$SalesPersonID."', SalesPerson = '".addslashes($SalesPerson)."',SalesPersonType = '".$SalesPersonType."', VendorSalesPerson = '".$vendorSalesPersonID."', InvoiceID = '".$InvoiceID."',
...
$inv = $this->query($selInvoice, 1);
     ...
     /* Set Override and spiff  based on product */
$objItem=new items();	
$arryProduct=$objItem->GetItemByCategoryID(0,$arryDetails['item_id'.$i]);
...
/* End Override and spiff  based on product */                            
			...
			if($OrderID>0){
				...
				if($module=='Quote'){	
					...
					$InvoicePaid = $arrySale[0]['InvoicePaid'];
  					if($InvoicePaid=='Unpaid' && $arrySale[0]['PaymentTerm']=='Credit Card' && ($arrySale[0]['OrderPaid']==1 || $arrySale[0]['OrderPaid']==3 || $arrySale[0]['OrderPaid']==4)){
						$InvoicePaid = 'Paid';
					}
					/************/
...
City = '".addslashes($City)."',State = '".addslashes($State)."', Country = '".addslashes($Country)."', ZipCode = '".addslashes($ZipCode)."', Mobile = '".$Mobile."', Landline = '".$Landline."', Fax = '".$Fax."', Email = '".addslashes($Email)."',
			ShippingName = '".addslashes($ShippingName)."', ShippingCompany = '".addslashes($ShippingCompany)."', ShippingAddress = '".addslashes(strip_tags($ShippingAddress))."', ShippingCity = '".addslashes($ShippingCity)."',ShippingState = '".addslashes($ShippingState)."', ShippingCountry = '".addslashes($ShippingCountry)."', ShippingZipCode = '".addslashes($ShippingZipCode)."', ShippingMobile = '".$ShippingMobile."', ShippingLandline = '".$ShippingLandline."', ShippingFax = '".$ShippingFax."', ShippingEmail = '".addslashes($ShippingEmail)."',ShippingMethod='".addslashes($ShippingMethod)."',ShippingMethodVal ='".addslashes($ShippingMethodVal)."',CountryId ='".addslashes($CountryId)."',StateID ='".addslashes($StateID)."',CityID ='".addslashes($CityID)."',ShippingCountryID ='".addslashes($ShippingCountryID)."',ShippingStateID ='".addslashes($ShippingStateID)."',ShippingCityID ='".addslashes($ShippingCityID)."', TaxRate='".addslashes($TaxRate)."',freightTxSet='".addslashes($freightTxSet)."',tax_auths='".addslashes($tax_auths)."'  ".$addSQLQuery." where OrderID='".$OrderID."'  ";                
				$this->query($strSQLQuery, 0);                                                     
...
function AddShipmentInvoiceItem($order_id, $arryDetails)
 ...
 $query_parts = array();
    ...
    if(!empty($arryDetails['EdiConfirm']) && !empty($arryDetails['InvPOId'])){
           $arryItemOrd=$this->checkItemOrder($arryDetails['OID'],$arryDetails['item_id'.$i],$arryDetails['Condition'.$i]);
           $id = $arryItemOrd[0]['id'];
...
return 1;
    ...
    function salesOrderRecurring(){       
          global $Config;
	  $Config['CronEntry']=1;
	// $Config['TodayDate'] = '2018-11-01 05:48:55'; //temp
          $arryDate = explode(" ", $Config['TodayDate']);
   	  $arryDay = explode("-", $arryDate[0]);
	...
	//pr($arrySaleLineItem);exit;
	  ...
	  foreach($arrySaleLineItem as $value)
	  {
		...
		$YearOrder = $arryDt[0]; 
		$MonthOrder = $arryDt[1]; 
		$YearMonthOrder = $arryDt[0].'-'.$MonthOrder;
		...
		$MonthChild = $arryDtChild[1]; 
		$YearMonthChild = $arryDtChild[0].'-'.$MonthChild;
		
 		/**************/
		$lastEntryDate = !empty($ChildDate)?($ChildDate):($ModuleDate);	
		$arryLastDate = explode("-",$lastEntryDate); 
		list($year, $month, $day) = $arryLastDate;
		$TempDate  = mktime(0, 0, 0, $month+2 , $day, $year);		
		$ThisYearMonth = date("Y-m",$TempDate);
		if($ThisYearMonth<$YearMonth){  
			$ThisEntryMonth = $Month; //set current month for old data
		}else{
			$ThisEntryMonth = (int)date("m",$TempDate);
		}
				...
				case 'semi_monthly':
				 ...
				 case 'monthly':					 
				  ...
				  case 'bi_monthly':
					...
					if($value['EntryDate']==$Day && $ThisEntryMonth==$Month && $YearMonthChild!=$YearMonth)
					{ 
						$OrderFlag=1;
					}
					break;
		...
		if($OrderFlag==1){
			...
			$order_id = $this->addSalesRecurring($value['OrderID'],$value['Module'],$value['PaymentTerm'], $value['item_id'],$taxAmnt,$TotalAmount,$line_qty,$line_price,$line_amount);
	...
	return true;
	   ...
	   function addSalesRecurring($order_id,$Module,$PaymentTerm,$item_id,$taxAmnt,$TotalAmount,$line_qty,$line_price,$line_amount){
			global $Config;
			/*****************/
			...
			if($Module=="Invoice"){
				...
				if($PaymentTerm=='Credit Card'){
					$strAdd .= " , BalanceAmount ='".$TotalAmount."' ";
				}
...
return $rs = $this->query($strSQLQuery, 1);
		...
		function  SalesCommissionReport($FromDate,$ToDate,$SalesPersonID,$SalesPersonType=0)
		{
			...
			//$strAddQuery .= (!empty($SalesPersonID))?(" and  FIND_IN_SET('".$SalesPersonID."',o.SalesPersonID)>0 "):("");
			
			if($SalesPersonType=="1"){
				$strAddQuery .= (!empty($SalesPersonID))?(" and c.SuppID ='".$SalesPersonID."'"):("");
				$SalesPersonCol = " IF(s.SuppType = 'Individual' and s.UserName!='', s.UserName, s.CompanyName) as SalesPerson";
				$columns  = " c.SuppID as SalesPersonID";
				$comCondition = "inner join h_commission c on FIND_IN_SET(c.SuppID,o.VendorSalesPerson)>0  left outer join p_supplier s on c.SuppID=s.SuppID ";
				$condition = "and o.VendorSalesPerson>0 ";
				$groupBy = "group by c.SuppID";
			}else{
				$strAddQuery .= (!empty($SalesPersonID))?(" and c.EmpID ='".$SalesPersonID."'"):("");
				 
				$SalesPersonCol = " e.UserName as SalesPerson";
				$columns  = " c.EmpID as SalesPersonID";
				$comCondition = " inner join h_commission c on  FIND_IN_SET(c.EmpID,o.SalesPersonID)>0  left outer join h_employee e on c.EmpID=e.EmpID";
                $condition = "and o.SalesPersonID>0 ";
				$groupBy = "group by c.EmpID";
			}
			...
			if($Config['ConversionType']==1){
			   ...
			   $strSQLQuery = "select  sum(IF(c.CommPaidOn='Paid', DECODE(p.DebitAmnt,'". $Config['EncryptKey']."'), ".$ConvertedAmount.")) as TotalSales , ".$SalesPersonCol." , o.OrderID as InID,c.CommOn,c.CommPaidOn, sum(i.qty_invoiced) as QtyInvoiced,sum(i.avgCost) as Cost,".$columns." from s_order o ".$comCondition." left outer join f_payments p on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='')  left outer join s_order_item i on o.OrderID=i.OrderID where o.Module='Invoice' and o.SalesPerson!='' ".$condition." and o.PostToGL='1'   ".$strAddQuery.$groupBy." order by SalesPerson asc"; 
 
			// if(!empty($_GET['pk567'])) echo $strSQLQuery;
				
			return $this->query($strSQLQuery, 1);		
		}


		function  GetSalesPersionForCommission($FromDate,$ToDate,$SalesPersonType=0,$EmpIDMulti='')
		{
            		global $Config;
			$strAddQuery = "";			
			if(!empty($FromDate) && !empty($ToDate)){				
			...
			if($SalesPersonType=="1"){	
				$strAddQuery .= (!empty($EmpIDMulti))?(" and c.SuppID in (".$EmpIDMulti.")"):("");		  
				$comCondition = "inner join h_commission c on FIND_IN_SET(c.SuppID,o.VendorSalesPerson)>0  left outer join p_supplier s on c.SuppID=s.SuppID ";
				$condition = "and o.VendorSalesPerson>0 ";
				$groupBy = "group by c.SuppID";
			}else{				 
				$strAddQuery .= (!empty($EmpIDMulti))?(" and c.EmpID in (".$EmpIDMulti.")"):("");
				$comCondition = " inner join h_commission c on  FIND_IN_SET(c.EmpID,o.SalesPersonID)>0  left outer join h_employee e on c.EmpID=e.EmpID";
				$condition = "and o.SalesPersonID>0 ";
				$groupBy = "group by c.EmpID";
			}

			$strSQLQuery = "select c.EmpID as SalesPersonID, c.CommOn, c.CommType, c.CommPaidOn from s_order o ".$comCondition." left outer join f_payments p on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='') where o.Module='Invoice' and o.SalesPerson!='' ".$condition." and o.PostToGL='1'   ".$strAddQuery.$groupBy." order by SalesPerson asc"; 
		...
		function  CommReport($arrySalesCommission, $FromDate,$ToDate,$SalesPersonID,$SalesPsType=0 )
		{
                        global $Config;
 
  			extract($arrySalesCommission);
 
			$strAddQuery = "";						
			if($SalesPsType=="1"){
				$strAddQuery .= (!empty($SalesPersonID))?(" and  c.SuppID ='".$SalesPersonID."'"):("");				
				$comCondition = "inner join h_commission c on FIND_IN_SET(c.SuppID,o.VendorSalesPerson)>0 ";
				$commCondition ="o.VendorSalesPerson>0";
			}else{
				$strAddQuery .= (!empty($SalesPersonID))?(" and  c.EmpID ='".$SalesPersonID."'"):("");
				$comCondition = " inner join h_commission c on FIND_IN_SET(c.EmpID,o.SalesPersonID)>0 ";
				$commCondition ="o.SalesPersonID>0";
			}
			 

			 

			if($CommOn=="1" && $CommType!='Spiff'){ //per invoice payment
			...
			if($CommPaidOn == 'Paid' && $CommType!='Spiff'){ 
				$strAddQuery .= " and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid')  ELSE 1 END = 1  ";
			}
			...
			$strSQLQuery = "select p.*, SUM(DECODE(p.DebitAmnt,'". $Config['EncryptKey']."')) as DebitAmnt, SUM(DECODE(p.CreditAmnt,'". $Config['EncryptKey']."')) as CreditAmnt,   o.InvoiceDate,o.Fee,o.OrderDate,o.InvoiceID as MainInvoiceID, o.OrderID as InID,o.CustomerName,o.SalesPersonID, o.InvoicePaid, o.TotalAmount, o.TotalInvoiceAmount, o.Freight, o.Fee, o.taxAmnt, o.ConversionRate, o.CustomerCurrency, c.CommOn,c.CommPercentage,c.CommPaidOn,c.CommType,c.SpiffType,c.spiffBasedOn,c.amountType from s_order o ".$comCondition."  left outer join f_payments p  on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='')  where o.Module='Invoice' and o.SalesPerson!='' and ".$commCondition." and o.PostToGL='1' ".$strAddQuery."  ".$groupby.$orderby; 
    ...
    global $Config;
            ...
            $comCondition = 'FIND_IN_SET(c.EmpID,o.SalesPersonID)>0';
			if(!empty($SuppID)){			
				//$strAddQuery .= " and FIND_IN_SET('".$SuppID."',o.SalesPersonID)> 0  and o.SalesPersonType='1' "; 
				$strAddQuery .= " and c.SuppID = '".$SuppID."'  and o.SalesPersonType='1' "; 
				//$comCondition = 'o.SalesPersonID=c.SuppID';
				$comCondition = 'FIND_IN_SET(c.SuppID,o.SalesPersonID)>0';
				$commiCondi = " and o.VendorSalesPerson>0";

			}else if(!empty($EmpID)){	
				$strAddQuery .= " and c.EmpID = '".$EmpID."'  and o.SalesPersonType='0' "; 	 
				$commiCondi = " and o.SalesPersonID>0 ";
			...
			if($Config['ConversionType']==1){
		     ...
		     $strSQLQuery = "select sum(".$ConvertedAmount.") as TotalSales from s_order o   inner join h_commission c on ".$comCondition." where o.Module='Invoice'  and o.SalesPerson!=''  ".$commiCondi." and o.PostToGL='1' ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1  order by o.InvoiceDate asc, o.OrderID asc";
		...
		function  GetInvPaymentNonResidual($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0){
			...
			if(!empty($SuppID)){			 
				...
				$comCondition = 'FIND_IN_SET(c.SuppID,o.VendorSalesPerson)>0';
				$strJoinQuery  = " and  cs.VenSpId ='".$SuppID."'" ;
				$commiCondi = "o.VendorSalesPerson>0";
			}else if(!empty($EmpID)){	
				$strAddQuery .= " and c.EmpID = '".$EmpID."'  ";
				$strJoinQuery  = " and  cs.EmpSpId ='".$EmpID."'";	 
				$commiCondi = "o.SalesPersonID>0";
			}	
			$sql_invoice = "select o.InvoiceID from s_order o inner join h_commission c on ".$comCondition."  where o.Module='Invoice' and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1'  ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 group by o.InvoiceID order by o.InvoiceDate asc,o.OrderID asc limit 0,1";
			...
			if(!empty($arryInvoice[0]["InvoiceID"])){
				...
				$strInnerQuery = (!empty($OrderID))?(" and o.OrderID='".$OrderID."'"):("");		
			 	$strSQLQuery = "select (".$ConvertedAmount.") as TotalSales , cs.CommPercentage,o.InvoiceID from s_order o left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice' and o.InvoiceID='".$arryInvoice[0]["InvoiceID"]."' ".$strInnerQuery."";
		...
		function  GetInvPaymentPer($PaymentID,$FromDate,$ToDate,$EmpID,$SuppID=0)
		{
			...
			if(!empty($SuppID)){			
			 ...
			 $strSQLQuery = "select DECODE(p.DebitAmnt,'". $Config['EncryptKey']."') as TotalSales from f_payments p left outer join s_order o on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='') inner join h_commission c on ".$comCondition." where o.Module='Invoice'  and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1' and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 ".$strAddQuery." ";
		...
		function  GetInvPaymentNonResidualPer($PaymentID,$FromDate,$ToDate,$EmpID,$SuppID=0)
		{
			...
			if(!empty($SuppID)){			
				$strAddQuery .= " and o.SalesPersonID='".$SuppID."' and o.SalesPersonType='1' "; 
				$comCondition = 'o.VendorSalesPerson=c.SuppID';
    $commiCondi = "o.VendorSalesPerson>0";
			}else if(!empty($EmpID)){	
				$strAddQuery .= " and o.SalesPersonID='".$EmpID."' and o.SalesPersonType='0' "; 	
				$commiCondi = "o.SalesPersonID>0"; 
			 ...
			 $sql_invoice = "select p.InvoiceID from f_payments p left outer join s_order o on (p.InvoiceID=o.InvoiceID and p.PaymentType='Sales' and p.InvoiceID!='') inner join h_commission c on ".$comCondition." where o.Module='Invoice' and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1' ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 group by p.InvoiceID order by p.PaymentDate asc,p.PaymentID asc limit 0,1";
			$arryInvoice = $this->query($sql_invoice, 1);
	...
	if($Config['ConversionType']==1){
			$ConvertedItemAmount =  "(o.setSpiffamt)/(o.ConversionRate)" ;
		}else{
			$ConvertedItemAmount = "(o.setSpiffamt)*(o.ConversionRate)";
		}


			 $strSQLQuery = "select  sum(".$ConvertedAmount.") as TotalSales,sum(".$ConvertedItemAmount.") as TotalItemSales, o.CustomerCurrency,o.ConversionRate,o.OrderID as InID, o.InvoiceID, o.TotalInvoiceAmount,o.Freight,o.Fee ,o.TDiscount,c.CommPaidOn,cs.CommPercentage  from s_order o inner join h_commission c on ".$comCondition." left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice' and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1'  ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc";
		...
		function  GetInvPaymentNonResidualMargin($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0)
		{
			...
			if(!empty($SuppID)){			
		 	...
		 	$strSQLQuery = "select  o.InvoiceID from s_order o  inner join h_commission c on ".$comCondition." left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice'  and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1' ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc limit 0,1";
			return $this->query($strSQLQuery, 1);
							
		}

function checkCustomerPO($CustomerPO)
		{
			
			 $strSQLQuery = "select CustomerPO from s_order where CustomerPO='".$CustomerPO."'";
	...
	function  GetInvPaymentCal($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0){
		global $Config;
		$strAddQuery = "";			 
		if(!empty($FromDate) && !empty($ToDate)){				
			$strAddQuery .= " and o.InvoiceDate>='".$FromDate."' and o.InvoiceDate<='".$ToDate."' ";
	...
	if($Config['ConversionType']==1){
		...
		$strSQLQuery = "select sum(".$ConvertedAmount.") as TotalSales,sum(".$ConvertedItemAmount.") as TotalItemSales,cs.CommPercentage from s_order o   inner join h_commission c on ".$comCondition." left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice'  and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1' ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1   group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc";

		return $arryRow = $this->query($strSQLQuery, 1);

	}

	function  GetInvoiceSpiff($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0){
		global $Config;
		$strAddQuery = "";			 
		if(!empty($FromDate) && !empty($ToDate)){				
			$strAddQuery .= " and o.InvoiceDate>='".$FromDate."' and o.InvoiceDate<='".$ToDate."' ";
	...
	if($Config['ConversionType']==1){
		...
		$strSQLQuery = "select sum(".$ConvertedAmount.") as TotalSales,sum(".$ConvertedItemAmount.") as TotalItemSales,o.InvoiceID from s_order o inner join h_commission c on ".$comCondition."  where o.Module='Invoice'  and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1' ".$strAddQuery."   group by o.InvoiceID order by o.InvoiceDate asc, o.OrderID asc";
 		 
		return $arryRow = $this->query($strSQLQuery, 1);

	}

	function UpdateEdiPO($arryDetails){
		
		global $Config;
			extract($arryDetails);
			
			
		  	 $strSQLselQuery = "select EdiPoID,EDICompId,EDICompName from s_order  where OrderID = '".$OrderID."'"; 
		    $rows = $this->query($strSQLselQuery, 1);
		  
		 
		# pr($rows); exit;
			$DB = '';
			if(!empty($rows[0]['EdiPoID']) && $rows[0]['EDICompName']!='' ){
			
			$DB = 'erp_'.$rows[0]['EDICompName'].'.';
			
			  $strSQLQuery = "update ".$DB."p_order set  Approved='".$Approved."', Status='".$Status."',   ClosedDate='".$ClosedDate."',  DeliveryDate='".$DeliveryDate."',Comment='".addslashes(strip_tags($Comment))."', TotalAmount='".addslashes($TotalAmount)."' ,Freight='".addslashes($Freight)."'	,UpdatedDate = '".$Config['TodayDate']."', PaymentMethod='".addslashes($PaymentMethod)."', ShippingMethod='".addslashes($ShippingMethod)."', PaymentTerm='".addslashes($PaymentTerm)."', Taxable='".addslashes($Taxable)."' , taxAmnt ='".addslashes($taxAmnt)."', tax_auths='".addslashes($tax_auths)."', TaxRate='".addslashes($MainTaxRate)."', AccountID ='".addslashes($AccountID)."' ,PrepaidFreight ='".addslashes($PrepaidFreight)."' ,ShippingMethodVal ='".addslashes($ShippingMethodVal)."',freightTxSet ='".addslashes($freightTxSet)."',TrackingNo ='".addslashes($TrackingNo)."',CreditCardVendor ='".addslashes($CreditCardVendor)."',ShippingAccountNumber ='".$ShipAccountNumber."', wName = '".addslashes($ShippingCompany)."', wAddress = '".addslashes(strip_tags($ShippingAddress))."', wCity = '".addslashes($ShippingCity)."',wState = '".addslashes($ShippingState)."', wCountry = '".addslashes($ShippingCountry)."', wZipCode = '".addslashes($ShippingZipCode)."', wMobile = '".$ShippingMobile."', wLandline = '".$ShippingLandline."', wEmail = '".addslashes($ShippingEmail)."' ".$strAddSql." where OrderID='".mysql_real_escape_string($rows[0]['EdiPoID'])."'";   
			...
			$this->query($strSQLQuery, 0);
			//if($_GET['tttt']==1){
		 $strSQLdelQuery = "delete from ".$DB."p_order_item  where OrderID='".$rows[0]['EdiPoID']."'";			
			 $this->query($strSQLdelQuery, 0);
			
			
			$order_id =$rows[0]['EdiPoID'];
				$arryItem = $this->GetsaleItem($OrderID);
				...
				for($i=1;$i<=$NumLine;$i++){
					...
					if($arryDetails['qty'.$i]>0){
					//$amount = $arryItem[$Count]["avgCost"]*$arryDetails['qty'.$i];
						$qty = $arryDetails['qty'.$i];
						$sql = "insert into ".$DB."p_order_item(OrderID, item_id,sku, description, qty,  price, tax_id, tax, amount, Taxable,`Condition`,  WID) values('".$order_id."', '".$arryItem[$Count]["item_id"]."' , '".$arryItem[$Count]["sku"]."', '".$arryItem[$Count]["description"]."', '".$arryItem[$Count]["qty"]."', '".$arryItem[$Count]["price"]."', '".$arryItem[$Count]["tax_id"]."', '".$arryItem[$Count]["tax"]."', '".$arryDetails['amount'.$i]."', '".$arryItem[$Count]["Taxable"]."','".$arryItem[$Count]["Condition"]."','".$arryItem[$Count]["WID"]."')";
	...
	/*************
	function UpdateSalesPersonPdf346436(){

			$objConfigure=new configure();	
		  	  $strSQLQuery = "select * from s_order where CustID in (15867,15868,15869,15870) "; 
		    $rows = $this->query($strSQLQuery, 1);
		  	  ...
		  	  foreach($rows as $value) { 
					...
					if($value['Module']=="Order"){
						...
						$PdfArray['TableName'] =  "s_order";
						$PdfArray['OrderColumn'] =  "OrderID";
						$PdfArray['OrderID'] =  $value['OrderID'];		 
						$objConfigure->GeneratePDF($PdfArray);	
					}else{
	...
	*************/
	...
	function  GetInvPaymentOneTime($FromDate,$ToDate,$EmpID,$SuppID=0,$OrderID=0,$SpiffTye=0){
                        global $Config;
			$strAddQuery = "";
			if(!empty($FromDate) && !empty($ToDate)){				
				$strAddQuery .= "  and o.InvoiceDate>='".$FromDate."' and o.InvoiceDate<='".$ToDate."' ";
    ...
    if(!empty($SpiffTye)){	
       $strAddQuery .= " and o.setSpiffamt >0  ";
       if($Config['ConversionType']==1){
         $ConvertedItemAmount =  "(o.setSpiffamt)/(o.ConversionRate)" ;
       }else{
         $ConvertedItemAmount = "(o.setSpiffamt)*(o.ConversionRate)";
       ...
       $ConvertedItemAmount = " ,(".$ConvertedItemAmount.") as TotalItemSales  ";
	 ...
	 $sql_invoice = "select o.InvoiceID from s_order o inner join h_commission c on ".$comCondition."  where o.Module='Invoice' and o.SalesPerson!='' and ".$commiCondi." and o.PostToGL='1'  ".$strAddQuery." and CASE WHEN c.CommPaidOn='Paid' THEN o.InvoicePaid in ('Paid','Part Paid') ELSE 1 END = 1 group by o.InvoiceID order by o.InvoiceDate asc,o.OrderID asc limit 0,1";
			...
			if(!empty($arryInvoice[0]["InvoiceID"])){
				if($Config['ConversionType']==1){
				 ...
				 $strInnerQuery = (!empty($OrderID))?(" and o.OrderID='".$OrderID."'"):("");		
			 	$strSQLQuery = "select (".$ConvertedAmount.") as TotalSales , cs.CommPercentage,o.InvoiceID ".$ConvertedItemAmount." from s_order o left outer join customer_sales_commission cs on o.CustID=cs.CustomerId ".$strJoinQuery." where o.Module='Invoice' and o.InvoiceID='".$arryInvoice[0]["InvoiceID"]."' ".$strInnerQuery."";
		...
		function checkItemOrder($OrderID,$ItemID,$Condition){
		
		     $strSQLQuery = "select id from s_order_item  where OrderID = '".$OrderID."' and item_id='".$ItemID."' and `condition`='".$Condition."'  "; 

       return $arrRow = $this->query($strSQLQuery, 1);
		
		}
	function getEdiCurrency($currency_id,$Status)
		{
		global $Config;
			$sql  = " where 1 "; 
			...
			$sql="select * from ".$Config['DbMain'].".currencies ".$sql." order by name";
			return $this->query($sql);
		}
		
		 function GetEdiLocationData($id = 0, $Status = 0,$Db) {
	global $Config;
        $sql = " where 1";
        $sql .= (!empty($id)) ? (" and l.locationID = '" . mysql_real_escape_string($id) . "'") : ("");
        $sql .= (!empty($Status) && $Status == 1) ? (" and l.Status = '" . $Status . "'") : ("");

        $sql = "select l.*,c.code as CountryCode, s.code as StateCode from ".$Db."location l inner join ".$Config['DbMain'].".country c on  l.country_id =  c.country_id left outer join ".$Config['DbMain'].".state s on  l.state_id =  s.state_id " . $sql . " order by l.locationID Asc";
        return $this->query($sql, 1);
    }
    
    
    function AddEDISale($arryDetails,$Db=''){
	     ...
	     $DBName=''; 
		     ...
		     if($EDIPurchaseCompName!='' && !empty($EDICompId)) { 
  ...
  $EdiPOFromSO = $objConfigure->getSettingVariable('EDI_PO_SO');
 // Add EDI purchase
		if($EDICompId!='' && $EDIPurchaseCompName!='' && $EdiPOFromSO=='yes'){
			$DBName='';
			if($EDIPurchaseCompName!='') {
				$DBName='erp_'.$EDIPurchaseCompName.'.';

				$ParentCmpDatabase = $_SESSION['CmpDatabase'];
				$_SESSION['CmpDatabase'] = 'erp_'.$EDIPurchaseCompName;				
			}
			
			$sqlc= "select ConversionType from ".$Config['DbMain'].".company where CmpID='".addslashes($EDICompId)."' ";
			$cmpdetail = $this->query($sqlc);

			$ParentConversionType = $_SESSION['ConversionType'];
			$_SESSION['ConversionType'] = $cmpdetail[0]['ConversionType'];


			$sql= "select * from ".$DBName."p_supplier where EDICompId='".addslashes($_SESSION['CmpID'])."' ";
			$supplierdetail = $this->query($sql);
			
			$sqladd= "select * from ".$DBName."p_address_book where SuppID='".addslashes($supplierdetail[0]['SuppID'])."' and AddType='billing' ";
			$arrySuppAdd = $this->query($sqladd);
			
			$sqlwareadd= "select * from ".$DBName."w_warehouse where 1 and `Default` =1 ";
			$arrywarehouse = $this->query($sqlwareadd);
			
			//Edi PO Shipping address from Edi SO
			//$arryDetails['wName']=$arrywarehouse[0]['warehouse_name'];
			$arryDetails['wName']=$ShippingCompany;
   $arryDetails['wCode']=$arrywarehouse[0]['warehouse_code'];
   $arryDetails['wAddress']=$ShippingAddress;
   $arryDetails['wCity']=$ShippingCity;
   $arryDetails['wState']=$ShippingState;
   $arryDetails['wCountry']=$ShippingCountry;
   $arryDetails['wZipCode']=$ShippingZipCode;
   $arryDetails['wMobile']=$ShippingMobile;
   $arryDetails['wLandline']=$ShippingLandline;
   $arryDetails['wEmail']=$ShippingEmail;
   		
   $arryDetails['SuppCode']=$supplierdetail[0]['SuppCode'];
   $arryDetails['SuppCompany']=$arrySuppAdd[0]['Name'];
   $arryDetails['SuppContact']=$supplierdetail[0]['Mobile'];
   $arryDetails['SuppCurrency']=$supplierdetail[0]['Currency'];
   $arryDetails['Currency']=$CustomerCurrency;
   $arryDetails['PrefixPO']='PO';
   
   //wName, wContact, wAddress, wCity, wState, wCountry, wZipCode, wMobile, wLandline, wEmail
			
			//update Date And Currency
			$arryEdiLocation=$this->GetEdiLocationData(1,0,$DBName);
			$Config['TodayDate'] = getLocalTime($arryEdiLocation[0]['Timezone']);
 
			$arryDetails['OrderDate'] =$Config['TodayDate'];
			$arryDetails['PostedDate'] =$Config['TodayDate'];
			
			if($arryEdiLocation[0]['currency_id']>0){
				$arryEdiSelCurrency = $this->getEdiCurrency($arryEdiLocation[0]['currency_id'],'');
				$Config['Currency'] = $arryEdiSelCurrency[0]['code'];
...
//update Date And Currency
			...
			$arryDetails['ModuleID']='PurchaseID';
				...
				$arryDetails['ShippingAccountVendor']=$ShippingAccountCustomer;
				//$arryDetails['ShippingAccNO']=$ShippingAccNO;
			...
			$_SESSION['CmpDatabase'] = $ParentCmpDatabase;
			$_SESSION['ConversionType'] = $ParentConversionType;
		
//echo '<br>After: '.$_SESSION['CmpDatabase'].'#'.$_SESSION['ConversionType'];die;
		...
		$arrySal = $this->query($SElectSaleQury, 1);
			 ...
			 $strSQL = "update s_order set EDIRefNo='EDI/".$_SESSION['CmpID']."/".$EDICompId."/".$order_id."/".$arryPur[0]['PurchaseID']."',PONumber = '".$arryPur[0]['PurchaseID']."',EdiPoID ='".$order_id."',CustomerPO='".$arryPur[0]['PurchaseID']."' where OrderID='".$OrderID."'"; 
			$this->query($strSQL, 0);

			$strSQL = "update ".$DBName."p_order set EDIRefNo='EDI/".$_SESSION['CmpID']."/".$EDICompId."/".$OrderID."/".$arrySal[0]['SaleID']."',EDICompId='".addslashes($_SESSION['CmpID'])."',EDICompName='".addslashes($_SESSION['DisplayName'])."',EdiSoID ='".$OrderID."',SuppCompany ='".$arrySuppAdd[0]['Name']."', Address='".$arrySuppAdd[0]['Address']."', City='".$arrySuppAdd[0]['City']."', State='".$arrySuppAdd[0]['State']."', Country='".$arrySuppAdd[0]['Country']."', ZipCode='".$arrySuppAdd[0]['ZipCode']."',   Mobile='".$arrySuppAdd[0]['Mobile']."', Landline='".$arrySuppAdd[0]['Landline']."', Fax='".$arrySuppAdd[0]['Fax']."', Email='".$arrySuppAdd[0]['Email']."' where OrderID='".$order_id."'";
			$this->query($strSQL, 0);
    ...
    function AddUpdateSpiffInfo($Order_ID, $arryDetails)
		{      
			global $Config;
			extract($arryDetails);

			if(!empty($DelItemSpiff)){
				$strSQLQuery = "delete from s_order_spiff where ID in(".$DelItemSpiff.")"; 
				$this->query($strSQLQuery, 0);
			}	 
			
			for($i=1;$i<=$SpiffNumLine;$i++){     
				if(!empty($arryDetails['SpiffSuppCode'.$i])){
					
					
					if(!empty($arryDetails['SpiffID'.$i])){	 					
                                                  $sql = "update s_order_spiff set OrderID='".$Order_ID."', SuppCode='".addslashes($arryDetails['SpiffSuppCode'.$i])."', Amount= '".$arryDetails['SpiffAmount'.$i]."'  where ID='".$arryDetails['SpiffID'.$i]."' "; 
                                                  $this->query($sql, 0);
                                                       
					 }else{
                                            
                                            $sql = "insert into s_order_spiff set OrderID='".$Order_ID."', SuppCode='".addslashes($arryDetails['SpiffSuppCode'.$i])."',Amount= '".$arryDetails['SpiffAmount'.$i]."' ";
                                            $this->query($sql, 0);	
                                                
					}
					
				}else if(!empty($arryDetails['SpiffID'.$i])){	
					$strSQLQuery = "delete from s_order_spiff  where ID='".$arryDetails['SpiffID'.$i]."' "; 
					$this->query($strSQLQuery, 0);
				}
			}
	...
	function GetSpiffEntry($OrderID,$SaleID)
		{       
		        global $Config;
			 $strSQLQuery = "Select s.*, IF(p.SuppType = 'Individual' and p.UserName!='', p.UserName, p.CompanyName) as VendorName from s_order_spiff s inner join s_order o on (s.OrderID=o.OrderID and s.OrderID>0)  inner join p_supplier p on (s.SuppCode=p.SuppCode and s.SuppCode!='') where 1 ";	
			$strSQLQuery .= (!empty($OrderID))?(" and s.OrderID = '".$OrderID."' "):("");
			$strSQLQuery .= (!empty($SaleID))?(" and o.SaleID = '".$SaleID."' and o.Module='Order' "):("");
			return $this->query($strSQLQuery, 1);
		
		}
    
    
    	function  GetSaleItemBySku($OrderID,$Sku,$Cond,$WID,$Db=''){
       global $Config;
       $strAddQuery = (!empty($OrderID))?(" and i.OrderID='".$OrderID."'"):("");
       $strAddQuery .= (!empty($Sku))?(" and i.sku='".$Sku."'"):("");
       $strAddQuery .= (!empty($Cond))?(" and i.`Condition`='".$Cond."'"):("");
       $strAddQuery .= (!empty($WID))?(" and i.WID='".$WID."'"):("");
       $strAddQuery .= (!empty($Config['droppick']))?(" and i.DropShipCheck='0'"):("");
