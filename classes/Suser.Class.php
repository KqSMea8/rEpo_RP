...
class Suser extends dbClass
{
		...
		function  UserList($arryDetails){
			...
			if($Config['GetNumRecords']==1){
			 ...
			 $strSqlQuery = "select ".$Columns." from u_user order by Firstname asc ".$strAddQuery;		
     ...
     function AddUser($arryDetails)
		{  
			...
			$strSQLQuery = "insert into u_user(FirstName,LastName,Gender ,uEmail, upassword,Designation, Status) values('".addslashes($FirstName)."', '".addslashes($LastName)."', '".addslashes($Gender)."','".addslashes($uEmail)."', '".md5($uPassword)."', '".addslashes($Designation)."', '".$Status."')";
...
function UpdateRolePermission($arryDetails)
	...
	function ChangePassword($newPassword,$Uid)
		...
		function ValidateUser($Email,$Password)
		{
			...
			{
				$strSQLQuery ="select * from u_user where LCASE(uEmail) ='".strtolower(trim($Email))."' and upassword='".md5($Password)."' and Status='1' ";			
				return $this->query($strSQLQuery, 1);
	...
	function GetHeaderMenusUser($Uid) {
		...
		if(!empty($Uid)){
			$strAddQuery ='';
			$strAddQuery .= (!empty($_SESSION['projectID']))?(" and m.depID='".$_SESSION['projectID']."'"):("");
			$strSQLQuery ="SELECT m.*,p.* FROM admin_modules m join permission p on m.ModuleID=p.ModuleID  WHERE m.Parent='0' and  p.UserID='".$Uid."' ".$strAddQuery." Order by m.OrderBy  asc"; 
	 ...
	 function GetHeaderMenusBySuperadmin($Adminid){
		...
		if(!empty($Adminid)){
			$strAddQuery ='';
			$strAddQuery .= (!empty($_SESSION['projectID']))?(" and depID='".$_SESSION['projectID']."'"):("");
			$strSQLQuery ="SELECT * FROM admin_modules WHERE Status='1'  and Parent='0'  ".$strAddQuery." Order by OrderBy  asc";
			return $this->query($strSQLQuery, 1);
		}
	...
	function GetHeaderSubmenuUser($Uid,$Parent)
	 {
	
    if(!empty($Uid))
     {
	// $strid="select*from admin_modules where UserID=$Uid";
	 $strAddQuery ="SELECT M.*,P.* FROM admin_modules as M join permission as P on M.ModuleID=P.ModuleID  WHERE    Parent='".$Parent."' and  P.UserID='".$Uid."' Order by M.OrderBy  asc"; 
 //echo  $strAddQuery;exit;
	 return $this->query($strAddQuery, 1);
	 
	  }
	 }	
	 function GetHeaderSubmenuadmin($Uid,$Parent)
	 {
	
    if(!empty($Uid))
     {
	
	 $strAddQuery ="SELECT * FROM admin_modules   WHERE  Parent='".$Parent."'  Order by OrderBy  asc"; 
...
function GetHeaderParent($pageurml)
    ...
    if(!empty($pageurml))
	 ...
	 $strAddQuery ="SELECT * FROM admin_modules   WHERE  link='".$pageurml."'  Order by OrderBy  asc"; 
    //echo   $strAddQuery ;
	 return $this->query($strAddQuery, 1);
   ...
   function GetrightmenuByuser($AdminID)
	 {
	 	 if(!empty($AdminID))
     {
    $strAddQuery ="SELECT M.*,P.* FROM admin_modules as M join permission as P on M.ModuleID=P.ModuleID  WHERE   M.Parent='2' and  P.UserID='".$AdminID."' order by M.ModuleID asc, M.OrderBy  asc "; 
	 ...
	 function GetHdMenu($Uid,$Parent)
	 {
	    ...
	    if(!empty($Uid))
		 ...
		 $strSqlQuery ="SELECT M.*,P.* FROM admin_modules as M join permission as P on M.ModuleID=P.ModuleID  WHERE  M.Parent!='2' and  Parent='".$Parent."' and  P.UserID='".$Uid."' ".$strAddQuery." Order by M.OrderBy  asc"; 
	 ...
	 function GetHdMenuAdmin($Parent)
	 {
		...
		$strSqlQuery ="SELECT M.* FROM admin_modules as M  WHERE M.Parent!='2' and  Parent='".$Parent."'   Order by M.OrderBy  asc"; 
	 ...
	 function GetPageByLink($Link)
	 {
	    ...
	    if(!empty($Link))
	     {
		 $strAddQuery ="SELECT M.* FROM admin_modules as M WHERE  M.Link='".$Link."' "; 
 		 $arryRow = $this->query($strAddQuery, 1);
		if(!empty($arryRow[0]["Parent"])){			 
			 $strSql ="SELECT M.* FROM admin_modules as M WHERE  M.ModuleID='".$arryRow[0]["Parent"]."' "; 
 			 $arryRow2 = $this->query($strSql, 1);

			$arryRow2[0]["ModuleParent"] = $arryRow[0]["Parent"];
 	...
 	function GetSIteMenuByLink($Uid,$Link)
	 {
	
	    if(!empty($Uid))
	     {
		 $strAddQuery ="SELECT M.*,P.* FROM admin_modules as M join permission as P on M.ModuleID=P.ModuleID  WHERE  P.UserID='".$Uid."' and  M.Link='".$Link."' Order by OrderBy  asc"; 
 		//echo  $strAddQuery;
	 return $this->query($strAddQuery, 1);
...
function GetrightmenuByadmin($AdminID)
	 {
	 	 if(!empty($AdminID))
     {
    $strAddQuery ="SELECT * FROM admin_modules   WHERE    Parent='2' Order by ModuleID asc, OrderBy  asc "; 
	...
	$strSQLQuery ="select m.*,p.UserID,p.id as CheckedID from company m left outer join permission_cmp p on (m.CmpID=p.CmpID ".$OuterADD.") where 1  Order by CompanyName  asc";
	 ...
	 $strAddQuery ="SELECT P.*,CP.* FROM company as P left outer join permission_cmp as CP on P.CmpID=CP.CmpID WHERE UserID ='".$Uid."'  Order by CompanyName  asc"; 
   // echo $strAddQuery;exit;
	 return $this->query($strAddQuery, 1);
    ...
    function UpdateRolePermissionCompany($arryDetails)
		{
	
			global $Config;	
			extract($arryDetails);
	    		
			$sql = "delete from permission_cmp where UserID ='".$Uid."'";			
			$rs = $this->query($sql,0);
			
				foreach ($inercheck as $CmpID)
					{
						...
						if($CmpID>0)
						{
							...
							$sql = "insert ignore into permission_cmp(UserID,CmpID) values('".$Uid."', '".$CmpID."')";
