	...
	function Customer(){
		...
		$this->dbClass();
			...
			if($SearchKey=='active' && ($SortBy=='c1.Status' || $SortBy=='') ){
				...
				$strAddQuery .= " and g.GroupName like '%".$SearchKey."%'";
			}
                        else{
				$strAddQuery .= (!empty($SearchKey))?(" and (c1.FirstName like '".$SearchKey."%' or c1.LastName like '%".$SearchKey."%' or c1.Email like '%".$SearchKey."%' or  g.GroupName like '%".$SearchKey."%') "):("");
			}
			...
			if($Config['GetNumRecords']==1){
				...
				if($Config['RecordsPerPage']>0){
					$strAddQuery .= " limit ".$Config['StartPage'].",".$Config['RecordsPerPage'];
                     ...
                     $SqlCustomer = "select ".$Columns." from e_customers c1 left outer join e_customer_group g on c1.GroupID=g.GroupID ".$strAddQuery;
                ...
                function getCustomerGroupId($str)
                {
                    ...
                    if(!empty($arrayGroup[0]['GroupID'])) return $arrayGroup[0]['GroupID'];
                ...
                function ForgotPassword($Email)
		{
			...
			function deleteCustomer($Cid)
			{
 			...
 			if($_SESSION['companyType']=='dealer' && $rs[0]['Status']=='No' && $rs[0]['Password']==''){

				$this->activateDealer($rs[0]);
				}
             ...
             /****************************************Subscriber Functions Started********************************************/   
               function checkSubcribeEmail($email)
                ...
                function deleteSubcriber($id)
				{
				 	...
				 	// added by karishma 12 oct 2015
					$SqlCustomer = "SELECT Email FROM e_emails where EmailId = '".$id."'"; 
				    $arrayEmail = $this->query($SqlCustomer, 1);
				    $email = $arrayEmail[0]['Email'];
				    
							$SqlCustomer = "UPDATE e_customers SET Newsletters='No'  WHERE Email = '".addslashes($email)."'"; 
				    $this->query($SqlCustomer,0);
                ...
                function changeSubcriberStatus($id)
                {
                        ...
                        if(sizeof($rs))
                        {
							    ...
							    $email = $arrayEmail[0]['Email'];
										...
										$SqlCustomer = "UPDATE e_customers SET Newsletters='$Status'  WHERE Email = '".addslashes($email)."'"; 
							    $this->query($SqlCustomer,0);
            ...
            /******************************************Shipping Address Functions Started************************************************************/    
              ...
              function getCustomerGroupName($customerGroupID) {
                ...
                $arrayGroup = $this->query($SqlCustomer, 1);                
                if(!empty($arrayGroup[0]['GroupName']))return $arrayGroup[0]['GroupName'];
             }
             
               function exportCustomers()
               {
                     $SqlCustomer = "SELECT c1.*, g.GroupName FROM e_customers c1 left outer join e_customer_group g on c1.GroupID=g.GroupID WHERE c1.Login <> 'ExpressCheckoutUser' ORDER BY c1.Cid ASC";
...
/*****************Added by karishma****************************/
			function getCustomerGroupsetting()
                {
                    $sqlSettings = "SELECT * from e_customer_group where GroupID!=1";
                    return $this->query($sqlSettings, 1);
                }
              ...
              /********Added by karishma for dealer on 6 Oct 2016********/
	function addCompanyUser($Cid)
	{
		$_SESSION['CmpID'];
		$strSQLQuery="select * from e_customers where Cid = '".$Cid."'";
		$rs = $this->query($strSQLQuery);
		if($_SESSION['companyType']=='dealer'){
			$strSQLQuery="update e_customers set custType='posdealer' where Cid = '".$Cid."'";
			$this->query($strSQLQuery,0);
			$SqlCustomer = "INSERT INTO erp.company_user_pos SET fname = '".addslashes($rs[0]['FirstName'])."', 
			lname = '".addslashes($rs[0]['LastName'])."',user_name = '".addslashes($rs[0]['Email'])."',
			password = '".addslashes($rs[0]['Password'])."',user_type = 'vendorpos', ref_id = '".addslashes($Cid)."', 
			comId = '".$_SESSION['CmpID']."', date = '".date('Y-m-d H:i:s')."' , isvendor_admin = 'Yes',
			address1 = '".addslashes($rs[0]['Address1'])."',address2 = '".addslashes($rs[0]['Address2'])."',
			city = '".addslashes($rs[0]['City'])."',state = '".addslashes($rs[0]['State'])."',
			country = '".addslashes($rs[0]['Country'])."'";
			#echo "=>".$SqlCustomer;exit;
			$this->query($SqlCustomer, 1);
	...
	function GetReqWalletAmtMsg($type){
		$strSQLQuery="select * from e_dealer_message where msgType = '".addslashes($type)."' order by id asc limit 0,5";
	...
	function activateDealer($arryDetails){
		global $Config;
		$Password=md5($arryDetails['Phone']);
		$strSQLQuery="update e_customers set Password='".addslashes($Password)."' where Cid = '".$arryDetails['Cid']."'";

		$this->query($strSQLQuery,0);
		$arryDetails['Password']=$Password;
		$objConfig=new admin();
		...
		$main_state_id = $arryDetails['State'];
		$OtherState = $arryDetails['OtherState'];
		$main_city_id = $arryDetails['City'];
		$OtherCity = $arryDetails['OtherCity'];
		...
		$this->customerRegistrationEmail($arryDetails,$CountryName,$StateName,$CityName);
	}

	/********End by karishma for dealer on 6 Oct 2016********/ 
	
	
	 function getCustomersCurrentYear($id=0,$Status,$SearchKey,$SortBy,$AscDesc)
                {
                        $strAddQuery = '';
			$SearchKey   = strtolower(trim($SearchKey));
			$strAddQuery .= (!empty($id))?(" where c1.Cid=".$id):(" where 1 and Login <> 'ExpressCheckoutUser'");
			...
			$strAddQuery .= ($Status>0)?(" and c1.Status=".$Status.""):("");
			$strAddQuery .= " and (DATE(CreatedDate) BETWEEN '".date('Y').'-01-01'."' AND '".date('Y').'-12-31'."')";
