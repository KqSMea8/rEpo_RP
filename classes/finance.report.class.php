...
function CreateAPInvoiceForVendorSpiff($arryOrder){
	extract($arryOrder);	
	global $Config; 
	/*********************/
	$objConfigure=new configure();
	...
	$strSQLQuery = "SELECT s.*,c.CommPaidOn from s_order_spiff s inner join s_order o on (s.OrderID=o.OrderID and o.Module='Order' and o.SaleID='".$SaleID."') inner join p_supplier p on (s.SuppCode=p.SuppCode and s.SuppCode!='') inner join h_commission c on (c.SuppID=p.SuppID and c.SuppID>0 and c.CommType='Spiff' and c.SpiffOn='1')  where s.SuppCode!='' and s.Amount>0 order by s.ID asc";
	$arrySpiff = $this->query($strSQLQuery, 1);
 

    if(!empty($arrySpiff) && !empty($CommissionFeeAccount)){ 
        foreach($arrySpiff as $key=>$values){ 
		$TotalCommission=0;
  		if($values['CommPaidOn']=="Paid"){
			if($InvoicePaid=='Paid' || $InvoicePaid=='Part Paid'){
				$TotalCommission=$values['Amount'];
			}
	...
	if($TotalCommission>0){
		...
		$strSQLQuery2 = "SELECT p.InvoiceID as PoInvoiceID from p_order p inner join f_expense e on (p.ExpenseID=e.ExpenseID and p.ExpenseID>0) where p.PurchaseID='".$InvoiceID."'  and p.Module='Invoice' and p.InvoiceEntry='2' and p.SuppCode='".$values['SuppCode']."' and p.TotalAmount='".$TotalAmount."' and p.ArInvoiceID='".$InvoiceID."'  and e.ExpenseTypeID='".$CommissionFeeAccount."' and p.InvoiceComment='Vendor Spiff'   order by p.OrderID asc ";
	...
	if($arryRow[0]['InvoiceEntry']==2 && !empty($TotalAmount)  && $IncomeID>0){ //Start GL Account
		...
		if($PID>0){
		   ...
		   if($TransactionID>0 && !empty($arryRow[0]['VendorSalesPerson'])  && ($arryRow[0]['InvoiceEntry']=='0' || $arryRow[0]['InvoiceEntry']=='1') ){
			...
			/***Ap Invoice for Vendor Spiff***/
			if($Config['spiffDis']=="1"  && $arryRow[0]['Spiff']=='Yes' && !empty($arryRow[0]['SaleID'])  && $arryRow[0]['InvoiceEntry']=='0' ){
				$arryRow[0]['PostToGLDate'] = $PostToGLDate;
				$this->CreateAPInvoiceForVendorSpiff($arryRow[0]);      
			}  
			/**************************************/
...
function UpdateRefNoForActualFreight(){
	...
	/***Ap Invoice for Vendor Spiff***/
	if($TransactionID>0 && $Config['spiffDis']=="1"  && !empty($CommissionFeeAccount) ){
		$sql = "select o.OrderID, o.InvoiceID, o.ConversionRate, o.SaleID,  o.InvoiceEntry, o.Spiff,o.InvoicePaid from s_order o inner join f_transaction_data t on (t.OrderID=o.OrderID and t.CustID=o.CustID and t.OrderID>'0' ) WHERE t.TransactionID = '".$TransactionID."' and t.PaymentType='Invoice' and t.Module='AR' and t.InvoiceID!='' and o.Spiff='Yes' and o.Module='Invoice' and o.SaleID!='' and o.InvoiceEntry in ('0')  ";
            	$arryInvData = $this->query($sql, 1);
            	//pr($arryInvData,1);
		foreach($arryInvData as $key=>$values){			
			$values['PostToGLDate'] = $Date;
			$values['TransactionID'] = $TransactionID;
			$this->CreateAPInvoiceForVendorSpiff($values);  
		}  
	}  
	/**************************************/
