...
//$Columns = "  c1.*,IF(c1.CustomerType = 'Company' and c1.Company!='', c1.Company, c1.FullName) as CustomerName  ";
	...
	//SqlCustomer = "select ".$Columns." from s_customers c1 ".$strAddQuery;
		...
		function  ListCustomer($arryDetails)
		{
			...
			$strSQLQuery = "SELECT c.*,IF(c.CustomerType = 'Company' and c.Company!='', c.Company, c.FullName) as FullName, ab.CountryName ,ab.StateName,ab.CityName from s_customers c left outer join s_address_book ab ON (c.Cid = ab.CustID and ab.AddType = 'billing' ) ".$strAddQuery."";
...
//By Chetan 6July//
                        $unsetArray = array("AddID","CustID","Title","Department","LeadSource","Submit","AssignTo","Reference","DoNotCall","NotifyOwner",
                            "EmailOptOut","ajax_state_id","ajax_city_id","Description","Status",'main_state_id','main_city_id','state_id','city_id','SubmitContact','CurrentDivision');
...
//LEFT OUTER JOIN h_employee as e on FIND_IN_SET(e.EmpID, c.SalesID)
				...
				sp.Mobile as sMobile,sp.Landline as sLandline,sp.Fax as sFax, sp.Email as sEmail,  c.SalesId as SalesPersonID ,c.SalesPerson as SalesPerson,c.SalesPersonType as SalesPersonType FROM s_customers as c 
...
$this->query($strSQLQuery, 0);
 ...
 /********* By Karishma for MultiStore 22 Dec 2015******/
	...
	function GetCard($CardID,$CustID,$DefaultCard){
		...
		//if($CardID>0){echo $strSQLQuery;exit;}
		return $this->query($strSQLQuery, 1);
	}
...
function ListCustShipAccount($Type,$CustID){
		if(!empty($Type)){
