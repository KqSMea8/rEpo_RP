...
function GetItemBySku($Sku) {
	...
	if(!empty($key)) $SearchKey = strtolower(trim($key));
        ...
        $strAddQuery .= (!empty($InData)) ? (" and p1.CategoryID IN (" . $InData . ")") : ("");
...
if($Config['GetNumRecords']==1){
 ...
 $Join = " left outer join  inv_stock_adjustment ast on ast.adjID=a.adjID ";
        $strAddQuery .="group by ast.adjID";
        $strAddQuery .= (!empty($SortBy)) ? (" order by " . $SortBy . " ") : (" order by a.adjID ");
        $strAddQuery .= (!empty($AscDesc)) ? ($AscDesc) : (" Desc");	
...
$strAddQuery .= (!empty($WID))?(" and `warehouse`='".$WID."' "):(" and `warehouse`='1' ");
...
$sql = "select SUM(i.qty-i.qty_invoiced) as sales_qty,SUM(i.qty_invoiced) as qtyInvoice,i.Condition FROM s_order as s	inner join s_order_item as i on s.OrderID=i.OrderID	where s.Module='Order'	and i.sku='" . $sku . "'";
	...
	$strAddQuery .= ($Interval == 'd') ? (" AND s_order.OrderDate >= DATE_SUB(NOW(), INTERVAL ".$Num." DAY)") : ("");
        ...
        $strAddQuery .= (" and s_order.CustomerName!='' ");
	if($allItemid!='')
	{
...
function GetMainCategory($CategoryID){
		//$Sku = strtolower(trim($Sku));
if($CategoryID!=''){
		  $CatSql = "select * from inv_categories where CategoryID = '".$CategoryID."' and ParentID!='".$CategoryID."' and Status=1 ";
...
function GetMainCatFor(){
		//$Sku = strtolower(trim($Sku));

		  $CatSql = "select * from inv_categories where ParentID = '0' and Status=1 ";
		 $arryRow = $this->query($CatSql, 1);
...
function getTotQtBySerial($Sku,$Condi,$WID)   //added 3rd parameter by chetan on 30Mar2017//
{
	...
	$strAddQuery.= (!empty($WID))?(" and warehouse = '".addslashes($WID)."' "):("and warehouse = '1' ");
	  $strSQLQuery = "select COUNT(serialID) as condition_qty from inv_serial_item where UsedSerial=0 and Status=1 and serialNumber!='' and Sku ='".$Sku."' ".$strAddQuery."  ";
...
function BinLocationByWarehouse($WID,$binid=0){
...
function UpdateTotQtBySerial53534($Sku,$Condition,$WID,$Qty){
...
function UpdateTotQtBySerial($ItemID,$Sku,$Condition,$WID,$Qty){

if(empty($WID)) $WID=1; 

   $strAddQuery = (!empty($WID))?(" and WID = '".addslashes($WID)."' "):(" ");
   
    $strQuery = "select * from inv_item_quanity_condition where WID='".$WID."' and `condition`='".$Condition."' and Sku = '".$Sku."' and itemID ='".$ItemID."' and SalePrice>0 ";
    
    $row = $this->query($strQuery, 1);
    #pr($row);
   
   if(!empty($row)){  $insrt =",SalePrice,LastPrice,pricetype,fprice,prpercent,qtyfrom,qtyto "; 
   $val = ",'".$row[0]['SalePrice']."','".$row[0]['LastPrice']."','".$row[0]['pricetype']."','".$row[0]['fprice']."','".$row[0]['prpercent']."','".$row[0]['qtyfrom']."','".$row[0]['qtyto']."'";
   }
   
   
   
   $strDelQuery = "delete from inv_item_quanity_condition where WID='".$WID."' and `condition`='".$Condition."' and Sku = '".$Sku."' and itemID ='".$ItemID."' ";
   
    $strSQLQuery = "insert into inv_item_quanity_condition(ItemID,WID,condition_qty,`condition`,Sku ".$insrt.") 
						values ('" . addslashes($ItemID) . "','".$WID."','".$Qty."','".addslashes($Condition)."','" . addslashes($Sku) . "'".$val.")";

   $this->query($strDelQuery, 1);
...
function GetSerialValuationReport($FilterBy,$FromDate,$ToDate,$Month,$Year,$valuationType,$Sku=''){
...
$strAddQuery .= (!empty($key['binid']))?(" and `binid`='".$key['binid']."' "):("");
$strAddQuery .= (!empty($Sku))?(" and Sku='".$Sku."' "):("");
  $Sql2 = "select count(Sku) as srQt ,SUM(UnitCost) as conAmt,Sku,ReceiptDate  from inv_serial_item where 1 and Status=1 and UsedSerial=0 and serialNumber!=''   ".$strAddQuery." group by Sku,`Condition`";
...
function categoryChild($id) {
    $Sql = "SELECT CategoryID,Name FROM inv_categories WHERE ParentID = '".$id."'";
    $rs = $this->query($Sql, 1);
   ...
   (!isset($category))?($category=""):("");
    ...
    if(count($rs) > 0) {
        ...
        foreach($rs as $values) {
           ...
           $category .=  $values['CategoryID'].$prefix ;
         $category .= $this->categoryChild($values['CategoryID'], $prefix . '-');
        }
    }
...
function GetSerialqtyAvg($FilterBy,$FromDate,$ToDate,$Month,$Year,$valuationType,$Sku='',$CategoryID=''){

$strAddQuery = "";

	if($FilterBy=='Year'){
			$strAddQuery .= " and YEAR(s.ReceiptDate)='".$Year."'";
		}else if($FilterBy=='Month'){
			$strAddQuery .= " and MONTH(s.ReceiptDate)='".$Month."' and YEAR(s.ReceiptDate)='".$Year."'";
		}else{
			//$strAddQuery .= (!empty($FromDate))?(" and t.TransactionDate>='".$FromDate."'"):("");
			$strAddQuery .= (!empty($ToDate))?(" and (s.ReceiptDate <='".$ToDate."'  )"):("  ");
		}
...
$strAddQuery .= (!empty($CategoryID))?(" and i.CategoryID IN(".$CategoryID." )"):("  ");
$strAddQuery .= (!empty($Sku))?(" and s.Sku='".$Sku."' "):("");
  $Sql2 = "select count(s.serialID) as srQt ,SUM(s.UnitCost) as conAmt,s.Sku,s.`Condition`,i.CategoryID from inv_serial_item s inner join inv_items i on i.Sku =s.Sku where 1 and s.Status=1 and s.serialNumber!='' and s.UsedSerial=0   ".$strAddQuery." group by s.Sku,s.`Condition` ";
...
function getserialqtybycat($Sku,$Condition){

$strAddQuery .= (!empty($Sku))?(" and s.Sku='".$Sku."' "):("");
$strAddQuery .= (!empty($Condition))?(" and s.`Condition`='".$Condition."' "):("");

$Sql2 = "select count(s.serialID) as srQt ,SUM(s.UnitCost) as conAmt from inv_serial_item s  where 1 and s.Status=1 and s.serialNumber!='' and s.UsedSerial=0   ".$strAddQuery."  ";
$rs2 =  $this->query($Sql2, 1);
...
function GetItemByCategoryID($CategoryID,$ItemID=0) {
$strAddQuery = ' where 1 ' ;
        $strAddQuery .= ($CategoryID > 0) ? (" and CategoryID='" . $CategoryID."'") : (" and CategoryID>0 ");
         $strAddQuery .= ($ItemID > 0) ? (" and ItemID='" . $ItemID."'") : (" ");
        $strSQLQuery = "select ItemID,Sku,description,OverrideItem,OverspiffAmt,spiffType,spiffAmt,Spiff from inv_items  " . $strAddQuery ;
      # echo $strSQLQuery; exit;
        return $this->query($strSQLQuery, 1);
    }


    function UpdateOverrideItem($arryDetails){
            extract($arryDetails);
...
$OverspiffAmt =0.00;
            ...
            //if($arryDetails['OverrideItem']==1 && $_POST['Spiff']==1){
            
            if(!empty($dd)){ $adcond = " and ItemID NOT IN (" . $dd. ") ";}else{ $adcond = " ";}
            $strSQLQuery1 = "update inv_items set Spiff='" . addslashes($Spiff) . "', spiffType='" . addslashes($spiffType) . "', spiffAmt='" . $spiffAmt . "',OverrideItem='0',OverspiffAmt='".$OverspiffAmt."' where CategoryID = '" . $CategoryID. "'  $adcond  "; 
             #echo $strSQLQuery1; exit;
            $this->query($strSQLQuery1, 0);
            
           
           if(!empty($arryDetails['ItemID'])){
             $strSQLQuery = "update inv_items set Spiff='" . addslashes($Spiff) . "', spiffType='" . addslashes($spiffType) . "', spiffAmt='" . $spiffAmt . "',OverrideItem='" . addslashes($OverrideItem) . "',OverspiffAmt='" . addslashes($OverspiffAmt) . "' where ItemID IN (" . $dd. ")"; 
           // }else{

             //$strSQLQuery = "update inv_items set Spiff='" . addslashes($Spiff) . "', spiffType='" . addslashes($spiffType) . "', spiffAmt='" . $spiffAmt . "' where CategoryID = '" . $CategoryID. "'"; 
...
function CheckSerialAvailable($Sku,$Condition){

$strAddQuery .= (!empty($Sku))?(" and s.Sku='".$Sku."' "):("");
$strAddQuery .= (!empty($Condition))?(" and s.`Condition`='".$Condition."' "):("");

$Sql2 = "select count(s.serialID) as srQt ,SUM(s.UnitCost) as conAmt from inv_serial_item s  where 1 and  s.serialNumber!=''    ".$strAddQuery."  ";
$rs2 =  $this->query($Sql2, 1);
...
return $rs2;
 ...
 function GetDuplicatSerial(){
 
 $Sql = "select n1.* FROM inv_serial_item n1, inv_serial_item n2 WHERE n1.serialID < n2.serialID AND n1.serialNumber = n2.serialNumber  and n1.`Condition` =n2.`Condition` and n1.Sku=n2.Sku and n1.Status=1 and n2.Status=1 and n1.UsedSerial=0 and n2.UsedSerial=0";
