...
#echo $strSQLQuery; exit;
	...
	$strAddQuery='';
        ...
        $strAddQuery .= (!empty($ReceiptID)) ? (" and o.ReceiptNo='" . $ReceiptID . "'") : ("");
        $strAddQuery .= (!empty($Module)) ? (" and o.Module='" . $Module . "'") : ("");
        $strSQLQuery = "select o.*,p.ConversionRate, p.Restocking as RmaRestocking, p.Restocking_fee as RestockingFee,p.tax_auths,p.Currency from w_receiptpo o  left outer join p_order p on o.OrderID = p.OrderID  where 1" . $strAddQuery . " order by o.OrderID desc";
        return $this->query($strSQLQuery, 1);
    }
	...
	$arryPurchase[0]["ReceiptNo"] = $ReceiptNo;
            ...
            if (!empty($id) && $arryDetails['qty' . $i] > 0) {
                ...
                if (!empty($receipt_id)) {
                    ...
                    if ($id > 0) {
                       ...
                       $sql = "insert into w_receiptpo_item(ReceiptID,OrderID, item_id, ref_id, sku, description, on_hand_qty, qty, qty_received,qty_invoiced,qty_returned,qty_receipt , price, tax_id, tax, amount, Taxable, req_item,SerialNumbers,`Condition`,Type,Action,Reason,fee) values('" . $receipt_id . "','" . $arryItem[$Count]["OrderID"] . "', '" . $arryItem[$Count]["item_id"] . "' , '" . $arryDetails['id' . $i] . "', '" . $arryItem[$Count]["sku"] . "', '" . $arryItem[$Count]["description"] . "', '" . $arryItem[$Count]["on_hand_qty"] . "', '" . $arryItem[$Count]["qty"] . "', '" . $SQ_RECT . "', '" . addslashes($arryDetails['received_qty' . $i]) . "','" . $arryItem[$Count]["qty_returned"] . "','" . $qty_receipt . "', '" . $arryItem[$Count]["price"] . "', '" . $arryItem[$Count]["tax_id"] . "', '" . $arryItem[$Count]["tax"] . "', '" . $arryDetails['amount' . $i] . "', '" . $arryItem[$Count]["Taxable"] . "', '" . addslashes($arryItem[$Count]["req_item"]) . "','" . $SerialNumbers . "' ,'" . addslashes($arryDetails['Condition' . $i]) . "', '" . $Type . "' , '" . $Action . "' , '" . $Reason . "', '" . addslashes($arryDetails['restocking_fee' . $i]) . "')";
                     // echo "=>".$sql;exit;
                    }
                } else {
                    $sql = "insert into w_receiptpo_item(ReceiptID,OrderID, item_id, sku, description, on_hand_qty, qty, qty_received,qty_invoiced,qty_returned,qty_receipt , price, tax_id, tax, amount, Taxable, req_item,SerialNumbers,`Condition`,Type,Action, Reason, fee) values('" . $receipt_id . "','" . $arryItem[$Count]["OrderID"] . "', '" . $arryItem[$Count]["item_id"] . "' ,  '" . $arryItem[$Count]["sku"] . "', '" . $arryItem[$Count]["description"] . "', '" . $arryItem[$Count]["on_hand_qty"] . "', '" . $arryItem[$Count]["qty"] . "', '" . $SQ_RECT . "', '" . addslashes($arryDetails['received_qty' . $i]) . "','" . $arryItem[$Count]["qty_returned"] . "','" . $qty_receipt . "', '" . $arryItem[$Count]["price"] . "', '" . $arryItem[$Count]["tax_id"] . "', '" . $arryItem[$Count]["tax"] . "', '" . $arryDetails['amount' . $i] . "', '" . $arryItem[$Count]["Taxable"] . "', '" . addslashes($arryItem[$Count]["req_item"]) . "','" . $SerialNumbers . "' , ,'" . addslashes($arryDetails['Condition' . $i]) . "', '" . $Type . "' , '" . $Action . "' , '" . $Reason . "' , '" . addslashes($arryDetails['restocking_fee' . $i]) . "')";
                   // echo "=>=>".$sql;exit;
                }
                $this->query($sql, 0);
    ...
    function AddReceiptOrderPO($arryDetails) {
       ...
       $strSQLQuery = "INSERT INTO w_receiptpo SET Module = 'Receipt',ModuleType ='Purchase', 
        OrderID='" . $OrderID . "', ReceiptNo ='" . $ReceiptNo	 . "', 
PurchaseID ='" . $PurchaseID . "', 
        ...
        $sql = "UPDATE w_receiptpo SET ReceiptNo='" . $ReceiptNo . "' WHERE ReceiptID='" . $ReceiptID . "'";
        $this->query($sql, 0);*/

	$objConfigure = new configure();
	$objConfigure->UpdateModuleAutoReceiptID('w_receiptpo','Receipt',$ReceiptID,'ReceiptID',$ReceiptNo);
  ...
  function AddCreditPOFromReceipt($ReceiptID) {    
	...
	$RestockingFee=0; 
		...
		if($arryItem[$Count]['Type']=="C"){
			...
			$RestockingFee+= $arryItem[$Count]['fee'];	
	...
	if(sizeof($arryCreditItem)>0){
		$arryPurchase[0]['RestockingFee'] = $RestockingFee;
		$arryPurchase[0]["TotalAmount"] = $TotalCreditItem;
		$CreditID = $this->AddWhPurchaseCreditRMA($arryPurchase[0]);
		foreach($arryCreditItem as $arryCrItem){		    
   	  	   $sqlCrd = "insert into p_order_item(OrderID, item_id, sku, description, on_hand_qty, qty, price, amount, `Condition`, DesComment, SerialNumbers, fee) values('".$CreditID."', '".$arryCrItem['item_id']."', '".addslashes($arryCrItem['sku'])."', '".addslashes($arryCrItem['description'])."', '".$arryCrItem['on_hand_qty']."', '".$arryCrItem['qty']."', '".$arryCrItem['price']."',  '".$arryCrItem['amount']."', '".addslashes($arryCrItem['Condition'])."','".addslashes($arryCrItem['DesComment'])."','".addslashes($arryCrItem['SerialNumbers'])."', '".addslashes($arryCrItem['fee'])."')";					
   	  	   $this->query($sqlCrd, 0);
						
	    	}
		$_SESSION["WPR_CreditID"] = $CreditID;	  
	...
	if(sizeof($arryReplacementItem)>0){ 
		...
		foreach($arryReplacementItem as $arryRpItem){				
			 $sqlPO = "insert into p_order_item(OrderID, item_id, sku, description, on_hand_qty, qty,  price,  amount, `Condition`, DesComment, SerialNumbers) values('".$PurchaseID."', '".$arryRpItem['item_id']."' , '".addslashes($arryRpItem['sku'])."', '".addslashes($arryRpItem['description'])."', '".$arryRpItem['on_hand_qty']."', '".$arryRpItem['qty']."', '".$arryRpItem['price']."', '".$arryRpItem['amount']."', '".addslashes($arryRpItem['Condition'])."','".addslashes($arryRpItem['DesComment'])."', '".addslashes($arryRpItem['SerialNumbers'])."')";		       	
			 $this->query($sqlPO, 0);	
...
//echo "</pre>";print_r($arryDetails);die;
	...
	if($Config['GetNumRecords']==1){
		...
		if($Config['RecordsPerPage']>0){
         ...
         $strSQLQuery = "select ".$Columns." from w_receiptpo w  left outer join p_supplier s on w.SuppCode = s.SuppCode left outer join p_order o on w.OrderID = o.OrderID left outer join h_employee em on (w.AdminID=em.EmpID and w.AdminType='employee') " . $strAddQuery;
...
$selectsql="select * from w_receiptpo_item where ReceiptID='".mysql_real_escape_string($ReceiptID)."'";
				$rsitem = $this->query($selectsql);

//print_r($rsitem);die;
$NumLine = count($rsitem);
$responce=array();
  $objItem=new items();
	for($r=0;$r<$NumLine;$r++){

/************************************/
$checkProduct=$objItem->checkItemSku($rsitem[$r]["sku"]);

		//By Chetan 9sep// 
		if(empty($checkProduct))
		{
		$arryAlias = $objItem->checkItemAliasSku($rsitem[$r]['sku']);
			if(count($arryAlias))
			{

					$rsitem[$r]['sku'] = $arryAlias[0]['sku'];
					$rsitem[$r]['item_id'] = $arryAlias[0]['ItemID'];
			}
		}else{

	$rsitem[$r]['sku']  = 	$rsitem[$r]['sku'];
}
...
/***********************************/
		...
		$allserials = explode(',',$rsitem[$r]['SerialNumbers']);
		#print_r($allserials);die;
if(empty($rsitem[$r]['WID'])){ $rsitem[$r]['WID']=1;}
		foreach($allserials as $serial){
				if(1!=1){
					$responce['errors']['items'][$rsitem[$r]['SerialNumbers']][]=$serial;
				}else{
				
					 $sqlUpdate ="Update inv_serial_item set UsedSerial='1' Where serialNumber='".$serial."' AND Sku='".$rsitem[$r]['sku']."' AND `Condition`='".$rsitem[$r]['Condition']."' ";
					$this->query($sqlUpdate, 0);
					 
					 $sqlyuUpdate ="Update inv_item_quanity_condition SET condition_qty=condition_qty-1 WHERE Sku='".$rsitem[$r]['sku']."'  AND `condition`='".$rsitem[$r]['Condition']."' "; 
					 $this->query($sqlyuUpdate, 0);
				}
		}
	...
	/*for ($r=0;$r<=$NumLine; $r++){
			$count = $r+1;
			if($rsitem[$r]['SerialNumbers']!=''){
						$qty=$rsitem[$r]['qty_receipt';
						$SerialValue=$rsitem[$r]['SerialNumbers'];
						...
						$_POST['serial_value'.$count]=implode(',',$remainserial);
			
						}else{
								$_POST['serial_value'.$count]=implode(',',$allSerialArray);
				...
				if(sizeof($rs))
				{
				 ...
				 if($rs[0]['ReceiptStatus'] == "Completed"){
					...
					$sqltt="update w_receiptpo set ReceiptStatus='$Status' where ReceiptID='".mysql_real_escape_string($ReceiptID)."'";
					$this->query($sqltt,0);					
				}	
			}
...
function AddWhPurchaseCreditRMA($arryDetails)
      ...
      function AddWhPurchaseReplacementRMA($arryDetails)
		{  
			...
			$strSQLQuery = "insert into p_order(Module, OrderType, OrderDate,  PurchaseID, QuoteID, InvoiceID, CreditID, wCode, Approved, Status, DropShip, DeliveryDate, ClosedDate, Comment, SuppCode, SuppCompany, SuppContact, Address, City, State, Country, ZipCode, Currency, SuppCurrency, Mobile, Landline, Fax, Email, wName, wContact, wAddress, wCity, wState, wCountry, wZipCode, wMobile, wLandline, wEmail, TotalAmount, Freight, CreatedBy, AdminID, AdminType, PostedDate, UpdatedDate, ReceivedDate, ExpiryDate, InvoicePaid, InvoiceComment, PaymentMethod, ShippingMethod, PaymentTerm, AssignedEmpID, AssignedEmp, Taxable, SaleID, taxAmnt , tax_auths, TaxRate,CustCode,ReturnID,ConversionRate,IPAddress,CreatedDate) values('".$Remodule."', '".$OrderType."', '".$OrderDateRep."',  '".$PurchaseID."', '".$QuoteID."', '".$InvoiceID."', '".$CreditID."', '".$wCode."', '".$Approved."','".$Status."', '".$DropShip."', '".$DeliveryDate."', '".$ClosedDate."', '".addslashes(strip_tags($Comment))."', '".addslashes($SuppCode)."', '".addslashes($SuppCompany)."', '".addslashes($SuppContact)."', '".addslashes($Address)."' ,  '".addslashes($City)."', '".addslashes($State)."', '".addslashes($Country)."', '".addslashes($ZipCode)."',  '".$Currency."', '".addslashes($SuppCurrency)."', '".addslashes($Mobile)."', '".addslashes($Landline)."', '".addslashes($Fax)."', '".addslashes($Email)."' , '".addslashes($wName)."', '".addslashes($wContact)."', '".addslashes($wAddress)."' ,  '".addslashes($wCity)."', '".addslashes($wState)."', '".addslashes($wCountry)."', '".addslashes($wZipCode)."', '".addslashes($wMobile)."', '".addslashes($wLandline)."',  '".addslashes($wEmail)."', '".addslashes($TotalAmount)."', '0', '".addslashes($_SESSION['UserName'])."', '".$_SESSION['AdminID']."', '".$_SESSION['AdminType']."', '".$Config['TodayDate']."', '".$Config['TodayDate']."', '".$ReceivedDate."', '".$ExpiryDate."', '".$InvoicePaid."', '".addslashes(strip_tags($InvoiceComment))."', '".addslashes($PaymentMethod)."', '".addslashes($ShippingMethod)."', '".addslashes($PaymentTerm)."', '".addslashes($EmpID)."', '".addslashes($EmpName)."', '".addslashes($Taxable)."', '".addslashes($SaleID)."', '".addslashes($taxAmnt)."', '".addslashes($tax_auths)."', '".addslashes($MainTaxRate)."','".$CustCode."','".$ReturnID."','".$ConversionRate."','".$IPAddress."', '".$Config['TodayDate']."' )";
            //echo $strSQLQuery;exit;
	...
	/**************************************/
	function APInvoiceForActualFreightPO($ReceiptID){
		global $Config;	
		$objConfig =new admin(); 
		$objReport = new report();	 
	 	if(!empty($ReceiptID)){

                     $strSQLQuery = "select o.* from p_order o  inner join w_receiptpo w on o.OrderID = w.OrderID   where w.ReceiptID='" . $ReceiptID."'";
                        $arryRow = $this->query($strSQLQuery, 1);	
			...
			if(!empty($OrderID)){ 				   
				$arryShippInfo = $objConfig->getStandaloneShipmentByID("PurchaseRMA", $OrderID);		   	 
				...
				if(!empty($arryShippInfo[0]['TotalFreight'])){	
					...
					$arryRow[0]['InvoiceComment'] = 'PurchaseRMA';
