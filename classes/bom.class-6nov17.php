...
function UpdateOptionBill($arryDetails) {
 ...
 function AddOptionCat($bom_id, $arryDetails) {
    ...
    function AddAssemble($arryDetails) {
        ...
        $strSQLQuery = "insert into inv_assembly(warehouse_code,assembly_qty,item_id,Sku,unit_cost,total_cost,on_hand_qty,asmDate,created_by,created_id,Status,serial_name,serial_qty,bomCondition,serial_Price,serial_desc,Comment) 
				values('" . $warehouse . "','" . $assembly_qty . "','" . $item_id . "', '" . $Sku . "',  '" . $price . "', '" . $TotalValue . "', '" . $on_hand_qty . "', '" . $Config['TodayDate'] . "','" . $_SESSION['AdminType'] . "','" . $_SESSION['AdminID'] . "','" . $Status . "','" . $serial_value . "','" . $serial_qty . "','".$bomCondition."','".$serial_Price."','".addslashes($serial_desc)."','" . addslashes($Comment) . "')";
...
//$arryDetails['warehouse'] =1;
 ...
 $strSQLQuery = "insert into inv_serial_item (adjustment_no,warehouse,serialNumber,Sku,assembleID,`Condition`,description,type,UnitCost,ReceiptDate)  values ('".$adjustID."','" . $arryDetails['warehouse'] . "','" . $serial_no_add[$j] . "','" . $Sku . "','" . $materialID . "','".$bomCondition."','" . addslashes($serial_desc[$j]) . "','Assemble', '".$SRP."','".$Config['TodayDate']."')";
    ...
    /*********Added By karishma based on Condition 6 Jan 2016*********************/
					if($bomCondition!=''){
						...
						if($restbl[0]['total']==0){
							...
							'" . addslashes($assembly_qty) . "','" . $arryDetails['warehouse'] . "')";
							$this->query($strSQLQuery, 0);
						}else{
							// update in tbl
...
//$warehouse=$arryDetails['warehouse'];
	if($arryDetails['serial_value' ]!=''){						
				...
				$SQLQuery = "insert into inv_serial_item (warehouse,serialNumber,Sku,assembleID,`Condition`,description,type,UnitCost,ReceiptDate)  values ('" . $warehouse . "','" . $serial_noum[$j] . "','" . addslashes($Sku) . "','" . $asmID . "','".$bomCondition."','" . addslashes($serial_desc[$j]) . "','Assemble', '".$srPrice."','".$Config['TodayDate']."')";
    ...
    function AddAssembleItem($AID, $arryDetails) {
        ...
        for ($i = 1; $i <= $NumLine; $i++) {
            ...
            if (!empty($arryDetails['sku' . $i])) {
                ...
                $sql = "insert into inv_item_assembly (asmID,bomID,bom_refID, item_id, sku, description,valuationType,available_qty, qty, unit_cost, total_bom_cost,serial,`Condition`,req_item,serialPrice,serialdesc) values('" . $AID . "','" . $bomID . "','" . $id . "','" . $arryDetails['item_id' . $i] . "', '" . addslashes($arryDetails['sku' . $i]) . "', '" . addslashes($arryDetails['description' . $i]) . "','" . addslashes($arryDetails['valuationType' . $i]) . "','" . addslashes($arryDetails['on_hand' . $i]) . "', '" . addslashes($arryDetails['qty' . $i]) . "', '" . addslashes($arryDetails['price' . $i]) . "','" . addslashes($arryDetails['amount' . $i]) . "','" . addslashes($arryDetails['serial_value' . $i]) . "', '" . addslashes($arryDetails['Condition' . $i]) . "','" . addslashes($arryDetails['req_item' . $i]) . "','" . addslashes($arryDetails['serialPrice' . $i]) . "','" . addslashes($arryDetails['serialdesc' . $i]) . "')";
...
/*********Added By karishma based on Condition 6 Jan 2016*********************/
				...
				$strSQL = "update inv_serial_item set UsedSerial = '1' where serialNumber='".trim($serial_no[$k])."' and Sku ='" . addslashes($arryDetails['sku' . $i]) ."' and `Condition`='".$arryDetails['Condition' . $i]."' and warehouse = '".$arryDetails['warehouse']."'  "; 
								$this->query($strSQL, 0);
    ...
    function AddUpdateAssembleItem($AID, $arryDetails) {
        ...
        for ($i = 1; $i <= $NumLine; $i++) {
            ...
            if (!empty($arryDetails['sku' . $i])) {
                ...
                $id = $arryDetails['id' . $i];
                if ($id > 0) {
                    $sql = "update inv_item_assembly set item_id='" . $arryDetails['item_id' . $i] . "', sku='" . addslashes($arryDetails['sku' . $i]) . "', description='" . addslashes($arryDetails['description' . $i]) . "', qty='" . addslashes($arryDetails['qty' . $i]) . "', unit_cost='" . addslashes($arryDetails['price' . $i]) . "', total_bom_cost='" . addslashes($arryDetails['amount' . $i]) . "' ,serial='" . addslashes($arryDetails['serial_value' . $i]) . "',`Condition` = '" . addslashes($arryDetails['Condition' . $i]) . "',req_item='" . addslashes($arryDetails['req_item' . $i]) . "'  where id='" . $id. "'";
                } else {

                    $sql = "insert into inv_item_assembly (asmID,bomID,bom_refID, item_id, sku, description,valuationType,available_qty, qty, unit_cost, total_bom_cost,serial,`Condition`,req_item) values('" . $AID . "','" . $bomID . "','" . $id . "','" . $arryDetails['item_id' . $i] . "', '" . addslashes($arryDetails['sku' . $i]) . "', '" . addslashes($arryDetails['description' . $i]) . "','" . addslashes($arryDetails['valuationType' . $i]) . "','" . addslashes($arryDetails['on_hand' . $i]) . "', '" . addslashes($arryDetails['qty' . $i]) . "', '" . addslashes($arryDetails['price' . $i]) . "','" . addslashes($arryDetails['amount' . $i]) . "','" . addslashes($arryDetails['serial_value' . $i]) . "','" . addslashes($arryDetails['Condition' . $i]) . "','" . addslashes($arryDetails['req_item' . $i]) . "')";
                }
                $this->query($sql, 0);
                if ($arryDetails['Status'] == 2) {
                    $strSQLItem = "update inv_items set qty_on_hand = qty_on_hand-'" . $arryDetails['qty' . $i] . "' where Sku='" . $arryDetails['sku' . $i] . "'";
...
if($arryDetails['serial_value']!=''){
    ...
    function AddDisassemble($arryDetails) {
        ...
        $strSQLQuery = "insert into inv_disassembly  (WarehouseCode,disassembly_qty,item_id,Sku,unit_cost,`total_cost`,`on_hand_qty`,disassemblyDate,created_by,created_id,Status,serial_Num,bomCondition,total_dis_cost) 
		        values('".addslashes($warehouse)."','".addslashes($disassembly_qty)."','".addslashes($item_id)."','".addslashes($Sku)."',  '" . addslashes($price) . "', '" . addslashes($TotalValue) . "', '" . $on_hand_qty . "', '" . $Config['TodayDate'] . "','" . $_SESSION['AdminType'] . "','".$_SESSION['AdminID']."','".$Status."','".addslashes($serial_Num)."','".addslashes($bomCondition)."','".addslashes($total_dis_cost)."')";
...
$arryDetails['warehouse']=1;
                        for ($j = 0; $j <= sizeof($serial_noum) - 1; $j++) {
                            $strSQLQuery = "insert into inv_serial_item (warehouse,serialNumber,Sku,disassembly,`Condition`,type,description,UnitCost)  values ('" . $arryDetails['warehouse'] . "','" . $serial_noum[$j] . "','" . addslashes($arryDetails['sku' . $i]) . "','" . $AID . "','" . addslashes($arryDetails['Condition' . $i]) . "','disassemble',addslashes($serial_des[$j]),'". addslashes($arryDetails['price' . $i])."')";
                            $this->query($strSQLQuery, 0);
                            //echo   $serial_no[$i]."<br/>"; 
                        }
                    }
...
$strSQLQuery = "insert into inv_serial_item (warehouse,serialNumber,Sku,UsedSerial,disassembly,`Condition`,type,UnitCost,ReceiptDate)  values ('".$arryDetails['warehouse']."','".$serial_noum[$j]."','".addslashes($arryDetails['sku'.$i])."','0','".$AID."','".addslashes($arryDetails['Condition'.$i])."','Receive Order','".$arryDetails['price'.$i]."','".$Config['TodayDate']."')";
 ...
 $strAddQuery = (!empty($Condition)) ? (" and `Condition`='" . $Condition. "'") : ("");
    ...
    function addSerailNumberForInvoice($arryDetails) {
       ...
       function getBinBySku($Wcode,$Sku){
                    ...
                    $strAddQuery = (!empty($Wcode))?(" where s.Wcode='".$Wcode."'"):("");
                     ...
                     $strSQLQuery = "select b.*,s.Sku,s.Wcode,s.bin from w_binlocation b inner join inv_bin_stock s on BINARY b.warehouse_code= BINARY s.Wcode ".$strAddQuery." ";
                    return $this->query($strSQLQuery, 1);
    ...
    function NextPrevBom($bomID,$Next) {
		...
		if($bomID>0){		
			...
			$strSQLQuery = "select i.bomID  from inv_bill_of_material i where i.bomID".$operator."'" . $bomID . "'  ". $strAddQuery. " order by i.bomID ".$asc." limit 0,1";

			$arrRow = $this->query($strSQLQuery, 1);
			return $arrRow[0]['bomID'];
		}
	}
