...
function DashboardIconRoleGroupNew($depID,$GroupID=0) {
    ...
    function AddNotification($arryDetails,$DBNAme='') {
        global $Config;
        extract($arryDetails);
    ...
    function getSettingVariable($settingKey,$DB='') {
        $strSQLQuery = "select setting_value from ".$DB."settings where setting_key ='" . trim($settingKey) . "'";
        $arryRow = $this->query($strSQLQuery, 1);
        $settingValue = $arryRow[0]['setting_value'];
        return $settingValue;
    }

    function getSettingVariableAll($settingKey,$DB='') {
        $strSQLQuery = "select * from ".$DB."settings where setting_key ='" . trim($settingKey) . "'";
        $arryRow = $this->query($strSQLQuery, 1);
        return $arryRow;
    }
   ...
   function getSettingCaption($settingKey) {
    ...
    function UpdateModuleAutoID($Table, $Module, $OrderID, $ModuleIDValue,$DB='') {
        if ($OrderID > 0) {
            if (!empty($ModuleIDValue)) { //update if entered in box
                ...
                $AutoID = filter_var($ModuleIDValue, FILTER_SANITIZE_NUMBER_INT);
		#$AutoID = preg_replace('/[^0-9]/', '', $AutoID);

                $strSQLFinal = "update ".$DB.$Table . "   set AutoID='" . $AutoID . "' where OrderID='" . $OrderID . "'";
		       ...
		       case 'Shipment': 
                ...
                $strSQL = "select max(AutoID) as MaxAutoID from ".$DB.$Table . "   where OrderID != '" . $OrderID . "' and Module='" . $Module . "'";
                $arryAuto = $this->query($strSQL, 1);


                $PrefixModule = $this->getSettingVariable($PrefixString,$DB);
                $StartNum = $this->getSettingVariable($StartString,$DB);
                ...
                $strSQLFinal = "update ".$DB.$Table ." set AutoID='" . $AutoID . "', " . $ModuleID . "='" . $ModuleIDValue . "' where OrderID='" . $OrderID . "'";
            }
	...
	global $Config;
	if (!empty($ShippingMethod) && !empty($ShippingMethodVal)) {
...
/*********************************/
	   ...
	   function RecurringHoliday(){ 
			...
			$arryHoliday = $this->query($sql, 1);
	             	...
	             	//$Config['TodayDate'] = '2017-12-31';//temp
			...
			$Month = $arryDay[1];
			 ...
			 if($Month == '01'){  
				   for($i=0;$i<sizeof($arryHoliday);$i++) { 
						...
						if($arryHoliday[$i]['holidayDateTo']>0){
							$arryHolidayDateTo = explode("-", $arryHoliday[$i]['holidayDateTo']);							
							$H_Month2 = $arryHolidayDateTo[1];
							$H_Day2 = $arryHolidayDateTo[2];
							$holidayDateTo = $Year.'-'.$H_Month2.'-'.$H_Day2;	
						}
					...
					$sql = "update h_holiday set holidayDate = '".$holidayDate."',holidayDateTo = '".$holidayDateTo."' where holidayID = '".$arryHoliday[$i]['holidayID']."'";
					$this->query($sql,0);
				      	//echo $sql.'<br>';	
...
//updated on 8May2018 by chetan/saiyed//
	function GeneratePDF($PdfArray){
		...
		(empty($UploadPrefix))?($UploadPrefix=""):("");
	...
	function IsSecurityQstAnsExist($RefID,$UserType) {	
		if(!empty($RefID) && !empty($UserType)){	
			$strSQLQuery = "select ID from user_secure where RefID='".$RefID."' and UserType='".$UserType."'"; 
			$arryRow = $this->query($strSQLQuery, 1);
			if(!empty($arryRow[0]['ID'])){
				return true;
			}
	...
	function ResetAuthSecretKey($RefID,$UserType){ 
		global $Config;	 
		if($UserType == "admin"){	
			$strSQLQuery = "update ".$Config['DbMain'].".company set AuthSecretKey=''  where CmpID='".$RefID."'";
		}else{
			$strSQLQuery = "update user set AuthSecretKey='' where UserID='".$RefID."'";
		}
	...
	function UpdateSecuritySettingSA($arryDetails){   
		extract($arryDetails);
		if(!empty($AdminID)){				
			 $strSQLQuery = "update admin set SecurityLevel='".mysql_real_escape_string($SecurityLevel)."', AllowSecurity='".mysql_real_escape_string($AllowSecurity)."' where AdminID='".mysql_real_escape_string($AdminID)."'";
			 $this->query($strSQLQuery, 0);
		}
		return 1;
	}
	
	function DeleteSecurityProfileSA($RefID,$UserType) {
		if(!empty($RefID) && !empty($UserType)){			
	...
	function UpdateAuthSecretKeySA($AuthSecretKey){ 
		global $Config;	 
		if($_SESSION['AdminType'] == "admin"){	
			$strSQLQuery = "update admin set AuthSecretKey='".$AuthSecretKey."'  where AdminID='".$_SESSION['AdminID']."'";
		}else{
			$strSQLQuery = "update u_user set AuthSecretKey='".$AuthSecretKey."' where Uid='".$_SESSION['AdminID']."'";
		}
		$this->query($strSQLQuery, 0);
		return 1;
	}

	function ResetAuthSecretKeySA($RefID,$UserType){ 
		global $Config;	 
		if($UserType == "admin"){	
			$strSQLQuery = "update admin set AuthSecretKey=''  where AdminID='".$RefID."'";
		}else{
			$strSQLQuery = "update u_user set AuthSecretKey='' where Uid='".$RefID."'";
		}
	...
	function UpdateSecurityLevelSA($arryDetails){ 			
		global $Config;
		extract($arryDetails);		
		$AllowSecurityUser = rtrim($AllowSecurityUser,',');	
	
		if($_SESSION['AdminType'] == "admin"){	
			$strSQLQuery = "update admin set UserSecurity='".$UserSecurity."',AllowSecurityUser='".$AllowSecurityUser."' where AdminID='".$_SESSION['AdminID']."'";
		}else{
			 $strSQLQuery = "update u_user set UserSecurity='".$UserSecurity."',AllowSecurityUser='".$AllowSecurityUser."' where Uid='".$_SESSION['AdminID']."'";
		}
		$this->query($strSQLQuery, 0);
		return 1;
	}

	function GetFreightTaxVal($TaxName,$TaxVal,$Class) {	 
		$strSQLQuery = "SELECT FreightTax from inv_tax_rates WHERE  RateDescription = '" . $TaxName . "' and TaxRate = '" . $TaxVal . "' and ClassId like '%" . $Class . "%' limit 0,1";
                $arryRow = $this->query($strSQLQuery, 1);
		if(!empty($arryRow[0]['FreightTax'])){
			return $arryRow[0]['FreightTax'];
		}
	}
