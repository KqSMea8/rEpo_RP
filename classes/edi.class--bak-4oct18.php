...
class edi extends dbClass
{
	...
	function getEDICompanies($search_str){
	  ...
	  $sql ="select CmpID,CompanyName,DisplayName,Email,Description,
		...
		and DisplayName='" . addslashes($search_str) . "' "; 
	...
	function sendRequestforedi($EDICompId,$name,$EDIType){

		$Added_date=date('Y-m-d');
		$RequestedCompDB='erp_'.$name;
	...
	function acceptrejectEDI($ID,$mod){
		global $Config;

		$AcceptRejectDate=date('Y-m-d');
	  $sql="select EDICompName,RequestedCompName,EDICompId,RequestedCompID,EDIType from edi_company
			where RequestedCompID='" . addslashes($ID) . "'";
...
#print_r($details); exit;
 ...
 $sqlCust="select * from ".$RequestedCompNameDB.".s_customers	where Email='".addslashes($arryDetails['Email'])."'";
								  $arryCustomer=$this->query($sqlCust, 1);

										if(is_array($arryCustomer)){
...
#echo "<pre>";print_r($arryDetails); exit;
 ...
 $sqlData="select * from ".$EDICompDB.".s_customers	where Email='" . addslashes($arrycurrCust['Email']) . "'";
										$arryCust=$this->query($sqlData, 1);
										if(is_array($arryCust)){
...
$sqlSuppSql="select * from ".$RequestedCompNameDB.".p_supplier	where Email='" . addslashes($arryDetails['Email']) . "'";
								  $arrySupp=$this->query($sqlSuppSql, 1);
										if(is_array($arrySupp)){

													 $sqlReqUpdate = "UPDATE ".$RequestedCompNameDB.".p_supplier SET EDICompId='" . addslashes($details[0]['EDICompId']) . "',EDICompName='" . addslashes($details[0]['EDICompName']) . "'	WHERE Cid =" . $arrySupp[0]['Cid']; 
		$this->query($sqlReqUpdate, 0);
	...
	$sql2sql="select * from ".$EDICompDB.".p_supplier	where Email='" . addslashes($arryDetails['Email']) . "'";
								  $arrySupplier=$this->query($sql2sql, 1);
										if(is_array($arrySupplier)){
	...
	function getCompanyInfoForSupplier($CmpID,$RequestedCompID){
		...
		echo $sql="select * from erp.company	where CmpID='" . addslashes($CmpID) . "'";
		$compdetails=$this->query($sql, 1);
