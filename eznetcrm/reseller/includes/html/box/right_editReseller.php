...
<div id="imgGal">
		<? if($arryReseller[0]['Image'] !='' && file_exists('../upload/reseller/'.$arryReseller[0]['Image']) ){ ?>
			...
			<div  id="ImageDiv" align="center"><a class="fancybox" data-fancybox-group="gallery" href="../upload/reseller/<?=$arryReseller[0]['Image']?>"  title="<?=stripslashes($arryReseller[0]['FullName'])?>"><? echo '<img src="../resizeimage.php?w=120&h=120&img=upload/reseller/'.$arryReseller[0]['Image'].'" border=0 >';?></a>
			<br />
			
			<!--</h1><a class="fancybox fancybox.iframe" href="includes/iframe/reseller_img.php">Change Photo</a>-->
			
			<a href="Javascript:void(0);" onclick="Javascript:DeleteFile('../upload/reseller/<?=$arryReseller[0]['Image']?>','ImageDiv')"><?=$delete?></a>
			
			
				</div>
		<?	}else{ ?>
		
		<div  id="ImageDiv" align="center"><img src="../resizeimage.php?w=120&h=120&img=images/no.jpg" title="<?=$arryReseller[0]['UserName']?>" /></div>
		<? } ?>
	</div>	
