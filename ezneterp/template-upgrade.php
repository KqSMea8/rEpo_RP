...
function autoSetAmount(){
	...
	var MaxUser = $('#MaxUser').val();
        var sendParam='&action=autoSetAmount&r='+Math.random()+"&UpgradeAmountToPay="+escape(TotalAmount)+"&UpgradeMaxUser="+escape(MaxUser); 
	 $.ajax({
                type: "GET",
                async:false,
                url: 'ajax_main.php',
	...
	function calculateprice(UserLess){
		...
		if(MaxUser>0){
			...
			var UserLabel = MaxUser+' user';
			if(MaxUser>1){
				UserLabel = MaxUser+' users';
			}


			var datahtml = '<h3>You have choosen a subscription for '+DurationLabel+' for '+UserLabel+' <br>After deduction of $'+Deduction+' for time '+DaysLeft+' day(s):</h3> <h3><br>Final Price: $<span id="TotalAmountSpan">'+TotalAmount+'</span> <br><br>Additional Space: '+AdditionalSpace+'  '+AdditionalSpaceUnit+'<br>Free Space: '+FreeSpace+'  '+FreeSpaceUnit+'</h3>';
...
<form accept-charset="UTF-8" id="pre-payment-calculation-form"
	method="post" action="upgrade-payment"
	novalidate="novalidate" onsubmit="return validate(this);">
<div>

<div id="prepricetext">
<h3>Your current subscription is for <?php echo $arrayCompany[0]['MaxUser'];?>
user(s) till <?=date("jS F, Y",strtotime($arrayCompany[0]['ExpiryDate']))?>
.</h3>
<br>
<h3><?php
...
<select class="form-select required" name="PlanDuration"
	...
	<option  value="<?php echo $pdchkVal;?>"><?php echo $pdchkVal;?></option>	
	<?php } ?>
</select>
</div>

<div id="CouponCd" style="display: none;" class="form-item form-type-select form-item-num-users">
    <label for="edit-num-users">Coupon Code </label> 
