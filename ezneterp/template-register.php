...
if ($_POST) {
    if ($_SESSION["code"] != $_POST['ImageVerify']) {
        $_SESSION['mess_company'] = '<div class="redmsg">Wrong Image Verification Code !</div>';
        ...
        $_POST["Password"] = substr(md5(rand(100, 10000)), 0, 8);
        /*         * ******************** */

        $CmpID = $objCompany->AddCompany($_POST);
        //$objCmp->SendActivationMail($CmpID);
        $ReturnFlag = $objCmp->ActiveCompany($_POST);
...
<script language="JavaScript1.2" type="text/javascript">
    ...
    function getCaptcha() {
        var CaptchaCode = '';
        var sendParam = '&action=getCaptcha&r=' + Math.random();
        $.ajax({
            type: "GET",
            async: false,
            url: 'ajax_main.php',
            ...
            success: function (responseText) {
                document.getElementById("ImageVerifyHidden").value = responseText;
            }
        });
    ...
    function validateCompany(frm) {
        ...
        if (ValidateMandRange(frm.DisplayName, "User Name", 3, 30)
                ...
                && ValidateForSimpleBlank(frm.ImageVerify, "Image Verification Code")
                ) {

            /*
             if(document.getElementById("ImageVerify").value!=document.getElementById("ImageVerifyHidden").value){
             alert('Wrong Image Verification Code !');
             return false;
             }*/
...
<?= $arryCountry[$i]['name'] ?>
                                        ...
                                        ><div class="form-item form-type-textfield form-item-name">
                                            <label for="edit-name" class="in-field-labels-processed"
                                                   style="opacity: 1;">Image Verification Code <span
                                                    title="This field is required." class="form-required">*</span>
                                            </label> 
                                            <input type="text" maxlength="20" size="60" value="" name="ImageVerify" id="ImageVerify" class="username form-text required">
                                            <input type="hidden"  readonly value="" name="ImageVerifyHidden" id="ImageVerifyHidden" >
                                        ...
                                        <div id="edit-actions" class="form-actions form-wrapper">
                                            <input type="submit" class="form-submit"
                                                   value="Create new account" name="op" id="edit-submit" title="Create New Account">
                                        </div>
                                    </div>
                                </form>
