...
<SCRIPT LANGUAGE=JAVASCRIPT>
	...
	function calculateprice(UserLess){
		...
		var ResellerDiscount = $('#ResellerDiscount').val();
		...
		rsDiscount = (TotalAmount*ResellerDiscount)/100;

		TotalAmount= TotalAmount - Math.round(rsDiscount);
		...
		if(MaxUser>0){
			...
			var datahtml = '<h3>You have chosen a subscription for '+DurationLabel+' for '+UserLabel+' <br></h3> <h3><br><br>Final Price: $<span id="TotalAmountSpan">'+TotalAmount+'</span> <br><br>Additional Space: '+AdditionalSpace+'  '+AdditionalSpaceUnit+'<br>Free Space: '+FreeSpace+'  '+FreeSpaceUnit+'</h3>';
...
<? if(!empty($rSDiscount)){?>
<br>
<span class="label">Discount:-</span> <span
	class="value">
<?php 
echo $rSDiscount .'%';
?> 
...
<form accept-charset="UTF-8" id="pre-payment-calculation-form"
	method="post" action="createCompany.php" novalidate="novalidate"
...
<input type="hidden" name="ResellerDiscount" id="ResellerDiscount" value="<?php echo $rSDiscount;?>">
	
	
<input type="hidden" name="FreeSpace"
