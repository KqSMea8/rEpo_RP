<?php
ValidateCrmSession();
require_once("../classes/erp.cmp.class.php");
require_once("../classes/erp.company.class.php");
...
if(!empty($_POST['paySubmit'])){
   ...
   CleanPost();
	if($_POST['TotalAmount']!=$_SESSION["UpgradeAmountToPay"] || $_POST['MaxUser']!=$_SESSION["UpgradeMaxUser"]){
		...
		header('location:index.php?slug=upgrade&pack_id='.$_POST['pack_id']);
		exit;
	}


	$packData=array();
	$packData=$objCmp->ezneterpPackagesByName($_POST['PaymentPlan']);
	
	$packDataUnserialize=unserialize($packData[0]['features']);
	$packageFeatureId = implode(",", $packDataUnserialize);

    $inventryModule = $packData[0]['inventryModule'];
	...
	if($ack!="SUCCESS")
	{
		...
		//echo '<pre>'; print_r($resArray);exit;
		/*$_SESSION["PricingMsg"] = '<table width="500" cellpadding="0" cellspacing="0"><tr>
		 <td colspan="2" style="font-weight:bold;color:red;" align="center">Payment Error: Please check that you have provided all information correctly.</td>
		 </tr>
		<tr>
		 <td align="right">Error Code:</td>
		 <td>'.$resArray["L_ERRORCODE0"].'</td>
		 </tr>
		 <tr>
		 <td align="right">Short Message:</td>
		 <td>'.$resArray["L_SHORTMESSAGE0"].'</td>
		 </tr>
		 <tr>
		 <td align="right">Long Message:</td>
		 <td>'.$resArray["L_LONGMESSAGE0"].'</td>
		 </tr>
		 <tr>
		 <td align="right">Correlation ID:</td>
		 <td>'.$resArray['CORRELATIONID'].'</td>
		 </tr></table>';*/
		if(!empty($resArray["L_LONGMESSAGE0"]))
		$_SESSION["PricingMsg"] = 'Payment Error : '.$resArray["L_LONGMESSAGE0"];
		else
	...
	else
	{
		...
		///////////////////////////////////
           ...
           if(in_array(2,$packDataUnserialize)){
          	$maxProduct = $packData[0]['maxProduct'];
          }else{
    ...
    $_POST['CrmAdminID'] = $CmpID;	
	 ...
	 $OrderID=$objCmp->AddErpOrder($_POST);
        ...
        $objCmp->UpdateDepartment($CmpID,$packageFeatureId,$maxProduct);
	...
	/* invoice end here */
		...
		if($OrderID>0){
			...
			if($UpdateAdminMenu == 1){
				$DbName = $Config['DbMain']."_".$_SESSION['CrmDisplayName'];
				$Config['DbName'] = $DbName;
				$objConfig->dbName = $Config['DbName'];
				$objConfig->connect();
				$objCompany->UpdateAdminModules($_SESSION['CrmAdminID'],$packageFeatureId);
/* trackinventry 0 */

                $_POST['TrackInventory']=0;
				...
				//$objCmp->UpdateAdminSubModules($_SESSION['CrmAdminID'],5,strtolower($_POST['PaymentPlan']),$packageFeatureId);
			      ...
			      if(in_array('3,4,6,7,8',$packDataUnserialize)){
...
if(!empty($_POST['pack_id'])){
	$arrayPkj=$objCmp->ezneterpPackagesById($_POST['pack_id']);
...
<div class="content" id="upgrade-payment">
