     ...
     <td width="12%" class="head1" >Display Name</td>
      <td width="10%" class="head1" >Image</td> -->
			...
			<tr align="left" bgcolor="<?=$bgcolor?>">
				...
				<td height="50"><strong><?=stripslashes($values["username"])?></strong></td>
	        ...
	        echo '<a href="addCompany.php?active_id='.$values["userID"].'&status='.$values["status"].'&curP='.$_GET["curP"].'" class="'.$status.'">'.$status.'</a>';

	   ?></td>
				<td align="center" class="head1_inner"><a
					href="addCompany.php?edit=<?=$values['userID']?>&curP=<?=$_GET['curP']?>" title="edit company details"><?=$edit?></a>
				<!--<a
					...
					<a href="changePassword.php?changepwd_id=<?php echo $values['userID']; ?>" title="change password"><?= $changepwd ?></a>
				</td>
			</tr>
			<?php } // foreach end //?>
