...
<table <?=$table_bg?>>
  ...
  <tr align="left"  bgcolor="<?=$bgcolor?>">
	...
	<td class="blacknormal"> <?=stripslashes($values->TemplateName)?>  </td>
        <td class="blacknormal"> <?php echo ucfirst($values->TemplateType);?>  </td>
    <td class="blacknormal"><?php if($values->TemplateType!=''){?>
    	<img width="205px;" height="150px;" src="<?php echo isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off' ? 'https' : 'http'.'://'.$_SERVER['SERVER_NAME'].'/'.$Config['ecomfolder'];?>/template/<?php echo $values->TemplateName;?>/<?php echo $values->Thumbnail;?>" >
...
?></td>
  ...
  <?php }else{?>
  	<tr align="center">
  	  <td height="20" colspan="5"  class="no_record">No Template found. </td>
  </tr>

  <?php } ?>
    
  <tr >  <td height="20" colspan="5">Total Record(s) : &nbsp;<?php echo $num;?> 
