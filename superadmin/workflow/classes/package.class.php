 ...
 */
    ...
    function getPackage($arg = array(), $pckg_id) {
        extract($arg);
        if (isset($pckg_id)) {
            $sql = "SELECT * FROM `plan_package` where pckg_id='" . $pckg_id . "'";
     ...
     function getPackageelement($arg = array(), $pckg_id) {
           extract($arg);
            $sql = "SELECT * FROM `plan_package_element` where package_id='" . $pckg_id . "'";
            return $this->get_results($sql);
    ...
    function AddPackage($arryDetails) {
        ...
        if (!empty($arryDetails['elementdata'])) {
            foreach ($arryDetails['elementdata'] as $keyelemnt => $valueelement) {
                ...
                $tempdata = array('package_id' => $pckgID, 'ele_key' => $keyelemnt, 'ele_value' => $valueelement);
                $this->insert('plan_package_element', $tempdata);
            }
        }
        return $pckgID;
    }
    ...
    function UpdatePackage($arryDetails, $pckg_id) {
       ...
       //print_r($arryDetails); die;
        ...
        $result = $this->update('plan_package', $arryDetails['packagedata'], array('pckg_id' => $pckg_id));
        if (!empty($pckg_id)) {
            ...
            foreach ($elementresult as $keyelemnt => $valueelement) {
            	if(array_key_exists($valueelement->ele_key,$arryDetails['elementdata'])){
            		 ...
            		 $this->update('plan_package_element', array('ele_value'=>$value), array('package_id' => $pckg_id,'ele_key'=>$valueelement->ele_key));
            	}
    ...
    function DeletePackage($pckg_id) {
        ...
        $this->delete('plan_package', array('pckg_id' => $pckg_id));
        if(!empty($pckg_id)){
        $this->delete('plan_package_element', array('package_id' => $pckg_id));
    ...
    function changePackageStatus($arryDetails, $pckg_id) {
        ...
        if (!empty($arryDetails)) {
            ...
            $this->update('plan_package', $arryDetails, array('pckg_id' => $pckg_id));
        }
        return true;
    }
