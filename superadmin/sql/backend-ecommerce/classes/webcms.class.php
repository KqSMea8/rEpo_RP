...
class webcms extends dbClass
{
	...
	function getTemplates($TemplateType) {
		...
		$objConfig->connect();
		/*******************************************/
		$sql = "SELECT * from templates where TemplateType='" . addslashes($TemplateType) . "'   ORDER BY TemplateId DESC ";
    ...
    IF(b.FullName != '',
        b.FullName,
        IF(b.Company != '', b.Company, 'Owned')) as FullName,
		c.Sitename
FROM
    `web_assign_customer` as a
        left join
    s_customers b ON b.Cid = a.CustomerId
left join
    web_setting c ON c.CustomerID = a.CustomerId ";
		return $this->query($sql, 1);
	}
	...
	function getCustomers()
	{
		...
		foreach($res as $val){
			...
			$SqlCustomer = "select c1.Cid,c1.FullName,c1.Company,b.* from s_customers c1
	...
	function assignCustomer($arryDetails){
		...
		if($res[0]['total']==0){
			...
			$sitename=trim(str_replace(' ','',$sitename) );	
