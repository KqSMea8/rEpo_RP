...
require_once("includes/theme_function.php");
...
require_once("classes/variant.class.php");   //By Chetan11Sep//
...
if(!empty($_GET["c"])){
	...
	if(empty($arryCompany[0]["CmpID"]) || substr_count($arryCompany[0]['Department'],2)==0){
		...
		if(!$objConfig->connect_check()){
			...
			// cms object
			$objWebcms = new webcms();

			$arryGlobalSetting = $objWebcms->GetGlobalSetting();
			
			//$arryGlobalSetting[0]['TemplateId']=1;
			$themeObj=new theme();
			$ThisPage = GetPage();
			if($ThisPage!='index.php'){
				$pageArray=explode('.',$ThisPage);
			$page=$pageArray[0];
			}else{
				$page='home';
			}
			
			$arryPageTemplate = $themeObj->GetTemplate($arryGlobalSetting[0]['TemplateId'],$page);
			
			$cssfiles=$themeObj->scantemplate($_SERVER['DOCUMENT_ROOT'].'/erp/template/'.$arryPageTemplate[0]['themeUploadedName'] );
			//$arryGlobalTemplate = $objWebcms->GetTemplate($arryGlobalSetting[0]['TemplateId']);

			//$cssfiles=$objWebcms->scantemplate($_SERVER['DOCUMENT_ROOT'].'/erp/template/'.$arryGlobalTemplate[0]['TemplateName'] );
...
$arrySocialSetting = $objcartsettings->getSocialsettings();
 
// get group discount
$arryDiscountSetting = $objcartsettings->getgroupdiscount();
 
$GroupId=$_SESSION['GroupID'];
 
$groupDiscont=($GroupId!='' && count($arryDiscountSetting)>0)?$arryDiscountSetting[$GroupId]:'';

// end of group discount
...
$Config['MailFooter'] = '['.stripslashes($settings['StoreName']).']';
...
$arryCartItems = $objOrder->GetItemCart($Cid);
