<?php	
	...
	function redirect( $message, $url, $waitTime = 2, $alt = 1 ){ 
	 ...
	 function getDurationFormat($TimeDuration){
		...
		$time='';
		if($timeArray[0]>0) $time .= (int)$timeArray[0].' hr, ';
		if($timeArray[1]>0) $time .= (int)$timeArray[1].' min, ';
		if($timeArray[2]>0) $time .= (int)$timeArray[2].' sec, ';
		$time= rtrim($time,', ');
		return $time;
	}
	...
	function ConvertToSecond($TimeDuration){
		$timeArray = explode(":",$TimeDuration);
		$time=0;
		if($timeArray[0]>0) $time += (int)$timeArray[0]*3600;
		if($timeArray[1]>0) $time += (int)$timeArray[1]*60;
		if($timeArray[2]>0) $time += (int)$timeArray[2];
