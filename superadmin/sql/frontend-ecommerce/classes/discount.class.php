...
class discount extends dbClass
{
	...
	public function checkPromoCode($SubtotalAmount,$promo_code,$Cid)
	{
		...
		if (!empty($checkPromoID) && $settings["DiscountsPromo"] == "Yes" && $checkprostatus == true)
		{
			...
			$strSQLQuery = "SELECT * FROM e_promo_codes WHERE DateStart < NOW() AND DateStop > NOW() AND PromoCode='".trim($promo_code)."' AND Active='Yes' AND MinAmount <= ".$subtotal." AND IF(DiscountType = 'amount', Discount<= ".$subtotal." , Discount!='') ";
