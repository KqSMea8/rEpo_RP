...
//echo '<pre>';print_r($arryCustomRule);exit;
		...
		if(sizeof($arryCustomRule)>0){
			...
			//echo '<pre>'; print_r($LeaveAllowedTotal); 

			if(sizeof($LeaveAllowedTotal)>0){
				$MainIndex = 0;							
...
/************************************/
		...
		function checkLeaveHoliday($EmpID,$Date)
		{
			global $Config;
			//holiday check
			$strH ="select holidayID from `h_holiday` where holidayDate='".$Date."' ";
			...
			if(!empty($arryH[0]['holidayID'])) {
				...
				$strL ="select LeaveID,DateDiff('".$Date."',FromDate)+1 as NumDay,LastBalance as LaB from `h_leave` where EmpID='".$EmpID."' and Status='Approved'  and WEEKDAY('".$Date."') not in (5,6)  and (FromDate<='".$Date."' and ToDate>='".$Date."') having NumDay<=LaB ";

				
				if($_GET['p']==1){echo $strL;}
