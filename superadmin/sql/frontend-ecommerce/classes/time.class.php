...
//echo $EmpWorkingHour.'='.$arryLocation[0]['HalfDayHour'];exit;
	...
	function getAttendence($depID,$attID=0,$EmpID=0,$attDate,$year,$month)
	{
		...
		$sql = "select a.*,TIMEDIFF(a.OutTime,a.InTime) as TimeDuration, s.shiftName,s.LunchPaid,s.ShortBreakPaid, e.EmpCode, e.UserName,e.JobTitle, d.Department from h_attendence a inner join h_employee e on a.EmpID=e.EmpID left outer join  h_department d on e.Department=d.depID left outer join  h_shift s on e.shiftID=s.shiftID ".$sql." order by e.UserName asc, a.attDate Asc "; 
...
order by e.UserName asc, a.attDate Asc "; 
	...
	function updateAttPunchingAdmin($punchID,$InTime,$OutTime)
	{
		global $Config;
		...
		if(!empty($punchID)){
			$AddSql = '';
			if($InTime!='') $AddSql .= " , InTime='".$InTime."' ";
			if($OutTime!='') $AddSql .= " , OutTime='".$OutTime."' ";
			$sql = "update h_att_punching set UpdatedDate='".$Config['TodayDate']."' ".$AddSql."  where punchID = '".mysql_real_escape_string($punchID)."'";
			$rs = $this->query($sql,0);		
