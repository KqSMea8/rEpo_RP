...
<tr>
	...
	<input name="OldImage" type="hidden" value="<?=$arryReseller[0]['Image']?>">
...
<? } ?>
 ...
 <? if($_GET["tab"]=="term"){
	...
	$arryPaymentMethod = $objCommon->GetAttributeValue('PaymentMethod','');
 ?>
<tr>
<td colspan="2">&nbsp;</td>
</tr>
	...
	<td   align="left">
	  <select name="PaymentTerm" class="inputbox" id="PaymentTerm">
	  	<option value="">--- None ---</option>
			...
			?>
				<option value="<?=$PaymentTerm?>" <?  if($PaymentTerm==$arryReseller[0]['PaymentTerm']){echo "selected";}?>><?=$PaymentTerm?></option>
			<? } ?>
	</select> 
	</td>
</tr>
		  ...
		  <select name="PaymentMethod" class="inputbox" id="PaymentMethod">
		  	<option value="">--- None ---</option>
				<? for($i=0;$i<sizeof($arryPaymentMethod);$i++) {?>
					<option value="<?=$arryPaymentMethod[$i]['attribute_value']?>" <?  if($arryPaymentMethod[$i]['attribute_value']==$arryReseller[0]['PaymentMethod']){echo "selected";}?>>
