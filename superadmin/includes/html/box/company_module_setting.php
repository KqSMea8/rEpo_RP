<?php 
	...
	if($_GET['edit']>0){
		...
		$RedirectUrl = 'editCompany.php?edit='.$CmpID.'&curP='.$_GET['curP'].'&d='.$_GET['d'].'&tab=mod';	
			...
			if(!$objConfig->connect_check()){
				...
				if ($_POST) {
					...
					if(!empty($_POST['numModule'])) {
						$objConfigure->updateMenuSettings($_POST);
						$_SESSION['mess_mod'] = MODULE_SETTING_UPDATED;
					}
...
function ShowListing(){
	ShowHideLoader('1','L');	
	location.href = "<?php echo 'editCompany.php?edit='.$CmpID.'&mode='.$_GET['mode'].'&tab=mod';?>&d="+document.getElementById("DepartmentID").value;
