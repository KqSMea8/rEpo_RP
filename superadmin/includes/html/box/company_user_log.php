<?php 
	...
	$SessionTimeout = ($arryCompany[0]['SessionTimeout']>0)?($arryCompany[0]['SessionTimeout']):(7200);
	$CurrentCmpTime = getLocalTime($arryCompany[0]['Timezone']);
...
<?
	...
	$OnlineUrl = 'editCompany.php?edit='.$CmpID.'&mode=online&tab=UserLog';	
	$OfflineUrl = 'editCompany.php?edit='.$CmpID.'&mode=offline&tab=UserLog';	
?>
...
<table <?=$table_bg?>>
    ...
    <tr align="left"  >
	  ...
	  <td width="8%"  class="head1" >User Type</td>     
...
$TotalDuration=0;
  if(is_array($arryUserLog) && $num>0){
	...
	$values['CurrentCmpTime'] = $CurrentCmpTime; 
	$arryStatusDetail = CheckLoginStatus($values);
	$Status = $arryStatusDetail[0];
	$stClass = $arryStatusDetail[1];
	$CheckHide = $arryStatusDetail[2];



	 if($values['UserType']=='employee'){
		$UserLink = '<a class="fancybox fancybox.iframe" href="userInfo.php?view='.$values['EmpID'].'&cmp='.$CmpID.'" >'.stripslashes($values["UserName"]).'</a>';
		$Email = $values['Email'];
	 }else if($values['UserType']=='customer'){
		$UserLink = '<a class="fancybox fancybox.iframe" href="custInfo.php?CustID='.$values['UserID'].'&cmp='.$CmpID.'">'.stripslashes($values['CustomerName']).'</a>';
		$Email = $values['CustomerEmail'];
	}else if($values['UserType']=='customerContact'){
		$UserLink = stripslashes($values['CustContactName']);
		$Email = $values['CustContactEmail'];
	}else if($values['UserType']=='vendor'){
		$UserLink = '<a class="fancybox fancybox.iframe" href="vendorInfo.php?SuppID='.$values['UserID'].'&cmp='.$CmpID.'" >'.stripslashes($values["VendorName"]).'<a>';
