...
function SetPageWithRight(){
	document.getElementById("inner_mid").style.width="85%";
	$(".main-container .main .left-main-nav").hide();
...
function ValidateMandExcelCSV(p_Field,p_Message){
	if(p_Field.value !=''){
		var FileExtension = p_Field.value.substr(p_Field.value.length-3,p_Field.value.length);
		switch(FileExtension.toLowerCase()){
			case 'xls':
			...
			case 'csv':
				return 1;
			default:
				alert('Please upload file in .xls or .csv format only.');
				p_Field.select();
				return 0;
		}
	}else{
...
function ValidateMandExcel(p_Field,p_Message){
	if(p_Field.value !=''){
		var FileExtension = p_Field.value.substr(p_Field.value.lastIndexOf('.') + 1,p_Field.value.length);
		switch(FileExtension.toLowerCase()){
			case 'xls':
				return 1;
case 'xlsx':
...
function ValidateOptionalDocFiles(ext){
