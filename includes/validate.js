...
function confirmAction(obj, module, message)
{
	...
	var CustomLoader = '';
	if(document.getElementById("CustomLoader") != null){
		CustomLoader = $("#CustomLoader").val();
	}
...
function confirmAlert(obj,module, message)
{
    var response = 0;
    $("#dialog-modal").html(message);
    $("#dialog-modal").dialog(
...
function isPhoneKey(evt){
    ...
    if (charCode > 31 && (charCode < 43 || charCode > 57 || charCode == 44 || charCode == 46 || charCode == 47))
        return false;
    return true;
}
...
function isSSN(formInput) {
  var pattern = /^\d{3}-\d{2}-\d{4}$/;

   var ssn = formInput.value;
...
function isEIN(formInput) {
  var pattern = /^\d{2}-\d{7}$/;
   ...
   if(ssn.match(pattern)){	 
	 ...
	 alert("Please Enter Valid EIN");  
	 formInput.select();
	return 0;
    }
}

/*function DisableModifyOptions(){
	$(".add").hide();
	$(".add_quick").hide();
	$(".editicon").hide();
	$(".delicon").hide();
...
function DisableModifyOptions(AddLabel,EditLabel,DeleteLabel,ApproveLabel){
...
function DefaultDateFormat(DateVal){
		 if(DateVal!='' && DateFormatForm!= 'Y-m-d'){
			var arryDate = DateVal.split('/');

			if(DateFormatForm=="Y/m/d"){
				y = arryDate[0]; m = arryDate[1]; d = arryDate[2];
			}else if(DateFormatForm=="d/m/Y"){
				d = arryDate[0]; m = arryDate[1]; y = arryDate[2];
			}else if(DateFormatForm=="m/d/Y"){
