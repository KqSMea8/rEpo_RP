	...
	function ClearSpecialChars(obj){	
		if(obj.value!=''){
			var str = obj.value.replace(/[^a-zA-Z0-9]/g,''); 
			obj.value = str; 	
		}
	}
	function RemoveSpecialChars(obj){	
		if(obj.value!=''){
			var str = obj.value.replace(/[^a-zA-Z0-9_ \-\.]/g,''); 
			obj.value = str; 	
		}
	}
		...
		httpObj.onreadystatechange = function StateListRecieve(){
			if (httpObj.readyState == 4) {
				...
				$("#state_id option").each(function(i){
					if(stateval=='')stateval = $(this).val(); 
				});	
				if(stateval<=0){
					$("#state_td").hide();
					$("#state_td").closest('td').prev().hide();
					if(document.getElementById("city_td") != null){
						CityListSend(); 
					}
				} 
	...
	function CityListSend(){
		...
		if(document.getElementById("state_id") != null){
			...
			ShowHideLoader('1','L');
			var SendUrl = 'ajax.php?action=city&state_id='+document.getElementById("state_id").value+'&country_id='+document.getElementById("country_id").value+'&current_city='+document.getElementById("main_city_id").value+SelectOption+OtherOption+ByCountry+'&r='+Math.random()+'&select=1';
	...
	function SetMainCityId(){
		...
		if(document.getElementById("OtherState") != null){
			if(stateDisplay=='none')document.getElementById("OtherState").value = '';
			if(document.getElementById("state_id").value > 0 || stateDisplay=='none' ){
...
/*****************************************/
function RemoveFile(file_dir,file_name,file_div){


	var Url = "ajax.php?action=delete_upload_file&file_dir="+escape(file_dir)+"&file_name="+escape(file_name)+"&r="+Math.random(); 
...
/*****************************************/
function DeleteFileRefresh(file_dir,file_name,file_div){
	var Url = "ajax.php?action=delete_upload_file&file_dir="+escape(file_dir)+"&file_name="+escape(file_name)+"&r="+Math.random(); 
...
/*****************************************/
function RemoveFileRefresh(file_dir,file_name,file_div){
	var Url = "ajax.php?action=delete_upload_file&file_dir="+escape(file_dir)+"&file_name="+escape(file_name)+"&r="+Math.random(); 
					...
					httpObj.onreadystatechange = function DeleteFileRecieve(){
						if (httpObj.readyState == 4) {
							if(httpObj.responseText == 1){
								...
								var QuestMark = selfPage.indexOf("?");
								if(QuestMark>0){
									selfPage = selfPage+'&FileExist=1';
								}else{
									selfPage = selfPage+'?FileExist=1';
								}
...
function SetCountyByZip(){
	...
	if(ZipCode!=''){
		$.ajax({
			...
			data: "&action=SetRegionByZip&ZipCode="+escape(ZipCode)+"&r"+Math.random(),
			success: function (responseText) {
				if(responseText["country_id"]>0){
				
				if(responseText["state_id"]!=document.getElementById("state_id").value){
				document.getElementById("country_id").value=responseText["country_id"];
				document.getElementById("main_state_id").value=responseText["state_id"];
				document.getElementById("main_city_id").value=responseText["city_id"];
				StateListSend();
				}
				}
...
function TrackShippAPI(TrackingNo,ShippingMethod){	 	 
	if(TrackingNo!='' && ShippingMethod!=''){
		var TrackUrl = '';
 		if(ShippingMethod=='fedex'){
			TrackUrl = 'tracking.details.php';
		}else if(ShippingMethod=='ups'){
			TrackUrl = 'upstracking.details.php';
		}else if(ShippingMethod=='dhl'){
			TrackUrl = 'dhltracking.details.php';
		}else if(ShippingMethod=='usps'){
			TrackUrl = 'uspstracking.details.php';
		}
		 
			
		if(TrackUrl !='' ){
			$("#TrackingStatusTR").show();	
			$("#TrackingStatusTD").html('<img src="../images/loading.gif">');

			$.ajax({
				type: "GET",				
				url: "../shipping/"+TrackUrl,
				data: "&action=TrackStatus&view="+escape(TrackingNo)+"&r"+Math.random(),
				success: function (responseText) {
					var resArray = responseText.split("#");
				        var HtmlResponse = '<span class=bluemsg>'+resArray[0]+'</span>';
					var EstimatedDelivery = resArray[3];
;					if(resArray[1]!=''){ 
						HtmlResponse += '<br><img class="trackload" src="../images/tracking/'+resArray[1]+'.png" >';
					}
					//alert(resArray[2]);
					$("#TrackingStatusTD").html(HtmlResponse);
					if(EstimatedDelivery!=''){
						$("#ExpectedDateTd").html(EstimatedDelivery);		
					}
				}
			});	
...
function TrackShippAPINew(TrackingNo,ShippingMethod,ln){	
//alert(TrackingNo); 	 
	if(TrackingNo!='' && ShippingMethod!=''){
		var TrackUrl = '';
 		if(ShippingMethod=='fedex'){
		...
		if(TrackUrl !='' ){
			$("#TrackingStatusTR"+ln).show();	
			$("#TrackingStatusTD"+ln).html('<img src="../images/loading.gif">');
...
;					if(resArray[1]!=''){ 
					...
					$("#TrackingStatusTD"+ln).html(HtmlResponse);
					if(EstimatedDelivery!=''){
						$("#ExpectedDateTd").html(EstimatedDelivery);		
					}
				}
...
function TrackShippAPICustom(TrackingNo,ShippingMethod,PO){	 	
//alert(PO);
		...
		if(TrackUrl !='' ){
			...
			$.ajax({
				...
				success: function (responseText) {
					...
					var EstimatedDelivery = resArray[3];
;					if(resArray[1]!=''){ 
						HtmlResponse += '<br><img src="../images/tracking/'+resArray[1]+'.png" >';
					}
					//alert(resArray[2]);
					//$("#TrackingStatusTD"+PO).html(HtmlResponse);
//alert(EstimatedDelivery);
					if(EstimatedDelivery!=''){
						$("#ExpectedDateTd"+PO).html(EstimatedDelivery);		
					}
				}
			});	
...
function loadState(countryId,selectId){
 ...
 data: {
        ...
        selectId: selectId
               ...
               // data:{'country_id='+countryId,'selectId='+selectId,},
...
function loadCity(state_id,selectId){
        if(state_id){
            $.ajax({
                type:'POST',
                url:'../sales/ajaxData.php',
 data: {
        ...
        selectId: selectId
               ...
               // data:{'city_id='+countryId,'selectId='+selectId,},
                success:function(html){
                    //$('#ShippingCountry').val(countryName);
...
function RemoveRecord(thisobj,id,ModuleName,Action){
	
	var recordCount = parseInt($("#recordCount").html());

	var poptitle = capitalizeFirstLetter(Action)+" "+ModuleName;
	var poptitleaction = capitalizeFirstLetter(Action)+ModuleName.split(' ').join('');
 
	var Url = "ajax.php?action="+poptitleaction+"&id="+escape(id)+"&r="+Math.random(); 

	$("#dialog-modal").html("Are you sure you want to "+Action+" this "+ModuleName+"?");
    $("#dialog-modal").dialog(
    {
        	title: poptitle,
		modal: true,
		...
		buttons: {
			"Ok": function() {
				 ...
				 ShowHideLoader(1,'P');
					...
					httpObj.open("GET", Url, true);
					httpObj.onreadystatechange = function RemoveDataRecieve(){
						if (httpObj.readyState == 4) {
							if(httpObj.responseText == 1){
								$(".message").html(ModuleName+" has been "+Action+"ed successfully.");
								$(thisobj).closest("tr").hide(500); 
								 recordCount = recordCount -1;
								 $("#recordCount").html(recordCount);
							}else if(httpObj.responseText == 0){
								alert(poptitle+" failed. Please try again later.");
