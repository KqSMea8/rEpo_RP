...
require_once($Prefix."classes/purchase.class.php");	
	...
	if(!empty($SuppID)){	
		if(!empty($arrySupplier[0]['SuppID'])){
			$arryInvoice=$objPurchase->GetSuppPurchase($arrySupplier[0]['SuppCode'],'','','Invoice');
    ...
    <tr align="left"  >
     <!-- <td width="0%" class="head1" ><input type="checkbox" name="SelectAll" id="SelectAll" onclick="Javascript:SelectCheckBoxes('SelectAll','OrderID','<?=sizeof($arryInvoice)?>');" /></td>-->
      ...
      <td width="8%" align="center" class="head1" >Currency</td>
       ...
       <td class="head1" width="8%" align="center" >Action</td>
  ...
  ?>
    <tr align="left" >
      <!--<td ><input type="checkbox" name="OrderID[]" id="OrderID<?=$Line?>" value="<?=$values['OrderID']?>" /></td>-->
       <td><a class="fancybox fancybig fancybox.iframe" href="../admin/finance/vPoInvoice.php?view=<?=$values['OrderID']?>&pop=1"><?=$values["InvoiceID"]?></a></td>
	   <td height="20">
    ...
    <td align="center"><? 
	 ...
	 ?></td>
<td><a href="../admin/finance/pdfPoInvoice.php?o=<?php echo $values['OrderID'];?>&amp;module="><img src="<?php echo _SiteUrl?>admin/images/download.png" border="0" onmouseover="ddrivetip('<center>Download</center>', 60,'')" ;="" onmouseout="hideddrivetip()"></a></td>
    </tr>
    <?php } // foreach end //?>
