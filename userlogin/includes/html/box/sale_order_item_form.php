...
//By chetan 21Sep//
<?php if($Config['TrackVariant']==1) {?>
if(responseText['variantDisplay']!='' && responseText['variantDisplay']!=null){ 
...
<textarea name="DesComment<?=$Line?>" id="DesComment<?=$Line?>" style="height:100px;" maxlength="100"><?=stripslashes($arrySaleItem[$Count]["DesComment"])?></textarea>
       ...
       <td align="center"><input type="checkbox" disabled="" class="disabled" name="DropshipCheck<?=$Line?>" id="DropshipCheck<?=$Line?>" <?php if($arrySaleItem[$Count]["DropshipCheck"] == 1){echo "checked";}?> onclick="return dropshipcost(<?=$Line?>);"></td>
       ...
       <td><input type="text" readonly="" class="disabled" name="discount<?=$Line?>" id="discount<?=$Line?>" class="textbox" size="3" maxlength="10" onkeypress="return isDecimalKey(event);" value="<?=stripslashes($arrySaleItem[$Count]["discount"])?>"/></td>
       <td>
